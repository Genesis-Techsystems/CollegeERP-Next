...
import { StudentAssignedListComponent } from './student-assigned-list/student-assigned-list.component';

@Component({
  selector: 'app-multi-evaluator-assign',
  templateUrl: './multi-evaluator-assign.component.html',
  styleUrls: ['./multi-evaluator-assign.component.scss']
})
export class MultiEvaluatorAssignComponent implements OnInit {
  ...
  checkedevaluator(check, item) {
    ...
    for (let i = 0; i < this.evaluationListData.length; i++) {
      if (check == false) {
        ...
        this.timeTableDetIds.push(this.evaluationListData[i].pk_exam_timetable_det_ids);
      }
    }
  }
  markEvaluators() {
    ...
    for (let i = 0; i < this.searchNameData.length; i++) {
      if (this.checkevaluator) {
        ...
        this.timeTableDetIds.push(this.searchNameData[i].pk_exam_timetable_det_ids);
      } else {
  ...
  Assign() {
    ...
    let request = [
      { paramName: 'in_flag', paramValue: 'MultipleUpdateEvaluationAssignment' },
