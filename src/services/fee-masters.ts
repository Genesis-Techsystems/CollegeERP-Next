...
import type { FeeParticular, FeeParticularPayload } from '@/types/fee-particular'
import type {
...
import { buildQuery, cmsDomainList, domainCreate, domainList, domainUpdate, getAllRecords, postDetails } from './crud'
...
export async function createFeeCategory(data: FeeCategoryPayload): Promise<FeeCategory> {
...
export async function createFeeParticular(data: FeeParticularPayload): Promise<FeeParticular> {
...
export async function listCollegeFeeStructures(params: {
  ...
  const res = await fetch(NEXT_API.PROXY(FEE_API.FEE_STRUCTURES_LIST) + `?${query}`, {
    cache: 'no-store',
    credentials: 'include',
  })
  ...
  if (!body.success) {
    throw new AppError('API_ERROR', body.message ?? 'Failed to load fee structures')
  }
...
export async function listUnivFeeStructures(
  ...
  if (!universityId || !courseId || !academicYearId || !courseGroupId) return []
...
export async function createUnivFeeStructure(
  ...
  return domainCreate<UnivFeeStructureRow>(ENTITIES.UNIV_FEE_STRUCTURE.name, data)
}
...
export async function listFeeCategoriesByCollege(collegeId: number): Promise<FeeCategory[]> {
  if (!collegeId) return []
  return domainList<FeeCategory>(
    ENTITIES.FEE_CATEGORY.name,
...
export async function listFeeParticularsByCollege(collegeId: number): Promise<FeeParticular[]> {
  if (!collegeId) return []
  return domainList<FeeParticular>(
    ENTITIES.FEE_PARTICULAR.name,
    buildQuery({ 'College.collegeId': collegeId, isActive: true }),
...
export async function listCourseYearsForFeeStructure(
  ...
  const queries = [
    ...
    buildQuery({ courseId, isFeeYear: true, isActive: true }, { field: 'sortOrder', direction: 'ASC' }),
  ]
  ...
  const filtered = isLateral ? rows.filter((r) => Number(r.yearNo ?? 0) !== 1) : rows
