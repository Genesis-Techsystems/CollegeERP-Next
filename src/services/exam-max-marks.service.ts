 ...
 * Spring Boot entity: ExamMarkssetup (lowercase 's')
 *
 * Import in 'use client' components:
 *   import { fetchMarksSetup, saveMarksSetup } from '@/services/exam-max-marks.service'
 */

import { EXAM_MASTERS_API } from '@/config/constants/api'
import { AppError, parseApiError } from '@/lib/errors'
import { domainList, buildQuery, getAllRecords } from '@/services/crud.service'
...
export async function fetchMarksSetup(
  ...
  // Spring Boot ExamMarkssetup entity uses field name 'disabled' (not 'isForDisabled').
  ...
  return domainList<ExamMarksSetup>(EXAM_MASTERS_API.EXAM_MARKS_SETUP_ENTITY, query)
}
...
export async function saveMarksSetup(rows: ExamMarksSetup[]): Promise<void> {
  const res = await fetch(`/api/proxy/${EXAM_MASTERS_API.EXAM_MARKS_SETUP_SAVE}`, {
    method: 'POST',
  ...
  if (!body.success || body.statusCode !== 200) {
    throw new AppError('API_ERROR', body.message ?? 'Failed to save marks setup')
  }
}
