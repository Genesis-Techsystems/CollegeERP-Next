...
import { domainList, fetchDetails, getAllRecords, postDetails } from "./crud";
...
export function buildStaffAttendanceNotMarkedSmsMessage(
  row: AnySmsRow,
): string {
...
export async function listStaffAttendanceNotMarked(params: {
  ...
  const raw = await getAllRecords<unknown>(
    COMMUNICATION_API.S_REP_ATTENDANCE_NOT_TAKEN_STAFF,
    {
...
export async function sendBulkEmailToEmployeesForAttendance(
  rows: AnySmsRow[],
): Promise<void> {
  await postDetails<unknown>(
    COMMUNICATION_API.SEND_BULK_EMAIL_EMPLOYEES_ATTENDANCE,
    rows,
  );
}
...
function absentStudentsArrayFromEnvelope(
 ...
 */
export async function fetchAbsentStudentsForSms(
  payload: Record<string, unknown>,
): Promise<AnySmsRow[]> {
  const res = await fetch(
    `${PROXY_BASE}/${COMMUNICATION_API.GET_SMS_TO_ABSENT_STUDENTS}`,
    {
      method: "POST",
      credentials: "include",
...
/**
 * Angular `listByTwoIds(emailsentlistbydateUrl, collegeId, day, 'collegeId', 'messageSentDate')`.
 * `messageSentDate` must be `YYYY/MM/DD` (Angular `momentFormatYMD`).
 */
export async function listEmailLogsForCollege(params: {
  collegeId: number;
  messageSentDate: string;
}): Promise<AnySmsRow[]> {
  const { collegeId, messageSentDate } = params;
  if (!collegeId || !messageSentDate) return [];

  const data = await fetchDetails<unknown>(
    COMMUNICATION_API.EMAILSENTLISTBYDATE,
    {
      collegeId,
      messageSentDate,
    },
  );
  return unwrapEmailOrSmsHistoryRows(data);
}

/** Angular `crudService.add(sendBulkSmsToMultiUsersUrl, students)`. */
export async function sendBulkSmsToMultiUsers(
  rows: AnySmsRow[],
): Promise<void> {
  await postDetails<unknown>(
    COMMUNICATION_API.SEND_BULK_SMS_TO_MULTI_USERS,
    rows,
  );
}
...
export async function sendBulkSmsToStudents(
  payload: SendBulkSmsToStudentsPayload,
): Promise<void> {
...
export async function sendLoginDetailsSms(
  rows: SendLoginDetailsUserRow[],
): Promise<void> {
...
function assertLegacyMasterPost(
  body: Record<string, unknown> | null,
  fallback: string,
): void {
...
export async function uploadFileForEmail(formData: FormData): Promise<string> {
  const res = await fetch(
    `${PROXY_BASE}/${COMMUNICATION_API.UPLOAD_FILE_FOR_EMAIL}`,
    {
      method: "POST",
      credentials: "include",
      body: formData,
...
export async function sendBulkEmailToStudents(
  payload: Record<string, unknown>,
): Promise<void> {
  const res = await fetch(
    `${PROXY_BASE}/${COMMUNICATION_API.SEND_BULK_EMAIL_TO_STUDENTS}`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
...
export async function sendBulkEmailToSectionStudents(
  payload: Record<string, unknown>,
): Promise<void> {
  const res = await fetch(
    `${PROXY_BASE}/${COMMUNICATION_API.SEND_BULK_EMAIL_TO_SECTION_STUDENTS}`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
...
export async function sendBulkEmailToStudentsByCourseYears(
  payload: Record<string, unknown>,
): Promise<void> {
  const res = await fetch(
    `${PROXY_BASE}/${COMMUNICATION_API.SEND_BULK_EMAIL_TO_STUDENTS_BY_CY}`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
  ...
  > | null;
  if (!res.ok) throw parseApiError(res, body);
  assertLegacyMasterPost(
    body,
    "Send bulk email to students by course years failed",
  );
}
 ...
 */
export async function sendBulkEmailToEmployeesDepartmentWise(
  payload: Record<string, unknown>,
): Promise<void> {
  const res = await fetch(
    `${PROXY_BASE}/${COMMUNICATION_API.SEND_BULK_EMAIL_TO_EMPLOYEES}`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
 ...
 */
export async function sendBulkEmailToAdminFromStaff(
  payload: Record<string, unknown>,
): Promise<void> {
  const res = await fetch(
    `${PROXY_BASE}/${COMMUNICATION_API.SEND_BULK_EMAIL_TO_ADMIN}`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
