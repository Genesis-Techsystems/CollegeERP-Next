...
export async function getExamTimetableFilterRows(args: {
  ...
  const data = await getAllRecords<Record<string, unknown>>(
  ...
  const usable = flat.filter(
    (r) => Number(r.fk_course_id ?? 0) > 0 || Number(r.fk_exam_id ?? 0) > 0,
  );
...
export async function listUnivExamBagsActive(): Promise<AnyRow[]> {
  return domainList<AnyRow>(
    UNIV_EXAM_CENTER_API.EXAM_BAGS,
...
export async function createUnivExamBag(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
  return domainCreate<AnyRow>(UNIV_EXAM_CENTER_API.EXAM_BAGS, payload);
}

export async function updateUnivExamBag(
...
export async function createUnivExamBundle(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
...
export function pickUnivExamBagTransportationId(row: AnyRow): number {
  return num(
    row.univExamBagTransportationId ?? row.univ_exam_bag_transportation_id,
  );
}

export async function listAllActiveUnivExamRegionalCenters(): Promise<
  AnyRow[]
> {
  return domainList<AnyRow>(
...
export async function listExamBagTransportationByFilters(
  ...
  if (!univExamReionalCenterId || !univExamcenterId || !univExamBagId)
    return [];
  const queries = [
    buildQuery({
...
export async function createExamBagTransportation(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
...
export async function listAllActiveUnivExamAnswerPaperBags(): Promise<
  AnyRow[]
> {
  return domainList<AnyRow>(
...
export function pickUnivExamAnswerPaperBagId(row: AnyRow): number {
  return num(
    row.univExamAnswerPaperBagId ??
      row.univExamAnswerPaperBagsId ??
      row.univ_exam_answer_paper_bag_id,
  );
}

export async function createUnivExamAnswerPaperBag(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
...
export async function createUnivExamBagCollection(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
...
export async function listExamEvaluatorProfilesByRole(
  roleId: number,
): Promise<AnyRow[]> {
  if (!roleId) return [];
  return domainList<AnyRow>(
...
export async function addListUnivEcProfiles(
  payload: Record<string, unknown>[],
): Promise<void> {
...
export async function saveExamCenterAttendance(
...
function flattenExamCenterDetailsFirstGroup(
...
function normalizeBundleListQuestionPaperCode(
...
export async function listExamBundlesByCode(args: {
  ...
  const params = {
    ...
    in_questionpaper_code: normalizeBundleListQuestionPaperCode(
      args.questionPaperCode,
    ),
  };
  try {
    const data = await getAllRecords<{ result?: unknown }>(
...
export async function listExamSeatnoBarcodeDetails(args: {
  ...
  try {
    const data = await getAllRecords<{ result?: unknown }>(
      UNIV_EXAM_CENTER_API.BARCODE_DETAILS,
      params,
    );
    return flattenExamCenterDetailsFirstGroup(data);
  } catch (error: unknown) {
...
export async function listExamScanProfilesByGroup(
  examGroupId: number,
): Promise<AnyRow[]> {
...
export async function getExamScanBundleStickers(args: {
  ...
  try {
    const data = await getAllRecords<{ result?: unknown }>(
      UNIV_EXAM_CENTER_API.EXAM_CENTER_SCAN_BY_CODE,
      {
...
export async function searchExamOmrSerialNo(query: string): Promise<AnyRow[]> {
  const res = await fetch(
    NEXT_API.PROXY(
      `${UNIV_EXAM_CENTER_API.SEARCH_BY_OMR_SERIAL}?query=${encodeURIComponent(query)}`,
    ),
  );
 ...
 */
export async function saveScanBundleDetails(
  rows: Record<string, unknown>[],
): Promise<void> {
  if (!rows.length) return;
...
export async function createUnivEcProfile(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
  return domainCreate<AnyRow>(UNIV_EXAM_CENTER_API.EXAM_SCAN_PROFILES, payload);
}

export async function updateUnivEcProfile(
  univEcProfileId: number,
  payload: Record<string, unknown>,
): Promise<AnyRow> {
  const pks = [
    "examScanProfileId",
...
function flattenScanByCodeFirstGroup(
  data: { result?: unknown } | undefined,
): AnyRow[] {
  const raw = data?.result;
  if (!Array.isArray(raw)) return [];
...
export function pickExamScanBundleDetailId(row: AnyRow): number {
  return num(
    row.examScanBundleDetailId ??
      row.exam_scan_bundle_detail_id ??
      row.unvExamBundleDetailId,
  );
}

export async function listAllActiveExamScanBundleDetails(): Promise<AnyRow[]> {
  return domainList<AnyRow>(
    UNIV_EXAM_CENTER_API.EXAM_SCAN_BUNDLE_DETAILS,
    buildQuery({ isActive: true }),
  );
}

export async function createExamScanBundleDetail(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
  return domainCreate<AnyRow>(
    UNIV_EXAM_CENTER_API.EXAM_SCAN_BUNDLE_DETAILS,
    payload,
  );
}
...
export function pickUnivEcQuestionPaperConfigId(row: AnyRow): number {
  return num(
    row.univEcQuestionPaperConfigId ?? row.univ_ec_question_paper_config_id,
  );
}
...
export async function createUnivEcQuestionPaperConfig(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
...
function pickUnivExamCenterRoomCenterId(row: AnyRow): number {
  ...
  const fromNested =
    (nestedA && typeof nestedA === "object"
      ? num(
          (nestedA as AnyRow).univExamcenterId ??
            (nestedA as AnyRow).univExamCenterId,
        )
      : 0) ||
    (nestedB && typeof nestedB === "object"
      ? num(
          (nestedB as AnyRow).univExamcenterId ??
            (nestedB as AnyRow).univExamCenterId,
        )
      : 0);
  return num(
...
function pickUnivExamCenterRoomBuildingId(row: AnyRow): number {
  ...
  const nestedId =
    nested && typeof nested === "object"
      ? num((nested as AnyRow).buildingId)
      : 0;
...
function filterUnivExamCenterRoomsClient(
  ...
  return rows.filter((r) => {
    ...
    if (buildingId > 0 && pickUnivExamCenterRoomBuildingId(r) !== buildingId)
      return false;
    return true;
  });
}
  ...
  for (const where of queryVariants) {
    try {
      ...
      return buildingId > 0 && !("building.buildingId" in where)
        ? filterUnivExamCenterRoomsClient(
            list,
            examId,
            univExamcenterId,
            buildingId,
...
export async function addListUnivExamCenterRooms(
  payload: Record<string, unknown>[],
): Promise<void> {
  if (!payload.length) return;
...
export async function listUnivExamRegionalCentersByUniversity(
  universityId: number,
): Promise<AnyRow[]> {
...
export async function listActiveCities(): Promise<AnyRow[]> {
  try {
    return await domainList<AnyRow>(
      "City",
      buildQuery({ isActive: true }, { field: "cityCode", direction: "ASC" }),
    );
  } catch {
    return domainList<AnyRow>(
      "City",
      buildQuery({}, { field: "cityCode", direction: "ASC" }),
    );
  }
}
...
export async function listUnivExamCentersByUniversity(
  universityId: number,
): Promise<AnyRow[]> {
  if (!universityId) return [];
  const queries = [
...
export async function addListUnivEcColleges(
  payload: Record<string, unknown>[],
): Promise<void> {
  if (!payload.length) return;
...
export async function updateUnivEcCollege(
  univEcCollegeId: number,
  payload: Record<string, unknown>,
): Promise<AnyRow> {
  return domainUpdate<AnyRow>(
...
export async function addListUnivEcStudents(
  payload: Record<string, unknown>[],
): Promise<void> {
  if (!payload.length) return;
...
export async function listUnivEcCollegeDetailsByUnivEcCollege(
  univEcCollegeId: number,
): Promise<AnyRow[]> {
  if (!univEcCollegeId) return [];
  const queries = [
    buildQuery({
      "univEcColleges.univEcCollegeId": univEcCollegeId,
      isActive: true,
    }),
    buildQuery({
      "UnivEcColleges.univEcCollegeId": univEcCollegeId,
  ...
  for (const q of queries) {
    try {
      return await domainList<AnyRow>(
        UNIV_EXAM_CENTER_API.EC_COLLEGE_DETAILS,
        q,
      );
    } catch {
      /* try next */
...
export async function addUnivEcCollegeDetails(
  payload: Record<string, unknown>[],
): Promise<void> {
  if (!payload.length) return;
...
export async function updateInActiveUnivEcCollegeDetails(
  payload: Record<string, unknown>,
): Promise<void> {
  await putDetails(
    UNIV_EXAM_CENTER_API.UPDATE_INACTIVE_EC_COLLEGE_DETAILS,
    payload,
  );
}
 ...
 */
export async function listUniversitiesForExamGroup(
  orgId: number,
  employeeId: number,
): Promise<AnyRow[]> {
 ...
 */
export async function listExamGroupingsByUniversity(
  universityId: number,
): Promise<AnyRow[]> {
...
export function pickExamGroupingId(row: AnyRow): number {
  return num(
    row.univExamGroupId ??
      row.univ_exam_group_id ??
      row.examGroupingId ??
      row.exam_grouping_id,
  );
}

async function findExamGroupInUniversityList(
  id: number,
  universityId: number,
): Promise<AnyRow | null> {
...
export async function getExamGroupingById(
  ...
  const fromHint = await findExamGroupInUniversityList(
    id,
    num(universityIdHint),
  );
  if (fromHint) return fromHint;

  return findExamGroupByScanningUniversities(
    id,
    num(scanOrg?.orgId),
    num(scanOrg?.employeeId),
  );
}

export async function createExamGrouping(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
...
export async function listExamGroupExamLines(
  univExamGroupId: number,
): Promise<AnyRow[]> {
  ...
  try {
    const rows = await domainList<AnyRow>(
      UNIV_EXAM_CENTER_API.UNIV_EXAM_GROUP_DETAILS,
      q,
    );
    return rows.filter((r) => r.isActive !== false);
  } catch {
    const qLoose = buildQuery({
      "univExamGroup.univExamGroupId": univExamGroupId,
    });
    return domainList<AnyRow>(
      UNIV_EXAM_CENTER_API.UNIV_EXAM_GROUP_DETAILS,
...
export async function createExamGroupExamLine(
  payload: Record<string, unknown>,
): Promise<AnyRow> {
...
export async function getExamGroupByCodeRows(args: {
  ...
  const data = await getAllRecords<{ result: unknown }>(
    UNIV_EXAM_CENTER_API.EXAM_GROUP_BY_CODE,
    {
...
export function pickLineExamLabel(row: AnyRow): string {
  ...
  const n =
    row.examName ?? row.exam_name ?? row.examShortName ?? row.exam_short_name;
  return typeof n === "string" ? n : "-";
}
