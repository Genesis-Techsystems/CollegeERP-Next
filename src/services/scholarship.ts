...
import { getFeeMasterCollegeFilters, type FeeMasterCollegeFilters } from './fee-masters'
...
function normalizeFeeSchStructure(row: FeeSchStructureRow): FeeSchStructureRow {
  ...
  const collegeId = num(row.collegeId, r.collegeId, college?.collegeId, r['College.collegeId'])
  const courseId = num(row.courseId, r.courseId, course?.courseId, r['Course.courseId'])
  const batchId = num(row.batchId, r.batchId, batch?.batchId, r['Batch.batchId'])
...
function normalizeScholarshipValue(row: ScholarshipValueRow): ScholarshipValueRow {
  const r = row as AnyRow
  ...
  return {
    ...
    feeSchStructureId: num(row.feeSchStructureId, r.feeSchStructureId, r['FeeSchStructure.feeSchStructureId']),
    feeCategoryId: num(row.feeCategoryId, r.feeCategoryId, category?.feeCategoryId, r['FeeCategory.feeCategoryId']),
    ...
    scholarshipAmount: Number(row.scholarshipAmount ?? row.feeAmount ?? r.scholarshipAmount ?? r.feeAmount ?? 0),
    feeAmount: Number(row.feeAmount ?? row.scholarshipAmount ?? r.feeAmount ?? r.scholarshipAmount ?? 0),
    categoryName: str(row.categoryName, category?.categoryName, category?.categoryCode),
    particularName: str(row.particularName, row.particularsName, particular?.particularsName),
...
): Promise<ApiResponse<T> & { totalCount?: number; page?: number; pageSize?: number }> {
...
export async function createScholarshipType(data: ScholarshipTypePayload): Promise<ScholarshipType> {
...
export async function listScholarshipApplications(
  ...
  return domainList<ScholarshipApplication>(
    ...
    buildQuery(
      { 'College.collegeId': collegeId, 'AcademicYear.academicYearId': academicYearId },
...
export async function createScholarshipApplication(
  data: ScholarshipApplicationPayload,
): Promise<unknown> {
  return postDetails(SCHOLARSHIP_API.SCHOLARSHIP_APP, data)
}
...
export async function listSchPreceedings(params: {
  ...
  const body = await proxyGet<SchPreceeding[]>(SCHOLARSHIP_API.SCH_PRECEEDINGS, {
  ...
  if (!body.success) {
    throw new AppError('API_ERROR', body.message ?? 'Failed to load proceedings')
  }
...
export async function createSchPreceeding(data: SchPreceedingPayload): Promise<unknown> {
...
export async function listSchStdPreceedings(schPreceedingId: number): Promise<SchStdPreceeding[]> {
...
export async function listSchAccountsPreceedings(collegeId?: number): Promise<SchAccountsPreceeding[]> {
  const query = collegeId
    ? buildQuery({ 'College.collegeId': collegeId, isActive: true })
    : buildQuery({ isActive: true })
  return domainList<SchAccountsPreceeding>(ENTITIES.SCH_ACCOUNTS_PRECEEDING.name, query)
}

export async function createSchAccountsPreceeding(data: Record<string, unknown>): Promise<unknown> {
...
export async function listNullPreceedings(collegeId: number): Promise<SchPreceeding[]> {
  const data = await fetchDetails<SchPreceeding[]>(SCHOLARSHIP_API.GET_NULL_PRECEEDINGS, { collegeId })
  return Array.isArray(data) ? data : []
}
...
export async function listFeeSchStructures(filters: {
  ...
  if (filters.academicYearId) conditions['AcademicYear.academicYearId'] = filters.academicYearId
  ...
  return domainList<FeeSchStructureRow>(ENTITIES.FEE_SCH_STRUCTURE.name, buildQuery(conditions))
}

export async function createFeeSchStructure(data: FeeSchStructurePayload): Promise<FeeSchStructureRow> {
  return domainCreate<FeeSchStructureRow>(ENTITIES.FEE_SCH_STRUCTURE.name, data)
}
...
export async function getFeeSchStructureById(
  ...
  for (const query of queries) {
    try {
      const rows = await domainList<FeeSchStructureRow>(ENTITIES.FEE_SCH_STRUCTURE.name, query)
...
export async function listScholarshipValuesByStructure(
  ...
  const queries = [
    buildQuery({ 'FeeSchStructure.feeSchStructureId': feeSchStructureId, isActive: true }),
  ...
  for (const query of queries) {
    try {
      const rows = await domainList<ScholarshipValueRow>(ENTITIES.SCHOLARSHIP_VALUE.name, query)
...
export async function createScholarshipValue(
  ...
  return domainCreate<ScholarshipValueRow>(ENTITIES.SCHOLARSHIP_VALUE.name, data)
}
...
export async function deleteScholarshipValue(scholarshipValueId: number): Promise<void> {
...
export async function getScholarshipTypeAndValues(params: {
  ...
  const data = await fetchDetails<unknown>(SCHOLARSHIP_API.GET_SCHOLARSHIP_TYPE_AND_VALUES, query)
  return unwrapDetailRows(data).map(normalizeScholarshipTypeAndValue)
}

function normalizeScholarshipTypeAndValue(row: AnyRow): ScholarshipTypeAndValue {
  ...
  return {
    ...
    scholarshipTypeCode: str(row.scholarshipTypeCode, row.scholarshipTypeDesc, row.name),
    scholarshipAmount: Number(
      firstStructure?.scholarshipAmount ?? row.scholarshipAmount ?? row.amount ?? 0,
    ),
  }
}
...
export async function getStudentsScholarshipDetails(params: {
  ...
  if (params.scholarshipTypeId) query.scholarshipTypeId = params.scholarshipTypeId
  const data = await fetchDetails<unknown>(
    SCHOLARSHIP_API.GET_STUDENTS_SCHOLARSHIP_DETAILS,
    query,
  )
  const rows = unwrapDetailRows(data)
  return rows.map((row) => normalizeAssignScholarshipStudent(row as AssignScholarshipStudent))
}
...
function normalizeAssignScholarshipStudent(row: AssignScholarshipStudent): AssignScholarshipStudent {
  const r = row as AnyRow
  const details = (r.students_details ?? r.StudentDetail ?? r.studentDetail) as AnyRow | undefined
  ...
  const hasScholarshipRecord = scholarship != null && typeof scholarship === 'object'
  const unAssigned = hasScholarshipRecord && isTruthyFlag(scholarship?.unAssigned)
  ...
  return {
    ...
    isAssigned,
    scholarshipAmount: Number(
      scholarship?.amount ?? row.scholarshipAmount ?? r.scholarshipAmount ?? r.value ?? 0,
    ),
    assignedType: str(
      row.assignedType,
    ...
    scholarshipValueId: num(row.scholarshipValueId, scholarship?.scholarshipValueId) || undefined,
    studentScholarshipId:
      num(row.studentScholarshipId, scholarship?.studentScholarshipId) || undefined,
...
export async function uploadStdPreceedings(formData: FormData): Promise<unknown> {
  const res = await fetch(NEXT_API.PROXY(SCHOLARSHIP_API.UPLOAD_STD_PRECEEDINGS), {
    method: 'POST',
    credentials: 'include',
    body: formData,
  })
...
export async function listFinancialYearsByUniversity(universityId: number): Promise<AnyRow[]> {
...
export async function searchStudentsForScholarship(q: string, collegeId: number): Promise<AnyRow[]> {
  const term = q.trim()
  if (term.length < 4 || !collegeId) return []
  const data = await fetchDetails<AnyRow[]>('studentsearch', { q: term, collegeId })
  return Array.isArray(data) ? data : []
}
