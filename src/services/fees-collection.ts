...
export async function listStudentFeeDue(params: {
  ...
  if (params.courseGroupId) query.set('courseGroupId', String(params.courseGroupId))
  if (params.courseYearId) query.set('courseYearId', String(params.courseYearId))
  if (params.quotaId) query.set('quotaId', String(params.quotaId))

  const res = await fetch(NEXT_API.PROXY(FEE_API.STUDENT_FEE_LIST) + `?${query}`, {
    cache: 'no-store',
    credentials: 'include',
  })
  ...
  if (!body.success) {
    throw new AppError('API_ERROR', body.message ?? 'Failed to load student fee due list')
  }
...
export async function searchStudentsForFeeCollection(term: string): Promise<StudentFeeSearchRow[]> {
  const q = term.trim()
  if (q.length < 2) return []
  const data = await fetchDetails<StudentFeeSearchRow[]>(FEE_API.STUDENT_FEE_SEARCH, {
    isActive: 'true',
    q,
  })
...
export async function listCourseYearsForFeeCollection(courseId: number) {
  ...
  for (const query of queries) {
    try {
      const rows = await domainList<Record<string, unknown>>(ENTITIES.COURSE_YEAR.name, query)
...
export async function getFeeStudentData(params: {
  ...
  if (!collegeId || !academicYearId || !studentId || !feeStructureId) return null

  const data = await fetchDetails<FeeStudentData[] | FeeStudentData>(FEE_API.FEE_STUDENT_DATA, {
    collegeId,
    academicYearId,
    studentId,
    feeStructureId,
...
export async function getFinancialYearForReceiptDate(
  ...
  const data = await fetchDetails<FinancialYearRow[] | FinancialYearRow>(FEE_API.FINANCIAL_YEAR_DATE, {
    collegeId,
    checkDate,
  })
...
export async function getFeePaymentLookups(): Promise<FeePaymentLookups> {
  ...
  const withoutOnline = (rows: GeneralDetail[]) =>
    rows.filter((r) => String(r.generalDetailCode ?? '').toUpperCase() !== 'ONLINE')
  return {
...
export async function listFeeStructureParticularsForPayment(
  ...
  const queries = [
    buildQuery({ 'feeStructure.feeStructureId': feeStructureId, isActive: true }),
    buildQuery({ 'FeeStructure.feeStructureId': feeStructureId, isActive: true }),
    buildQuery({ feeStructureId, isActive: true }),
  ]
  for (const query of queries) {
    try {
      const rows = await domainList<Record<string, unknown>>('FeeStructureParticular', query)
      if (rows.length > 0) return rows
    } catch {
      // try next shape
    }
...
export async function generateFeeTransactions(feeStdDataId: number): Promise<void> {
  if (!feeStdDataId) return
  await putDetails(`${FEE_API.GENERATE_TRANSACTIONS}/${feeStdDataId}`, {})
}

export async function submitFeeReceipt(payload: FeeReceiptPaymentPayload): Promise<unknown> {
...
export async function searchStudentsInCollege(
  ...
  if (options?.courseGroupId) params.courseGroupId = String(options.courseGroupId)
  const data = await fetchDetails<StudentFeeSearchRow[]>(FEE_API.STUDENT_FEE_SEARCH, params)
  return Array.isArray(data) ? data : []
}
...
export async function listFeeStructuresForStudentCourseYear(params: {
  ...
  for (const query of queries) {
    try {
      ...
      if (academicYearId) {
        return rows.filter((r) => Number(r.academicYearId ?? 0) === academicYearId)
      }
      return rows
    } catch {
      // try next query shape
...
export async function deleteFeeReceipt(feeReceiptsId: number): Promise<void> {
  ...
  if (!body.success) {
    throw new AppError('API_ERROR', body.message ?? 'Failed to delete fee receipt')
  }
}
...
export async function syncInitiatedFeePayments(): Promise<string> {
  ...
  if (Array.isArray(data) && data[0] && typeof data[0] === 'object' && data[0] !== null) {
...
export async function syncAdmissionInitiatedPayments(): Promise<string> {
  ...
  if (data && typeof data === 'object' && 'message' in data) {
    return String((data as { message?: string }).message ?? 'Admission payment sync completed.')
  }
...
export async function listFeeConcessions(params: {
  ...
  if (!body.success) {
    throw new AppError('API_ERROR', body.message ?? 'Failed to load fee concessions')
  }
...
export async function searchEmployeesForTransport(term: string): Promise<EmployeeSearchRow[]> {
  const q = term.trim()
  if (q.length < 5) return []
  const data = await fetchDetails<EmployeeSearchRow[]>(FEE_API.EMPLOYEE_SEARCH, {
    q,
    empStatus: 'ACTV',
  })
  return Array.isArray(data) ? data : []
...
export async function getEmployeeDetailsForTransport(
  ...
  const data = await fetchDetails<EmployeeProfileRow>(EMPLOYEE_API.DETAILS_BY_USER_ID, {
    employeeId,
  })
...
export async function listTransportAllocationsByEmployee(
  ...
  const data = await fetchDetails<TransportAllocationRow[] | { resultList?: TransportAllocationRow[] }>(
    FEE_API.TRANSPORT_ALLOCATION,
    { employeeId, status: 'true' },
  )
  if (Array.isArray(data)) return data
  if (data && typeof data === 'object' && Array.isArray(data.resultList)) return data.resultList
  return []
}
...
export async function listFeeStructuresForAllocation(params: {
  ...
  if (params.academicYearId) filters['academicYear.academicYearId'] = params.academicYearId
...
export async function sendFeePaymentMailNotification(payload: unknown): Promise<unknown> {
...
function splitPaylinkFilterGroups(groups: Record<string, unknown>[][]): FeePaylinkCollegeFilters {
...
function readFeeMgmtStr(row: Record<string, unknown>, ...keys: string[]): string {
  for (const k of keys) {
    const v = row[k]
    if (v != null) {
...
export function normalizeFeeManagementStudent(
  ...
  return {
    ...raw,
    studentId,
    firstName: readFeeMgmtStr(raw, 'firstName', 'first_name', 'studentName', 'student_name', 'name'),
    ...
    admissionNumber: readFeeMgmtStr(raw, 'admissionNumber', 'admission_number', 'admission_no'),
    ...
    mobile: readFeeMgmtStr(raw, 'mobile', 'mobileNo', 'mobile_no', 'student_mobile'),
    fatherName: readFeeMgmtStr(raw, 'fatherName', 'father_name'),
    studentStatusCode: readFeeMgmtStr(raw, 'studentStatusCode', 'student_status_code'),
    studentStatusDisplayName: readFeeMgmtStr(
      raw,
      'studentStatusDisplayName',
      'student_status_display_name',
    ),
    collegeId: Number(raw.collegeId ?? raw.college_id ?? raw.fk_college_id ?? 0) || undefined,
    academicYearId:
      Number(raw.academicYearId ?? raw.academic_year_id ?? raw.fk_academic_year_id ?? 0) ||
      undefined,
    courseGroupId:
      Number(raw.courseGroupId ?? raw.course_group_id ?? raw.fk_course_group_id ?? 0) || undefined,
    courseYearId:
      Number(raw.courseYearId ?? raw.course_year_id ?? raw.fk_course_year_id ?? 0) || undefined,
    studentAppId: Number(raw.studentAppId ?? raw.student_app_id ?? 0) || undefined,
    isLateral: raw.isLateral != null ? Boolean(raw.isLateral) : raw.is_lateral != null ? Boolean(raw.is_lateral) : undefined,
    feeManagmentStdDetailsDtos,
  }
}

export function normalizeFeeManagementStudents(rows: unknown): FeeManagementStudentRow[] {
...
export async function searchFeeManagementStudents(
  ...
  const q = term.trim()
  if (q.length < 5) return []
  const data = await fetchDetails<unknown>(FEE_API.FEE_MANAGEMENT_SEARCH, { q })
...
export async function listFeeManagementStudentsByFilters(params: {
  ...
  if (!collegeId || !academicYearId || !courseYearId || !courseGroupId) return []
  const data = await fetchDetails<unknown>(FEE_API.GET_FEE_MANAGEMENT_STD_DETAILS, {
    collegeId: String(collegeId),
...
export async function listPaySchedulesForFeeManagement(): Promise<GeneralDetail[]> {
