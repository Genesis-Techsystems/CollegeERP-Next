...
export async function listStudentApplicationForms(params: {
  ...
  return rows.sort(
    (a, b) => Number(b.studentAppId ?? 0) - Number(a.studentAppId ?? 0),
  );
}
...
export async function createStudentEnquiry(
...
export async function updateStudentEnquiry(
  ...
  await domainUpdate(
    ADMISSION_API.STUDENT_ENQUIRY,
    ENTITIES.STUDENT_ENQUIRY.pk,
    enquiryId,
    payload,
  );
}

export async function getStudentEnquiryById(
  enquiryId: number,
): Promise<StudentEnquiryRow | null> {
  ...
  for (const query of queries) {
    try {
      const rows = await domainList<StudentEnquiryRow>(
        ADMISSION_API.STUDENT_ENQUIRY,
        query,
      );
...
export async function listQualificationGroupsByQualification(
...
export async function createCasteQuota(
  payload: CasteQuotaPayload,
): Promise<void> {
  await domainCreate(UNIVERSITY_API.CASTE_QUOTA, payload);
}

export async function updateCasteQuota(
  payload: CasteQuotaPayload,
): Promise<void> {
  const id = payload.casteQuotaId;
  if (!id)
    throw new AppError("VALIDATION", "casteQuotaId is required for update");
  await domainUpdate(
    UNIVERSITY_API.CASTE_QUOTA,
    ENTITIES.CASTE_QUOTA.pk,
    id,
    payload,
  );
}
...
export async function listCollegeCounselling(params: {
  ...
  return domainList<CollegeCounsellingRow>(
    UNIVERSITY_API.COLLEGE_COUNSELLING,
    query,
  );
}

export async function createCollegeCounselling(
  payload: CollegeCounsellingPayload,
): Promise<void> {
  await domainCreate(UNIVERSITY_API.COLLEGE_COUNSELLING, payload);
}

export async function updateCollegeCounselling(
  payload: CollegeCounsellingPayload,
): Promise<void> {
  ...
  if (!id)
    throw new AppError(
      "VALIDATION",
      "univCollegeCounsellingId is required for update",
    );
  await domainUpdate(
    UNIVERSITY_API.COLLEGE_COUNSELLING,
...
export async function listUnivStdApplications(): Promise<
  UnivStdApplicationRow[]
> {
  const query = buildQuery(
    { isActive: true },
  ...
  return domainList<UnivStdApplicationRow>(
    UNIVERSITY_API.STD_APPLICATIONS,
    query,
  );
}

export async function updateUnivStdApplication(
  formData: FormData,
): Promise<void> {
  const res = await fetch(
    `/api/proxy/${ADMISSION_API.UPDATE_UNIV_STUDENT_APPLICATION}`,
    {
      method: "POST",
      credentials: "include",
      body: formData,
  ...
  if (!body.success)
    throw new AppError("API_ERROR", body.message ?? "Update failed");
}

export async function searchUnivStdApplications(
  q: string,
): Promise<UnivStdApplicationRow[]> {
  if (q.trim().length < 4) return [];
  const query = buildQuery({ q: q.trim() });
  return domainList<UnivStdApplicationRow>(
    ADMISSION_API.STD_APPLICATIONS_SEARCH,
    query,
  );
}
...
export async function listAdmissionAllotments(
  collegeId: number,
): Promise<AdmissionAllotmentRow[]> {
  // Include inactive rows so Status can show Inactive in the grid.
  const query = buildQuery({ "college.collegeId": collegeId });
  return domainList<AdmissionAllotmentRow>(
...
export async function listAdmissionAllotmentConsolidate(
  ...
  const data = await fetchDetails<
    AllotmentDetailApiRow[] | AllotmentDetailApiRow
...
export async function createAdmissionAllotment(
...
export type StudentReportsParams = {
  flag: string;
  fromDate?: string;
  toDate?: string;
  collegeId: number;
...
/**
 * Angular student admission reports —
 * `listByFourteenIds(getStudentsReports, flag, …)` →
 * `getAllRecords/s_get_student_reports?in_flag=&in_fdate=&…`
 */
export async function fetchStudentReports(
  params: StudentReportsParams,
): Promise<Record<string, unknown>[]> {
  if (!params.flag || !params.collegeId) return [];
  const data = await getAllRecords<{ result: Record<string, unknown>[][] }>(
    "s_get_student_reports",
    {
      in_flag: params.flag,
      in_fdate: params.fromDate ?? "1990-01-01",
      in_tdate: params.toDate ?? "1990-01-01",
      in_collegeId: params.collegeId,
      in_courseId: params.courseId ?? 0,
      in_academicYearId: params.academicYearId ?? 0,
      in_courseGroupId: params.courseGroupId ?? 0,
      in_courseYearId: params.courseYearId ?? 0,
      in_studentquotaId: params.quotaId ?? 0,
      in_regId: params.regulationId ?? 0,
      in_iscurrentyr: params.isCurrentYear ?? -1,
      in_islateral: params.isLateral ?? -1,
      in_app_status: params.appStatus ?? 0,
      in_std_status_ids: params.studentStatusIds ?? 0,
    },
  );
