...
export function resolveExamLoginEmpId(
 ...
 */
export async function getUnivExamFiltersAll(
  employeeId: number,
): Promise<any[]> {
 ...
 */
export async function getUnivExamFiltersForExamFeeSetup(
  employeeId: number,
): Promise<any[]> {
  const data = await getAllRecords<{ result?: any[][] }>(
    "s_get_exam_filters_bycode",
  ...
  try {
    ...
    for (const g of groups) {
      ...
      if (String(head?.flag ?? head?.FLAG ?? "") === "univ_exam_rest_filters") {
        return g.filter(
          (r) => r && (r as Record<string, unknown>).fk_college_id != null,
        );
      }
    }
    return groups
...
export async function getCollegeFilters(
  orgId: number,
  empId: number,
): Promise<CollegeFiltersResult> {
...
export async function updateExamSession(
  ...
  return domainUpdate(
    ENTITIES.EXAM_SESSION.name,
    ENTITIES.EXAM_SESSION.pk,
    id,
    payload,
  );
}
...
export async function listExamGrades(filters?: {
  courseId?: number;
  regulationId?: number;
  ...
  if (filters?.regulationId)
    where["Regulation.regulationId"] = filters.regulationId;
  // Spring Boot entity field is `disabled` (Angular form calls it `isForDisabled`)
  if (filters?.isForDisabled !== undefined)
    where.disabled = filters.isForDisabled;

  // Mirror Angular API query shape exactly for this screen.
  const hasAny = Object.keys(where).length > 0;
  return domainList(
    ENTITIES.EXAM_GRADE.name,
    hasAny ? buildQuery(where) : undefined,
  );
}
...
export async function updateExamMasterSvc(
  examId: number,
  payload: Record<string, unknown>,
) {
  return domainUpdate<ExamMaster>(
    ENTITIES.EXAM_MASTER.name,
    ENTITIES.EXAM_MASTER.pk,
    examId,
    payload,
  );
}
...
export async function uploadExamNotificationFiles(
  ...
  if (feeNotificationFile)
    fd.append("feeNotificationFilePath", feeNotificationFile);
...
export async function saveExamFeeStructure(
  ...
  if (!body.success) {
    throw new AppError(
      "API_ERROR",
      body.message ?? "Failed to save exam fee structure",
    );
  }
  return body;
}
...
export async function updateExamFeeStructure(
  id: number,
  payload: Record<string, unknown>,
) {
...
export async function listExamFeeTypeGeneralDetails(): Promise<
  GeneralDetail[]
> {
...
export async function getExamTimetableDetails(
  courseYearId: number,
  courseId: number,
  examId: number,
): Promise<any> {
  ...
  const res = await fetch(
    NEXT_API.PROXY(
      `${EXAM_API.EXAM_TIMETABLE_DETAILS}?courseYearId=${courseYearId}&courseId=${courseId}&examId=${examId}`,
    ),
  );
...
export async function listExamMarksSetup(
  courseId: number,
  regulationId: number,
  disabled: boolean,
) {
  // Angular listDetailsByFourIds — no order clause
  return domainList(
...
export async function saveExamMarksSetup(
  rows: any[],
...
export async function saveExamTimetable(rows: any[]): Promise<{
  ok: boolean;
  statusCode: number;
  success: boolean;
  message: string;
