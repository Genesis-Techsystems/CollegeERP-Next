...
function arraysFromStudentByPattern(
  student: AnyRow,
  patterns: RegExp[],
): AnyRow[] {
  const rows: AnyRow[] = [];
  for (const [key, val] of Object.entries(student)) {
    ...
    for (const item of val) {
      if (item && typeof item === "object" && !Array.isArray(item))
        rows.push(item as AnyRow);
    }
  }
  return rows;
}

function mergeProfileArrays(
  student: AnyRow,
  keys: string[],
  patterns: RegExp[],
): AnyRow[] {
...
export function getStudentProfileContext(
  student: AnyRow,
...
function rowCourseYearId(row: AnyRow): number {
  ...
  if (
    nestedYear &&
    typeof nestedYear === "object" &&
    !Array.isArray(nestedYear)
  ) {
    const fromNested = num(nestedYear as AnyRow, [
      "courseYearId",
      "fk_course_year_id",
    ]);
...
function academicYearIdForSemester(
  ...
  for (const row of academicDetails) {
    ...
    if (
      toCy === courseYearId ||
      fromCy === courseYearId ||
      rowCourseYearId(row) === courseYearId
    ) {
      const ay = num(row, [
        "academicYearId",
        "fk_academic_year_id",
...
function resolveRegulationId(student: AnyRow): number {
  return deepNum(student, [
    "regulationId",
    "fk_regulation_id",
...
function subjectDedupeKey(row: AnyRow): string {
  const id = num(row, ["studentSubjectId", "student_subject_id", "id"]);
  if (id) return String(id);
  const cy = rowCourseYearId(row);
  return `${cy}-${text(row, ["subjectCode", "subject_code"])}-${text(row, ["academicYear", "academic_year"])}`;
}

function semesterLabelMatches(row: AnyRow, semesterLabel: string): boolean {
  if (!semesterLabel) return false;
  const code = text(row, ["courseYearCode", "course_year_code"]);
  const name = text(row, ["courseYearName", "course_year_name"]);
...
async function fetchAllStudentSubjects(
  ...
  await Promise.all(
    semesterList.map(async (sem) => {
      const ayId = academicYearIdForSemester(
        sem.courseYearId,
        academicDetails,
        ctx.academicYearId,
      );
      const rows = await fetchSubjectsForSemester(
        student,
        ctx,
        sem.courseYearId,
        ayId,
  ...
  return dedupeRows(merged, (r) => {
    ...
    return String(
      id ||
        `${cy}-${text(r, ["subjectCode", "subject_code"])}-${text(r, ["academicYear", "academic_year"])}`,
    );
  });
}

/** Angular `batchwiseElectiveUrl` — listByIds(studentId), filter by semester tab label. */
...
/** Angular `batchwiseStudentSecUrl` — listByIds(studentId), filter by semester tab label. */
...
export async function loadStudentCurriculumShell(
  student: AnyRow,
): Promise<StudentCurriculumShell> {
  const ctx = getStudentProfileContext(student);
  const courseId = resolveCourseId(student);
  const courseYears = courseId
    ? await listCourseYearsByCourse(courseId).catch(() => [])
    : [];
...
export async function loadStudentCurriculumSemester(
  ...
  const subjects = dedupeRows(rows, subjectDedupeKey);
  const label =
    semesterLabel ||
    text(student, ["courseYearName", "course_year_name", "courseYearCode"]) ||
    "";
  const electives = await fetchElectivesForSemester(ctx, label);
  const labBatches = await fetchLabBatchesForSemester(ctx, label);
  return { subjects, electives, labBatches };
}

export async function loadStudentCurriculumData(
  student: AnyRow,
): Promise<StudentCurriculumData> {
  const ctx = getStudentProfileContext(student);
  const courseId = resolveCourseId(student);
  ...
  const academicDetails = await fetchStudentAcademicDetails(student, ctx);
  const allSubjects = await fetchAllStudentSubjects(
    student,
    ctx,
    semesters,
    academicDetails,
  ...
  for (const sem of semesters) {
    ...
    const semesterSubjects = allSubjects.filter(
      (r) => rowCourseYearId(r) === cyId,
    );
    subjectsBySemester[cyId] = dedupeRows(semesterSubjects, subjectDedupeKey);
    electivesBySemester[cyId] = await fetchElectivesForSemester(ctx, sem.label);
    labBatchesBySemester[cyId] = await fetchLabBatchesForSemester(
      ctx,
      sem.label,
    );
  }
  ...
  return {
    ...
    subjectsBySemester,
    electivesBySemester,
    labBatchesBySemester,
    academicDetails,
  };
}

async function fetchStudentAcademicDetails(
  student: AnyRow,
  ctx: StudentProfileContext,
): Promise<AnyRow[]> {
...
async function loadTimetable(
  ...
  const { loadAngularStudentTimetable } =
    await import("@/services/student-timetable");
...
async function loadAttendance(
  ...
  if (
    ...
    try {
      const raw = await getAllRecords<unknown>(
        procName("getAllRecords/s_rep_tt_std_attendance_per"),
        {
  ...
  const procParams: Record<string, string | number>[] = [
...
async function loadCounselor(
  ...
  const mappings = await loadStudentCounselorMappings(student);
  if (mappings.length > 0) return flattenCounselorActivities(mappings);
...
async function loadBooks(
  ...
  try {
    const data = await fetchDetails<unknown>("bookIssuedetails", {
      studentId: ctx.studentId,
    });
    const rows = flattenRecordRows(data);
    if (rows.length > 0) return rows;
  ...
  try {
    const raw = await getAllRecords<unknown>(
      procName(DASHBOARD_API.LIBRARY_SUMMARY),
      {
        in_student_id: ctx.studentId,
        in_college_id: ctx.collegeId,
      },
...
export async function loadStudentExamResultsShell(
  student: AnyRow,
): Promise<{ semesters: StudentCurriculumSemester[] }> {
  const ctx = getStudentProfileContext(student);
  const courseId = resolveCourseId(student);
  const courseYears = courseId
    ? await listCourseYearsByCourse(courseId).catch(() => [])
  ...
  for (const cy of courseYears) {
    const cyId = num(cy, ["courseYearId", "fk_course_year_id"]);
    if (cyId === ctx.courseYearId) {
      currentSemNo = num(cy, ["semesterNo", "semester_no", "semNo", "sem_no"]);
      break;
    }
  }
  if (!currentSemNo) {
    currentSemNo = num(student, [
      "semNo",
      "sem_no",
      "semesterNo",
      "semester_no",
  ...
  let semesters: StudentCurriculumSemester[] = courseYears
    .filter((cy) => {
      const sem = num(cy, ["semesterNo", "semester_no", "semNo", "sem_no"]);
      return !currentSemNo || !sem || sem <= currentSemNo;
    })
    .map((cy) => ({
      courseYearId: num(cy, ["courseYearId", "fk_course_year_id"]),
      label: semesterLabel(cy),
...
export async function fetchStudentProfileAcademicDetails(
  student: AnyRow,
): Promise<AnyRow[]> {
  const nested = mergeProfileArrays(
    student,
    ...
    [
      /education|qualification|stdacademic|schoolacademic|pre.*academic/i,
      /academicdetail/i,
    ],
  );
...
async function loadPlacements(
  ...
  const queries: Record<string, string | number>[] = [
    { studentId: ctx.studentId },
    { studentId: ctx.studentId, isRegistered: "true" },
  ];
  for (const q of queries) {
    try {
      const data = await fetchDetails<unknown>("stdregdetails", q);
      const rows = flattenRecordRows(data);
      if (rows.length > 0) return rows;
    } catch {
      // try next
...
export async function loadStudentProfileTabData(
