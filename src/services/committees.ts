...
export async function listActiveCommittees(
  organizationId: number,
): Promise<UnivCommittee[]> {
  return domainList<UnivCommittee>(
    EC.name,
...
export async function listAllCommitteePositions(): Promise<
  UnivCommitteePosition[]
> {
  return domainList<UnivCommitteePosition>(
    EP.name,
...
export async function createCommitteePosition(
...
export async function getCommitteeDetailsByFlag(
  flag: string,
  overrides: Record<string, string | number> = {},
) {
  const data = await getAllRecords<{ result: CommitteeFilterRow[][] }>(
    COMMITTEE_PROC,
    {
      in_flag: flag,
      ...BASE_COMMITTEE_PROC,
...
export async function getCommitteeMemberFilters(): Promise<
  CommitteeFilterRow[]
> {
...
export async function getCommitteeCreateFormData() {
  const data = await getAllRecords<{ result: Record<string, unknown>[][] }>(
    ...
    {
      in_flag:
        "univ_exams,MasterUniversityDepartments,evaluationroles,committee_members",
      ...BASE_COMMITTEE_PROC,
    },
  ).catch(() => ({ result: [] as Record<string, unknown>[][] }));

  const committees = procRows<Record<string, unknown>>(data, 0);
  const exams = distinctBy(
    procRows<Record<string, unknown>>(data, 1),
    (r) => r.pk_university_exam_id,
  );
...
export async function getCommitteeExamSubjects(universityExamId: number) {
  const rows = await getCommitteeDetailsByFlag("univ_exam_subjects", {
    in_univ_exam_id: universityExamId,
  });
...
export async function getEvaluatorProfilesForRecruitment(params: {
  ...
  const rows = await getCommitteeDetailsByFlag(
    "committee_recruiitment_profiles",
    {
...
export async function listScheduledMeetings(params: {
  ...
  const data = await fetchDetails<{
    univCommitteeMeetingDetails?: UnivCommitteeMeeting[];
...
export async function addMultipleProfileRecruitments(
  payload: unknown[],
): Promise<unknown> {
...
export async function listRemunerationSettings(): Promise<
  UnivRemunerationSetting[]
> {
  // Angular `ListDetails(UnivRemunerationSettingUrl)` — unfiltered domain list.
...
/**
 * Angular `listDetailsByTwoIds(generalDetailsUrl, 'RemunerationDesignation', 'true', generalDetailsByCodeUrl, isActive)`.
 */
...
> {
  const rows = await listGeneralDetailsByMaster(
    GM_CODES.REMUNERATION_DESIGNATION,
  );
  return rows.map((r) => ({
    generalDetailId: Number(r.generalDetailId),
    generalDetailDisplayName: r.generalDetailDisplayName ?? r.generalDetailName,
    remunerationDesignationCatDetId: Number(r.generalDetailId),
    remunerationDesignationName:
      r.generalDetailDisplayName ?? r.generalDetailName,
    name: r.generalDetailDisplayName ?? r.generalDetailName,
  }));
}

/** Angular `addDetails(UnivRemunerationSettingUrl, details)` — parent also sets organizationId. */
export async function createRemunerationSetting(
...
/** Angular `updateDetails(..., univRemunerationSettingId, 'univRemunerationSettingId')`. */
export async function updateRemunerationSetting(
  id: number,
  data: Partial<UnivRemunerationSetting>,
): Promise<UnivRemunerationSetting> {
  return domainUpdate<UnivRemunerationSetting>(ERS.name, ERS.pk, id, {
    univRemunerationSettingId: id,
    ...data,
  });
}
...
export async function listUnivExamFilters(
  orgId: number,
  employeeId: number,
): Promise<UnivExamFilterRow[]> {
  const data = await getAllRecords<{ result: UnivExamFilterRow[][] }>(
    EXAM_EVAL_PROC,
    {
...
export async function bulkUpdateRemunerationStatus(
  payload: unknown[],
): Promise<unknown> {
...
export async function listRemunerationPaymentSummary(params: {
  ...
  const data = await getAllRecords<{ result: RemunerationPaymentSummary[][] }>(
    REM_REPORT_PROC,
    {
...
export async function submitRemunerationPayment(
  ...
  payload: RemunerationPaymentWritePayload,
): Promise<unknown> {
  return postDetails(
    `${UNIV_COMMITTEE_API.UPDATE_REMUNERATION_DETAILS}/${remunerationIds}`,
    payload,
  );
}
