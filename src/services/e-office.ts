...
async function domainGet<T>(entity: string, pkField: string, pkValue: number): Promise<T | null> {
...
async function postMultipart(path: string, formData: FormData): Promise<unknown> {
  const res = await fetch(`${PROXY}/${path}`, {
    method: 'POST',
    body: formData,
    credentials: 'include',
...
export async function createOfficeLetterFormat(
  ...
  return domainCreate<OfficeLetterFormatRow>(ENTITIES.OFFICE_LETTER_FORMATS.name, data)
}
...
export async function getInternalIndentById(internalIndId: number): Promise<InvInternalIndentRow | null> {
...
export async function createInternalIndent(payload: Record<string, unknown>): Promise<unknown> {
  return postDetails(E_OFFICE_API.INV_INTERNAL_INDENT, payload)
}

export async function updateInternalIndent(payload: Record<string, unknown>): Promise<unknown> {
...
export async function listInternalIndentTransactionTypes(): Promise<GeneralDetailOption[]> {
  const rows = await listGeneralDetailsByMaster('TRANSTYPE')
  return rows.filter(
    (r) => String(r.generalDetailCode ?? '').toUpperCase() === 'INTERNAL INDENT',
...
export async function getPurchaseOrderById(poId: number): Promise<InvPurchaseOrderRow | null> {
...
export async function listTransactionTypes(): Promise<GeneralDetailOption[]> {
  return listGeneralDetailsByMaster('TRANSTYPE') as Promise<GeneralDetailOption[]>
}

/** Indents approved for PO linkage (workflow stage 3 or 4). */
export async function listIndentsForPaymentNote(): Promise<InvInternalIndentRow[]> {
...
export async function createPurchaseOrderMultipart(
  ...
  if (files?.comparative) formData.append('poRefFileDoc1', files.comparative, files.comparative.name)
  if (files?.note) formData.append('poRefFileDoc2', files.note, files.note.name)
...
export async function completePurchaseOrder(poId: number, poStatusCatdetId: number): Promise<void> {
  await updatePurchaseOrderMultipart({ poId, poStatusCatdetId })
}

export async function getFinanceBudgetDetails(params: Record<string, string | number>): Promise<AnyRow> {
  const groups = await getAllRecords<AnyRow[][]>(E_OFFICE_API.FIN_BUDGET_DETAILS, params)
  return groups?.[0]?.[0] ?? {}
}
