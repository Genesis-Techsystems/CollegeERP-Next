...
export async function getPayrollCategoryById(
...
export async function listPayrollCategoriesByCollege(
  collegeId: number,
): Promise<AnyRow[]> {
  if (!collegeId) return [];
  const queries = [
 ...
 */
export async function getPayrollGroupById(
  payrollGroupId: number,
): Promise<AnyRow | null> {
  if (!payrollGroupId) return null;
  const rows = await domainList<AnyRow>(
    "PayrollGroup",
    buildQuery({ payrollGroupId }),
  );
  let row = rows[0] ?? null;
  const groups = row?.payrollCategoryGroups;
  if (row && (!Array.isArray(groups) || groups.length === 0)) {
    try {
      ...
      const fromList = all.find(
        (g) => Number(g.payrollGroupId) === payrollGroupId,
      );
      if (fromList) {
        row = {
          ...row,
...
export async function listLeaveTypeDurations(): Promise<AnyRow[]> {
  const queries = [
    buildQuery({
      "GeneralMaster.generalMasterCode": GM_CODES.LEAVE_TYPE_DURATION,
      isActive: true,
    }),
    buildQuery({
      generalMasterCode: GM_CODES.LEAVE_TYPE_DURATION,
      isActive: true,
    }),
  ];
  for (const q of queries) {
...
export async function getLeaveYears(): Promise<string[]> {
  const raw = await fetchDetails<Record<string, unknown>>(
    LEAVE_API.LEAVE_YEARS,
  );
...
export async function listLeaveTypesForEntitlement(
...
export async function listLeaveEntitlementsByDept(
  ...
  let path: string = LEAVE_API.LEAVE_ENTITLEMENT_BY_DEPT;
...
export function buildLeaveEntitlementEmployeeRows(
  ...
  return employees.map((emp) => {
    ...
    const existing = entitlements.filter(
      (e) => Number(e.employeeId) === employeeId,
    );
...
export async function getStaffPayrollList(
  ...
  const data = await getAllRecords<{ result: AnyRow[][] }>(
    HR_PAYROLL_API.STAFF_PAYROLL_LIST,
    params,
  );
...
export async function listAllActiveEmployeePayrollGroups(
  size = 999,
): Promise<AnyRow[]> {
  const data = await fetchDetails<unknown>(
    HR_PAYROLL_API.EMPLOYEE_PAYROLL_GROUP,
...
export async function listActiveEmployeesForPayrollAssign(params: {
  ...
  return rows.filter(
    (r) => !PAYROLL_ASSIGN_EXCLUDED_ROLES.has(String(r.roleName ?? "")),
  );
}
...
export async function calculatePayroll(
  ...
  const data = await postDetails<unknown>(
    HR_PAYROLL_API.CALCULATE_PAYROLL,
    payload,
  );
...
export async function getActiveEmployeeDetailById(
  employeeId: number,
): Promise<AnyRow | null> {
...
export async function getEmployeeDetailById(
  employeeId: number,
): Promise<AnyRow | null> {
  if (!employeeId) return null;
  const rows = await domainList<AnyRow>(
...
export async function listEmployeePayslipGenerationsByCollege(
  ...
  return all.filter((r) => {
    const cid = Number(
      r.collegeId ?? (r.college as AnyRow | undefined)?.collegeId ?? 0,
    );
...
export async function listEmployeeDetails(): Promise<AnyRow[]> {
  // Angular employee-list: listAllDetails → domain/list/EmployeeDetail?query=order(createdDt=desc)
  const data = await domainList<AnyRow>(
    ENTITIES.EMPLOYEE_DETAIL.name,
    buildQuery({}, { field: "createdDt", direction: "DESC" }),
  );
...
export async function sendEmployeeMails(
  rows: Array<{ employeeId: number; collegeId: number }>,
): Promise<unknown> {
  return postDetails(EMPLOYEE_API.SEND_MAILS, rows);
}
...
export async function listEmployeeReportingByEmployee(
  employeeId: number,
): Promise<AnyRow[]> {
...
export async function getEmployeeByIdForHr(
  employeeId: number,
): Promise<AnyRow | null> {
  if (!employeeId) return null;
...
export async function searchEmployeesForManagerAssign(
  term: string,
): Promise<AnyRow[]> {
  const q = term.trim();
  if (q.length < 4) return [];
...
export async function assignEmployeeReportingManager(
  payload: AnyRow,
): Promise<unknown> {
...
export async function listPerformanceAssessmentByEmployee(
  employeeId: number,
): Promise<AnyRow[]> {
  if (!employeeId) return [];
  const rows = await domainList<AnyRow>(
  ...
  return [...rows].sort(
    (a, b) =>
      Number(b.assessmentFeedbackId ?? 0) - Number(a.assessmentFeedbackId ?? 0),
  );
}

export async function listSelfAppraisalFormsByCollege(
  collegeId: number,
): Promise<AnyRow[]> {
  if (!collegeId) return [];
  return domainList<AnyRow>(
...
export async function saveSelfAppraisalFormDetails(
  details: AnyRow[],
): Promise<unknown> {
...
export async function getPerformanceAssessmentQuestions(): Promise<AnyRow[]> {
  const data = await fetchDetails<unknown>(
    HR_PAYROLL_API.GET_EMP_PERF_ASSESSMENT,
  );
  if (Array.isArray(data)) return data;
  return normalizeListPayload(data);
}
...
export async function listEmployeeShiftsByEmployee(
  employeeId: number,
): Promise<AnyRow[]> {
  if (!employeeId) return [];
  return domainList<AnyRow>(
...
export async function listActiveOrganizationsForEmployeeEnrollment(): Promise<
  AnyRow[]
> {
  return domainList<AnyRow>(
...
export async function listActiveCollegesByOrganizationForEmployeeEnrollment(
  organizationId: number,
): Promise<AnyRow[]> {
  if (!organizationId) return [];
  const queries = [
  ...
  for (const q of queries) {
    try {
      const rows = await domainList<AnyRow>(ENTITIES.COLLEGE.name, q);
      if (rows.length > 0) return rows;
    } catch {
      // next query shape
    }
...
export async function listEmployeeEnrollmentGeneralDetails(
  code: string,
): Promise<AnyRow[]> {
  return listGeneralDetailsByCode(code);
}

export async function listActiveCastesForEmployeeEnrollment(): Promise<
  AnyRow[]
> {
  return domainList<AnyRow>(
    ENTITIES.CASTE.name,
...
export async function listSubCastesByCasteForEmployeeEnrollment(
  casteId: number,
): Promise<AnyRow[]> {
  if (!casteId) return [];
  const queries = [
  ...
  for (const q of queries) {
    try {
      const rows = await domainList<AnyRow>(ENTITIES.SUB_CASTE.name, q);
      if (rows.length > 0) return rows;
    } catch {
      // next query shape
    }
...
export async function listDepartmentsByCollegeForEmployeeEnrollment(
  collegeId: number,
): Promise<AnyRow[]> {
  if (!collegeId) return [];
  const queries = [
  ...
  for (const q of queries) {
    try {
      const rows = await domainList<AnyRow>(ENTITIES.DEPARTMENT.name, q);
      if (rows.length > 0) return rows;
    } catch {
      // next query shape
    }
...
export async function listQualificationsByOrganizationForEmployeeEnrollment(
  organizationId: number,
): Promise<AnyRow[]> {
  if (!organizationId) return [];
  const queries = [
  ...
  for (const q of queries) {
    try {
      const rows = await domainList<AnyRow>(ENTITIES.QUALIFICATION.name, q);
      if (rows.length > 0) return rows;
    } catch {
      // next query shape
    }
...
export async function listCountriesForEmployeeEnrollment(): Promise<AnyRow[]> {
  return domainList<AnyRow>(
    ENTITIES.COUNTRY.name,
    buildQuery({ isActive: true }),
  );
}

export async function listStatesByCountryForEmployeeEnrollment(
  countryId: number,
): Promise<AnyRow[]> {
  if (!countryId) return [];
  const queries = [
    buildQuery({ "Country.countryId": countryId, isActive: true }),
    buildQuery({ countryId, isActive: true }),
  ];
  for (const q of queries) {
    try {
      const rows = await domainList<AnyRow>(ENTITIES.STATE.name, q);
      if (rows.length > 0) return rows;
    } catch {
      // next query shape
    }
...
export async function listDistrictsByStateForEmployeeEnrollment(
  stateId: number,
): Promise<AnyRow[]> {
  if (!stateId) return [];
  const queries = [
    buildQuery({ "State.stateId": stateId, isActive: true }),
    buildQuery({ stateId, isActive: true }),
  ];
  for (const q of queries) {
    try {
      const rows = await domainList<AnyRow>(ENTITIES.DISTRICT.name, q);
      if (rows.length > 0) return rows;
    } catch {
      // next query shape
    }
...
export async function listCitiesByDistrictForEmployeeEnrollment(
  districtId: number,
): Promise<AnyRow[]> {
  if (!districtId) return [];
  const queries = [
    buildQuery({ "District.districtId": districtId, isActive: true }),
    buildQuery({ districtId, isActive: true }),
  ];
  for (const q of queries) {
    try {
      const rows = await domainList<AnyRow>(ENTITIES.CITY.name, q);
      if (rows.length > 0) return rows;
    } catch {
      // next query shape
    }
...
export async function listEmployeeDocumentsByCollegeForEnrollment(
  collegeId: number,
): Promise<AnyRow[]> {
  if (!collegeId) return [];
  // Angular: isForEmp==true.and.isActive==true.and.College.collegeId=={collegeId}
  return domainList<AnyRow>(
...
export async function getEmployeeEnrollmentById(
  employeeId: number,
): Promise<AnyRow | null> {
  if (!employeeId) return null;
  const rows = await domainList<AnyRow>(
...
export async function createEmployeeEnrollment(
  payload: AnyRow,
): Promise<unknown> {
  return postDetails(EMPLOYEE_API.EMPLOYEEAPPLICATION, payload);
}

export async function updateEmployeeEnrollment(
  payload: AnyRow,
): Promise<unknown> {
  return putDetails(EMPLOYEE_API.EMPLOYEEAPPLICATION, payload);
}

export async function uploadEmployeeEnrollmentFiles(
  formData: FormData,
): Promise<unknown> {
