...
export async function getSeatingPlanRows(
  ...
  return domainList<ExamRoomAllotment>('ExamRoomAllotment', query)
}
 ...
 *
 * The Spring Boot payload uses nested FK objects:
 * {
 *   ExamMaster: { examId },
...
export async function updateSeatingPlanRow(
  ...
  const payload = buildSeatingPayload(data)
  return domainUpdate<ExamRoomAllotment>('ExamRoomAllotment', 'examRoomAllotmentId', id, payload)
}
...
export async function deleteSeatingPlanRow(id: number): Promise<void> {
  return domainSoftDelete('ExamRoomAllotment', 'examRoomAllotmentId', id)
}
...
export async function getRooms(): Promise<RoomLookup[]> {
  ...
  return domainList<RoomLookup>('Room', query)
}
