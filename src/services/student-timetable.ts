...
export async function fetchWeeklyTimetableReportRows(ids: WeeklyTimetableReportIds): Promise<AnyRow[]> {
  const {
    collegeId,
    courseId,
    courseGroupId,
  ...
  try {
    const raw = await getAllRecords<unknown>(
      procNameFromStoredProcPath(TIMETABLE_REPORT_API.REP_TT_GET_TIMETABLE_DETAILS),
      {
...
function unwrapApiData(body: ApiResponse<unknown> & { result?: unknown; resultList?: unknown }): unknown {
  const hasPayload =
    body?.data != null ||
...
async function fetchProxyPayload(path: string, params: Record<string, string | number>): Promise<unknown> {
  const res = await fetch(`/api/proxy/${path}${toQuery(params)}`)
  if (!res.ok) {
    throw new Error(`Timetable request failed (${res.status})`)
  }
  const body = (await res.json()) as ApiResponse<unknown> & { result?: unknown }
...
function formatClockAmPm(value: unknown): string {
  ...
  if (!raw) return ''
  if (/AM|PM/i.test(raw)) {
    return raw.replace(/\s+/g, ' ').replace(/\b(am|pm)\b/gi, (m) => m.toUpperCase())
  }
...
function formatTimeLabel(row: AnyRow): string {
  const from = text(row, ['fromTime', 'startTime', 'from_time', 'periodFromTime', 'period_from_time'])
  const to = text(row, ['toTime', 'endTime', 'to_time', 'periodToTime', 'period_to_time'])
...
function breakVariantFromLabel(label: string): 'short' | 'lunch' | 'tea' | 'other' {
...
function isBreakRow(row: AnyRow): boolean {
  ...
  const periodType = text(row, ['periodType', 'period_type', 'sessionType', 'session_type']).toUpperCase()
...
function subjectTitle(row: AnyRow): string {
  const code = text(row, ['subjectCode', 'subject_code', 'subCode', 'sub_code', 'Subject_Code'])
  const name = text(row, [
    'subjectName',
    'subject_name',
    'Subject_Name',
...
function slotKey(row: AnyRow): string {
  const period = num(row, ['periodNo', 'period_no', 'periodNumber', 'period', 'Period', 'periodId', 'period_id'])
  const from = parseTimeValue(
    row.fromTime ?? row.startTime ?? row.from_time ?? row.periodFromTime ?? row.period_from_time,
  )
  const to = parseTimeValue(row.toTime ?? row.endTime ?? row.to_time ?? row.periodToTime ?? row.period_to_time)
...
function dayFromPropertyKey(key: string): string {
  const normalized = key.trim().toLowerCase().replace(/[^a-z]/g, '')
...
function dayFromDetailListKey(key: string): string {
  ...
  for (const { prefix, day } of DAY_KEY_PREFIXES) {
    ...
    if (
      ...
      /(details?|list|slots|subjects|cells|entries|dto|data|wise)/i.test(normalized.slice(prefix.length))
    ) {
      return day
    }
  }
...
function resolveDayFromRow(row: AnyRow): string {
  ...
  if (weekDayObj && typeof weekDayObj === 'object' && !Array.isArray(weekDayObj)) {
...
function expandPerDayDetailArrays(row: AnyRow, inheritedPeriod: AnyRow): AnyRow[] {
  const slots: AnyRow[] = []
  for (const [key, value] of Object.entries(row)) {
    ...
    for (const item of value) {
      if (item && typeof item === 'object' && !Array.isArray(item)) {
        slots.push({ ...inheritedPeriod, ...row, ...(item as AnyRow), weekDay: day })
      }
    }
  }
  return slots
...
function expandWideMatrixRow(row: AnyRow): AnyRow[] {
  ...
  for (const [key, value] of Object.entries(row)) {
    ...
    if (!day) {
      if (!dayFromPropertyKey(key) && !/^(mon|tue|wed|thu|fri|sat|sun)/i.test(key)) {
        shared[key] = value
      }
      continue
    }
...
export function expandTimetableRows(data: unknown): AnyRow[] {
  ...
  function walk(node: unknown, inheritedDay = '', inheritedPeriod: AnyRow = {}): void {
    if (node == null) return
    ...
    if (Array.isArray(gridSource) && gridSource.length > 0) {
      const dayLabels = (Array.isArray(row.weekDays) ? row.weekDays : row.weekdays ?? DAY_ORDER) as unknown[]
    ...
    if (Array.isArray(row.weekDays) && Array.isArray(row.periods)) {
      for (const period of row.periods as AnyRow[]) {
        ...
        for (let i = 0; i < cells.length; i++) {
          ...
          walk({ ...row, ...period, ...(cells[i] as AnyRow), weekDay: day }, day, row)
        }
      }
      return
    }
    ...
    const schedule = row.schedule ?? row.scheduleList ?? row.periods ?? row.periodList
    ...
    const hasSlotData =
      ...
      num(row, ['periodNo', 'period_no', 'periodNumber', 'period', 'Period']) > 0
...
function collectSlotLikeRows(data: unknown): AnyRow[] {
  ...
  function collect(node: unknown): void {
    ...
    const hasPeriod =
      num(row, ['periodNo', 'period_no', 'periodNumber', 'period', 'Period']) > 0 ||
      Boolean(text(row, ['fromTime', 'from_time', 'periodFromTime', 'period_from_time', 'startTime']))
 ...
 */
export async function fetchStudentTimetableRows(params: StudentTimetableFetchParams): Promise<AnyRow[]> {
  const {
    studentId = 0,
    collegeId = 0,
    academicYearId = 0,
  ...
  if (studentId) {
    ...
    if (collegeId && academicYearId) attempts.push({ studentId, collegeId, academicYearId })
  }
  ...
  if (groupSectionId && collegeId && academicYearId) {
    ...
    if (courseGroupId) attempts.push({ collegeId, academicYearId, groupSectionId, courseGroupId })
    if (courseYearId) attempts.push({ collegeId, academicYearId, groupSectionId, courseYearId })
  }

  const paths = ['timetablescurr', 'timetablesCurr', 'studenttimetable', 'sectiontimetable']
...
export function timetableDateRangeLabel(student: AnyRow, rows: AnyRow[]): string {
  const fromKeyGroups = [
    [
  ...
  let from = pickFromRow(student, [...fromKeyGroups[0]]) || pickFromNestedAcademicYear([...fromKeyGroups[0]])
  let to = pickFromRow(student, [...fromKeyGroups[1]]) || pickFromNestedAcademicYear([...fromKeyGroups[1]])
  ...
  if (!from || !to) {
    if (rows.length > 0) {
      const fromRow =
        rows.find((r) => {
          const x =
            r.fromDate ?? r.startDate ?? r.timetableFromDate ?? r.effectiveFromDate ?? r.Academic_Year_From
          return x != null && String(x).trim() !== ''
        }) ?? rows[0]
      const toRow =
        rows.find((r) => {
          const x = r.toDate ?? r.endDate ?? r.timetableToDate ?? r.effectiveToDate ?? r.Academic_Year_To
          return x != null && String(x).trim() !== ''
        }) ?? rows[0]
      if (!from && fromRow) {
        from =
...
export function buildStudentTimetableGrid(rowsInput: unknown, student?: AnyRow): StudentTimetableGrid {
  const rows = Array.isArray(rowsInput)
    ? (rowsInput as AnyRow[])
    : normalizeTimetableRows(rowsInput)
  const daySet = new Set<string>()
  const slotMap = new Map<string, { sortOrder: number; byDay: Map<string, AnyRow[]> }>()
  ...
  const gridRows: TimetableGridRow[] = [...slotMap.entries()]
    ...
    .map(([key, slot]) => {
      const cells: TimetableGridCell[] = dayLabels.map((day) => {
        ...
        const entries: TimetableSlotEntry[] = dayRows.filter((r) => !isBreakRow(r)).map((r) => ({
          title: subjectTitle(r),
          faculty: facultyNames(r),
          room: roomCode(r),
        }))

        const timeLabel = formatTimeLabel(dayRows.find((r) => formatTimeLabel(r)) ?? dayRows[0])
        return {
  ...
  return {
    dayLabels,
    rows: gridRows,
    dateRangeLabel: student ? timetableDateRangeLabel(student, rows) : timetableDateRangeLabel({}, rows),
  }
}
...
function mapSubjectResource(res: AnyRow): TimetableSubBatch {
  return {
    ...
    studentBatchName: text(res, ['studentBatchName', 'student_batch_name', 'batchName']),
    shortName: text(res, ['shortName', 'short_name', 'subjectShortName']),
    subjectCode: text(res, ['subjectCode', 'subject_code']),
    staffName: text(res, ['staffName', 'staff_name', 'employeeName', 'facultyName']),
...
function buildSubBatches(subjectResource: AnyRow[]): TimetableSubBatch[] {
  ...
  for (const res of subjectResource) {
    const typeCode = text(res, ['subjectTypeCode', 'subject_type_code']).toUpperCase()
    ...
    if (typeCode === 'LAB' && batchId > 0) {
      ...
      if (existing) {
        ...
        if (extra) {
          existing.staffName = existing.staffName ? `${existing.staffName} , ${extra}` : extra
        }
      } else {
...
/** Break cells use neutral tones — not the weekday pastel (Angular differentiates breaks). */
export function timetableBreakCellBg(classTimingName: string, isBreak: boolean): string {
  if (!isBreak) return ''
  const u = classTimingName.trim().toUpperCase()
  if (u.includes('LUNCH')) return '#FFF3CD'
  if (u.includes('SHORT')) return '#E9ECEF'
  if (u.includes('TEA')) return '#D1ECF1'
  return '#F5F5F5'
}

function mapScheduleTiming(timing: AnyRow, subBatches: TimetableSubBatch[]): TimetableDayTiming {
  const weekdayName = text(timing, ['weekdayName', 'weekday_name', 'weekDayName'])
  const resources = subjectResourcesFromTiming(timing)
  const cellGroupId = resources[0] ? text(resources[0], ['cellGroupId', 'cell_group_id']) : ''
  const isBreak = Boolean(timing.isBreak ?? timing.is_break)
  const classTimingName = text(timing, ['classTimingName', 'class_timing_name', 'periodName'])

  return {
    weekdayId: num(timing, ['weekdayId', 'fk_weekday_id', 'weekday_id']),
    weekdayName,
    startTime: text(timing, ['startTime', 'start_time', 'fromTime', 'from_time']),
...
export function buildAngularStudentTimetable(
  ...
  type WeekdayAcc = { weekdayId: number; weekdayName: string; classTimings: AnyRow[] }
  ...
  for (const raw of scheduleTimings) {
    ...
    if (existing) {
      ...
      weekdayMap.set(weekdayId, { weekdayId, weekdayName, classTimings: [timing] })
    }
  }
  ...
  for (const weekday of weekdayMap.values()) {
    ...
    for (const classTiming of weekday.classTimings) {
      ...
      if (resources.length > 0 && cellGroupId) {
        const existing = timings.find((t) => t.cellGroupId && t.cellGroupId === cellGroupId)
        if (existing) {
  ...
  let dateRangeLabel = ''
  if (timetableMeta) {
    const from = formatHeaderDate(timetableMeta.startDate ?? timetableMeta.start_date)
    const to = formatHeaderDate(timetableMeta.endDate ?? timetableMeta.end_date)
 ...
 */
export async function loadAngularStudentTimetable(student: AnyRow): Promise<AngularStudentTimetable | null> {
  const collegeId = num(student, ['collegeId', 'fk_college_id'])
  const academicYearId = num(student, ['academicYearId', 'fk_academic_year_id'])
  const groupSectionId = num(student, ['groupSectionId', 'fk_group_section_id', 'group_section_id'])
  if (!collegeId || !academicYearId || !groupSectionId) return null

  const timetables = await fetchTimetableScurrList(collegeId, academicYearId, groupSectionId)
  if (timetables.length === 0) return null

  const timetable =
    timetables.find((t) => num(t, ['timetableId', 'fk_timetable_id']) > 0) ?? timetables[0]
...
export function timetableCellHeightPx(startTime: string, endTime: string): number {
  const start = parseTimeValue(startTime)
  const end = parseTimeValue(endTime)
  if (start >= Number.MAX_SAFE_INTEGER || end >= Number.MAX_SAFE_INTEGER) return 96
