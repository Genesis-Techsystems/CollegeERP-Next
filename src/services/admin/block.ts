...
type BlockWriteInput = Partial<Omit<Block, "blockId">> &
...
export async function createBlock(
  data: Omit<Block, "blockId">,
): Promise<{ data: Block; message: string }> {
  const payload = buildAngularBlockPayload(data);
  return domainCreateResult<Block>(ENTITIES.BLOCK.name, payload);
}
...
export async function updateBlock(
  ...
  return domainUpdateResult<Block>(
    ENTITIES.BLOCK.name,
    ENTITIES.BLOCK.pk,
    blockId,
    payload,
