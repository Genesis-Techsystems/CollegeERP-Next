...
export async function listGeneralUserAccountsByCollege(
  collegeId: number,
...
export async function getGeneralUserAccountById(
  userId: number,
): Promise<GeneralUserAccount | null> {
  const rows = await domainList<GeneralUserAccount>(
    "User",
    buildQuery({ userId }),
  );
  return rows[0] ?? null;
...
export async function listUserTypesByOrganization(
  organizationId: number,
): Promise<UserTypeOption[]> {
  ...
  const rows = await fetchRows(
    `/api/proxy/domain/list/Usertype?size=99999&query=${encodeURIComponent(query)}`,
  );
  ...
  return rows
    .map((row): UserTypeOption | null => {
      ...
      const nested = (src.UserType ?? src.userType ?? null) as Record<
        string,
        unknown
      > | null;
      ...
      return {
        ...
        organizationId:
          Number(
            src.organizationId ?? src.orgId ?? nested?.organizationId ?? 0,
          ) || undefined,
...
/**
 * Angular UserRolesModal `getRoles`:
 * `user.userId=={id}.and.isActive==true.order(createdDt=DESC)`
 * (query key is lowercase `user.userId`, not `User.userId`).
 */
