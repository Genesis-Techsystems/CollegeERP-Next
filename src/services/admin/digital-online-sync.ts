...
export async function getCourseYearsForDigitalSync(params: {
  ...
  for (const query of paramVariants) {
    try {
      const rows = await fetchDetails<Array<Record<string, unknown>>>(
        "courseyears",
        query,
      );
...
function extractIdsFromCourseYearRows(
  rows: Array<Record<string, unknown>>,
): Set<number> {
  const ids = new Set<number>();
  for (const row of rows) {
    const regLists = [row.subjectregulations, row.subjectRegulations].filter(
      Array.isArray,
...
export async function collectSubjectRegulationIdsForSync(params: {
  ...
  const courseYearRows = (await getCourseYearsForDigitalSync(params)) as Array<
...
export async function syncSubjectRegulationIds(
  ...
  const data = await postDetails<unknown>(
    SUBJECT_API.SUBJECT_REGULATION_DATA_SYNC,
    payload,
  );
