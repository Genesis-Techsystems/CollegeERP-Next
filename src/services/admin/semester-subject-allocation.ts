...
export async function listRegulationsByCourse(
  courseId: number,
): Promise<AnyRow[]> {
  if (!courseId) return [];
  // Angular: listDetailsByTwoIdsWithSort(Regulation, courseId, true, desc, Course.courseId, isActive, regulationCode)
...
export async function listGroupSections(
  ...
  // Angular: listDetailsByFourIds(GroupSection, courseYearId, academicYearId, courseGroupId, true, ...)
  return domainList<AnyRow>(
...
export async function listSubjectRegulationsByRegulation(params: {
  ...
  if (
    ...
    return [];
  // Angular getSubjectRegulationDetailsByReg: subjectregulations/?collegeId&academicYearId&coursegroupId&courseyearId&regulationId
...
export async function listMapRegulationSubjects(params: {
  ...
  const { universityId, courseId, courseGroupId, courseYearId, regulationId } =
    params;
  if (
    !universityId ||
    !courseId ||
