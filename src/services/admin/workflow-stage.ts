...
type WorkflowStageWriteInput = Partial<Omit<WorkflowStage, "workflowStageId">> &
...
function buildAngularWorkflowStagePayload(
  ...
  const isSelfAvailable =
    data.isSelfAvailable === true || data.isSelfAvailable === 1;
  const availableForRaw = data.availableFor ?? existing?.availableFor;
  const availableForNum =
    availableForRaw == null || String(availableForRaw).trim() === ""
      ? null
      : Number(availableForRaw);
  const goBackPointRaw = data.goBackPoint ?? existing?.goBackPoint;
  const goBackPointNum =
    goBackPointRaw == null ||
    goBackPointRaw === false ||
    String(goBackPointRaw).trim() === ""
      ? null
      : Number(goBackPointRaw);
  ...
  const payload: Record<string, unknown> = {
    ...
    goBackPoint: Number.isFinite(goBackPointNum) ? goBackPointNum : null,
...
export async function listActiveOrganizationsForWorkflowStages(): Promise<
  Organization[]
> {
