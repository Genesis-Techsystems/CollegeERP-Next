...
function buildAngularUniversityPayload(
  ...
  const payload: Record<string, unknown> = {
    ...
    reportLine1: asNullableString(data.reportLine1 ?? existing?.reportLine1),
    reportLine2: asNullableString(data.reportLine2 ?? existing?.reportLine2),
    reportLine3: asNullableString(data.reportLine3 ?? existing?.reportLine3),
    isActive,
