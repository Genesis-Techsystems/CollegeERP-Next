...
import { buildQuery, domainList, getAllRecords, postDetails, putDetails } from '../crud'
...
function flattenProcRows(payload: Record<string, unknown> | undefined): AnyRow[] {
  if (!payload) return []
  const groups = [
    payload.result,
  ...
  for (const group of groups) {
    ...
    for (const item of group) {
      if (Array.isArray(item)) {
        for (const row of item) if (row && typeof row === 'object') out.push(row as AnyRow)
...
export async function addRoomDetailsList(payload: RoomDevicePayload[]): Promise<void> {
...
export async function listActiveBlocksByBuilding(buildingId: number): Promise<Block[]> {
...
export async function listActiveFloorsByBlock(blockId: number): Promise<Floor[]> {
  if (!blockId) return []
...
export async function listActiveEttlDevices(): Promise<EttlDevice[]> {
  try {
    return await domainList<EttlDevice>(ROOM_DETAILS_API.ETTL_DEVICES, buildQuery({ isActive: true }))
  } catch {
