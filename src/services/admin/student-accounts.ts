...
import { buildQuery, domainList, domainUpdate, fetchDetails } from '@/services/crud'
...
function departmentIdFromStudentRow(s: Record<string, unknown>): number | undefined {
  let id = Number(s.departmentId ?? s.fk_department_id ?? s.fk_departmentId ?? 0) || 0
  if (id) return id
...
function extractStudentAccountFromCreateResponse(body: unknown): StudentAccount | null {
 ...
 */
async function createStudentUserViaCmsApi(payload: Record<string, unknown>): Promise<StudentAccount> {
  ...
  if (body && typeof body === 'object' && 'success' in body) {
    ...
    if (env.success === false) {
      throw new AppError('API_ERROR', env.message ?? 'Failed to create student user')
    }
  }
  ...
  if (body && typeof body === 'object' && 'success' in body && (body as ApiResponse<unknown>).success === true) {
    return { userId: 0 }
  }
  throw new AppError('API_ERROR', 'Create user succeeded but the response had no user id.')
}
 ...
 */
export async function listStudentAccountsPage(page: number, pageSize: number): Promise<StudentAccountsPage> {
  ...
  if (body.success === false) {
    throw new AppError('API_ERROR', body.message ?? 'Failed to load student accounts')
  }
  ...
  if (Array.isArray(d)) {
    ...
    if (chunk) {
      ...
      if (Array.isArray(list)) rows = list as StudentAccount[]
...
export async function getStudentAccountById(userId: number): Promise<StudentAccount | null> {
  const q = buildQuery({ userId, 'Usertype.userTypeCode': STUDENT_USER_TYPE_CODE })
  const rows = await domainList<StudentAccount>('User', q)
  return rows[0] ?? null
}

export async function updateStudentAccount(userId: number, data: Partial<StudentAccount>): Promise<StudentAccount> {
  return domainUpdate<StudentAccount>('User', 'userId', userId, { ...data, userId })
}
...
function mergeStudentRow(row: Record<string, unknown>): Record<string, unknown> {
  const nestedBlocks = [
    row.studentDetail,
    row.StudentDetail,
    row.studentProfile,
    ...
    Boolean(v) && typeof v === 'object' && !Array.isArray(v)) as Record<string, unknown>[]
...
function hallticketForPick(m: Record<string, unknown>): string {
  return str(
    m.hallticketNumber ?? m.hallTicketNumber ?? m.rollNumber ?? m.roll_no ?? m.admissionNumber ?? m.hallTicket,
  )
}
...
function rowsFromSearchPayload(data: unknown): Record<string, unknown>[] {
  ...
  if (data && typeof data === 'object') {
    ...
    if (Array.isArray(d.resultList)) return d.resultList as Record<string, unknown>[]
...
function mapSearchRowsToPicks(rawRows: Record<string, unknown>[]): StudentForAccountPick[] {
  ...
  for (const raw of rawRows) {
    ...
    const fn = str(merged.firstName ?? merged.first_name) || str(merged.studentName) || display || undefined
...
export async function searchStudentsByCollegeForStudentAccount(
  ...
  try {
    const data = await fetchDetails<unknown>(USER_MANAGEMENT_API.STUDENT_SEARCH, { collegeId, q })
    return mapSearchRowsToPicks(rowsFromSearchPayload(data))
  } catch {
    try {
      const data = await fetchDetails<unknown>('studentsearch', { collegeId, q, isActive: 'true' })
      return mapSearchRowsToPicks(rowsFromSearchPayload(data))
    } catch {
      return []
    }
 ...
 */
export async function listStudentsByCollegeForStudentAccount(collegeId: number): Promise<StudentForAccountPick[]> {
  if (!collegeId) return []
  const queries = [
...
export async function createStudentUserAccountForStudent(params: {
  ...
  const rows = await domainList<Record<string, unknown>>('Student', buildQuery({ studentId }))
  const raw = rows[0]
  ...
  const studType = types.find((t) => String(t.userTypeCode ?? '').toUpperCase() === STUDENT_USER_TYPE_CODE)
  if (!studType?.userTypeId) {
    throw new AppError('CONFIG', 'No STUDENT user type is configured for this organization.')
  }

  const hallDefault = str(
    s.hallticketNumber ?? s.hallTicketNumber ?? s.rollNumber ?? s.roll_no ?? s.admissionNumber,
  )
  ...
  if (account) {
    firstName = str(account.firstName) || str(s.firstName ?? s.studentName) || hallDefault
    ...
    email = emailFromForm || emailFromStudentRow(s) || (userName ? `${userName}@student.local` : '')
    ...
    if (!pwd || pwd !== pwdConfirm) {
      throw new AppError('VALIDATION', 'Password and confirm password must match.')
    }
  } else {
    if (!hallDefault) {
      throw new AppError('VALIDATION', 'Student has no hall ticket / roll number to use as user name.')
    }
    ...
    mobileNumber = str(s.mobileNumber ?? s.mobile_no ?? s.student_mobile_no) || undefined
    pwd = 'Student@123'
    pwdConfirm = pwd
  }
