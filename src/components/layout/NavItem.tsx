...
export function NavItem({ item, depth = 0, layoutHydrated }: NavItemProps) {
  ...
  const forcedRoute = (() => {
    ...
    // Admin → Institutional Masters (Buildings / Blocks / Floors / Rooms…)
    // must beat sidebar pins that map the same labels to Exam Papers Delivery.
    ...
    if (examinationSectionRoute) return examinationSectionRoute;
    ...
    if (
      ...
      (hrefLower.includes("student-academics") &&
        (labelLower.includes("my subject") ||
          labelLower.replace(/[^a-z0-9]+/g, " ").trim() === "subjects"))
    ) {
    ...
    if (!hasChildren) {
      ...
      // Angular Accounts & Fees — Fee Receipts list.
      if (
        hrefLower.includes("fees-collection/fee-receipts") ||
      ...
      if (
        ...
        (labelLower.includes("academic history") &&
          labelLower.includes("report"))
      ) {
      ...
      // Attendance Reports (Angular admin-attendance-reports)
      if (
      ...
      if (
        ...
        (labelLower.includes("library") &&
          ...
          labelLower.includes("collection") &&
          (labelLower.includes("day") || labelLower.includes("list")))
      ) {
      ...
      // Finance Reports — Day-Wise Expenses
      if (
      ...
      const adminInstitutionalRoom = mapAdminInstitutionalRoomRoute(
        item.href,
        item.label,
      );
    ...
    if (
      (labelLower.includes("college courses") &&
        labelLower.includes("group")) ||
