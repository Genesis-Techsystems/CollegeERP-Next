...
import {
  ensureErpModuleNavChildren,
  mapErpModuleNavRoute,
} from "./erp-modules-navigation";
import {
...
export {
  EXAMINATION_SECTION_BASE,
  isExaminationSectionPath,
  mapExaminationSectionNavRoute,
} from "./examination-section-navigation";
...
export function normalizeHref(path: string): string {
  ...
  raw = raw
    ...
    // Angular Fee Masters — folder `fee-category` vs route `fee-categories`.
    .replace(
      /\/fee-masters\/fee-category(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/fee-categories",
    )
    .replace(
      /\/fee-masters\/fee-categories(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/fee-categories",
    )
    .replace(
      /\/fee-masters\/fee-particulars(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/fee-particular",
    )
    .replace(
      /\/fee-masters\/fee-particular(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/fee-particular",
    )
    .replace(
      /\/fee-masters\/fee-structure(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/fee-structure",
    )
    .replace(
      /\/fee-masters\/add-feeStructure(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/add-fee-structure",
    )
    .replace(
      /\/fee-masters\/edit-feeStructure(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/edit-fee-structure",
    )
    .replace(
      /\/fee-masters\/university-fee-structure-details(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/university-fee-structure/details",
    )
    .replace(
      /\/fee-masters\/university-fee-structure(?=\/|$)/gi,
      "/accounts-and-fees/fee-masters/university-fee-structure",
    )
    // Angular Wallet module — legacy slugs map to Angular-parity App Router paths.
    .replace(
      /\/wallet\/university-wallet-transactions(?=\/|$)/gi,
      "/wallet/university-payment-wallet-transactions",
    )
    .replace(
      /\/wallet\/wallet-transactions(?=\/|$)/gi,
      "/wallet/university-payment-wallet-transactions",
    )
    .replace(
    ...
    .replace(
      /\/apps\/admin-attendance-management\//gi,
      "/attendance-management/",
    )
    .replace(
      /\/staff-classes\/attendance-update\//gi,
    ...
    .replace(
      /\/notifications-&-announcements\//gi,
      "/notifications-and-announcements/",
    )
    .replace(
      /\/notifications-%26-announcements\//gi,
      "/notifications-and-announcements/",
    )
    .replace(
    ...
    .replace(
      /\/time-table-management\/timetable\//gi,
    ...
    .replace(
      /\/email-sms\/depart(?:ment)?-wise-email(?=\/|$)/gi,
      "/email-sms/department-wise-email",
    )
    /** Shorthand slug sometimes used in menus. */
    .replace(
      /\/email-sms\/dept-wise-email(?=\/|$)/gi,
      "/email-sms/department-wise-email",
    )
    /** Angular source folder `principal-to-dpt-email` (abbrev.) → App Router slug. */
    .replace(
      /\/email-sms\/principal-to-dpt-email(?=\/|$)/gi,
      "/email-sms/principal-to-dept-email",
    )
    /** Legacy casing `principal-to-Dept-email` → canonical segment. */
    .replace(
      /\/email-sms\/principal-to-dept-email(?=\/|$)/gi,
      "/email-sms/principal-to-dept-email",
    )
    /** Common menu typo `emial` → `email`. */
    .replace(
      /\/email-sms\/department-wise-emial(?=\/|$)/gi,
      "/email-sms/department-wise-email",
    )
    .replace(
      /\/email-sms\/send-student-sms(?=\/|$)/gi,
      "/email-sms/send-sms-to-students",
    )
    .replace(
      /\/email-sms\/send-login-details(?=\/|$)/gi,
      "/email-sms/send-login-details",
    )
    .replace(
      /\/email-sms\/send-absent-sms(?=\/|$)/gi,
      "/email-sms/send-sms-to-absents",
    )
    .replace(
      /\/email-sms\/send-staff-sms(?=\/|$)/gi,
      "/email-sms/send-sms-to-staff-attendance",
    )
    ...
    .replace(
      /\/email-sms\/principal-and-staff-to-admin-email(?=\/|$)/gi,
      "/email-sms/principal-staff-to-admin-email",
    )
    .replace(
      /\/email-sms\/principal-staff-to-admin-emial(?=\/|$)/gi,
      "/email-sms/principal-staff-to-admin-email",
    )
    .replace(
      /\/email-sms\/staff-principal-to-admin-email(?=\/|$)/gi,
      "/email-sms/principal-staff-to-admin-email",
    )
    /** Angular `principal-to-staff-email` — minimal "Send Email To Admin" screen. */
    .replace(
      /\/email-sms\/send-email-to-admin(?=\/|$)/gi,
      "/email-sms/principal-to-staff-email",
    )
    .replace(
      /\/email-sms\/principal-to-staff-emial(?=\/|$)/gi,
      "/email-sms/principal-to-staff-email",
    )
    ...
    // Student portal — Angular `student-exam-section` / `examination-section` module.
    .replace(/\/apps\/examination-section\//gi, "/examination-section/")
    .replace(/\/apps\/student-exam-section\//gi, "/examination-section/")
    .replace(/\/student-exam-section\//gi, "/examination-section/");
  ...
  for (
    let prefixLen = 1;
    prefixLen <= Math.floor(segments.length / 2);
    prefixLen++
  ) {
    const prefix = segments.slice(0, prefixLen);
    const next = segments.slice(prefixLen, prefixLen * 2);
    if (
      prefix.length === next.length &&
      prefix.every((s, i) => s === next[i])
    ) {
...
function overrideLegacyPreExamHref(href: string, label: string): string {
  ...
  const isPreExamHref =
    ...
    hrefLower.includes("/pre-examinations/");

  // Always pin register-subjects (label or legacy -update slug) to the real page.
  if (
    hrefLower.includes("exam-register-subjects") ||
    lower.includes("exam register subject")
  ) {
    return `${base}/exam-register-subjects`;
  }

  if (!isPreExamHref && !href.startsWith(base)) return href;
  ...
  if (lower.includes("invigilator allot"))
    return `${base}/invigilator-allotment`;
  if (lower.includes("additional exam fee"))
    return `${base}/additional-exam-fees`;
  if (
    lower.includes("exam attendance-wise") ||
    lower.includes("exam attendancewise")
  ) {
    return `${base}/exam-attendancewise-subject-barcode`;
  }
  if (lower.includes("student exam lab bat"))
    return `${base}/student-exam-lab-batches`;
  if (lower.includes("manual") && lower.includes("fee"))
    return `${base}/exam-registration-manual-feeless`;
  if (lower.includes("college exam timetable"))
    return `${base}/college-exam-timetable-view`;
  if (lower.includes("complete exam fee"))
    return `${base}/complete-exam-fee-registration`;
...
function overrideLegacyPostExamHref(href: string, label: string): string {
  ...
  if (
    lower.includes("internal") &&
    (lower.includes("exams average") ||
      lower.includes("exam average") ||
      lower.includes("internal exams avg"))
  ) {
...
function overrideExaminationSectionHref(
  ...
  const mapped = mapExaminationSectionNavRoute(href, pageLabel);
  return mapped ?? href;
}
...
export function normalizePageHref(href: string, pageLabel: string): string {
  ...
  const withExaminationSection = overrideExaminationSectionHref(
    href,
    pageLabel,
  );

  const withInstitutional = overrideInstitutionalMastersHref(
...
function mergeFlatPagesIntoModules(
  modules: Module[],
  flatPages: Page[],
): Module[] {
  ...
  return modules.map((module) => {
    ...
    const attached = flatPages.filter((page) => {
      if (page.moduleId != null && page.moduleId === module.moduleId)
        return true;
  ...
  return flatPages.filter((page) => {
    ...
    return !modules.some((m) => {
      const keys = [
        ...
        normalizeModuleKey(m.displayName ?? ""),
      ];
...
export function buildNavTree(modules: Module[], pages: Page[]): NavItem[] {
  ...
  if (!hasModules && hasPages) {
    return ensureErpModuleNavChildren(
      ensureTimetableNavChildren(buildStandalonePages(safePages)),
    );
  }
...
function buildStandalonePages(pages: Page[]): NavItem[] {
  ...
  return sorted.map((page) => {
    ...
    if (moduleUrl && subModuleUrl) {
      ...
      href =
        rawUrl.startsWith(fullPrefix + "/") || rawUrl === fullPrefix
          ? rawUrl
          : `${fullPrefix}/${rawUrl}`;
    } else if (moduleUrl) {
      href =
        rawUrl.startsWith(moduleUrl + "/") || rawUrl === moduleUrl
          ? rawUrl
          : `${moduleUrl}/${rawUrl}`;
    } else if (subModuleUrl) {
      href =
        rawUrl.startsWith(subModuleUrl + "/") || rawUrl === subModuleUrl
          ? rawUrl
          : `${subModuleUrl}/${rawUrl}`;
    }
...
function buildSubModuleItem(subModule: SubModule, moduleUrl: string): NavItem {
  ...
  if (subModule.pages && subModule.pages.length > 0) {
    const sorted = [...subModule.pages].sort(
      (a, b) => a.sortOrder - b.sortOrder,
    );
...
function breadcrumbChainPreference(chain: NavItem[]): number {
  ...
  const underAdminExam =
    ...
    labels.some(
      (l) => l.includes("examination report") || l.includes("exam report"),
    );
