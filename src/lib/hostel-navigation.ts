...
export function mapHostelNavRoute(
  href?: string,
  label?: string,
): string | null {
  ...
  // Admin master-settings screen — not hostel. Keep Hostel "View Room Details"
  // available when its href belongs to the Hostel module.
  if (
    (hrefLower.includes("room-detail") &&
      (hrefLower.includes("/admin/") ||
        hrefLower.includes("institutional") ||
        hrefLower.includes("master-settings"))) ||
  ...
  if (
    hrefLower.includes("/hostel/view-room-details") ||
    (labelLower.includes("view") && labelLower.includes("room"))
  ) {
...
export function isHostelRoomAllocationPath(pathname: string): boolean {
  ...
  return (
    path === "/hostel/rooms-list" || path.startsWith("/hostel/room-allocation")
  );
}
