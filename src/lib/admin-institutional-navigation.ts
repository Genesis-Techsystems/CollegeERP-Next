...
export function mapLegacyInstitutionalMastersHref(href?: string): string | null {
  if (!href) return null
...
export function isAdminInstitutionalRoomContext(href?: string, label?: string): boolean {
  const hrefLower = (href ?? '').toLowerCase()
  const labelLower = (label ?? '').toLowerCase().trim()

  if (hrefLower.includes('institutional-masters')) return true
  if (hrefLower.includes('rooms-type') || hrefLower.includes('room-types') || hrefLower.includes('room-type')) {
    return true
  }
  if (hrefLower.startsWith('/admin/room') || hrefLower.includes('/admin/rooms')) return true
  if (labelLower.includes('room type') || labelLower === 'room types') return true
  if (
...
export function mapAdminInstitutionalRoomRoute(href?: string, label?: string): string | null {
  const hrefLower = (href ?? '').toLowerCase()
  const labelLower = (label ?? '').toLowerCase().trim()
  ...
  if (hrefLower === '/admin/room-types' || hrefLower.startsWith('/admin/room-types/')) {
    return '/admin/room-types'
  }
  if (hrefLower === '/admin/rooms' || hrefLower.startsWith('/admin/rooms/')) {
    return '/admin/rooms'
  }
  if (hrefLower === '/admin/room-details' || hrefLower.startsWith('/admin/room-details/')) {
