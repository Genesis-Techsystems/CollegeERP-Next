...
export const NOTIFICATIONS_ANNOUNCEMENTS_BASE = '/notifications-and-announcements'
...
export function mapAttendanceLabelToRoute(label?: string): string | null {
  ...
  if (key.includes('markattendance') || (key.includes('mark') && key.includes('attendance'))) {
...
export function mapAttendanceNavRoute(href?: string, label?: string): string | null {
  ...
  if (hrefLower.includes('staff-classes/attendance-update')) {
    if (hrefLower.includes('view-attendance')) return `${ATTENDANCE_MGMT_BASE}/mark-attendance/view-attendance`
    if (hrefLower.includes('mark-attendance')) return `${ATTENDANCE_MGMT_BASE}/mark-attendance/mark-attendance`
    return `${ATTENDANCE_MGMT_BASE}/mark-attendance`
  }
  if (hrefLower.includes('exam-attendance')) return `${ATTENDANCE_MGMT_BASE}/exam-attendance`
  ...
  if (slug && hrefLower.includes('attendance')) return `${ATTENDANCE_MGMT_BASE}/${slug}`
...
export function mapMentorshipLabelToRoute(label?: string): string | null {
  ...
  if (key.includes('assignmentor') || (key.includes('assign') && key.includes('mentor'))) {
    return `${MENTORSHIP_BASE}/assign-mentor-to-students`
  }
  if (key.includes('assigncounselor')) return `${MENTORSHIP_BASE}/assign-counselor`
  if (key.includes('counselingdashboard')) return `${MENTORSHIP_BASE}/counseling-dashboard`
  if (key.includes('activitytype')) return `${MENTORSHIP_BASE}/activity-type`
  if (key.includes('meetinghistory')) return `${MENTORSHIP_BASE}/meeting-history`
  if (key.includes('studentmeeting')) return `${MENTORSHIP_BASE}/student-meetings`
  return null
}

export function mapMentorshipNavRoute(href?: string, label?: string): string | null {
  ...
  for (const seg of ['staff-mentorship', 'admin-counseling'] as const) {
    const mapped = mapModuleTail(hrefRaw, seg, MENTORSHIP_BASE, MENTORSHIP_SLUGS, 'counseling-dashboard')
  ...
  if (slug && (hrefLower.includes('counsel') || hrefLower.includes('mentor') || hrefLower.includes('ptm'))) {
...
export function mapEventsLabelToRoute(label?: string): string | null {
  if (!label) return null
  const key = normalizeLabelKey(label)
  if (key.includes('eventsdashboard') || key === 'events') return `${EVENTS_BASE}/events-dashboard`
  if (key.includes('addevent')) return `${EVENTS_BASE}/add-event`
  if (key.includes('eventtype')) return `${EVENTS_BASE}/event-type`
  if (key.includes('departmentevent')) return `${EVENTS_BASE}/department-events`
...
export function mapEventsNavRoute(href?: string, label?: string): string | null {
  ...
  if (hrefLower.includes('principal-communications/events') || hrefLower.includes('events-calendar')) {
    if (hrefLower.includes('school-calendar')) return `${EVENTS_BASE}/school-calendar`
    if (hrefLower.includes('staff-events')) return `${EVENTS_BASE}/staff-events`
    return `${EVENTS_BASE}/events-calendar`
  }

  const fromEvents = mapModuleTail(hrefRaw, 'events', EVENTS_BASE, EVENTS_SLUGS, 'events-dashboard')
  ...
  if (hrefLower.includes('/events/') || hrefLower.match(/\/events$/)) {
    ...
    const tail = hrefRaw.slice(idx + '/events'.length).replace(/^\/+/, '').split('?')[0]
...
export function mapNotificationsAnnouncementsLabelToRoute(label?: string): string | null {
  if (!label) return null
  const key = normalizeLabelKey(label)
  ...
  if (key.includes('addnotification')) return `${NOTIFICATIONS_ANNOUNCEMENTS_BASE}/add-notification`
  return null
}

export function mapNotificationsAnnouncementsNavRoute(href?: string, label?: string): string | null {
  ...
  if (
    ...
    if (start >= 0) {
      ...
      return tail.startsWith('/') ? tail.replace('notifications-&-announcements', 'notifications-and-announcements') : `/${tail}`
    }
  }
...
export function mapMyNotificationsNavRoute(href?: string, label?: string): string | null {
...
export function mapLibraryLabelToRoute(label?: string): string | null {
  if (!label) return null
  const key = normalizeLabelKey(label)
  if (key === 'library' || key === 'librarymanagement') return `${LIBRARY_BASE}/library-dashboard`
  ...
  if (key.includes('printbook') && key.includes('barcode')) return `${LIBRARY_BASE}/print-books-barcodes`
  if (key.includes('booksbarcode') || key.includes('bookbarcode')) return `${LIBRARY_BASE}/books-barcode`
  if (key === 'books' || key.includes('bookcatalogue')) return `${LIBRARY_BASE}/books`
  ...
  if (key.includes('librarysetting') || (key.includes('library') && key.includes('setting'))) {
    return `${LIBRARY_BASE}/library-settings`
  }
  if (key.includes('librarydetail')) return `${LIBRARY_BASE}/library-details`
  if (key.includes('librarydashboard')) return `${LIBRARY_BASE}/library-dashboard`
  return null
}

export function mapLibraryNavRoute(href?: string, label?: string): string | null {
  ...
  const mapped = mapModuleTail(hrefRaw, 'library/', LIBRARY_BASE, LIBRARY_SLUGS, 'library-dashboard')
  if (mapped) return mapped

  if (hrefLower.includes('/library/') || hrefLower.includes('/apps/library/')) {
    const idx = Math.max(hrefLower.indexOf('/library/'), hrefLower.indexOf('/apps/library/'))
    const segment = hrefLower.includes('/apps/library/') ? '/apps/library/' : '/library/'
    const tail = hrefRaw.slice(idx + segment.length).split('?')[0].replace(/^\/+/, '')
    if (!tail) return `${LIBRARY_BASE}/library-dashboard`
    const first = tail.split('/')[0]!.toLowerCase()
    const slug = LIBRARY_SLUGS[first] ?? LIBRARY_SLUGS[first.replace(/-/g, '')] ?? first
    const rest = tail.split('/').slice(1).join('/')
...
export function isNotificationsAnnouncementsModuleLabel(label?: string): boolean {
  if (!label) return false
  const key = normalizeLabelKey(label)
  return key.includes('notification') && key.includes('announcement') && !key.includes('my')
}

export function isMyNotificationsModuleLabel(label?: string): boolean {
  if (!label) return false
  const key = normalizeLabelKey(label)
  return key.includes('mynotification') || (key.includes('my') && key.includes('notification'))
}
