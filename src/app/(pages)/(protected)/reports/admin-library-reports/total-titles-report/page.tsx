...
/**
 * Titles Report —
 * Angular `reports/admin-library-reports/total-titles-report` parity.
 */
...
const PRINT_REPORT_TITLE = "Total No Of Titles";
...
function mapTitleRow(row: AnyRow): TitleRow {
  return {
...
export default function TotalTitlesReportPage() {
  ...
  const columnDefs = useMemo<ColDef<TitleRow>[]>(
    () => [
      COL_DEFS.siNo,
      COL_DEFS.titleName,
      COL_DEFS.author,
  ...
  const handleGetList = async () => {
    ...
    setCollegeName(name);
    try {
      const raw = await getTitlesReport(bid);
      if (raw.length === 0) {
        toastInfo("No records found.");
        return;
      }
      setRows(raw.map(mapTitleRow));
      setShowTable(true);
    } catch (err) {
  ...
  return (
    <FilteredListPage<TitleRow>
      title={
        showTable && dataDetails
          ? `Titles Report - ( ${dataDetails} )`
          : "Titles Report"
      }
      filters={
