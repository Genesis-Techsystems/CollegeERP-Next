 ...
 * Angular `reports/admin-transport-reports/driver-details-report` parity.
 */
...
const PRINT_REPORT_TITLE = "Driver Details Report";
...
const EXCEL_COLUMNS = [
  ...
  { key: "licenseValidUpto", header: "License Valid Upto" },
...
function mapRow(row: AnyRow): DriverRow {
  return {
...
export default function DriverDetailsReportPage() {
  ...
  const columnDefs = useMemo<ColDef<DriverRow>[]>(
    () => [
      COL_DEFS.siNo,
      COL_DEFS.driverName,
      COL_DEFS.gender,
  ...
  const handleGetList = async () => {
    ...
    setCollegeName(name);
    try {
      const raw = await getDriverDetailsReport(cid);
      if (raw.length === 0) {
