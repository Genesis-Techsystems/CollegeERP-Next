 ...
 * Angular `reports/admin-attendance-reports/faculty-subjects-attendance-report` parity.
 */
...
const PRINT_REPORT_TITLE = "Faculty Subjects Attendance Report";
const ALL0 = { value: "0", label: "All" };

export default function FacultySubjectsAttendanceReportPage() {
  ...
  const collegeLogo = useCollegeLogo(
    Number(collegeId) > 0 ? Number(collegeId) : null,
  );
  ...
  const filtersQuery = useQuery({
    queryKey: QK.attendanceReports.subjectFilters(orgId, empId),
    queryFn: () => getAttendanceSubjectFilters(orgId, empId),
  ...
  const subjectData = useMemo(() => {
    const fromSubject = (filtersQuery.data?.subjectData ?? []) as FilterRow[];
    return fromSubject.length > 0 ? fromSubject : filtersData;
  }, [filtersQuery.data?.subjectData, filtersData]);

  const collegeOptions = useMemo(() => {
    const rows = dedupeBy(
      filtersData.filter((r) => pickNum(r, ["fk_college_id", "collegeId"]) > 0),
  ...
  useEffect(() => {
    if (collegeId !== "0" || collegeOptions.length <= 1) return;
    const first = collegeOptions.find((o) => o.value !== "0");
    if (first) setCollegeId(first.value);
  ...
  const columnDefs = useMemo<ColDef<EmpAttendancePivotRow>[]>(() => {
    const fixed: ColDef<EmpAttendancePivotRow>[] = [
      ...
      { field: "subject_code", headerName: "Subject Code", minWidth: 120 },
    ];
  ...
  const handleExcelExport = () => {
    ...
    exportHtmlTableAsExcel(
      "Faculty Subjects Attendance Report.xls",
