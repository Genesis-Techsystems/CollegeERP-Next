...
/** Angular null Month/Year → "-". */
function nullableText(value: unknown): string {
  if (value == null || value === "") return "-";
  return String(value);
}

function discountAmountFormatter(
  p: ValueFormatterParams<FeeDiscountSummaryRow>,
) {
  return inrCurrency(p.data?.discount_amount);
}

function monthFormatter(p: ValueFormatterParams<FeeDiscountSummaryRow>) {
  return nullableText(p.data?.Month);
}

function yearFormatter(p: ValueFormatterParams<FeeDiscountSummaryRow>) {
  return nullableText(p.data?.Year);
}
...
const COL_DEFS = {
  siNo: {
    ...
    sortable: false,
    filter: false,
  } as ColDef<FeeDiscountSummaryRow>,
  college: {
    field: "college_name",
    headerName: "College",
    minWidth: 140,
  ...
  discount: {
    ...
    flex: 1,
    cellClass: "text-left",
    valueFormatter: discountAmountFormatter,
  } as ColDef<FeeDiscountSummaryRow>,
  approver: {
    field: "ApproverName",
    headerName: "Approver",
...
export default function DiscountReportPage() {
  ...
  const { data: filterBundle, isLoading: loadingFilters } = useQuery({
    queryKey: ["DiscountReport", "filters", orgId, employeeId],
  ...
  const {
    ...
    queryKey: QK.feeDiscountSummary(
    ...
    queryFn: () => {
      ...
      return fetchFeeDiscountSummary(p);
    },
  ...
  /** Angular `filter:searchText` — screen, Excel, and print. */
  ...
  function handleExportExcel() {
    ...
    exportHtmlTableAsExcel(excelRef.current, "Discount Report");
  }

  function handlePrint() {
    if (!printRef.current) return;
    printElementInIframe(printRef.current, "Discount Report", {
      extraCss: `
        @page { margin: 1cm; }
        html, body { background: #fff !important; }
        .discount-print { width: 100%; color: #000; }
        .discount-print .collegeName {
  ...
  return (
    <FilteredListPage<FeeDiscountSummaryRow>
    ...
    >
      {resultsVisible ? (
        <div ref={excelRef} className="hidden" aria-hidden>
          <strong>Discount Report &nbsp; ({dataDetails})</strong>
          <table>
            <thead>
              <tr>
                <th style={TH}>S.No</th>
                ...
                <th style={TH}>Discount Amount</th>
            ...
            <tbody>
              {filteredRows.map((row, i) => (
                <tr key={`excel-${i}`}>
                  <td style={TD}>{i + 1}</td>
                  <td style={TD}>{String(row.college_name ?? "")}</td>
                  <td style={TD}>{String(row.district_name ?? "")}</td>
                  <td style={TD}>{String(row.fee_category_name ?? "")}</td>
                  <td style={TD}>{inrCurrency(row.discount_amount)}</td>
                  <td style={TD}>{String(row.ApproverName ?? "")}</td>
                  <td style={TD}>{nullableText(row.Month)}</td>
                  <td style={TD}>{nullableText(row.Year)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {resultsVisible ? (
        <div className="pointer-events-none absolute -left-[9999px] top-0 w-[900px] bg-white text-black">
          <div
            ref={printRef}
            className="discount-print bg-white p-4 text-black"
          >
            ...
            <p className="title">Discount Report</p>
            <table>
              <thead>
                <tr>
                  <th>S.No</th>
              ...
              <tbody>
                {filteredRows.map((row, i) => (
                  <tr key={`print-${i}`}>
                    <td>{i + 1}</td>
                    <td>{String(row.college_name ?? "")}</td>
                    <td>{String(row.district_name ?? "")}</td>
                    <td>{String(row.fee_category_name ?? "")}</td>
                    <td>
                      {row.discount_amount == null
                        ? ""
                        : String(row.discount_amount)}
                    </td>
                    <td>{String(row.ApproverName ?? "")}</td>
