 ...
 * Angular `reports/student-admission-reports/students-gender-count` parity.
 * Get List: `getAllRecords/s_get_student_reports?in_flag=adm_quota_std_count&…`
 * (Angular uses `adm_quota_std_count` for this report's Get List payload.)
 */
...
const REPORT_TITLE = "Student Count By Gender Report";
...
const EXCEL_COLUMNS = [
  ...
  { key: "Student_Count", header: "Student Count" },
] as const;

/** Proc / driver may return PascalCase or snake_case aliases. */
function rowText(row: AnyRow | undefined | null, keys: string[]): string {
  if (!row) return "";
  for (const key of keys) {
    const v = row[key];
    if (v != null && String(v).trim() !== "") return String(v);
  }
  // Case-insensitive fallback (MySQL drivers sometimes change casing).
...
function normalizeGenderCountRow(row: AnyRow): AnyRow {
  return {
    ...row,
    College_Code: rowText(row, [
      "College_Code",
      "college_code",
      "CollegeCode",
      "collegeCode",
...
export default function StudentsGenderCountPage() {
  ...
  const handleGetList = async () => {
    ...
    try {
      ...
      if (raw.length === 0) {
        toastInfo("No student count records found.");
        return;
      }
      setRows(raw.map(normalizeGenderCountRow));
      setShowTable(true);
    } catch (err) {
