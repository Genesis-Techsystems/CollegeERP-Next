...
import { FileSpreadsheet, LayoutGrid, Printer } from "lucide-react";
...
const REPORT_TITLE = "Scholarship Preceeding Report";
const PRINT_REPORT_TITLE = "Scholarship Preceeding Report";

const EXCEL_COLUMNS = [
  { key: "siNo", header: "SI.No" },
  { key: "preceedingNo", header: "Preceeding No." },
  { key: "preceedingTitle", header: "Preceeding Title" },
  { key: "caste", header: "Caste" },
  { key: "noOfStudents", header: "No. of Students" },
  { key: "preceedingAmount", header: "Amount" },
];
...
export default function ScholarshipPreceedingsReportPage() {
  ...
  const exportRows = useMemo(
    () =>
      rows.map((row, i) => ({
        ...
        preceedingAmount: row.preceedingAmount ?? "",
      })),
    [rows],
  );
  ...
  const handleExcelExport = () => {
    ...
    exportHtmlTableAsExcel(
      "Scholarship Preceeding Report.xls",
        ...
        >
          <DialogHeader className="shrink-0 border-b border-border pb-3">
            <div className="flex items-center gap-2">
              <LayoutGrid className="h-4 w-4 text-[#0c51a4]" aria-hidden />
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto py-2">
            {studentsHeader ? (
              <div className="space-y-1.5 rounded-md border border-sky-300 bg-sky-50/40 p-3 text-sm">
                <p>
                ...
                <p>
                  <span className="text-foreground">Preceeding Amount :</span>{" "}
                  <span className="font-medium text-blue-700">
            ...
            {/* Angular modal: Search + table (headers always; empty body when no rows) */}
            <input
              className="h-9 w-full max-w-xs border-0 border-b border-[#0c51a4] bg-transparent px-1 text-sm outline-none focus:border-[#0c51a4]"
              placeholder="Search"
                ...
                <table className="w-full table-fixed text-left text-xs">
                  <thead className="bg-[#e8f1fb]">
                    <tr className="border-b">
                      <th className="w-[7%] px-2 py-2 font-medium">Sl.No</th>
                      <th className="w-[14%] px-2 py-2 font-medium">
                        Application No.
                      </th>
                      <th className="w-[12%] px-2 py-2 font-medium">Roll No.</th>
                      <th className="w-[14%] px-2 py-2 font-medium">Student</th>
                      <th className="w-[18%] px-2 py-2 font-medium">Course</th>
                      ...
                      <th className="w-[11%] px-2 py-2 font-medium">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredStudentRows.map((r, i) => {
                      ...
                      return (
                        ...
                        >
                          <td className="truncate px-2 py-1.5 text-center">
                            {i + 1}
                          </td>
                          <td className="truncate px-2 py-1.5">
                            {String(r.schApplicationNo ?? "")}
                          </td>
                          <td className="truncate px-2 py-1.5">{roll}</td>
                          <td className="truncate px-2 py-1.5">
                            {String(r.applicantName ?? r.firstName ?? "")}
                          </td>
                          <td className="truncate px-2 py-1.5">{courseLine}</td>
                          <td className="truncate px-2 py-1.5">
