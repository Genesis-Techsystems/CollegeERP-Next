...
import { ViewProxiesModal } from "./ViewProxiesModal";

type ActionName = "RECOMMEND" | "REJECT";
...
function isPendingLeave(row: AnyRow): boolean {
  return String(row.leaveprocessStatusCode ?? "") === "LPSAPPLIED";
}
...
export function LeaveApprovalsPage() {
    ...
    async (year: number) => {
      ...
      try {
        const rows = sortLeaveApplicationsDesc(
          await listAssignedLeaveApplications(employeeId, year),
        );
        const pending: AnyRow[] = [];
        const processed: AnyRow[] = [];
        for (const row of rows) {
          if (isPendingLeave(row)) pending.push(row);
  ...
  async function handleActionSave(payload: {
    reason: string;
    leaveCode?: string;
  }) {
    ...
    const statusCode =
      actionName === "RECOMMEND" ? "LPSRECOMMENDED" : "LPSREJECTED";
    ...
    // Angular leave-application.confirm: leaveTypeCode + reason only.
    item.leaveTypeCode = payload.leaveCode;
        ...
        cellRenderer: (p: ICellRendererParams<AnyRow>) => {
          ...
          return (
            <div className="flex flex-wrap gap-1 py-1">
              <Button
                size="sm"
                className="h-7 px-2 text-xs bg-amber-400 hover:bg-amber-500 text-black"
                disabled={saving}
    ...
    () => [
      ...
      },
      {
        ...
        cellRenderer: (p: ICellRendererParams<AnyRow>) => {
          ...
          if (code === "LPSAPPROVED") return null;
          return <span>--</span>;
        },
      },
    ],
      ...
      filters={
        <GlobalFilterBarRow>
          ...
          <div className="flex items-center gap-2 pt-6">
            <Checkbox
              id="processed-leave-list"
              checked={showProcessed}
              onCheckedChange={(checked) => setShowProcessed(checked === true)}
            />
            <Label htmlFor="processed-leave-list" className="font-normal">
              Processed Leave Requests
            </Label>
          </div>
        </GlobalFilterBarRow>
