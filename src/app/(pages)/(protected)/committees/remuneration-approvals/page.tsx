...
function remunerationStatusVariant(
  statusId?: number,
...
function makeActionsRenderer(
  onEdit: (row: UnivExaminationRemuneration) => void,
) {
  return (p: ICellRendererParams<UnivExaminationRemuneration>) => {
    const row = p.data;
    if (!row) return null;
...
export default function RemunerationApprovalsPage() {
  ...
  const [editingRow, setEditingRow] =
    useState<UnivExaminationRemuneration | null>(null);
  ...
  const orgOptions = useMemo(
    () =>
      dedupeBy(filterRows, (r) =>
        pickText(r as Record<string, unknown>, ["org_code"]),
      ).map((r) => {
  ...
  const monthYearOptions = useMemo(() => {
    const base = orgCode
      ? filterRows.filter(
          (r) =>
            pickText(r as Record<string, unknown>, ["org_code"]) === orgCode,
        )
      : filterRows;
    return dedupeBy(base, (r) =>
      pickText(r as Record<string, unknown>, ["exam_month_yr"]),
    ).map((r) => {
  ...
  const examOptions = useMemo(() => {
    const base = filterRows.filter((r) => {
      ...
      const monthOk =
        !examMonthYear || pickText(row, ["exam_month_yr"]) === examMonthYear;
      return orgOk && monthOk;
    });
    return dedupeBy(base, (r) =>
      pickNum(r as Record<string, unknown>, ["pk_university_exam_id"]),
    ).map((r) => {
      const row = r as Record<string, unknown>;
  ...
  const selectedExamName = useMemo(() => {
    ...
    return match
      ? pickText(match as Record<string, unknown>, ["exam_name"])
      : "";
  }, [filterRows, examId]);
  ...
  useEffect(() => {
    if (!examMonthYear && monthYearOptions[0])
      setExamMonthYear(monthYearOptions[0].value);
  ...
  async function loadDetails() {
    ...
    try {
      ...
      const pending = rows.filter(
        (r) => r.remunerationStatusCatDetId === REMUNERATION_STATUS.PENDING,
      );
      const approved = rows.filter(
        (r) => r.remunerationStatusCatDetId === REMUNERATION_STATUS.APPROVED,
      );
      setPendingRows(pending);
      setApprovedRows(approved);
      setSelectedIds(new Set());
      toastSuccess(
        `Loaded ${rows.length} remuneration record${rows.length === 1 ? "" : "s"}`,
      );
    } catch (error) {
  ...
  async function handleBulkApprove() {
    ...
    try {
      ...
      toastSuccess(
        `Approved ${payload.length} record${payload.length === 1 ? "" : "s"}`,
      );
  ...
  const filtersLoading =
    sessionLoading || empResolving || loadingFilters || loadingRoles;
  ...
  return (
    <FilteredListPage
      ...
      filters={
        <div className="grid grid-cols-1 items-end gap-3 md:grid-cols-12">
          ...
          <div className="flex flex-wrap gap-2 md:col-span-3">
            ...
            >
              <RefreshCw
                className={`mr-1.5 h-4 w-4 ${loadingDetails ? "animate-spin" : ""}`}
              />
              Load
            </Button>
          </div>
      ...
      showTable={pendingRows.length > 0}
      resultsVisible={pendingRows.length > 0}
      height="360px"
