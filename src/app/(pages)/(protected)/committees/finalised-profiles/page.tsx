...
function meetingTitleRenderer(p: ICellRendererParams<UnivProfileRecruitment>) {
  ...
  const title =
    row?.meetingTitle ??
    (row as UnivProfileRecruitment & { committeeMeetingTitle?: string })
...
function makeReleaseRenderer(onRelease: (row: UnivProfileRecruitment) => void) {
...
export default function FinalisedProfilesPage() {
  ...
  const [confirmRow, setConfirmRow] = useState<UnivProfileRecruitment | null>(
    null,
  );
  ...
  const { data: meetings = [] } = useQuery({
    ...
    enabled: listRequested && filters.cascadeReady,
  });
  ...
  const {
    ...
    enabled: listRequested && filters.filtersReady && filters.orgId > 0,
  });
  ...
  const { data: evaluatorProfiles = [] } = useQuery({
    ...
    enabled: listRequested && filters.filtersReady && examNum > 0,
  });
  ...
  async function handleReleaseOffer() {
    ...
    const profileDetail = evaluatorProfiles.find(
      (p: EvaluatorProfileRow) =>
        Number(p.pk_exam_evaluator_profile_id) ===
        Number(confirmRow.examEvaluatorProfilesId),
    );
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="space-y-0.5">
            ...
            <Select
              ...
              searchable
              clearable
              disabled={!listRequested || !filters.cascadeReady}
            />
          </div>
