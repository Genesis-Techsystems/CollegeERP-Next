...
/**
 * Angular `InternalItemReturnComponent` parity — Internal Return list.
 */
 ...
 * id, storeCode, internalReturnNo, returnDate, fromEmpName, isActive, actions
 */
const COL_DEFS = {
  siNo: {
    headerName: "SI.No",
...
/** Angular `{{ row.returnDate | date:'MMM d, y' }}` */
...
function dateRenderer(p: ICellRendererParams<InvInternalReturn>) {
  return <span>{formatReturnDate(p.data?.returnDate)}</span>;
}
  ...
  return (p: ICellRendererParams<InvInternalReturn>) => {
    ...
    return (
      <Button
        ...
        onClick={() =>
          router.push(
            `/inventory-management/internal-item-return/edit?id=${row.interReturnId}`,
          )
        }
      >
...
export default function InternalItemReturnPage() {
  ...
  const columnDefs = useMemo<ColDef<InvInternalReturn>[]>(
    () => [
      ...
      { ...COL_DEFS.returnDate, cellRenderer: dateRenderer },
    ...
    <ListPage
      ...
      toolbarTrailing={
        <Button
          size="sm"
          onClick={() =>
            router.push("/inventory-management/internal-item-return/add")
          }
        >
