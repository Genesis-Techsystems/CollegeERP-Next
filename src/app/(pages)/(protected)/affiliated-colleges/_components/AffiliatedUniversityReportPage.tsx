...
function actionRenderer(slug: string) {
  return (p: ICellRendererParams<AffiliatedSummaryRow>) => {
    ...
    const segment = Object.entries(VIEW_ROUTES).find(([k]) =>
      flag.includes(k),
    )?.[1];
    if (!segment)
    ...
    return (
      <Button size="sm" variant="outline" asChild>
        <Link
          href={`/affiliated-colleges/university-affiliated-colleges/${segment}`}
        >
          View
        </Link>
      </Button>
...
export function AffiliatedUniversityReportPage({
  slug,
}: AffiliatedUniversityReportPageProps) {
