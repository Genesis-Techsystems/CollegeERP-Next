...
export function CollegeStudentSubjectsUploadPage() {
  ...
  const loadTemplate = useCallback(
    async (params: typeof filterParams, regId: number) => {
      if (regId <= 0) return;
  ...
  async function onUploadFile(selected: File) {
    ...
    setLoadedRows([]);
    try {
      const rows = await importAffiliatedStudentSubjectsFile(
        selected,
        importMeta,
      );
      setStagingRows(rows);
    ...
    <FilteredPage
      ...
      filters={
        <AffiliatedCollegeFilters
          ...
          cascade={cascade}
          onGetDetails={() =>
            void loadTemplate(filterParamsRef.current, regulationIdRef.current)
          }
      ...
      {showUploadCard ? (
        <div className="app-card">
          ...
          <AffiliatedExcelDownloadUpload
            downloadTitle="1. Download Subjects Sample"
            uploadTitle="2. Upload Subjects"
