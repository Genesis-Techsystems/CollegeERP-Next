...
function pickUploadFileId(rows: AnyRow[]): number {
  const first = rows[0] ?? {}
  return Number(first.univUploadFilesId ?? first.univ_uploadfile_id ?? first.univUploadFileId ?? 0)
}
...
export function CollegeStudentBulkUploadPage() {
  ...
  const importMeta = useMemo(() => {
    ...
    return {
      universityCode: pickAffiliatedText(college, ['university_code', 'universityCode']),
      ...
      fileDescription: cascade.contextLabel,
      fileUploadedByEmpId: resolveAffiliatedEmployeeId(),
      ...filterParams,
    }
  }, [cascade.colleges, cascade.courses, cascade.collegeId, cascade.courseId, cascade.contextLabel, filterParams])

  const templateMeta = useMemo(() => {
    const college = cascade.colleges.find(
      (c) => pickFilterRowId(c as AnyRow, ['fk_college_id', 'collegeId']) === cascade.collegeId,
    )
    const academicYear = cascade.academicYears.find(
      (a) => pickFilterRowId(a as AnyRow, ['fk_academic_year_id', 'academicYearId']) === cascade.academicYearId,
    )
    const course = cascade.courses.find(
      (c) => pickFilterRowId(c as AnyRow, ['fk_course_id', 'courseId']) === cascade.courseId,
    )
    const courseGroup = cascade.courseGroups.find(
      (g) => pickFilterRowId(g as AnyRow, ['fk_course_group_id', 'courseGroupId']) === cascade.courseGroupId,
    )
    const courseYear = cascade.courseYears.find(
      (y) => pickFilterRowId(y as AnyRow, ['fk_course_year_id', 'courseYearId']) === cascade.courseYearId,
    )
    return buildAffiliatedStudentTemplateMeta({
      universityCode: pickAffiliatedText(college, ['university_code', 'universityCode']),
      collegeCode: pickAffiliatedText(college, ['college_code', 'collegeCode']),
      academicYear: pickAffiliatedText(academicYear, ['academic_year', 'academicYear']),
      courseCode: pickAffiliatedText(course, ['course_code', 'courseCode']),
      courseGroup: pickAffiliatedText(courseGroup, ['group_code', 'groupCode']),
      courseYear: pickAffiliatedText(courseYear, ['course_year_name', 'courseYearName']),
    })
  }, [
    cascade.colleges,
  ...
  useEffect(() => {
    const collegeId =
      numParam(searchParams, 'collegeId') || (summaryContext?.fk_college_id ?? 0)
    if (collegeId <= 0) {
  ...
  async function onUploadFile(selected: File) {
    ...
    if (ext !== 'xlsx' && ext !== 'xls') {
      toastError('Invalid file type. Please upload an Excel file (.xlsx or .xls).')
      return
    }
    setUploading(true)
    setLoadedRows([])
  ...
  async function onVerify() {
    ...
    try {
      ...
      const flag = String(result[0]?.Flag ?? result[0]?.flag ?? '').toLowerCase()
      if (flag === 'success') {
        setSubmitOpen(true)
  ...
  return (
    <PageContainer>
      <PageHeader title="College Student Bulk Upload" />
        ...
        footerExtra={
          <div className="flex flex-wrap items-center gap-2 mr-auto">
            ...
            <Button
              ...
              size="sm"
              disabled={!templateData}
              onClick={() => templateData && downloadAffiliatedStudentDictionaryExcel(templateData)}
            >
      ...
      {templateData ? (
        <div className="app-card mt-4">
          <div className="flex items-center gap-2 border-b px-4 py-3">
            ...
            <h2 className="font-semibold text-base">Download &amp; Upload — Students Data</h2>
          </div>
          <div className="p-4 space-y-4">
            ...
            {cascade.contextLabel ? (
              <p className="text-sm text-primary font-medium">{cascade.contextLabel}</p>
            ) : null}
          </div>
        </div>
      ) : null}
      ...
      {stagingRows.length > 0 ? (
        <div className="mt-4 space-y-2">
          ...
          <div className="flex justify-end px-1">
            <Button type="button" onClick={() => void onVerify()} disabled={verifying}>
      ...
      <FormModal
        ...
        description="Data verified successfully. Add notes before submitting to the university."
        ...
        size="md"
      >
        <label className="text-sm font-medium text-primary">Notes / Terms</label>
        <Textarea
