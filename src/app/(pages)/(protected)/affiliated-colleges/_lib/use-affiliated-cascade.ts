...
import { listAffiliatedRegulationsForCourse, resolveRegulationFromFilterContext } from "./resolve-affiliated-regulation";
  ...
  const exams = useMemo(() => {
    if (options.courseFirstCascade) {
      ...
      if (options.examKind === "internal") {
        rows = rows.filter((r) => Boolean(r.is_internal_exam ?? r.isInternalExam));
