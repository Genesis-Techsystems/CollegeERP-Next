...
export function pivotAffiliatedAttendanceStagingRows(stagingRows: AnyRow[]): {
  rows: AffiliatedAttendancePivotRow[];
  subjectCodes: string[];
} {
  ...
  for (const item of stagingRows) {
    const hallticketno = String(
      item.hallticketno ?? item.hallTicketNumber ?? "",
    );
    ...
    if (!transformed[hallticketno]) {
      transformed[hallticketno] = {
        ...
        courseyearcode: String(
          item.courseyearcode ?? item.courseYearCode ?? "",
        ),
        regulationcode: String(
          item.regulationcode ?? item.regulationCode ?? "",
        ),
