...
export function enrichAffiliatedStudentTemplate(
  ...
  const copy = sampleExcelData.map((group) =>
  ...
  copy[10] =
    existing.length > 0
      ? [{ ...existing[0], ...headerRow, count: existing[0].count ?? 1 }]
      : [headerRow];
  return copy;
}
...
function applyHallTicketAutofill(
  template: AnyRow,
  headers: string[],
  ht: AnyRow,
): void {
  const hallTicket = pickHallTicketText(ht, [
    "hallticket_number",
    "hallTicketNumber",
    "hallticketNumber",
  ]);
  if (headers.includes("HallTicketNumber"))
    template.HallTicketNumber = hallTicket;
  if (headers.includes("HallTicket")) template.HallTicket = hallTicket;
  if (headers.includes("Batch"))
    template.Batch = pickHallTicketText(ht, ["batch_name", "batchName"]);
  if (headers.includes("Regulation")) {
    template.Regulation = pickHallTicketText(ht, [
      "regulation_code",
      "regulationCode",
    ]);
  }
  if (headers.includes("Group")) {
    template.Group = pickHallTicketText(ht, [
      "group_code",
      "groupCode",
      "group_name",
      "groupName",
...
export function pickAffiliatedExistingStudentsCount(
  meta: AnyRow[] | null,
): number {
  if (!meta?.length) return 0;
  const row = meta[0] ?? {};
  const raw =
    row.uploaded_students_count ?? row.uploadedStudentsCount ?? row.count ?? 0;
  const n = Number(raw);
...
export function downloadAffiliatedStudentTemplateExcel(
  ...
  if (headers.length === 0) {
    return {
      ok: false,
      message: "Template headers missing. Reload details and try again.",
    };
  }
...
export function downloadAffiliatedExistingStudentsExcel(
  ...
  const rowCount =
    hallTicketData.length > 0 ? hallTicketData.length : dataRows[0] ? 1 : 0;
  if (rowCount === 0) {
    return {
      ok: false,
      message: "No existing student rows available to export.",
    };
  }
