...
export function pivotAffiliatedExamRegStagingRows(
  stagingRows: AnyRow[],
): { rows: AffiliatedExamRegPivotRow[]; subjectCodes: string[] } {
  ...
  for (const item of stagingRows) {
    ...
    if (!transformed[hallticketno]) {
      transformed[hallticketno] = {
        sno: Number(
          pickStagingField(item, "srno", "sno", "Sno", "SNo") ?? 0,
        ),
        hallticketno,
        academicyear: String(
          pickStagingField(item, "academicyear", "academicYear", "AcademicYear") ??
            "",
        ),
        courseyearcode: String(
          pickStagingField(
