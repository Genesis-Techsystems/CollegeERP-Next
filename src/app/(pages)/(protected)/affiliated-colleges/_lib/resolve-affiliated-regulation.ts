...
export function resolveRegulationFromFilterContext(input: {
  ...
  const universityId =
    ...
    pickPositiveId(
      filtersData.find(
        (row) => pickPositiveId(row, ["fk_college_id", "collegeId"]) === collegeId,
      ),
  ...
  const exactMatch = filtersData.find(
    (row) =>
      ...
      rowMatchesFilterContext(row, collegeId, courseId, courseGroupId, courseYearId) &&
      pickPositiveId(row, ["fk_regulation_id", "regulationId"]) > 0,
  );
  const fromExact = pickPositiveId(exactMatch, ["fk_regulation_id", "regulationId"]);
  ...
  const fromLoose = pickPositiveId(looseMatch, ["fk_regulation_id", "regulationId"]);
...
export function resolveAffiliatedRegulationId(input: {
  ...
  const prefer = (id: number) => {
    ...
    const match = regulations.some(
      (row) =>
        pickPositiveId(row, ["fk_regulation_id", "regulationId"]) === id,
    );
    return match ? id : 0;
  };
  ...
  if (regulationCode && regulations.length > 0) {
    const match = regulations.find(
      (row) =>
        pickText(row, ["regulation_code", "regulationCode", "regulationcode"]) ===
        regulationCode,
    );
