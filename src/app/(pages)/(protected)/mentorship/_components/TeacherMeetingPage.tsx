...
const schema = z.object({
  ...
  newNextScheduledActivityDate: z.date({ message: 'Next schedule date is required' }),
})
...
export function TeacherMeetingPage() {
  ...
  const counselorActivityId = Number(searchParams.get('counselorActivityId') || 0) || 0
  ...
  const [activityDetails, setActivityDetails] = useState<MentorshipRow | null>(null)
  ...
  function goBackToSchedule() {
    ...
    router.push(`/mentorship/schedule-ptm?${params.toString()}`)
  }
  ...
  async function onSave(values: FormValues) {
    ...
    try {
      ...
      await updateCounselorActivity(Number(activityDetails.counselorActivityId), updated)
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          <div className="md:col-span-3 space-y-1.5">
            ...
            <DatePicker value={activityDate} onChange={() => undefined} disabled clearable={false} />
          </div>
          <Controller
            name="attendeesName"
            control={control}
            render={({ field }) => (
              <div className="md:col-span-4 space-y-1.5">
                ...
                {errors.attendeesName ? (
                  <p className="text-xs text-destructive">{errors.attendeesName.message}</p>
                ) : null}
              </div>
            )}
          />
        ...
        <div className="flex items-center gap-2 pt-2">
          <Button type="submit" disabled={isSubmitting || loading || !activityDetails}>
