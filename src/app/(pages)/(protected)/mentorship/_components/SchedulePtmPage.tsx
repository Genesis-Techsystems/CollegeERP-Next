...
const COL_DEFS = {
  siNo: { headerName: 'No.', valueGetter: rowIndexGetter, width: 70, flex: 0 } as ColDef<MeetingRow>,
  activityType: { field: 'activityTypeCode', headerName: 'Activity Type', minWidth: 120 } as ColDef<MeetingRow>,
  nextDate: { field: 'nextScheduledActivityDate', headerName: 'Schedule Date', minWidth: 130 } as ColDef<MeetingRow>,
  attendees: { field: 'attendeesName', headerName: 'Attendees Name', minWidth: 140 } as ColDef<MeetingRow>,
  ...
  activityDate: { field: 'activityDate', headerName: 'Activity Date', minWidth: 130 } as ColDef<MeetingRow>,
  status: { field: 'activityStatusCode', headerName: 'Status', minWidth: 110 } as ColDef<MeetingRow>,
  actions: { headerName: 'Actions', minWidth: 200, width: 200, flex: 0 } as ColDef<MeetingRow>,
}
...
export function SchedulePtmPage() {
  ...
  const { employeeId: loginEmployeeId, isResolving } = useLoginEmployeeId(user, sessionLoading)

  const userRole = String(user?.userRole ?? readStorage('userRole') ?? '').toUpperCase()
  const isAdmin = userRole === 'ADMIN' || userRole === 'SUPERADMIN'
  const staffCollegeId = Number(user?.collegeId ?? readStorageNum('collegeId')) || 0
  ...
  const loadStudents = useCallback(
    async (opts?: { preferStudentId?: number; silentEmpty?: boolean }) => {
      ...
      try {
        ...
        setStudents(list)
        if (list.length === 0 && !opts?.silentEmpty) {
  ...
  const loadMeetings = useCallback(
    async (sid: number, opts?: { silentEmpty?: boolean }) => {
      ...
      if (isAdmin) {
        collegeId =
          Number(students.find((s) => Number(s.studentId) === sid)?.collegeId ?? 0) ||
          staffCollegeId
      }
      if (!collegeId || !sid) {
      ...
      try {
        ...
        if (result.activities.length === 0 && !opts?.silentEmpty) {
          toastSuccess('No Record(s) found.')
        }
      } catch (e) {
        toastError(getErrorMessage(e))
  ...
  }, [sessionLoading, isResolving, querySeeded, isAdmin, staffEmployeeId, fromDate, toDate])
  ...
  function openSchedule() {
    ...
    const collegeId =
      (isAdmin
        ? Number(students.find((s) => Number(s.studentId) === studentId)?.collegeId ?? 0)
  ...
  function goToMeeting(row: MeetingRow) {
    ...
    router.push(`/mentorship/schedule-ptm/teacher-meeting?${params.toString()}`)
  }
  ...
  async function onScheduleSaved(payload: MentorshipRow) {
    ...
    if (activityId > 0) {
      ...
      const counselorId = Number(payload.counselorId ?? mapping?.counselorId ?? 0)
  ...
  const effectiveCollegeId =
    (isAdmin
      ? Number(students.find((s) => Number(s.studentId) === studentId)?.collegeId ?? 0)
      : staffCollegeId) || Number(mapping?.collegeId ?? 0) || 0
  ...
  return (
    ...
    >
      ...
      <MeetingOverviewModal
        ...
        row={overviewRow}
      />
      <CounselorSendSmsModal open={smsOpen} onClose={() => setSmsOpen(false)} onSubmit={onSmsSubmit} />
    </FilteredListPage>
  )
}
