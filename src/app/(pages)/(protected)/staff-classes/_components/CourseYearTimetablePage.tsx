...
function buildStaffClassTimetable(scheduleTimings: ScheduleTiming[]): {
  ...
  for (const row of scheduleTimings) {
    const resources = Array.isArray(row.subjectResource) ? row.subjectResource : []
    const colored: ScheduleTiming = {
      ...row,
      subjectResource: resources,
      color: row.colorCode != null || resources[0]?.colorCode != null ? '#fff' : '#000',
      colorCode: (resources[0]?.colorCode as string | undefined) ?? row.colorCode,
      colspan: 1,
    }
  ...
  // Header periods from first weekday (Angular uses weekdays[0].classTimings / timings)
...
export function CourseYearTimetablePage() {
  ...
  useEffect(() => {
    ...
    void fetchTimetableFilterRows('cls_timtable_filters', 0)
      .then((rows) => {
        ...
        const groups = courseGroupsFromFilterRows(rows, firstCollege, firstAy, firstCourse)
  ...
  const collegeOpts = useMemo(
    () => toOpts(collegesFromFilterRows(filterRows), 'fk_college_id', ['college_code']),
    [filterRows],
  )
  const ayOpts = useMemo(
    () =>
      collegeId
        ? toOpts(academicYearsFromFilterRows(filterRows, collegeId), 'fk_academic_year_id', [
            'academic_year',
          ])
        : [],
  ...
  const courseOpts = useMemo(
    () =>
      collegeId && academicYearId
        ? toOpts(coursesFromFilterRows(filterRows, collegeId, academicYearId), 'fk_course_id', [
            'course_code',
            'course_name',
          ])
        : [],
  ...
  const timetableOpts = useMemo(() => {
    if (!collegeId || !academicYearId || !courseId || !courseGroupId || !groupSectionId) {
  ...
  const dataDetails = [
    ...
    timetableOpts.find((o) => o.value === String(timetableId))?.label?.split(' (')[0],
  ]
    .filter(Boolean)
    .join(' / ')
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <Select
              ...
              onChange={(v) => {
                ...
                setTimetableId(id)
                const row = timetableOpts.find((o) => o.value === v)?.raw ?? null
                setSelectedTtRow(row)
              }}
              options={timetableOpts.map(({ value, label }) => ({ value, label }))}
              searchable
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-semibold text-[hsl(var(--card-title))]">
                Timetable allocations{dateRange ? ` - (${dateRange})` : ''}
              </p>
              {dataDetails ? (
            ...
            {gridLoading ? (
              ...
              <div className="overflow-x-auto rounded-sm border border-[#ddd] bg-white">
                <table className="w-full min-w-[720px] border-separate border-spacing-px text-sm">
                  <thead>
                    <tr>
                      <th className="w-14 px-[5px] py-[5px] text-left font-medium bg-[#C3D9FF]" />
                      {headerTimings.map((t, i) => {
                        const isBreak = Boolean(t.isBreak)
                        const label = isBreak
                          ? 'LUNCH'
                        ...
                        return (
                          <th
                            ...
                            className="px-[5px] py-[5px] text-center font-medium whitespace-nowrap bg-[#C3D9FF]"
                            style={isBreak ? { width: 56 } : undefined}
                          >
                            {label}
                          </th>
                        )
                  ...
                  <tbody>
                    {weekdays.map((day) => (
                      <tr key={day.weekdayId}>
                        <th className="px-[5px] py-[5px] text-left font-medium uppercase whitespace-nowrap bg-[#C3D9FF]">
                          {day.weekdayName}
                        </th>
                        {day.timings.map((timing, ti) => {
                          ...
                          // Angular: [ngStyle]={'background':timing.colorCode}; .break → #efefef
                          const apiColor =
                            typeof timing.colorCode === 'string' && timing.colorCode.trim()
                              ? timing.colorCode
                              : undefined
                          const fg =
                            (timing.color as string | undefined) ||
                            (apiColor ? '#ffffff' : '#000000')
                          let bg: string | undefined
                          if (isBreak) bg = '#efefef'
                          else if (apiColor) bg = apiColor
                          return (
                            <td
                              key={`${day.weekdayId}-${ti}`}
                              className="px-2 py-5 align-middle text-center"
                            ...
                            >
                              {isBreak ? (
                                <span className="font-medium uppercase text-xs">Lunch</span>
                              ) : hasSubject ? (
                                <div className="space-y-0.5">
                                  {resources.slice(0, 2).map((r, ri) => (
                                    <div key={ri}>
                                      <p
                                        className="font-medium leading-tight uppercase text-[15px]"
                                        style={{ color: fg }}
                                      >
