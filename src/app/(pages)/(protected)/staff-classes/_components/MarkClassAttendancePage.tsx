 ...
 */
export function MarkClassAttendancePage({ mode }: Readonly<{ mode: 'mark' | 'view' }>) {
  ...
  const studentbatchId = Number(searchParams.get('studentbatchId') || 0) || null
  ...
  const [pendingPayload, setPendingPayload] = useState<MarkAttendanceSaveItem[] | null>(
    null,
  )

  const collegeLine = `${searchParams.get('collegeCode') ?? ''} / ${searchParams.get('academicYear') ?? ''}`.trim()
  const courseLine = [
  ...
  const loadPeriods = useCallback(
    async (date: Date) => {
      if (!collegeId || !academicYearId || !employeeId || !groupSectionId || !subjectId) {
        setPeriods([])
        return
      }
  ...
  useEffect(() => {
    void (async () => {
      try {
        const methods = await listGeneralDetailsByCode(GM_CODES.TEACHING_METHODOLOGY)
        setMethodOptions([
          { value: '', label: '- None -' },
          ...methods.map((m) => ({
  ...
  const periodOptions = useMemo(() => {
    if (periods.length === 0) {
      return [{ value: '__none__', label: 'No periods assigned for today.', disabled: true }]
    }
  ...
  function onPeriodSelect(value: string | null) {
    ...
    if (isTheory) {
      ...
      const period = periods.find((p) => Number(p.timetableScheduleId) === id) ?? null
      setSelectedPeriod(period)
      setClassTimingId(period?.classTimingId != null ? Number(period.classTimingId) : null)
      const proxies = period?.staffProxies ?? []
      setIsAssignedProxy(proxies.length > 0)
    }
  }
  ...
  async function onSearch() {
    ...
    const period =
      ...
      (isTheory
        ...
        : periods.find((p) => Number(p.timetableScheduleId) === selectedPeriodIds[0])) ??
      null
    if (!period) {
    ...
    try {
      const list = await listStudentsForMarkAttendance({
        collegeId,
        courseGroupId,
        groupSectionId,
        academicYearId,
      ...
      setFlag(true)
      if (students.length === 0) toastSuccess('No Record(s) found.')

      const sid = Number(period.subjectResource?.[0]?.subjectId ?? subjectId)
      if (sid) {
  ...
  async function confirmSaveAttendance() {
    if (!pendingPayload || pendingPayload.length === 0 || !day || !selectedPeriod) {
      return
    }
  ...
  const previewSubjectName = useMemo(() => {
    const fromPeriod = String(selectedPeriod?.subjectResource?.[0]?.subjectName ?? '')
  ...
  const columnDefs = useMemo<ColDef<StudentRow>[]>(() => {
    const markRenderer = (p: ICellRendererParams<StudentRow>) => {
      ...
      if (mode === 'view') {
        return (
          <span className={checked ? 'text-emerald-600 font-semibold' : 'text-destructive font-semibold'}>
    ...
    return [
      ...
      },
      {
        headerName: 'Roll No.',
        minWidth: 120,
        valueGetter: (p) => String(p.data?.rollNumber ?? p.data?.admissionNumber ?? ''),
      },
      ...
      filters={
        <div className="space-y-4">
          <div className="rounded-sm border border-primary/30 bg-primary/5 px-4 py-3 space-y-1">
            ...
            <DetailRow label="Employee" value={searchParams.get('empName') ?? ''} />
            <DetailRow label="Subject" value={subjectLine} />
            <DetailRow label="Subject Code" value={searchParams.get('subjectCode') ?? ''} />
          </div>
          ...
          {isAssignedProxy && selectedPeriod?.staffProxies?.[0] ? (
            <p className="text-sm text-destructive">
              ...
              <span className="font-medium">
                ...
                {String(selectedPeriod.staffProxies[0].proxySubjecttypeDisplayName ?? '')})
              </span>
              .
            </p>
          ) : null}
    ...
    >
      <div className="app-data-table app-data-table-card flex flex-col">
        <div className="app-data-table-heading px-5 pt-5 pb-0">
          <h2 className="text-lg font-semibold tracking-tight text-foreground">Lesson Status</h2>
        </div>
        <div className="global-filter-bar__inner px-5 pb-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
            <Select
              ...
              options={unitOptions.length ? unitOptions : [{ value: '', label: '- None -' }]}
              searchable
            ...
            <Select
              ...
              value={topicId}
              onChange={setTopicId}
              options={topicOptions.length ? topicOptions : [{ value: '', label: '- None -' }]}
              searchable
              className="md:col-span-4"
            />
            <Select
              ...
              options={methodOptions.length ? methodOptions : [{ value: '', label: '- None -' }]}
              searchable
              className="md:col-span-4"
            />
          </div>
      ...
      {flag && rows.length > 0 ? (
        <div className="space-y-3">
          <p className="text-sm font-semibold text-[hsl(var(--card-title))] px-1">
            Attendance -{' '}
            <span className="font-normal text-muted-foreground">{attendanceHeader}</span>
          </p>
                    ...
                    <label className="inline-flex items-center gap-1.5 text-sm cursor-pointer">
                      <input
                        ...
                        onChange={() => {
                          ...
                          setRows((prev) =>
                            prev.map((r) => ({ ...r, checked: next, isPresent: next })),
                          )
                        }}
                      />
                      <span className="font-medium">
            ...
            <div className="lg:col-span-4 xl:col-span-3">
              <div className="rounded-sm border bg-card h-full min-h-[200px]">
                <div className="flex items-center justify-between border-b px-3 py-2 bg-muted/40">
                  <h3 className="text-sm font-semibold uppercase tracking-wide">Absentees</h3>
                  <span className="text-sm font-semibold">{absentees.length}</span>
                </div>
                  ...
                  {absentees.length === 0 ? (
                    ...
                    absentees.map((a) => (
                      <p key={String(a.studentId)}>
                        {String(a.firstName ?? '')} - {String(a.rollNumber ?? a.admissionNumber ?? '')}
                      </p>
                    ))
                  )}
                </div>
          ...
          <div className="rounded-sm border border-primary/20 bg-primary/5 px-4 py-3 flex flex-wrap items-end gap-3">
            <div className="space-y-1">
              ...
              />
              {classNotesFile ? (
                <p className="text-xs text-muted-foreground">{classNotesFile.name}</p>
              ) : null}
            </div>
            <div className="flex-1 min-w-[220px] space-y-1">
              <Label className="text-sm font-medium" htmlFor="class-notes-video">
