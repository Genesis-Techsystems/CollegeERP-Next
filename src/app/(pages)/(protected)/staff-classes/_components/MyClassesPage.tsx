...
function ActionItem({ label, icon, onClick, href }: Readonly<ActionItemProps>) {
  ...
  const content = (
    <>
      <span className="inline-flex shrink-0 [&_svg]:h-6 [&_svg]:w-6">{icon}</span>
...
function ClassActions({
  ...
  const go = (path: string) => router.push(`/staff-classes/my-classes/${path}?${qs}`)
...
export function MyClassesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const userName =
        String(user?.userName ?? readStorage('uName') ?? readStorage('userName') ?? '') || 'Host'
