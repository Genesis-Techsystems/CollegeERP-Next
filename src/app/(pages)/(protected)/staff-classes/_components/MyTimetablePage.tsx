...
import { printStaffMyTimetable } from "../_print/my-timetable-print";
...
function TimetableCell({ timing }: { timing: MyTimetableTiming }) {
    ...
    >
      {subBatches.length > 0 ? (
        subBatches.map((batch, i) => (
          ...
          >
            <p className="font-medium text-[15px]" style={{ color: fg }}>
            ...
            <p className="text-[10px]" style={{ color: fg }}>
...
function WeekTimetableTable({ schedule }: { schedule: MyTimetableSchedule }) {
  return (
      ...
      <tbody>
        {schedule.weekdays.map((weekday) => (
          <tr key={weekday.weekdayId}>
            <th className="border border-[#ddd] bg-[#C3D9FF] px-[5px] py-[5px] text-left font-medium uppercase">
              {weekday.weekdayName}
            </th>
...
export function MyTimetablePage() {
  ...
  function onPrintTimetable() {
    if (!hasWeekGrid) return;
    printStaffMyTimetable({
      collegeName,
      employeeName,
      schedule,
    });
        ...
        <div className="border-t border-border">
          {viewMode === "1" ? (
            ...
            <>
              ...
              <Tabs value={activeDay} onValueChange={onDayTabChange}>
