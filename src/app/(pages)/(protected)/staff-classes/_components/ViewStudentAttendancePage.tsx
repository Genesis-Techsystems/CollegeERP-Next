...
function isLessonMarked(lesson: unknown): boolean {
  ...
  if (typeof lesson === 'object') return Object.keys(lesson as object).length > 0
  return true
}
...
function statusRenderer(p: ICellRendererParams<AttendanceAbsentRow>) {
  ...
  return (
    <span className={absent ? 'text-destructive font-semibold' : 'text-emerald-600 font-semibold'}>
      {absent ? 'A' : 'P'}
    </span>
  )
}
...
export function ViewStudentAttendancePage() {
  ...
  const periodOptions = useMemo(() => {
    if (periods.length === 0) {
      return [{ value: '__none__', label: 'No periods are assigned for today.', disabled: true }]
    }
  ...
  async function onPeriodChange(value: string | null) {
    ...
    const subjectFromPeriod = Number(period.subjectResource?.[0]?.subjectId ?? subjectId)
  ...
  const showNotMarked =
    flag && lessonLoaded && !hasLesson && Boolean(classTimingId) && !isAssignedProxy
    ...
    <FilteredPage
      ...
      body={
        showTable ? (
          ...
          <p className="text-sm text-muted-foreground py-4">Loading attendance…</p>
        ) : null
      }
