...
export function StaffAttendanceNotMarkedListPage() {
  ...
  /** `'R'` = Dept Emp → `in_issamedept=1`; `'G'` = Other Dept Emp → `in_issamedept=0` */
  ...
  async function getStaff() {
    ...
    try {
      ...
      if (rows.length === 0) toastSuccess("No Record(s) found.");
    } catch (e) {
      toastError(e, "Failed to load staff");
    } finally {
      setLoading(false);
    }
  }
  ...
  // Angular: `Other Dept Emp ({{departmentsHead[0].groupCode}})` only when heads.length > 0
  const otherDeptGroupCode =
    deptHeads.length > 0 ? String(deptHeads[0]?.groupCode ?? "").trim() : "";
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="ml-[35px] flex items-center gap-2">
          ...
          >
            Other Dept Emp
            {otherDeptGroupCode ? ` (${otherDeptGroupCode})` : null}
          </Label>
        </div>
      </RadioGroup>
  ...
  // Angular: mat-card table only when `staff.length > 0`
  if (staff.length === 0) {
    return (
      <FilteredListPage
