...
import { ATTENDANCE_HUB_SECTIONS } from '../_lib/route-config'
    ...
    <PageContainer className="space-y-5">
      <div className="app-card overflow-hidden">
        <div className="border-b bg-gradient-to-br from-slate-50 to-white px-6 py-10 text-center">
          <ClipboardCheck className="mx-auto h-12 w-12 text-[hsl(var(--primary))] opacity-90" aria-hidden />
          <h1 className="mt-4 text-xl font-semibold tracking-tight text-[hsl(var(--card-title))]">
            Attendance Management
          </h1>
          <p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground">
            Student and staff attendance, workload adjustments, and exam attendance — mirrored from
            Angular admin-attendance-management and related modules.
          </p>
        </div>
        <div className="space-y-6 p-4">
          {ATTENDANCE_HUB_SECTIONS.map((section) => (
