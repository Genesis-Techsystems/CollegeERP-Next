...
export function StudentDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!nextProfile.collegeId)
        nextProfile.collegeId = positiveId(user.collegeId);
      ...
      if (!nextProfile.academicYear)
        nextProfile.academicYear = user.academicYear;
      ...
      const status = (
        nextProfile.studentStatusCode || "INCOLLEGE"
      ).toUpperCase();
      if (
        status === "INCOLLEGE" &&
            ...
            {events.length > 0 ? (
              <ul className="divide-y divide-[#e8e8e8]">
                {events.map((ev, idx) => (
                  <li
                    key={String(
                      (ev as AnyRow).eventId ?? (ev as AnyRow).event_id ?? idx,
                    )}
                    className="px-3 py-2"
                  >
