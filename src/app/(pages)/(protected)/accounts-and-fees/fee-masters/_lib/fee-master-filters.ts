...
export function dedupeBy<T>(
  rows: T[],
  keyFn: (r: T) => string | number | null | undefined,
): T[] {
...
export function filterCollegesByUniversity(
  rows: FilterRow[],
...
export function filterBatches(rows: FilterRow[], courseId: number | null) {
  return dedupeBy(
    ...
    (a, b) =>
      Number(pickText(b, ["batch_name"])) - Number(pickText(a, ["batch_name"])),
  );
}
...
export function filterAcademicYears(
  ...
  return dedupeBy(
    rows.filter(
      (r) => pickNum(r, UNI) === universityId || pickNum(r, UNI) === 0,
    ),
    (r) => pickNum(r, AY),
  ).sort(
    (a, b) =>
...
export function filterCoursesByUniversity(
  rows: FilterRow[],
  universityId: number | null,
) {
  return dedupeBy(
...
export function filterAcademicYearsByUniversity(
  rows: FilterRow[],
  universityId: number | null,
) {
  return dedupeBy(
    rows.filter(
      (r) => rowMatches(r, UNI, universityId) || pickNum(r, UNI) === 0,
    ),
    (r) => pickNum(r, AY),
  ).sort(
    (a, b) =>
...
export function collegeOption(row: FilterRow) {
  ...
  return {
    value: String(id),
    label: pickText(row, ["college_code", "collegeCode"]) || String(id),
  };
}
...
export function courseOption(row: FilterRow) {
  ...
  return {
    value: String(id),
    label: name && code ? `${code} (${name})` : code || String(id),
  };
}
...
export function batchOption(row: FilterRow) {
  ...
  return {
    value: String(id),
    label: pickText(row, ["batch_name", "batchName"]) || String(id),
  };
}
...
export function academicYearOption(row: FilterRow) {
  ...
  return {
    value: String(id),
    label: pickText(row, ["academic_year", "academicYear"]) || String(id),
  };
}
...
/** Angular regulation dropdown — `clg_filters_regulation` rows by university + course. */
export function filterRegulations(
  regulationData: FilterRow[],
  universityId: number | null,
  courseId: number | null,
) {
  return dedupeBy(
...
export function regulationOption(row: FilterRow) {
  const id = pickNum(row, ["fk_regulation_id", "regulationId"]);
  return {
    value: String(id),
    label: pickText(row, ["regulation_code", "regulationCode"]) || String(id),
  };
}
...
export function courseGroupOption(row: FilterRow) {
  ...
  return {
    value: String(id),
    label: pickText(row, ["group_code", "groupCode"]) || String(id),
  };
}
