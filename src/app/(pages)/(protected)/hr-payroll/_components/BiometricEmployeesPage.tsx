...
const COL_DEFS = {
  ...
  startDate: {
    field: "startAttendanceDate",
    headerName: "Start Attendance Date",
    minWidth: 140,
  } as ColDef<BioRow>,
  latestDate: {
    field: "latestattendanceDate",
    headerName: "Latest Attendance Date",
    minWidth: 150,
  } as ColDef<BioRow>,
  biometric: {
    field: "employeeName",
    headerName: "Biometric Employee",
    minWidth: 150,
  } as ColDef<BioRow>,
  numericCode: {
    field: "numericCode",
    headerName: "Number Code",
    minWidth: 110,
  } as ColDef<BioRow>,
...
export function BiometricEmployeesPage() {
  ...
  const { data, isFetching, error, refetch, dataUpdatedAt, errorUpdatedAt } =
    useQuery({
      queryKey: QK.hrPayroll.biometricEmployees(
        collegeId,
        page,
        PAGE_SIZE,
        unassignedOnly,
  ...
  useEffect(() => {
    if (!fetchEnabled || isFetching) return;
    if (error) {
      const message = getErrorMessage(error);
      if (/no\s*record/i.test(message)) {
        toast("No Record(s) found.", { id: "biometric-no-records" });
      } else {
        toastError(error, "Failed to load biometric employees");
      }
      return;
    }
    if (dataUpdatedAt > 0 && rows.length === 0) {
      toast("No Record(s) found.", { id: "biometric-no-records" });
    }
  }, [
    dataUpdatedAt,
  ...
  const invalidate = useCallback(() => {
    void queryClient.invalidateQueries({
      queryKey: ["HrPayroll", "biometricEmployees"],
    });
  }, [queryClient]);
  ...
  async function handleMapSave(empId: number) {
    ...
    setMapSaving(true);
    try {
      await updateBiometricAttendanceEmployee(attendanceEmpsId, {
        ...mapRow,
        empId,
      });
    ...
    () => [
      ...
      COL_DEFS.category,
      {
        ...
        valueFormatter: (p) =>
          formatDisplayDate(
            p.value,
            (p.data?.shiftDTO as BioRow | undefined) != null,
          ),
      },
      {
        ...COL_DEFS.latestDate,
      ...
      {
        ...COL_DEFS.actions,
        cellRenderer: makeActionsRenderer(onAssign, onShift),
      },
    ],
