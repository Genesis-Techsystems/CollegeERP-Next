...
function valuesFromRow(row: Batch): FormValues {
  ...
  return {
    universityId:
      universityId > 0 ? universityId : (undefined as unknown as number),
    courseId: courseId > 0 ? courseId : (undefined as unknown as number),
    regulationId:
      regulationId > 0 ? regulationId : (undefined as unknown as number),
...
export default function BatchModal({
  ...
  const universityOptions = useMemo(() => {
    ...
    if (
      ...
      !opts.some((o) => o.value === String(selectedUniversityId))
    ) {
      opts.unshift({
  ...
  const regulationOptions = useMemo(() => {
    ...
    if (
      ...
      !opts.some((o) => o.value === String(selectedRegulationId))
    ) {
      const label =
        source?.regulationCode ||
        source?.regulationName ||
    ...
    void (async () => {
      try {
        ...
        const merged: Batch = {
          ...
          batchId: row.batchId,
          universityId:
            num(detail?.universityId) || num(row.universityId) || undefined,
          courseId: num(detail?.courseId) || num(row.courseId) || undefined,
          regulationId:
            num(detail?.regulationId) || num(row.regulationId) || undefined,
          ...
          fromDate:
            ...
            row.batchFrom,
          toDate:
            detail?.toDate || detail?.batchTo || row.toDate || row.batchTo,
  ...
  useEffect(() => {
    ...
    void (async () => {
      for (const university of universities) {
        if (cancelled) return;
        try {
          const rows = await listActiveCoursesByUniversity(
            university.universityId,
          );
          const found = rows.some((c) => num(c.courseId) === courseId);
          if (found) {
            setValue("universityId", university.universityId, {
              shouldDirty: false,
            });
            setEditSource((prev) =>
              prev
