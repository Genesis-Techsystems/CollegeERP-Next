    ...
    occupancy: z.preprocess(
      (value) =>
        value === "" || value == null || Number.isNaN(value)
          ...
          : value,
      z
        .number({ error: "Occupancy is required" })
...
export default function RoomModal({
  ...
  const floorOptions = useMemo(
    () =>
      asOptions(
        ...
        (r) => r.floorId,
        (r) =>
          `${r.floorName ?? ""} ${r.floorNo ? `(${String(r.floorNo)})` : ""}`.trim(),
      ),
    [floors],
  );
  ...
  useEffect(() => {
    if (room) {
      ...
      reset({
        blockId:
          num(room.blockId ?? raw.block_id ?? raw.fk_block_id) ??
          (undefined as unknown as number),
        floorId:
          num(room.floorId ?? raw.floor_id ?? raw.fk_floor_id) ??
          (undefined as unknown as number),
        roomTypeId:
          num(room.roomTypeId ?? raw.room_type_id ?? raw.fk_room_type_id) ??
          (undefined as unknown as number),
        ...
        examcolumns: num(
          room.examcolumns ?? raw.examcolumns ?? raw.exam_columns,
        ),
      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <FormField label="Floor" required error={errors.floorId?.message}>
              <Controller
                name="floorId"
                control={control}
                render={({ field }) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:max-w-[66%]">
            ...
            <FormField
              label="Total Columns"
              htmlFor="examcolumns"
              error={errors.examcolumns?.message}
            >
              <Input
                id="examcolumns"
                type="number"
