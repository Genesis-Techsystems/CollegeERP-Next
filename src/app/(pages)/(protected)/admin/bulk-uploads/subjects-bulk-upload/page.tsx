...
const SUBJECT_COLS: ColDef<SubjectBulkRow>[] = [
  ...
  },
  {
    ...
    flex: 1.3,
    valueGetter: (p) =>
      [p.data?.university, p.data?.course].filter(Boolean).join(" / "),
  },
  {
    field: "subjectCode",
...
export default function SubjectsBulkUploadPage() {
  ...
  return (
    <FilteredListPage
      title="Subjects Bulk Upload"
      filters={
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <label
              htmlFor="subject-bulk-file"
              className="text-sm font-medium text-slate-700 shrink-0"
            >
              Upload Subjects :
            </label>
            <input
              id="subject-bulk-file"
              ref={fileInputRef}
              type="file"
              accept=".xls,.xlsx"
          ...
          {xlsxCount > 0 ? (
            <p className="text-xs text-red-600">
              Total number of Subjects listed in xlsx sheet are {xlsxCount}.
            </p>
          ) : null}
        </div>
      }
