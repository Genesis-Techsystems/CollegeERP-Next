...
const COL_DEFS = {
  ...
  landMark: {
    ...
    minWidth: 150,
    flex: 1,
    // API returns `landmark`; type/form historically used `landMark`.
    valueGetter: (p) => {
      const row = p.data as
        | (Building & { landmark?: string | null })
        | undefined;
      const value = row?.landMark ?? row?.landmark;
...
export default function BuildingsPage() {
  ...
  const columnDefs = useMemo<ColDef<Building>[]>(
    () => [
      ...
      {
        ...COL_DEFS.actions,
        cellRenderer: makeActionsRenderer(setEditingBuilding, setModalOpen),
      },
    ],
  ...
  return (
    ...
    >
      <BuildingModal
        key={editingBuilding?.buildingId ?? "add-building"}
        open={modalOpen}
        onClose={() => {
          setModalOpen(false);
          setEditingBuilding(null);
