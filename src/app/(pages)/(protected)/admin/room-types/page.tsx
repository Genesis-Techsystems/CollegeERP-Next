...
export default function RoomTypesPage() {
  ...
  const filteredData = useMemo(() => {
    if (!searchValue.trim()) return roomTypes
    const lower = searchValue.toLowerCase()
    return roomTypes.filter((row) =>
      Object.values(row).some((val) => toSearchText(val).toLowerCase().includes(lower)),
    )
  }, [searchValue, roomTypes])
    ...
    <PageContainer className="space-y-5">
      <div className="app-card overflow-hidden">
        <div className="px-3 py-2.5 border-b border-slate-200 bg-slate-50/60">
          <h2 className="text-[14px] font-semibold text-[hsl(var(--primary))]">Room Types</h2>
        </div>
