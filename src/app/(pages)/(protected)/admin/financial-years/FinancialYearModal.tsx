  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      const duplicate = existingRows.some(
        (row) =>
          row.financialYear.toLowerCase() ===
            data.financialYear.toLowerCase() &&
      ...
      if (duplicate) {
        setSubmitError(
          "Financial year already exists for the selected organization and university",
        );
        return;
      }
      ...
      setSubmitError(
        err instanceof Error ? err.message : "Failed to save financial year",
      );
    }
  }
