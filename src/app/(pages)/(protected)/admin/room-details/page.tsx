...
function deepFindTextByKeyTokens(
  ...
  for (const [key, child] of Object.entries(row)) {
    ...
    const hasInclude = includeTokens.some((token) =>
    ...
    const nested = deepFindTextByKeyTokens(
      child,
      includeTokens,
      excludeTokens,
      depth + 1,
...
const COL_DEFS = {
  ...
  campusName: {
    ...
    flex: 1,
    valueGetter: (p) =>
      pickText((p.data ?? {}) as RoomDetail, ["campusName", "campus_name"]),
  } as ColDef<RoomDetail>,
  blockName: {
  ...
  deviceName: {
    ...
    valueGetter: (p) => {
      ...
      if (tokenValue && !isLikelyWrongDeviceValue(tokenValue))
        return tokenValue;
      return "";
    },
  } as ColDef<RoomDetail>,
...
export default function RoomDetailsPage() {
  ...
  const roomOptions = useMemo(() => {
    // Room list payload has blockId/floorId only — not buildingId (Angular same shape).
    // Filter by block/floor; when only building is selected, restrict to that building's blocks.
    const selectedBlockId = Number(blockId ?? 0);
    const selectedFloorId = Number(floorId ?? 0);
    const selectedBuildingId = Number(buildingId ?? 0);
    const blockIdsForBuilding = new Set(blocks.map((b) => Number(b.blockId)));

    const filtered = rooms.filter((room) => {
      const roomBlockId = pickBlockId(room);
      const roomFloorId = pickFloorId(room);
      if (selectedFloorId && roomFloorId !== selectedFloorId) return false;
      if (selectedBlockId && roomBlockId !== selectedBlockId) return false;
      if (
        selectedBuildingId &&
        !selectedBlockId &&
        blockIdsForBuilding.size > 0 &&
        !blockIdsForBuilding.has(roomBlockId)
      ) {
        return false;
      }
      return true;
