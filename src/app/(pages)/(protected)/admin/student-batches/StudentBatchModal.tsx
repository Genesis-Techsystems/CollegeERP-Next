...
export default function StudentBatchModal({
  ...
  const universityId = useMemo(() => {
    ...
    if (defaultUniversityId && defaultUniversityId > 0)
  ...
  const collegeOptions = useMemo(() => {
    ...
    if (
      selectedCollegeId &&
      !opts.some((o) => o.value === String(selectedCollegeId))
    ) {
      opts.unshift({
        value: String(selectedCollegeId),
        label:
          row?.collegeCode ?? row?.collegeName ?? String(selectedCollegeId),
      });
    }
    return opts;
  ...
  const courseOptions = useMemo(() => {
    ...
    const opts = courses
      .map((c) => {
        ...
        const code = pickText(c, [
          "courseCode",
          "course_code",
          "course.courseCode",
          "Course.courseCode",
        ]);
        const name = pickText(c, [
          "courseName",
          "course_name",
          "course.courseName",
          "Course.courseName",
    ...
    if (
      selectedCourseId &&
      !opts.some((o) => o.value === String(selectedCourseId))
    ) {
      ...
      const label =
        code && name
          ? `${code} - ${name}`
          : code || name || String(selectedCourseId);
  ...
  const subjectTypeOptions = useMemo(() => {
    ...
    if (
      selectedSubtypeId &&
      !opts.some((o) => o.value === String(selectedSubtypeId))
    ) {
      opts.unshift({
  ...
  useEffect(() => {
    ...
    listActiveCollegesForStudentBatches()
      ...
      .catch(console.error);
    listSubjectTypesForStudentBatches()
      .then((rows) => setSubjectTypes(Array.isArray(rows) ? rows : []))
    ...
    if (row) {
      ...
      reset({
        collegeId:
          defaultCollegeId && defaultCollegeId > 0
            ? defaultCollegeId
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      ...
      setSubmitError(
        e instanceof Error ? e.message : "Failed to save student batch",
      );
    }
  }
  ...
  return (
    <FormModal
      open={open}
      onClose={onClose}
      title={isEditing ? "Edit Student Batch" : "Add Student Batch"}
