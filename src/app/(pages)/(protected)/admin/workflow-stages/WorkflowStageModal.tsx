...
export default function WorkflowStageModal({
  ...
  useEffect(() => {
    if (row) {
      reset({
        ...
        goBackPoint:
          row.goBackPoint != null && row.goBackPoint !== false
            ? String(row.goBackPoint)
            : "",
        isSelfAvailable:
          row.isSelfAvailable === true || row.isSelfAvailable === 1,
  ...
  const collegeOptions = useMemo(
    () =>
      colleges.map((college) => ({
        ...
        label: college.collegeCode ?? college.collegeName,
      })),
    [colleges],
  );
  ...
  async function onSubmit(data: FormValues) {
    ...
    const payload = {
      ...data,
      availableFor: data.availableFor?.trim()
        ? Number(data.availableFor)
        : null,
      goBackPoint: data.goBackPoint?.trim() ? Number(data.goBackPoint) : null,
    };
    try {
      if (isEditing)
        await updateWorkflowStage(row!.workflowStageId, payload, row!);
      else
        await createWorkflowStage(
          payload as Omit<WorkflowStage, "workflowStageId">,
        );
      onSaved();
      onClose();
    } catch (error: unknown) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2 py-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              {errors.wfName && (
                <p className="text-xs text-red-500">{errors.wfName.message}</p>
              )}
            </div>
            <div>
              ...
              {errors.wfCode && (
                <p className="text-xs text-red-500">{errors.wfCode.message}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              ...
              <Input
                id="wfStage"
                type="number"
                {...register("wfStage", { valueAsNumber: true })}
              />
            </div>
            <div>
