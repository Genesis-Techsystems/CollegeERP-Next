...
function nestedId(row: object, rel: string, idKey: string): number | undefined {
  const relObj = (row as Record<string, unknown>)[rel];
  if (!relObj || typeof relObj !== "object") return undefined;
  return numId((relObj as Record<string, unknown>)[idKey]);
}

function formValuesFromRow(row: DocumentRepository | null): FormValues {
  if (!row) {
    return {
      ...
      docTypeId: undefined as unknown as number,
      docFormId: undefined as unknown as number,
      fileNameFormat: "",
      docName: "",
      docCode: "",
      isForStudent: false,
  ...
  return {
    organizationId:
      numId(row.organizationId) ??
      nestedId(row, "organization", "organizationId") ??
      nestedId(row, "Organization", "organizationId") ??
      (undefined as unknown as number),
    universityId:
      numId(row.universityId) ??
      nestedId(row, "universities", "universityId") ??
      nestedId(row, "Universities", "universityId") ??
      nestedId(row, "university", "universityId") ??
      (undefined as unknown as number),
    collegeId:
      numId(row.collegeId) ??
      nestedId(row, "college", "collegeId") ??
      nestedId(row, "College", "collegeId") ??
      (undefined as unknown as number),
    courseId:
      numId(row.courseId) ??
      nestedId(row, "course", "courseId") ??
      nestedId(row, "Course", "courseId") ??
      (undefined as unknown as number),
    docTypeId:
      numId(row.docTypeId) ??
      nestedId(row, "docType", "generalDetailId") ??
      nestedId(row, "documentType", "generalDetailId") ??
      (undefined as unknown as number),
    docFormId:
      numId(row.docFormId) ??
      nestedId(row, "docForm", "generalDetailId") ??
      nestedId(row, "documentForm", "generalDetailId") ??
      nestedId(row, "docFormType", "generalDetailId") ??
      (undefined as unknown as number),
...
function repositoryIdOf(row: DocumentRepository | null): number | undefined {
  if (!row) return undefined;
  return (
    numId(row.documentRepositoryId) ??
    nestedId(row, "documentRepository", "documentRepositoryId")
  );
}
...
export default function DocumentRepositoryModal({
  ...
  const {
    ...
    defaultValues: formValuesFromRow(row),
  });
  ...
  const universityOptions = useMemo(
    () =>
      universities.map((item) => ({
        value: String(item.universityId),
        label: item.universityCode ?? item.universityName,
      })),
    [universities],
  );
  const collegeOptions = useMemo(
    () =>
      colleges.map((item) => ({
        value: String(item.collegeId),
        label: item.collegeCode ?? item.collegeName,
      })),
    [colleges],
  );
  ...
  const docTypeOptions = useMemo(
    () =>
      docTypes.map((item) => ({
        ...
        label: item.generalDetailCode ?? item.name ?? "-",
      })),
    [docTypes],
  );
  ...
  useEffect(() => {
    ...
    const fromRow = formValuesFromRow(row);
    Promise.all([
      ...
      .then(([collegeRows, courseRows]) => {
        ...
        if (fromRow.universityId === universityId) {
          if (fromRow.collegeId) setValue("collegeId", fromRow.collegeId);
          if (fromRow.courseId) setValue("courseId", fromRow.courseId);
        }
      })
      .catch((error) =>
        setSubmitError(
  ...
  }, [open, universityId, row, setValue]);
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      if (!isEditing) {
        ...
        await updateDocumentRepository(repositoryIdOf(row)!, values);
      }
      onSaved();
      onClose();
    } catch (error: unknown) {
      setSubmitError(
        error instanceof Error
          ? error.message
          : "Failed to save document repository",
      );
    }
  }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2 py-1">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <div>
              ...
              {errors.docName && (
                <p className="text-xs text-red-500">{errors.docName.message}</p>
              )}
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 pt-1">
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setValue("isForStudent", Boolean(checked))
                }
              />
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setValue("isForEmp", Boolean(checked))
                }
              />
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setValue("isMandatory", Boolean(checked))
                }
              />
