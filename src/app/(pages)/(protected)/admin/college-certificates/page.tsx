...
const COL_DEFS = {
  ...
  duplicateAmount: {
    field: "duplicateCertificateAmount",
    headerName: "Duplicate Amt",
    minWidth: 120,
    flex: 0.9,
  } as ColDef<CollegeCertificate>,
  college: {
...
function pickText(row: Record<string, unknown>, keys: string[]): string {
  for (const key of keys) {
    if (key.includes(".")) {
      const [rel, nestedKey] = key.split(".");
      const relObj = row[rel];
      if (relObj && typeof relObj === "object") {
        const nested = (relObj as Record<string, unknown>)[nestedKey];
  ...
  return (
    <ListPage
      ...
      toolbar={{
        ...
        pdfDocumentTitle: "College Certificates",
      }}
      toolbarTrailing={
        <Button
          size="sm"
