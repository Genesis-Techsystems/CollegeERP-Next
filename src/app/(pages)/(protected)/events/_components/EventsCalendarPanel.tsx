    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between px-3 py-2">
        <p className="truncate text-[13px] font-medium text-foreground">
          ...
          {event.isHoliday ? (
            <span className="ml-1 font-normal text-amber-600 dark:text-amber-400">(Week Off)</span>
          ) : null}
        </p>
        {dateRange ? (
          <p className="mt-1 text-right text-[11px] text-muted-foreground">{dateRange}</p>
        ) : null}
      </div>
    </button>
  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="min-w-0 border-b border-border lg:border-b-0 lg:border-r">
          <div className="grid grid-cols-7 border-b border-[hsl(var(--primary))]/20 bg-[hsl(var(--primary))]/10 text-center text-[11px] font-semibold text-[hsl(var(--primary))]">
            {WEEKDAYS.map((d) => (
              <div key={d} className="border-r border-[hsl(var(--primary))]/15 px-1 py-2 last:border-r-0">
                {d}
              </div>
            ))}
          </div>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isDaySelectable && inMonth && 'cursor-pointer hover:bg-muted/30',
                ...
                >
                  <div className="flex items-start justify-between gap-1">
                    ...
                    <span className="text-[12px] font-medium">{format(day, 'd')}</span>
                  </div>
                </div>
              )
            })}
        ...
        <div className="flex max-h-[520px] flex-col gap-2 overflow-y-auto p-3 lg:max-h-none">
          {monthEvents.length === 0 ? (
            <p className="py-6 text-center text-[13px] text-muted-foreground">No events this month</p>
          ) : (
            monthEvents.map((ev) => (
              <EventListCard
