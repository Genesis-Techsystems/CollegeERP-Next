...
export function CollegeEventsPage({ title, variant }: Readonly<CollegeEventsPageProps>) {
  ...
  useEffect(() => {
    if (!isCalendarView || useStorageFilters || collegeId != null || colleges.length === 0) return
  ...
  const loadEvents = useCallback(async (modeOverride?: 'month' | 'list') => {
  ...
  const calendarListColumnDefs = useMemo<ColDef<CollegeEventRow>[]>(
    () => [
      ...
      },
      {
        ...
        flex: 0.8,
        sortable: true,
        valueGetter: (p) => formatWeekday(String(p.data?.startDate ?? p.data?.eventDate ?? '')),
      },
      {
      ...
      <div className="space-y-3">
        ...
        {calendarLoaded ? (
          <RadioGroup
            ...
            onValueChange={(v) => onCalendarViewModeChange(v as 'month' | 'list')}
