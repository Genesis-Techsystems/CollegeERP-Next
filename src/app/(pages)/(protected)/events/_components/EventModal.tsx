...
export function EventModal({
  ...
  const showStudentFields =
    selectedAudienceCode === "STD" || selectedAudienceCode === "Parents";
  ...
  useEffect(() => {
    ...
    const activeAudiences = (row?.eventAudiences ?? []).filter(
      (a) => a.isActive !== false,
    ) as AudienceTableRow[];
  ...
  function audienceExists(next: AudienceTableRow): boolean {
    return eventAudiences.some((existing) => {
      if (Number(existing.audienceTypeId) !== Number(next.audienceTypeId))
        return false;
      if (existing.audienceTypeCode === "ALL") return true;
      return (
        String(existing.categoryValue ?? "") ===
        String(next.categoryValue ?? "")
      );
    });
  }
  ...
  function addAudience() {
    ...
    const audienceMeta = audienceTypes.find(
      (a) => Number(a.generalDetailId) === audienceTypeId,
    );
  ...
  const audienceColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Audience Type",
        width: 25,
        render: (r: AudienceTableRow) =>
          r.audienceTypeCode ?? r.audienceTypeName ?? "—",
      },
      {
        id: "categoryName",
        label: "Category",
    ...
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
        ...
        <div className="flex items-center gap-2 sm:col-span-1">
          ...
          <Label
            htmlFor="isHoliday"
            className={`${EVENTS_FIELD_LABEL_CLASS} cursor-pointer`}
          >
            Holiday
          </Label>
        </div>
