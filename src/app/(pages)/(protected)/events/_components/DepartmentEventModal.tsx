...
function pickDeptEventId(
  saved: DepartmentEventRow | null | undefined,
  fallback?: number,
): number {
...
export function DepartmentEventModal({
  ...
  const [audienceKind, setAudienceKind] = useState<"student" | "employee">(
    "student",
  );
    ...
    try {
      ...
      setStudentOptions(
        rows.map((s) => {
          ...
          return {
            value: String(id),
            label: `${name}${roll}`.trim() || String(id),
          };
        }),
      );
    } catch {
      ...
      ...prev,
      {
        studentDetailId:
          audienceKind === "student"
            ...
            : undefined,
        studentDetailName:
          audienceKind === "student" ? selectedStudentMeta.name : undefined,
        studentDetailRollNumber:
          audienceKind === "student" ? selectedStudentMeta.roll : undefined,
        employeeDetailId:
          audienceKind === "employee"
            ? (selectedEmployeeId ?? undefined)
            : undefined,
        employeeDetailName:
          audienceKind === "employee" ? selectedEmployeeMeta.name : undefined,
        employeeDetailNumber:
          audienceKind === "employee" ? selectedEmployeeMeta.number : undefined,
  ...
  function removeAudience(index: number) {
    setAudiences((prev) => {
      ...
      const pk =
        removed?.deptEventAudienceId ?? removed?.departmentEventAudienceId;
      if (removed && pk) {
  ...
  function addResource() {
    ...
    setResources((prev) => [
      ...prev,
      { name, institute, isActive: true, path: null },
    ]);
    clearResourceDraft();
  }
  ...
  function removeResource(index: number) {
    setResources((prev) => {
      ...
      if (removed?.deptResourceId) {
        setDeletedResources((d) => [
          ...d,
          { ...removed, isActive: false, path: null },
        ]);
      }
      return next;
    });
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (deptEventId) {
        const uploaded = await buildAndUploadFiles(
          deptEventId,
          saved?.departmentEventResourceDTOS,
        );
        if (uploaded) {
          toastSuccess(
            isEditing ? "Department event updated" : "Department event created",
          );
        } else {
          toastSuccess(
            isEditing ? "Department event updated" : "Department event created",
          ...
          <div className="space-y-1 py-1">
            {!r.profileUrl ? (
              <Input
                ...
                className={cn(
                  EVENTS_INPUT_CLASS,
                  "h-8 cursor-pointer text-[11px]",
                )}
                onChange={(e) =>
                  setResourceFile(index, e.target.files?.[0] ?? null)
                }
              />
            ) : null}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              {...register("deptEventName")}
            />
            {errors.deptEventName ? (
              <p className="text-xs text-destructive">
                {errors.deptEventName.message}
              </p>
            ) : null}
          </div>
          <Controller
          ...
          <div className="space-y-1">
            <Label
              htmlFor="totalRegisrationAmount"
              className={EVENTS_FIELD_LABEL_CLASS}
            >
              Registration Amount
            </Label>
            <Input
              ...
              <div className="md:col-span-7">
                {audienceKind === "student" ? (
                  ...
                  <Select
                    label="Employee"
                    value={
                      selectedEmployeeId ? String(selectedEmployeeId) : null
                    }
