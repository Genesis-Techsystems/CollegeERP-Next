...
function ActionLink({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="text-[14px] font-normal text-[#007bff] hover:underline"
    >
      {label}
    </button>
  );
...
function ActionSeparator({ double = false }: { double?: boolean }) {
  return <span className="px-3 text-[14px] text-[#6c757d] select-none">|</span>;
}
...
function StudentNameCell({
  ...
  const name = pickText(row, ["firstName", "studentName"]).toUpperCase();
  const statusCode = pickText(row, ["studentStatusCode"]);
  const statusText = statusLabel(row);
  const meta = [
  ...
  const idNameParts: string[] = [];
  if (row.rollNumber == null && admission) idNameParts.push(admission);
  if (hallticket) idNameParts.push(hallticket);
  if (name) idNameParts.push(name);
  const idNameLine = idNameParts.join(", ");

  return (
    <button type="button" onClick={onOpenProfile} className="w-full text-left">
      <p className="relative pr-16 text-[13px] font-bold leading-snug text-[#00008b]">
        <span>{idNameLine}</span>
        <span className="absolute right-0 top-0 text-[11px] font-semibold text-[#1a5fb4]">
          {lateralTag(row)}
        </span>
      </p>
      {meta ? (
        <p className="mt-1 text-[11px] leading-relaxed text-[#333333]">
          {meta}
        </p>
      ) : null}
      <p className="mt-0.5 text-[11px] leading-relaxed text-[#333333]">
        {mobile ? <span>{mobile}</span> : <span>-</span>}
        {statusText ? (
          <>
            <span> | </span>
            <span className={statusClass(statusCode || statusText)}>
              {statusText}
            </span>
          </>
        ) : null}
    ...
    >
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] border-collapse text-[12px]">
          <thead>
            <tr
              className="text-left text-[12px] font-bold text-foreground"
              style={{ backgroundColor: COLORS.headerBg }}
            >
              <th
                className="w-24 border px-3 py-2 text-center"
                style={{ borderColor: COLORS.border }}
              >
                Photo
              </th>
              ...
              <th
                className="min-w-[300px] border px-3 py-2"
                style={{ borderColor: COLORS.border }}
              />
            </tr>
          </thead>
          <tbody>
            {pageRows.map((row, index) => {
              ...
              const showEdit = canNavigateEdit || canModalEdit;

              return (
                <tr
                  key={rowKey(row, safePage * size + index)}
                ...
                >
                  ...
                  >
                    ...
                    >
                      <img
                        src={photoSrc(
                          row.studentPhotoPath ?? row.student_photo_path,
                        )}
                        alt=""
                        className={cn(
                          "mx-auto h-[50px] w-[50px] rounded-full object-cover",
                          active
                  ...
                  >
                    <StudentNameCell
                      row={row}
                      onOpenProfile={() => onViewDetails(row)}
                    />
                  </td>
                  <td
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-center gap-y-1">
                      ...
                      <ActionSeparator double={showEdit} />
                      <ActionLink
                        label="View details"
                        onClick={() => onViewDetails(row)}
                      />
