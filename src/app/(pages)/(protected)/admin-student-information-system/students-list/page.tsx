...
function onSectionCascadeSelect(v: string | null, autoFillRef: MutableRefObject<boolean>) {
...
export default function StudentDetailsPage() {
  ...
  const [credRows, setCredRows] = useState<Array<{ studentId: number; collegeId: number }>>([])
  ...
  const specialEditRoles = ['Junior Accountant', 'Student Details', 'FEE COLLECTION']
  const canNavigateEdit = !isHod || isAdmin || specialEditRoles.includes(roleName)
  ...
  const universities = useMemo(
    () => dedupeBy(filtersData.filter((r) => pickNum(r, UNIV) > 0), (r) => pickNum(r, UNIV)),
    [filtersData],
  )

  const colleges = useMemo(() => {
    if (!universityId) return []
    return dedupeColleges(filtersData.filter((r) => pickNum(r, UNIV) === universityId))
  ...
  useEffect(() => {
    if (mode !== 'section' || loadingFilters || universities.length === 0) return
  ...
  const loadStudentsBySection = useCallback(
    async (sectionId: number | null = groupSectionId, showEmptyToast = false) => {
    ...
    [collegeId, academicYearId, courseId, courseGroupId, courseYearId, groupSectionId],
  )
  ...
  function navigateProfile(studentId: number) {
    router.push(`/admin-student-information-system/students-profile?studentId=${studentId}&check=${check}`)
  }

  function navigateEdit(studentId: number) {
    router.push(`/admin-student-information-system/edit-student?studentId=${studentId}&check=${check}`)
  }
  ...
  function navigateViewProfile(row: AnyRow) {
    ...
    router.push(`/principal-student-information-system/student-details?${params.toString()}`)
  }
  ...
  async function saveHodEdit(payload: AnyRow) {
    ...
    try {
      ...
      if (mode === 'student' && selectedStudentId) {
        const match = studentOptions.find((r) => pickNum(r, ['studentId', 'fk_student_id']) === selectedStudentId)
    ...
    <PageContainer className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-5 text-xs px-1">
        <label className="inline-flex items-center gap-1.5">
          <input type="radio" checked={mode === 'student'} onChange={() => resetForMode('student')} />
          <span>Search By Student</span>
        </label>
        <label className="inline-flex items-center gap-1.5">
          <input type="radio" checked={mode === 'section'} onChange={() => resetForMode('section')} />
      ...
      <div className="app-card overflow-hidden">
        <div className="px-4 py-3 border-b border-border bg-muted/40 flex items-center justify-between gap-2">
          <h2 className="app-card-title">{mode === 'section' ? 'Student Details' : 'Students Search'}</h2>
          <button
            type="button"
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
                  ...
                  <div className={selectClass()}>
                    <Select
                      label="Section"
                      value={groupSectionId === null ? null : String(groupSectionId)}
                      options={sectionOpts}
      ...
      <StudentsListTable
        ...
        onEditNavigate={(row) => navigateEdit(pickNum(row, ['studentId', 'fk_student_id']))}
        onEditModal={(row) => void openHodEdit(row)}
        onViewDetails={(row) => navigateProfile(pickNum(row, ['studentId', 'fk_student_id']))}
