...
export function StudentProfileHeader({ student, feeLedger }: StudentProfileHeaderProps) {
  const isLateral = student.isLateral === true
  const statusCode = pickText(student, ['studentStatusCode', 'statusCode'])
  const statusLabel = pickDisplay(student, ['studentStatusDisplayName', 'studentStatusName', 'statusName'])
  ...
  const metaFields: ProfileField[] = [
    {
      ...
      value: formatProfileDate(student.adminssionDate ?? student.admissionDate ?? student.admission_date),
    },
    { label: 'Quota', value: pickDisplay(student, ['quotaDisplayName', 'quotaName']) },
    {
      label: 'Category',
      value: pickDisplay(feeLedger, ['scholarship_type_code', 'scholarshipTypeCode']),
    },
  ...
  return (
    <div className="app-card overflow-hidden">
      <div className="flex items-center gap-2 border-b border-border bg-muted/40 px-4 py-2.5">
        ...
        <h2 className="text-sm font-semibold text-primary">Student Details</h2>
      </div>
      ...
      <div className="border-4 border-sky-200/80 bg-sky-50/20 p-3 sm:p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
          <div className="shrink-0">
            <img
              src={studentPhotoSrc(student.studentPhotoPath ?? student.student_photo_path)}
              alt=""
          ...
          <div className="min-w-0 flex-1 space-y-1 text-sm">
            <p className="font-medium text-foreground">
              ...
              <span className="font-semibold text-primary">({isLateral ? 'LATERAL' : 'REGULAR'})</span>
            </p>
            <p className="text-muted-foreground">{pickDisplay(student, ['hallticketNumber', 'rollNumber'])}</p>
            {pathLine ? <p className="text-muted-foreground">{pathLine}</p> : null}
            <p className="text-muted-foreground">{pickDisplay(student, ['mobile', 'mobileNumber', 'student_mobile_no'])}</p>
          </div>
            ...
            {metaFields.map((field) => (
              <p key={field.label} className="text-foreground">
                ...
                {field.label === 'Student Status' ? (
                  <span className={studentStatusClass(statusCode)}>{field.value}</span>
                ) : (
