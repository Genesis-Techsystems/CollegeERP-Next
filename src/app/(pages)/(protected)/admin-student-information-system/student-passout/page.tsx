...
export default function StudentPassoutPage() {
  ...
  const passedOutGdId = useMemo(() => resolvePassedOutGeneralDetailId(gmCodes), [gmCodes])
  ...
  const loadFilters = useCallback(async () => {
    ...
    try {
      ...
      setGmCodes(Array.isArray(r.studentStatusGmCodes) ? r.studentStatusGmCodes : [])
  ...
  const selectedCount = useMemo(
    () => students.filter((r) => r.checked && r.isPresent && statusUpper(r) !== 'PASSEDOUT').length,
    [students],
  )
  ...
  function handleMarkAllToggle() {
    ...
    setStudents((prev) =>
      prev.map((row) =>
        statusUpper(row) === 'PASSEDOUT' ? row : { ...row, checked: nextChecked, isPresent: nextChecked },
      ),
    )
  }
  ...
  const headerLine = useMemo(() => {
    ...
    return {
      ...
      yr: pickText(cy, ['course_year_name', 'courseYearName', 'course_year_code']) || '—',
    }
  }, [colleges, collegeId, academicYears, academicYearId, courses, courseId, courseGroups, courseGroupId, courseYears, courseYearId])

  function requestPassout() {
    if (!passedOutGdId) {
      toastError(new Error('Missing PASSEDOUT status'), 'General master STUDENTSTATUS is missing PASSEDOUT.')
      return
    }
    const chosen = students.filter((r) => r.checked && r.isPresent && statusUpper(r) !== 'PASSEDOUT')
    if (!chosen.length) {
  ...
  async function confirmPassout() {
    ...
    const chosen = students.filter((r) => r.checked && r.isPresent && statusUpper(r) !== 'PASSEDOUT')
    const payloads = chosen.map((row) => buildStudentPassoutPayload(row, passedOutGdId, dt))
    setSubmitting(true)
    try {
          ...
          <div className="rounded-lg border bg-card shadow-sm">
            <div className="border-b px-4 py-3">
              <p className="text-sm font-medium">
                Students — {headerLine.clg} / {headerLine.ac} / {headerLine.cr} / {headerLine.grp} / {headerLine.yr}{' '}
            ...
            <div className="flex flex-col gap-4 p-4 lg:flex-row">
                  ...
                  <table className="w-full min-w-[640px] text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50 text-left">
                        ...
                        <th className="px-3 py-2 font-medium">Hallticket No.</th>
                    ...
                    <tbody>
                      {loadingStudents ? (
                        ...
                        filteredStudents.map((row, index) => {
                          ...
                          const globalIdx = students.findIndex((s) => rowStudentKey(s) === k)
                          const siNo = globalIdx >= 0 ? globalIdx + 1 : index + 1
                          return (
                            <tr key={rowKey} className="border-b last:border-0">
                              ...
                              <td className="px-3 py-2">{pickText(row, ['firstName', 'first_name']) || '—'}</td>
                              <td className="px-3 py-2">{pickText(row, ['section', 'group_section_name']) || '—'}</td>
                              <td className="px-3 py-2">{pickText(row, ['studentStatusCode', 'student_status_code']) || '—'}</td>
                              <td className="px-3 py-2">
                                {disabled ? (
                                  '—'
                                ) : (
                                  <Checkbox
                                    ...
                                    onCheckedChange={(c) => handleCheckRow(rowStudentKey(row), c === true)}
                                  />
                                )}
                              </td>
                            </tr>
              ...
              <div className="w-full shrink-0 rounded-md border bg-muted/20 p-4 lg:w-72">
                <DatePicker label="Passed out on" value={passoutDate} onChange={setPassoutDate} />
                <Button
                ...
                {selectedCount > 0 && (
                  <p className="mt-2 text-xs text-muted-foreground">{selectedCount} student(s) selected</p>
                )}
              </div>
            </div>
          </div>
