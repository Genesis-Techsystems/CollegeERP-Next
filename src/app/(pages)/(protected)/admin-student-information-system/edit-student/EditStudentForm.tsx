...
export function EditStudentForm({ initialData, check }: EditStudentFormProps) {
  ...
  const [studentPhotoUrl, setStudentPhotoUrl] = useState(() =>
    photoSrc(initialData.studentPhotoPath),
  );
  const [fatherPhotoUrl, setFatherPhotoUrl] = useState(() =>
    photoSrc(initialData.fatherPhotoPath),
  );
  const [motherPhotoUrl, setMotherPhotoUrl] = useState(() =>
    photoSrc(initialData.motherPhotoPath, true, DEFAULT_MOTHER_PHOTO),
  );
  const [signaturePhotoUrl, setSignaturePhotoUrl] = useState(() =>
    photoSrc(initialData.studentSignaturePath),
  );

  const progress = useMemo(
    () => EDIT_STEPS.find((s) => s.id === activeStep)?.progress ?? 20,
    [activeStep],
  );
  ...
  const loadDocuments = useCallback(
    async (collegeId: number) => {
      const repos =
        await listDocumentRepositoriesForStudentByCollege(collegeId);
      setDocuments(
        mergeStudentDocuments(
          repos,
          ensureArray(initialData.studentDocumentCollections),
        ),
      );
    },
  ...
  const loadCollegeCascade = useCallback(
    async (collegeId: number, courseId?: number, courseGroupId?: number) => {
      ...
      const [ays, crs] = await Promise.all([
        ...
        univ
          ? listCoursesForUniversityCascade(univ)
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (collegeId) {
          await loadCollegeCascade(
            collegeId,
            num(initialData, ["courseId"]),
            num(initialData, ["courseGroupId"]),
          );
        }
        const casteId = num(initialData, ["casteId"]);
        if (casteId) setSubCastes(await listSubCastesByCasteId(casteId));
        if (initialData.presentCountryId)
          setPresentStates(
            await listStatesByCountry(Number(initialData.presentCountryId)),
          );
        if (initialData.presentStateId)
          setPresentDistricts(
            await listDistrictsByState(Number(initialData.presentStateId)),
          );
        if (initialData.presentDistrictId)
          setPresentCities(
            await listCitiesByDistrict(Number(initialData.presentDistrictId)),
          );
        if (initialData.permanentCountryId)
          setPermStates(
            await listStatesByCountry(Number(initialData.permanentCountryId)),
          );
        if (initialData.permanentStateId)
          setPermDistricts(
            await listDistrictsByState(Number(initialData.permanentStateId)),
          );
        if (initialData.permanentDistrictId)
          setPermCities(
            await listCitiesByDistrict(Number(initialData.permanentDistrictId)),
          );
      } catch (e) {
  ...
  function onSameAsPermanentChange(checked: boolean) {
    ...
    void (async () => {
      if (data.permanentCountryId)
        setPresentStates(
          await listStatesByCountry(Number(data.permanentCountryId)),
        );
      if (data.permanentStateId)
        setPresentDistricts(
          await listDistrictsByState(Number(data.permanentStateId)),
        );
      if (data.permanentDistrictId)
        setPresentCities(
          await listCitiesByDistrict(Number(data.permanentDistrictId)),
        );
    })();
  }
  ...
  function goBackToList() {
    ...
    if (check === 1) {
      ...
      if (data.academicYearId)
        params.set("academicYearId", String(data.academicYearId));
      ...
      if (data.courseGroupId)
        params.set("courseGroupId", String(data.courseGroupId));
      if (data.courseYearId)
        params.set("courseYearId", String(data.courseYearId));
      if (data.groupSectionId)
        params.set("groupSectionId", String(data.groupSectionId));
    }
    router.push(
  ...
  function buildPayload(): Record<string, unknown> {
    const quotaRow = quotas.find(
      (q) => num(q, ["generalDetailId"]) === num(data, ["quotaId"]),
    );
    const education = [
    ...
    for (const doc of documents) {
      const flagged =
        doc.isHardCopy || doc.isSoftCopy || doc.isOriginal || doc.isVerified;
      if (flagged || doc.stdDocCollId) {
        const row = {
          ...doc,
          docRepId: doc.documentRepositoryId,
          isActive: true,
        };
        if (!doc.path && !doc.filePath && doc.isSoftCopy)
          row.isSoftCopy = false;
    ...
    return {
      ...data,
      adminssionDate: toIsoDate(
        data.adminssionDate instanceof Date
          ? data.adminssionDate
          : data.adminssionDate
            ? new Date(data.adminssionDate)
            : null,
      ),
      dateOfRegistration: toIsoDate(
      ...
      dateOfExpiry: toIsoDate(
        data.dateOfExpiry instanceof Date
          ? data.dateOfExpiry
          : data.dateOfExpiry
            ? new Date(data.dateOfExpiry)
            : null,
      ),
  ...
  async function handleSubmit() {
    if (!data.firstName?.trim() || !data.mobile?.trim() || !data.dateOfBirth) {
      toastError(
        new Error(
          "Please complete required personal fields (name, mobile, date of birth).",
        ),
        "Validation",
      );
    ...
    try {
      ...
      const collections = ensureArray<AnyRow>(
        result?.studentDocumentCollections ??
          result?.data?.studentDocumentCollections,
      );
      ...
      formData.append(
        "orgCode",
        String(user?.organizationCode ?? readStorage("orgCode")),
      );
      formData.append(
        "collegeCode",
        String(user?.collegeCode ?? readStorage("collegeCode")),
      );
      ...
      formData.append(
        ...
        String(data.studentPhotoPath ?? ""),
      );
      formData.append(
        "fatherPhotoFileName",
        String(data.fatherPhotoPath ?? ""),
      );
      formData.append(
        "motherPhotoFileName",
        String(data.motherPhotoPath ?? ""),
      );
      ...
      if (photoFiles.current.student) {
        formData.append(
          "studentPhoto",
          photoFiles.current.student,
          photoFiles.current.student.name,
        );
        hasFiles = true;
      }
      if (photoFiles.current.father) {
        formData.append(
          "fatherPhoto",
          photoFiles.current.father,
          photoFiles.current.father.name,
        );
        hasFiles = true;
      }
      if (photoFiles.current.mother) {
        formData.append(
          "motherPhoto",
          photoFiles.current.mother,
          photoFiles.current.mother.name,
        );
        hasFiles = true;
      }
      ...
      if (photoFiles.current.signature) {
        ...
        sig.append(
          "file",
          photoFiles.current.signature,
          photoFiles.current.signature.name,
        );
        sig.append(
          "hallticketNumber",
          String(data.rollNumber ?? data.hallticketNumber ?? ""),
        );
      ...
      <div className="app-card overflow-hidden p-4" data-no-page-name>
        ...
        {activeStep === "personal" && (
          <PersonalInfoStep
            ...
            onSignaturePhoto={(f) => {
              photoFiles.current.signature = f;
              previewFile(f, setSignaturePhotoUrl);
            }}
          />
        )}
        {activeStep === "education" && (
          <EducationStep
            ...
            onRemoveEducation={(index, item) => {
              const next = ensureArray(data.studentEducationDetails).filter(
                (_, i) => i !== index,
              );
              if (item.appEducationId)
                setRemovedEducation((prev) => [
                  ...prev,
                  { ...item, isActive: false },
                ]);
        ...
        {activeStep === "activities" && (
          <ActivitiesStep
            ...
            onRemoveActivity={(index, item) => {
              const next = ensureArray(data.studentActivitiesDetails).filter(
                (_, i) => i !== index,
              );
              if (item.studentAppActivityId)
        ...
        <div className="mt-6 flex flex-wrap justify-between gap-2 border-t border-border pt-4">
          <Button
            ...
            onClick={
              isFirst
                ? goBackToList
                : () => setActiveStep(EDIT_STEPS[stepIndex - 1].id)
            }
          >
            {isFirst ? (
              "Back"
          ...
          {isLast ? (
            ...
            <Button
              type="button"
              onClick={() => setActiveStep(EDIT_STEPS[stepIndex + 1].id)}
            >
              Next
