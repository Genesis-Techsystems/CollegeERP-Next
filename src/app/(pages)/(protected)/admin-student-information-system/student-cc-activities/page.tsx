...
export default function StudentCcActivitiesPage() {
  ...
  const [ccactivityCatdetId, setCcactivityCatdetId] = useState<number | null>(
    null,
  );
  ...
  async function openEditDialog(row: AnyRow) {
    setEditingRow(row);
    setCcactivityCatdetId(
      pickNum(row, ["ccactivityCatdetId", "ccactivity_catdet_id"]),
    );
    setIsActive(Boolean(row.isActive ?? true));
    setReason(
      pickText(row, ["reason"]) || (row.isActive === false ? "" : "active"),
    );
    setDialogOpen(true);
    if (activityTypeRows.length > 0) return;
    setLoadingTypes(true);
  ...
  async function saveDialog() {
    ...
    try {
      ...
      toastError(
        e,
        editingRow ? "Failed to update activity" : "Failed to add activity",
      );
    } finally {
      setSaving(false);
    }
  ...
  const columnDefs = useMemo<ColDef<AnyRow>[]>(
    () => [
      { headerName: "SI.No", valueGetter: rowIndexGetter, width: 70, flex: 0 },
      {
        ...
        valueGetter: (p) =>
          pickText(p.data, [
            "ccactivityCatdetName",
            "ccactivity_catdet_name",
          ]) || "-",
      },
      {
  ...
  // Hide results until a student is selected and the API returned at least one row.
  // Do not show an empty/loading table card.
  const showTable = Boolean(studentId) && activityRows.length > 0;
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent hasDescription>
          <DialogHeader>
            ...
            <DialogDescription>
              Select activity and update status.
            </DialogDescription>
          </DialogHeader>
