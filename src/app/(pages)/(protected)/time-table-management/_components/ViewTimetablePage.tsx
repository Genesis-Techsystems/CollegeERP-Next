...
export function ViewTimetablePage() {
  ...
  const [selectedTimetableRow, setSelectedTimetableRow] = useState<AnyRow | null>(null)
      ...
      .then((rows) => {
        ...
        if (urlAy && urlCollege) {
          ...
          if (courses.length > 0) {
            ...
            setCourseId(cid)
            const groups = courseGroupsFromFilterRows(rows, firstCollege, firstAy, cid)
            if (groups.length > 0) {
              const gid = num(groups[0].fk_course_group_id)
              setCourseGroupId(gid)
              const years = courseYearsFromFilterRows(rows, firstCollege, firstAy, cid, gid)
              if (years.length > 0) {
                const yid = num(years[0].fk_course_year_id)
                setCourseYearId(yid)
                const secs = sectionsFromFilterRows(rows, firstCollege, firstAy, cid, gid, yid)
                if (secs.length > 0) {
  ...
  const loadGrid = useCallback(async () => {
    ...
    const ttRow = timetables.find((t) => num(t.pk_timetable_id ?? t.timetableId) === timetableId) ?? null
  ...
  useEffect(() => {
    ...
    if (!timetableId && timetables.length > 0) setTimetableId(num(timetables[0].pk_timetable_id ?? timetables[0].timetableId))
  ...
  const groupCode = courseGroups.find((g) => num(g.fk_course_group_id) === courseGroupId)
  const yearName = courseYears.find((y) => num(y.fk_course_year_id) === courseYearId)
  const sectionName = sections.find((s) => num(s.fk_group_section_id) === groupSectionId)
  ...
  const dateRange =
    selectedTimetableRow?.timetable_startdate || selectedTimetableRow?.timetable_enddate
  ...
  function openModalView() {
    if (!collegeId || !academicYearId || !courseGroupId || !courseYearId || !groupSectionId) return
    const collegeCode = text(collegeLabel ?? {}, ['college_code', 'collegeCode'])
    const ay = text(
  ...
  return (
    <PageContainer className="view-timetable-page space-y-4">
      ...
      {timetable && timetable.weekdays.length > 0 ? (
        <div className="app-card space-y-3 p-4 print:border-0 print:shadow-none">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="inline-flex items-center gap-2 text-[13px] font-semibold text-[#002b5c]">
              ...
              {dateRange ? <span className="font-normal">- ({dateRange})</span> : null}
            </h2>
            <div className="flex gap-2 print:hidden">
              <Button type="button" size="sm" variant="outline" onClick={openModalView}>
                Popup view
              </Button>
          ...
          <div className="hidden print:block">
            <TimetableWeeklyGrid timetable={timetable} variant="print" />
          </div>
        </div>
      ) : gridLoading ? (
