...
export function HostelVisitorModal({
    ...
    reset(
      row
        ? {
            ...
            outingDate: parseApiDate(row.outDate) ?? today,
            inDate: parseApiDate(row.inDate) ?? today,
            outingTime: parseApiTime(row.outTime),
            inTime: parseApiTime(row.inTime),
  ...
  async function onSubmit(data: FormValues) {
    ...
    const payload: Partial<HostelVisitor> = {
      ...
      outDate: toHostelApiDate(data.outingDate),
      inDate: toHostelApiDate(data.inDate),
      outTime: data.outingTime,
      inTime: data.inTime,
    ...
    try {
      ...
      toastError(
        e,
        isEditing ? "Update failed" : "Could not issue visitor pass",
      );
    }
  }
  ...
  const selectedOutingDate = watch("outingDate");
        ...
        <Controller
          ...
          render={({ field }) => (
            <DatePicker
              ...
              onChange={(d) => {
                const nextDate = d ?? new Date();
                field.onChange(nextDate);
                const currentInDate = watch("inDate");
                if (
                  currentInDate &&
                  currentInDate.getTime() < nextDate.getTime()
                ) {
                  setValue("inDate", nextDate);
                }
              }}
              className={HOSTEL_SELECT_CLASS}
            />
