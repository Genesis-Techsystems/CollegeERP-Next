...
export function UploadCourseContentPage() {
  ...
  const [subjectRegulationId, setSubjectRegulationId] = useState<number | null>(
    null,
  );
  const [subjects, setSubjects] = useState<OnlineCourseAcademicMapRow[]>([]);
  const [studentBatch, setStudentBatch] = useState<StaffSubjectClassRow | null>(
    null,
  );
  ...
  const onSelectClass = useCallback(
    (id: number | null) => {
      ...
      const row =
        myClasses.find((c) => num(c.subjectRegulationId) === id) ?? null;
      setStudentBatch(row);
  ...
  function assignUnits(data: OnlineCourseAcademicMapRow) {
    ...
    const qs = new URLSearchParams({
      ...
      academicYearId: String(
        data.academicYearId ?? studentBatch.academicYearId ?? "",
      ),
      courseYearId: String(
        data.courseYearId ?? studentBatch.courseYearId ?? "",
      ),
      courseGroupId: String(
        data.courseGroupId ?? studentBatch.courseGroupId ?? "",
      ),
