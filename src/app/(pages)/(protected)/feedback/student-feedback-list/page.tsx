...
export default function StudentFeedbackListPage() {
  ...
  useEffect(() => {
    ...
    const match = colleges.find((c) => Number(c.collegeId) === sessionCollegeId)
    if (match) {
      setCollegeId(String(match.collegeId))
      setCollegeLockedCode(String(match.collegeCode ?? match.collegeName ?? ''))
    }
  ...
  const feedbackQuery = useQuery({
    queryKey: QK.studentFeedbackList.feedback(
  ...
  const showTable =
    ...
    (feedbackQuery.isFetching || rows.length > 0)
