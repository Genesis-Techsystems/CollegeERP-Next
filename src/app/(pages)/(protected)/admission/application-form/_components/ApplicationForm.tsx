...
export function ApplicationForm({ mode = "add" }: ApplicationFormProps) {
  ...
  const orgId =
    resolveOrganizationId(user) || Number(readStorage("organizationId")) || 0;
  ...
  const [motherPhotoUrl, setMotherPhotoUrl] = useState(() =>
    photoSrc(null, true, DEFAULT_MOTHER_PHOTO),
  );
  ...
  const applyCollegeCascade = useCallback(
    (
      ...
      const ays = asAcademicYearRows(
        filterAcademicYears(ayRows, collegeId, filters),
      );
  ...
  const applyAcademicYearCascade = useCallback(
    (
      ...
      const courseId =
        preferCourseId &&
        crs.some((r) => num(r, ["courseId"]) === preferCourseId)
          ? preferCourseId
  ...
  const applyCourseCascade = useCallback(
    (
      ...
      const groups = asCourseGroupRows(
        filterCourseGroups(filters, collegeId, courseId),
      );
  ...
  const applyCourseGroupCascade = useCallback(
    (
      ...
      const years = asCourseYearRows(
        filterCourseYears(filters, collegeId, courseId, courseGroupId),
      );
  ...
  const hydrateGeoAndCaste = useCallback(async (row: AnyRow) => {
    ...
    if (presentCountryId) {
      setPresentStates(
        (await listStatesByCountry(presentCountryId)) as unknown as AnyRow[],
      );
    }
    if (presentStateId) {
      setPresentDistricts(
        (await listDistrictsByState(presentStateId)) as unknown as AnyRow[],
      );
    }
    if (presentDistrictId) {
      setPresentCities(
        (await listCitiesByDistrict(presentDistrictId)) as unknown as AnyRow[],
      );
    }
    ...
    if (permanentCountryId) {
      setPermStates(
        (await listStatesByCountry(permanentCountryId)) as unknown as AnyRow[],
      );
    }
    if (permanentStateId) {
      setPermDistricts(
        (await listDistrictsByState(permanentStateId)) as unknown as AnyRow[],
      );
    }
    if (permanentDistrictId) {
      setPermCities(
        (await listCitiesByDistrict(
          permanentDistrictId,
      ...
      try {
        if (isEdit) {
          ...
          if (!applicationNumberParam || !initialCollegeId) {
            toastError(
              new Error("Missing application number or college for edit."),
            );
        ...
        if (isEdit) {
          ...
          const courseGroupId = applyCourseCascade(
            collegeId,
            preferCourse,
            filters,
            preferGroup,
          );
          applyCourseGroupCascade(
            collegeId,
            preferCourse,
            courseGroupId,
            filters,
          ...
          setData({
            ...flagged,
            studentEducationDetails: educations.length
              ? educations
              : [emptyEducationRow()],
            studentActivitiesDetails: activities.length
              ? activities
              : [emptyActivityRow()],
            collegeId,
          ...
          setStudentPhotoUrl(
            photoSrc(txt(existing, ["studentPhotoPath"]) || null),
          );
          setFatherPhotoUrl(
            photoSrc(txt(existing, ["fatherPhotoPath"]) || null),
          );
          setMotherPhotoUrl(
            photoSrc(
              txt(existing, ["motherPhotoPath"]) || null,
              true,
              DEFAULT_MOTHER_PHOTO,
            ),
          );
        ...
        const collegeId =
          initialCollegeId &&
          clgRows.some((r) => num(r, ["collegeId"]) === initialCollegeId)
            ? initialCollegeId
        ...
        const courseId = applyAcademicYearCascade(
          collegeId,
          filters,
          initialCourseId,
        );
        const courseGroupId = applyCourseCascade(collegeId, courseId, filters);
        const courseYearId = applyCourseGroupCascade(
          collegeId,
          courseId,
          courseGroupId,
          filters,
        ...
        if (!cancelled) {
          toastError(
            ...
            isEdit
              ...
              : "Failed to load application form options",
          );
  ...
  async function onPresentCountryChange(countryId: number | null) {
    ...
    setPresentStates(
      countryId
        ? ((await listStatesByCountry(countryId)) as unknown as AnyRow[])
        : [],
    );
    setPresentDistricts([]);
    setPresentCities([]);
  ...
  async function onPresentStateChange(stateId: number | null) {
    ...
    setPresentDistricts(
      stateId
        ? ((await listDistrictsByState(stateId)) as unknown as AnyRow[])
        : [],
    );
    setPresentCities([]);
  }
  ...
  async function onPresentDistrictChange(districtId: number | null) {
    ...
    setPresentCities(
      districtId
        ? ((await listCitiesByDistrict(districtId)) as unknown as AnyRow[])
        : [],
    );
  }
  ...
  function onSameAsPermanentChange(checked: boolean) {
    ...
    void (async () => {
      if (data.permanentCountryId) {
        setPresentStates(
          (await listStatesByCountry(
            Number(data.permanentCountryId),
          )) as unknown as AnyRow[],
        );
      }
      if (data.permanentStateId) {
        setPresentDistricts(
          (await listDistrictsByState(
            Number(data.permanentStateId),
          )) as unknown as AnyRow[],
        );
      }
      if (data.permanentDistrictId) {
        setPresentCities(
          (await listCitiesByDistrict(
            Number(data.permanentDistrictId),
          )) as unknown as AnyRow[],
        );
      }
    })();
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const stage = workflowStages.find(
        (s) => num(s, ["workflowStageId"]) === workflowStageId,
      );
      ...
      const currentStage = workflowStages.find(
        (s) => num(s, ["workflowStageId"]) === currentWfId,
      );
      const currentStageName =
        txt(currentStage ?? {}, ["wfName", "name"]) || stageName;
      ...
      const payload: AnyRow = {
        ...
        dob:
          data.dob instanceof Date
            ? toIsoDate(data.dob)
            : data.dob
              ? toIsoDate(new Date(String(data.dob)))
              : null,
        dateOfIssue:
        ...
        stdAppDocCollections: mainDocuments.map(
      ...
      if (isEdit) {
        ...
        const studentAppId =
          num(data, ["studentAppId"]) || num(updated, ["studentAppId"]);
        if (studentAppId) {
          ...
          const docCollections = ensureArray<AnyRow>(
            updated.studentAppDocCollectionDTO,
          );
          for (const coll of docCollections) {
            ...
            const local = documents.find(
              (d) => d.documentRepositoryId === repId,
            );
      ...
      if (studentAppId) {
        ...
        formData.append(
          "applicationNumber",
          String(created.applicationNumber ?? ""),
        );
        formData.append(
          "admissionNumber",
          String(created.admissionNumber ?? ""),
        );
        ...
        const docCollections = ensureArray<AnyRow>(
          created.studentAppDocCollectionDTO,
        );
        for (const coll of docCollections) {
          if (!coll.isSoftCopy) continue;
          const repId = num(coll, ["documentRepositoryId", "docRepId"]);
      ...
      toastError(
        e,
        isEdit ? "Failed to update application" : "Failed to save application",
      );
    } finally {
      setSubmitting(false);
    }
        ...
        {activeStep === "office" && (
          <AppOfficeUseStep
            ...
            onGetDetails={() =>
              toastError(
                new Error(
                  "Get Details is not available for this admission type yet.",
                ),
              )
            }
          />
        ...
        {activeStep === "education" && (
          <AppEducationStep
            ...
            onRemoveEducation={(index) => {
              ...
              patchData({
                studentEducationDetails: next.length
                  ? next
                  : [emptyEducationRow()],
              });
            }}
        ...
        {activeStep === "activities" && (
          <AppActivitiesStep
            ...
            onRemoveActivity={(index) => {
              ...
              patchData({
                studentActivitiesDetails: next.length
                  ? next
                  : [emptyActivityRow()],
              });
            }}
