...
export default function BankAccountModal({
  ...
  useEffect(() => {
    const defaults = getDefaults(editData);
    // If editing but collegeId is missing (API omits it), fallback to the first active college like Angular
    if (open && editData && !defaults.collegeId && colleges.length > 0) {
      defaults.collegeId = colleges[0].collegeId;
    }
    // Pre-select college if there's only one
    if (open && !editData && !defaults.collegeId && colleges.length === 1) {
      defaults.collegeId = colleges[0].collegeId;
    }
    reset(defaults);
  }, [open, editData, colleges.length, reset]);
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2 py-1">
          ...
          <div className="space-y-0.5">
            ...
            {errors.ifscCode && (
              <p className="text-xs text-red-500">{errors.ifscCode.message}</p>
            )}
          </div>
          <div className="space-y-0.5">
