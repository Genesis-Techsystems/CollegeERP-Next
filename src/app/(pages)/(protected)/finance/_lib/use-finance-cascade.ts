...
export function useFinanceCascade(options?: {
  withAccountType?: boolean;
  withTransactionType?: boolean;
}) {
  const withAccountType = options?.withAccountType ?? false;
  const withTransactionType = options?.withTransactionType ?? false;
  const { organizationId, employeeId, contextLoading, contextReady } =
    useFinanceSessionIds();
  ...
  /** Angular accountTypeCatId — 0 = All */
  const [majorAccountTypeCatId, setMajorAccountTypeCatId] = useState(0);
  ...
  const accountTypes = useMemo<FinanceCascadeOption[]>(() => {
    if (!withAccountType || !collegeId || !accountEntityId || !financialYearId)
      return [];
    return filterFinanceAccountTypes(
      finRows,
      collegeId,
      accountEntityId,
      financialYearId,
      ...
      label:
        r.accounttype_code ??
        r.accounttype_name ??
        `Type ${r.pk_account_type_id}`,
    }));
  }, [finRows, collegeId, accountEntityId, financialYearId, withAccountType]);

  const transactionTypes = useMemo<FinanceCascadeOption[]>(() => {
    if (
      !withTransactionType ||
      !collegeId ||
      !accountEntityId ||
      ...
      return [];
    return filterFinanceTransactionTypes(
      finRows,
      collegeId,
      accountEntityId,
      financialYearId,
    ).map((r) => ({
      value: Number(r.fk_major_accounttype_catdet_id),
      label: r.transaction_type ?? `Type ${r.fk_major_accounttype_catdet_id}`,
    }));
  }, [
    finRows,
    collegeId,
  ...
  useEffect(() => {
    if (
      collegeId &&
      entities.length &&
      !entities.some((e) => e.value === accountEntityId)
    ) {
  ...
  useEffect(() => {
    if (
      accountEntityId &&
      years.length &&
      !years.some((y) => y.value === financialYearId)
    ) {
  ...
  useEffect(() => {
    if (
      ...
      financialYearId &&
      accountTypes.length &&
      !accountTypes.some((a) => a.value === accountTypeId)
    ) {
      setAccountTypeId(accountTypes[0]?.value ?? 0);
    }
  }, [withAccountType, financialYearId, accountTypes, accountTypeId]);

  // Angular selectedfinancialYear resets accountTypeCatId to 0 (All)
  useEffect(() => {
    if (!withTransactionType) return;
    setMajorAccountTypeCatId(0);
  }, [withTransactionType, financialYearId, accountEntityId, collegeId]);

  const filtersValid =
    collegeId > 0 && accountEntityId > 0 && financialYearId > 0;
  ...
  function toBudgetParams(extra: Record<string, string | number> = {}) {
    return {
      ...
      ...(withTransactionType
        ? { in_major_accounttype: majorAccountTypeCatId }
        : {}),
      ...extra,
    };
  }
  ...
  return {
    ...
    accountTypeId,
    setAccountTypeId,
    majorAccountTypeCatId,
    setMajorAccountTypeCatId,
    filtersValid,
    organizationId,
    employeeId,
    toBudgetParams,
