...
interface ApplyCertificateModalProps {
  open: boolean;
  onClose: () => void;
  collegeId: number;
...
export function ApplyCertificateModal({
  open,
  onClose,
  collegeId,
  collegeCertificateId,
  onSubmit,
}: Readonly<ApplyCertificateModalProps>) {
  ...
  const resetForm = useCallback(() => {
    ...
    setSelectedCertId(
      collegeCertificateId ? String(collegeCertificateId) : null,
    );
    setCheck("1");
    setPurpose(null);
    setForOther("");
  ...
  useEffect(() => {
    ...
    void listCollegeCertificatesByCollege(collegeId)
      .then((rows) => {
        ...
        if (collegeCertificateId) {
          setSelectedCertId(String(collegeCertificateId));
        } else if (filtered[0]) {
          setSelectedCertId(String(filtered[0].collegeCertificateId));
        }
      })
      .catch((e) => toastError(e, "Failed to load certificates"));
  }, [open, collegeId, collegeCertificateId, resetForm]);
  ...
  const onStudentSearch = useCallback(
    async (term: string) => {
      ...
      try {
        setStudents(await searchStudentsForTc({ collegeId, q }));
      } catch (error) {
  ...
  async function loadStudentSideData(student: StudentFeeSearchRow) {
  ...
  async function loadParticularsForAcademicYear(ayId: number) {
    ...
    try {
      const rows = await listFeeStudentDataParticulars({
        collegeId: Number(selectedStudent.collegeId ?? collegeId),
  ...
  async function handleSubmit(e: { preventDefault: () => void }) {
    e.preventDefault();
    if (!selectedStudent || !selectedCert || !studentId) {
      toastError(
        new Error("Please select student and certificate"),
        "Validation",
      );
      return;
    }
    ...
    try {
      ...
      const payload: ApplyCertificateRequestPayload = {
        ...
        academicYearId: Number(selectedStudent.academicYearId ?? 0),
      ...
      if (certificateCode === "CERBC" && check === "1") {
        ...
        payload.applicationComments = particulars
          .filter((p) => p.checked && p.particularsCode === "TF")
          .map((p) => String(p.particularsCode))
          .join(",");
        payload.academicYearId = Number(
          academicYearId ?? selectedStudent.academicYearId ?? 0,
        );
      ...
      <div className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <StudentSearchSelect
            ...
            onChange={(id, row) => {
              setStudentId(id);
              const student = (row as StudentFeeSearchRow | null) ?? null;
              setSelectedStudent(student);
              if (student) void loadStudentSideData(student);
              else {
                setFeeStudentData([]);
                setFinancialYears([]);
                setAcademicYears([]);
        ...
        {certificateCode === "CERBC" ? (
          ...
          >
            <div className="flex items-center gap-2">
              <RadioGroupItem value="1" id="apply-for" />
              <Label htmlFor="apply-for" className="font-normal">
                Apply For
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="2" id="for-income-tax" />
              <Label htmlFor="for-income-tax" className="font-normal">
                For Income Tax
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="3" id="for-tc" />
              <Label htmlFor="for-tc" className="font-normal">
                For Transfer Certificate
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="4" id="for-bank" />
        ...
        tfParticulars.length > 0 ? (
          <div className="space-y-2">
            ...
            {tfParticulars.map((item, index) => (
              <div
                key={`bank-${String(item.particularsCode)}-${index}`}
                className="flex items-center gap-2"
              >
                <Checkbox
                  checked={Boolean(item.checked)}
        ...
        {certificateCode === "CERCUST" ? (
          <div className="space-y-3">
            <div className="flex flex-wrap gap-4">
              {[
                { id: "ssc", label: "S.S.C", value: ssc, set: setSsc },
                {
                  id: "inter",
                  label: "Intermediate",
                  value: inter,
                ...
                { id: "btech", label: "B.Tech", value: btech, set: setBtech },
              ].map((opt) => (
                <div key={opt.id} className="flex items-center gap-2">
                  <Checkbox
                    id={`apply-${opt.id}`}
                    checked={opt.value}
                    onCheckedChange={(c) => opt.set(c === true)}
                  />
                  <Label htmlFor={`apply-${opt.id}`} className="font-normal">
                    {opt.label}
                  </Label>
                </div>
              ))}
            ...
            {btech ? (
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  <Checkbox
                    id="apply-provisional"
                    checked={provisional}
                    onCheckedChange={(c) => setProvisional(c === true)}
                  />
                  <Label htmlFor="apply-provisional" className="font-normal">
                    Provisional Certificate
                  </Label>
                </div>
                <div className="flex items-center gap-2">
                  <Checkbox
                    id="apply-consolidated"
                    checked={consolidated}
                    onCheckedChange={(c) => setConsolidated(c === true)}
                  />
                  <Label htmlFor="apply-consolidated" className="font-normal">
