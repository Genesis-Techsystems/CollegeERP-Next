...
export default function AssignRegulationToStudentsPage() {
  ...
  const regulationOptions = useMemo(
    () =>
      regulations.map((reg) => ({
        ...
        label:
          s(reg.regulationName) ||
          s(reg.regulationCode) ||
          String(n(reg.regulationId ?? reg.pk_regulation_id)),
      })),
    [regulations],
  );
  ...
  function updateRowRegulation(rowKey: string, nextId: number | null) {
    setRows((prev) =>
      prev.map((r) =>
        r.__rowKey === rowKey
          ? { ...r, assignedRegulationId: nextId, regulationId: nextId }
          : r,
      ),
    );
  }
  ...
  function applyBulkRegulation(regulationId: number | null) {
    ...
    setRows((prev) =>
      prev.map((r) => ({
        ...r,
        assignedRegulationId: regulationId,
        regulationId,
      })),
    );
  }
  ...
  function regulationRenderer(p: ICellRendererParams<AnyRow>) {
    ...
    return (
      <Select
        ...
        onChange={(v) =>
          updateRowRegulation(s(row.__rowKey), v ? Number(v) : null)
        }
  ...
  return (
    <FilteredListPage
      title="Assign Regulation to Students"
      filters={
