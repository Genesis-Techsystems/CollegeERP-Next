...
import { Eye, Pencil, PlusSquare, X } from "lucide-react";
 ...
 */
function toAngularTopicPayload(
  topic: TopicRow,
  fallbackCollegeId: number,
  fallbackSubjectRegulationId: number,
): AnyRow {
  ...
  out.subjectRegulationId =
    n(topic.subjectRegulationId) ||
    fallbackSubjectRegulationId ||
    topic.subjectRegulationId;
...
export default function AddSubjectUnitsPage() {
  ...
  function viewTopics(row: UnitRow) {
    const active = sortTopics(
      [...(row.subjectUnitTopicsDTOs ?? [])].filter(
        (t) => t.isActive !== false,
      ),
    );
    ...
    setTopicsDraft((prev) => {
      const index = prev.findIndex(
        (t) =>
          ...
          (n(t.subjectUnitTopicId) > 0 &&
            n(t.subjectUnitTopicId) === n(item.subjectUnitTopicId)),
      );
      if (index < 0) return prev;
      const row = prev[index];
      if (n(row.subjectUnitTopicId)) {
        setInactiveTopics((inactive) => [
          ...inactive,
          { ...row, isActive: false },
        ]);
      }
  ...
  const showUnitsTable = units.length > 0 || loading;

  return (
    <>
      {/* Angular: page-table-head outside the mat-card */}
      <PageContainer className="space-y-4">
        <div
          className="flex items-center gap-2 border-t-2 border-[#c3d9ff] bg-white px-4 py-3"
          data-no-page-name=""
        >
          <PlusSquare className="h-5 w-5 shrink-0 text-[#0c51a4]" aria-hidden />
          <strong className="text-[16px] font-semibold text-[#042956]">
            Subjects Units
            {contextTitle ? (
              <>
                {" "}
                (
                <span className="font-semibold text-black">{contextTitle}</span>
                )
              </>
            ) : null}
          </strong>
      ...
      <Dialog open={topicsOpen} onOpenChange={setTopicsOpen}>
        <DialogContent className="sm:max-w-4xl p-0 overflow-hidden">
          ...
          <DialogFooter className="px-4 pb-4">
            <Button variant="outline" onClick={() => setTopicsOpen(false)}>
              Close
            </Button>
            <Button
              disabled={saving}
      ...
      <Dialog open={viewTopicsOpen} onOpenChange={setViewTopicsOpen}>
        <DialogContent className="sm:max-w-3xl p-0 overflow-hidden">
          ...
          <DialogFooter className="px-4 pb-4">
            <Button variant="outline" onClick={() => setViewTopicsOpen(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
