...
function makeActionsRenderer(onUnmap: (row: AnyRow) => void) {
  return (p: ICellRendererParams<AnyRow>) => (
    <button type="button" className="inline-flex items-center" onClick={() => onUnmap(p.data ?? {})} title="Unmap">
...
export default function StaffSubjectUnmappingPage() {
  ...
  function openUnmap(row: AnyRow) {
    setSelectedRow(row)
    setEditToDate(toInputDate(row.toDate) || new Date().toISOString().slice(0, 10))
  ...
  async function saveUnmapping() {
    ...
    const payload = [{
      ...
      toDate: editToDate || selectedRow.toDate || new Date().toISOString().slice(0, 10),
    }]
    try {
      await saveStaffSubjectMappings(payload)
      toastSuccess('Staff subject updated successfully')
      setUnmapOpen(false)
      const refreshed = await listEmployeeMappedSubjects({ collegeId, employeeId })
      setRows(refreshed)
    } catch {
  ...
  function statusRenderer(p: ICellRendererParams<AnyRow>) {
    return <StatusBadge status={String(p.data?.status ?? 'active').toLowerCase() === 'active' ? 'active' : 'inactive'} />
  }

  const columnDefs = useMemo<ColDef<AnyRow>[]>(() => [
    { headerName: 'SI.No', valueGetter: (p: any) => (p.node?.rowIndex ?? 0) + 1, minWidth: 70, maxWidth: 80, flex: 0 },
    { field: 'courseDisplay', headerName: 'Course', minWidth: 240, flex: 1.3 },
    {
      headerName: 'Subject',
      minWidth: 260,
      flex: 1.4,
    ...
    { field: 'fromDate', headerName: 'From Date', minWidth: 120, flex: 0.8, valueGetter: (p: any) => toDateStr(s(p.data?.fromDate)) || '-' },
    { field: 'toDate', headerName: 'To Date', minWidth: 120, flex: 0.8, valueGetter: (p: any) => toDateStr(s(p.data?.toDate)) || '-' },
    { headerName: 'Status', minWidth: 100, flex: 0.6, cellRenderer: statusRenderer },
      ...
      <Dialog open={unmapOpen} onOpenChange={setUnmapOpen}>
        <DialogContent className="sm:max-w-3xl overflow-hidden">
          <DialogHeader className="px-5 py-3 pr-12 border-b bg-background">
            <DialogTitle className="text-base font-semibold text-[hsl(var(--primary))]">Staff Subject Edit</DialogTitle>
          </DialogHeader>
          <div className="px-5 py-3 space-y-3">
            <div className="rounded-md border border-cyan-100 p-3">
              <div className="grid grid-cols-[150px_1fr] gap-y-1.5 text-[11px]">
                <div className="font-semibold text-foreground">Course Details :</div>
                <div className="font-semibold text-blue-700 text-xs">{s(selectedRow?.courseDisplay) || '-'}</div>
                <div className="font-semibold text-foreground">Subject :</div>
                <div className="font-semibold text-blue-700 text-xs">
                  {s(selectedRow?.subjectName) || '-'} {s(selectedRow?.subjectCode) ? `(${s(selectedRow?.subjectCode)})` : ''}
                </div>
                <div className="font-semibold text-foreground">Subject Type :</div>
                <div className="font-semibold text-blue-700 text-xs">{s(selectedRow?.subjectType) || '-'}</div>
                <div className="font-semibold text-foreground">Date :</div>
                <div className="font-semibold text-blue-700 text-xs">
                  {(toDateStr(s(selectedRow?.fromDate)) || '-') + ' - ' + (toDateStr(s(selectedRow?.toDate)) || '-')}
                </div>
              </div>
            </div>
          ...
          <DialogFooter className="px-5 py-2 border-t">
            <Button size="sm" variant="outline" onClick={() => setUnmapOpen(false)}>Close</Button>
            <Button size="sm" onClick={() => { void saveUnmapping() }}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PageContainer>
