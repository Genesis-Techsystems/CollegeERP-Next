...
import {
  ...
  deleteElectiveGroupMapping,
  getDigitalOnlineSyncFilters,
  listElectiveGroupMappings,
  listElectiveGroupMappingsByCode,
...
export default function ElectiveGroupMappingPage() {
  ...
  const [viewRows, setViewRows] = useState<AnyRow[]>([]);
  ...
  const addElectiveOptions = useMemo(
    () =>
      electiveSubjects
        ...
        .map((x) => ({
          value: String(n(x.subjectId)),
          label: s(x.subjectName) || s(x.subjectCode),
        })),
    [electiveSubjects],
  );
  ...
  const electiveGroupName = useMemo(() => {
    const collegeCode = s(
      colleges.find((c) => n(c.fk_college_id) === (collegeId ?? 0))
        ?.college_code,
    );
    const groupCode = s(
      addCourseGroups.find(
        (g) => n(g.fk_course_group_id) === (addCourseGroupId ?? 0),
      )?.group_code,
    );
    const courseYearName = s(
      addCourseYears.find(
        (y) => n(y.fk_course_year_id) === (addCourseYearId ?? 0),
      )?.course_year_name,
    );
    const subject = s(
      electiveSubjects.find((x) => n(x.subjectId) === (addElectiveId ?? 0))
        ?.subjectName,
    );
    const staff = s(
      employees.find((e) => n(e.employeeId) === (addStaffId ?? 0))?.firstName,
    );
  ...
  useEffect(() => {
    if (!addCourseId && addCourses.length)
      setAddCourseId(n(addCourses[0].fk_course_id));
  }, [addCourses, addCourseId]);
  useEffect(() => {
    setAddCourseGroupId(null);
    setAddCourseYearId(null);
  }, [addCourseId]);
  useEffect(() => {
    if (!addCourseGroupId && addCourseGroups.length)
      setAddCourseGroupId(n(addCourseGroups[0].fk_course_group_id));
  }, [addCourseGroups, addCourseGroupId]);
  useEffect(() => {
    setAddCourseYearId(null);
  }, [addCourseGroupId]);
  useEffect(() => {
    if (!addCourseYearId && addCourseYears.length)
      setAddCourseYearId(n(addCourseYears[0].fk_course_year_id));
  }, [addCourseYears, addCourseYearId]);
  ...
  useEffect(() => {
    async function load() {
      ...
      setSections(
        (Array.isArray(list) ? list : []).map((sec) => ({
          ...sec,
          checked: false,
          check: true,
          batchwiseStudents: Array.isArray(sec.batchwiseStudents)
            ? sec.batchwiseStudents
            : [],
        })),
      );
    }
  ...
  // Angular deleteDialog — confirm first, then check batchwiseStudents before deleting
  function openDelete(row: AnyRow) {
    setDeleteTarget(row);
    setDeleteOpen(true);
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      if (hasStudents) {
        toastInfo("Students are alloted to this elective group.");
        setDeleteOpen(false);
        setDeleteTarget(null);
        return;
      }
      for (const m of maps) {
        const id = n(m.electiveGroupyrMappingId);
        if (id) await deleteElectiveGroupMapping(id);
      }
      toastSuccess("Elective group deleted successfully.");
      setDeleteOpen(false);
      setDeleteTarget(null);
      await loadElectives();
    } catch (err) {
      toastError(err, "Failed to delete elective group");
    } finally {
      setDeleting(false);
    }
  }
  ...
  function toggleSection(index: number, checked: boolean) {
    setSections((prev) =>
      prev.map((sec, i) =>
        i === index ? { ...sec, checked, check: checked } : sec,
      ),
    );
  }
  ...
  const columnDefs = useMemo<ColDef<AnyRow>[]>(
    () => [
      ...
      },
      {
        ...
        maxWidth: 130,
        flex: 0,
        cellRenderer: makeActionsRenderer(openDelete, openView),
      },
    ],
    [],
  );
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-1">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <Select
                ...
                options={addCourses.map((x) => ({
                  value: String(n(x.fk_course_id)),
                  label: s(x.course_code) || s(x.course_name),
                }))}
                searchable
              />
              <Select
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
              ...
              <div className="md:col-span-3 flex items-center gap-2 pb-1">
                <Checkbox
                  checked={addIsActive}
                  onCheckedChange={(v) => setAddIsActive(Boolean(v))}
                />
            ...
            <div className="w-[230px] border border-input p-3">
              ...
              {sections.length === 0 ? (
                <p className="mt-3 text-red-600 text-sm leading-tight">
                  No sections at present.
                </p>
              ) : (
                <div className="mt-2 space-y-2">
                  {sections.map((sec, i) => (
                    <label
                      key={n(sec.groupSectionId) || i}
                      className="flex items-center gap-2 text-sm"
                    >
                      <Checkbox
                        checked={Boolean(sec.checked)}
                        disabled={
                          Array.isArray(sec.batchwiseStudents) &&
                          sec.batchwiseStudents.length > 0
                        }
          ...
          <DialogFooter className="pt-1">
            ...
            <Button
              type="button"
              className="bg-primary hover:bg-[#123d79] text-white"
              onClick={() => {
                void onSave();
              }}
              disabled={saving}
