...
import { getUnivCurrFilters, listGroupYearRegulationDetails } from "@/services";
  ...
  return (p: ICellRendererParams<AnyRow>) => {
    ...
    return (
      <div className="text-xs inline-flex max-w-full flex-nowrap items-center gap-x-1.5 whitespace-nowrap">
        <Link
          href={unitsHref}
          className="shrink-0 text-blue-700 font-medium hover:underline"
        >
          Assign Units
        </Link>
...
export default function SubjectUnitTopicsPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      colleges.some((c) => n(c.fk_college_id) === pageParams.collegeId)
    ) {
  ...
  useEffect(() => {
    ...
    if (
      pageParams.courseId &&
      courses.some((c) => n(c.fk_course_id) === pageParams.courseId)
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      regulations.some((r) => n(r.fk_regulation_id) === pageParams.regulationId)
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      courseGroups.some(
        (g) => n(g.fk_course_group_id) === pageParams.courseGroupId,
      )
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      courseYears.some(
        (y) => n(y.fk_course_year_id) === pageParams.courseYearId,
      )
    ) {
      ...
      try {
        ...
        setRows(
          (Array.isArray(list) ? list : []).map((row) => ({
            ...
            regulationCode: s(
              row.regulationCode ?? row.regulation_code ?? row.regulationName,
            ),
            subjectId: n(
              row.subjectId ?? row.fk_subject_id ?? row.pk_subject_id,
            ),
  ...
  const contextLine = useMemo(() => {
    ...
    return [collegeCode, courseCode, courseGroupCode, courseYearName]
      .filter(Boolean)
      .join(" / ");
  ...
  return (
    <FilteredListPage
      ...
      tableHeader={
        <div
          className={cn(
            "padding-0 table-context-header !mx-0 !mb-0 w-full !items-center !justify-between gap-3",
          )}
        >
          ...
          {contextLine ? (
            <span
              className="shrink-0 text-right text-[15px] font-medium text-blue-700"
              title={contextLine}
            >
              {contextLine}
            </span>
      ...
      loading={loading}
      showTable={rows.length > 0 || loading}
      resultsVisible={rows.length > 0 || loading}
