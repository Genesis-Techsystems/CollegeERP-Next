...
const COLS = {
  ...
  markType: {
    colId: "examIntMarkTypeName",
    headerName: "Internal Marks Type",
    minWidth: 110,
    flex: 1,
  } as ColDef<AnyRow>,
  effectiveFrom: {
  ...
  const columnDefs = useMemo<ColDef<AnyRow>[]>(
    () => [
      COLS.siNo,
      {
        ...COLS.university,
        valueGetter: (p) =>
          pick(p.data ?? {}, ["universityCode", "universityName"]),
      },
      {
        ...COLS.course,
        valueGetter: (p) => pick(p.data ?? {}, ["courseCode", "courseName"]),
      },
      COLS.regCode,
      COLS.regName,
      {
        ...COLS.markType,
        valueGetter: (p) =>
          pick(p.data ?? {}, ["examIntMarkTypeName", "examIntMarkTypeCode"]),
      },
      {
        ...COLS.effectiveFrom,
        valueGetter: (p) => toDisplayDate(p.data?.effectiveFrom),
      },
      COLS.sortOrder,
