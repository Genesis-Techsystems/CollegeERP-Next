...
export default function CourseYearSubjectsPage() {
  ...
  const universities = useMemo(() => uniqBy(filtersData, 'fk_university_id'), [filtersData])
  const colleges = useMemo(
    () => uniqBy(filtersData.filter((r) => num(r.fk_university_id) === (universityId ?? 0)), 'fk_college_id'),
  ...
  useEffect(() => { if (!universityId && universities.length > 0) setUniversityId(num(universities[0].fk_university_id)) }, [universities, universityId])
  useEffect(() => { setCollegeId(null); setCourseId(null); setCourseGroupId(null); setCourseYearId(null); setAcademicYearId(null); setGroupSectionId(null); setRows([]) }, [universityId])
  useEffect(() => { if (!collegeId && colleges.length > 0) setCollegeId(num(colleges[0].fk_college_id)) }, [colleges, collegeId])
  ...
  useEffect(() => { if (!courseId && courses.length > 0) setCourseId(num(courses[0].fk_course_id)) }, [courses, courseId])
  useEffect(() => { setCourseGroupId(null); setCourseYearId(null); setAcademicYearId(null); setGroupSectionId(null); setRows([]) }, [courseId])
  useEffect(() => { if (!courseGroupId && courseGroups.length > 0) setCourseGroupId(num(courseGroups[0].fk_course_group_id)) }, [courseGroups, courseGroupId])
  useEffect(() => { setCourseYearId(null); setAcademicYearId(null); setGroupSectionId(null); setRows([]) }, [courseGroupId])
  useEffect(() => { if (!courseYearId && courseYears.length > 0) setCourseYearId(num(courseYears[0].fk_course_year_id)) }, [courseYears, courseYearId])
  useEffect(() => { setAcademicYearId(null); setGroupSectionId(null); setRows([]) }, [courseYearId])
  useEffect(() => {
    if (!academicYearId && academicYears.length > 0) {
      const current = [...academicYears].sort((a, b) => num(b.is_curr_ay) - num(a.is_curr_ay))[0]
  ...
  useEffect(() => {
    ...
    listGroupSections(courseYearId, academicYearId, courseGroupId)
      .then((list) => {
        setSections(list)
        setGroupSectionId(num(list[0]?.groupSectionId ?? list[0]?.pk_group_section_id) || null)
      })
      .catch(() => {
        setSections([])
        setGroupSectionId(null)
  ...
  useEffect(() => {
    async function loadSubjects() {
      ...
      setLoading(true)
      try {
        const list = await listSubjectCourseYearsBySection({ collegeId, academicYearId, groupSectionId })
        setRows(list)
      } finally {
        setLoading(false)
      }
  ...
  const columnDefs = useMemo<ColDef<AnyRow>[]>(() => [
    ...
    { field: 'subjectCode', headerName: 'Subject Code', minWidth: 150, flex: 1 },
    { field: 'subjectType', headerName: 'Subject Type', minWidth: 140, flex: 1 },
    { field: 'regulationName', headerName: 'Regulation', minWidth: 120, flex: 1 },
    { field: 'subCredits', headerName: 'Credit Points', minWidth: 120, maxWidth: 140, flex: 0 },
    { field: 'creditHours', headerName: 'Credit Hours', minWidth: 120, maxWidth: 140, flex: 0 },
  ], [])

  return (
    <FilteredListPage
      title="Course Year Subjects"
      filters={(
        <div className="grid grid-cols-1 gap-3 md:grid-cols-7">
          ...
          <Select label="Course Group" value={courseGroupId ? String(courseGroupId) : null} onChange={(v) => setCourseGroupId(v ? Number(v) : null)} options={courseGroups.map((x) => ({ value: String(num(x.fk_course_group_id)), label: text(x.group_code) }))} searchable disabled={!courseId} />
          <Select label="Course Year" value={courseYearId ? String(courseYearId) : null} onChange={(v) => setCourseYearId(v ? Number(v) : null)} options={courseYears.map((x) => ({ value: String(num(x.fk_course_year_id)), label: text(x.course_year_name) }))} searchable disabled={!courseGroupId} />
