...
export function ModerationRulePage({
  ...
  function onCollegeChange(value: string | null) {
    ...
    clearResults();
    if (!id) return;
    const collegeObj = colleges.find((c) => Number(c.collegeId) === id);
    const universityId = Number(
      collegeObj?.universityId ?? collegeObj?.fk_university_id ?? 0,
    );
    getModerationAcademicYears(id, universityId > 0 ? universityId : undefined)
  ...
  function onExamChange(value: string | null) {
    ...
    setSubjects([]);
    clearResults();
    if (!id || !collegeId || !academicYearId || !courseYearId || !courseGroupId)
      return;
    getExamCourseYearSubjects({
      collegeId,
      academicYearId,
  ...
  async function onGenerate() {
    ...
    try {
      const parts = [
        ...
        academicYears.find((x) => Number(x.academicYearId) === academicYearId)
          ?.academicYear,
        courses.find((x) => Number(x.courseId) === courseId)?.courseCode,
        courseGroups.find((x) => Number(x.courseGroupId) === courseGroupId)
          ?.groupCode,
        courseYears.find((x) => Number(x.courseYearId) === courseYearId)
          ?.courseYearName,
        ...
        <div className="space-y-4">
          ...
          {moderationInfo.length > 0 && students.length > 0 && (
            <div className="app-card p-3 grid grid-cols-1 md:grid-cols-4 gap-3 text-[12px]">
              {moderationInfo.map((info, idx) => (
                <div
                  key={`mi-${info.ModerationMarks_added ?? "na"}-${info.actual_pass_percentage ?? "na"}-${idx}`}
                  className="rounded border p-2 space-y-1 bg-white"
                >
                  <p>
                    <span className="text-slate-500">
                      Moderation Marks Added:
                    </span>{" "}
                    {info.ModerationMarks_added ?? "-"}
                  </p>
                  <p>
                    <span className="text-slate-500">
                      Actual Marks Percentage:
                    </span>{" "}
                    {info.actual_pass_percentage ?? "-"}%
                  </p>
                  <p>
                    <span className="text-slate-500">
                      Moderation Pass Percentage:
                    </span>{" "}
                    {info.moderation_pass_percentage ?? "-"}%
                  </p>
                  <p>
                    <span className="text-slate-500">
                      Students with Moderation Marks:
                    </span>{" "}
                    {info.NoOfStudentsPassed_With_ModerationMarks ?? "-"}%
                  </p>
                </div>
              ))}
            </div>
                ...
                <table className="w-full text-[12px]">
                  ...
                  <tbody>
                    {students.map((row, index) => (
                      <tr
                        key={`st-${row.hallticket_number ?? "na"}-${row.StudentName ?? "na"}-${index}`}
                        className="border-t"
                      >
