...
import { getExamRoomDetails, listActiveBuildings } from "@/services";
import {
...
function pickCourseId(row: Row): number {
  return num(
    row.fk_course_id ?? row.courseId ?? row.course_id ?? row.pk_course_id,
  );
}
...
function pickExamTimetableId(row: Row): number {
  return num(
    row.fk_exam_timetable_id ?? row.examTimetableId ?? row.exam_timetable_id,
  );
}
...
export default function UnivExamCenterRoomsPage() {
  ...
  const courses = useMemo(
    () => dedupeBy(allFilterRows, (r) => pickCourseId(r)),
    [allFilterRows],
  );
  const academicYears = useMemo(
    () =>
      dedupeBy(
        allFilterRows.filter((r) => pickCourseId(r) === Number(form.courseId)),
        (r) => pickAcademicYearId(r),
      ),
  ...
  const columnDefs = useMemo<ColDef<Row>[]>(
    () => [
      { headerName: "SI.No", valueGetter: rowIndexGetter, width: 80, flex: 0 },
      {
        ...
        minWidth: 180,
        valueGetter: (p) => txt(p.data?.examCenterName),
      },
      {
        headerName: "Building",
        minWidth: 130,
        valueGetter: (p) => txt(p.data?.buildingId),
      },
      {
  ...
  async function loadBaseFilters() {
    ...
    const empId = readLocalNumber(
      ["employeeId", "empId", "userId", "loginUserId"],
      0,
    );
    if (!empId) {
      toastError(
        "Employee id not found in session. Please re-login and retry.",
      );
      return;
    }
    setLoadingFilters(true);
    try {
      const rows = await getExamTimetableFilterRows({
  ...
  useEffect(() => {
    void (async () => {
      const [centerRows, buildingRows] = await Promise.all([
        listAllActiveUnivExamCenters().catch(() => []),
        listActiveBuildings().catch(() => []),
      ]);
      setCenters(Array.isArray(centerRows) ? centerRows : []);
      const list = (Array.isArray(buildingRows) ? buildingRows : []) as Row[];
      setBuildings(list);
      const firstBuildingId = list[0] ? String(num(list[0].buildingId)) : "0";
      setForm((f) =>
        f.buildingId && f.buildingId !== "0"
          ? f
          : { ...f, buildingId: firstBuildingId, blockId: "0", floorId: "0" },
      );
    })();
  }, []);
  ...
  useEffect(() => {
    const v = academicYears[0]
      ? String(pickAcademicYearId(academicYears[0]))
      : "";
  ...
  useEffect(() => {
    const v = centers[0]
      ? String(num(centers[0].univExamcenterId ?? centers[0].univExamCenterId))
      : "";
    if (!form.univExamcenterId && v && Number(v) > 0)
  ...
  function toggleOne(roomId: number, checked: boolean) {
    const next = vacancyRooms.map((r) =>
      num(r.roomId ?? r.pk_room_id) === roomId
  ...
  async function onSaveSelected() {
    ...
    const payload = selectedRooms.map((room) => ({
      ...
      collegeId,
      buildingId:
        Number(form.buildingId) || num(room.pk_building_id ?? room.buildingId),
      ...
      filters={
        <>
          <GlobalFilterBarRow className="global-filter-bar__row--ecr-r1">
            ...
            <GlobalFilterField
              label="Exam Center *"
              className="global-filter-field--fx22"
            >
              <Select
                options={centers
                  .map((r) => {
          ...
          <GlobalFilterBarRow className="global-filter-bar__row--ecr-r2">
            <GlobalFilterField
              label="Exam Center - Building"
              className="global-filter-field--fx50"
            >
              <Select
                options={buildings
                  .map((r) => {
                    const id = num(r.buildingId);
                    if (!id) return null;
                    const campus = txt(r.campusName);
                    const code = txt(r.buildingCode);
                    const name = txt(r.buildingName);
                    const label =
                      campus && code
                        ? `${campus} - ${code}`
                        : code && name
                ...
                value={form.buildingId === "0" ? "" : form.buildingId}
                onChange={(v) => {
                  hideResults();
            ...
            <GlobalFilterField
              label="Block"
              className="global-filter-field--fx16"
            >
              <Select
                options={blocks.map((r) => ({
                  value: String(num(r.blockId)),
                  label: txt(r.blockCode),
                }))}
                value={form.blockId === "0" ? "" : form.blockId}
                onChange={(v) => {
                  hideResults();
                    ...
                    <table className="w-full text-[12px]">
                      ...
                      <tbody>
                        ...
                        {selectedRooms.length === 0 && (
                          <tr>
                            <td className="px-2 py-6 text-center text-muted-foreground">
                              —
                            </td>
                          </tr>
                        )}
        ...
        <div className="space-y-1 text-sm">
          <p>
            ...
            <span className="text-blue-700">
              {txt(editRow?.examCenterName)}
            </span>
          </p>
          <p>
            <span className="font-medium">Building Name :</span>{" "}
            <span className="text-blue-700">
              {txt(editRow?.buildingName ?? editRow?.buildingId)}
            </span>
          </p>
          <p>
