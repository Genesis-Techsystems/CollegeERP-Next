...
const BACK =
  "/admin-examination-management/exam-papers-delivery-process/exam-group";
...
function toIsoExamMonthYr(
  examMonthDate: Date | null,
  examMonthRaw: string,
): string {
...
export default function ExamGroupDetailsPage() {
  ...
  const id = num(
    searchParams.get("univExamGroupId") ??
      searchParams.get("examGroupingId") ??
      0,
  );
  ...
  const universityIdFromRow = groupRow
    ? pickUniversityIdFromGroup(groupRow)
    : 0;
  const universityIdFromQuery = num(searchParams.get("universityId") ?? 0);
  const effectiveUniversityId =
    universityIdFromRow > 0 ? universityIdFromRow : universityIdFromQuery;
  ...
  const loadGroup = useCallback(async () => {
    ...
    try {
      ...
      const row = await getExamGroupingById(id, uidFromQuery, {
        orgId,
        employeeId,
      });
      setGroupRow(row);
      ...
      if (row) {
        ...
        setExamMonthDate(
          parseExamMonthYr(String(row.examMonthYr ?? row.examMonthYear ?? "")),
        );
  ...
  useEffect(() => {
    ...
    async function loadExams() {
      ...
      try {
        ...
        if (cancelled) return;
        const withExam = procRows.filter(
          (r) => num(r.fk_exam_id ?? r.exam_id ?? r.examId) > 0,
        );
      ...
      try {
        const list = await listExamsForExamGroupPicker(
  ...
  const academicYearSelectOptions: SelectOption[] = useMemo(() => {
    const base = academicYears
      .map((y) => ({
        value: String(
          num(y.academicYearId ?? y.academic_year_id ?? y.fk_academic_year_id),
        ),
        label: String(
          y.academic_year ?? y.academicYear ?? y.academicYearName ?? "-",
        ),
      }))
  ...
  const lineExamIds = useMemo(
    () => new Set(lines.map((r) => pickLineExamId(r)).filter((x) => x > 0)),
    [lines],
  );
  ...
  async function persistGroupHeader(): Promise<boolean> {
    ...
    const universityIdForPayload =
      pickUniversityIdFromGroup(groupRow) > 0
        ? pickUniversityIdFromGroup(groupRow)
        : universityIdFromQuery;
    if (!universityIdForPayload) {
      toastError(
        "University is missing. Open Group Details from the exam group list, or add ?universityId= to the URL.",
      );
      return false;
    }
    await updateExamGrouping(id, {
      ...
      examGroupCode: String(
        groupRow.examGroupCode ?? groupRow.exam_group_code ?? "",
      ),
      examGroupName: String(
        groupRow.examGroupName ?? groupRow.exam_group_name ?? "",
      ),
      academicYearId: Number(academicYearId),
  ...
  async function handleSave() {
    ...
    try {
      ...
      const refreshed = await getExamGroupingById(id, universityIdFromQuery, {
        orgId: oid,
        employeeId: eid,
      });
  ...
  async function handleAdd() {
    ...
    try {
      ...
      toastError(
        e,
        "Could not add exam — check UnivExamGroupDetails create payload with your API.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleRemove(line: AnyRow) {
    const examId = pickLineExamId(line);
    if (!examId) {
      toastError("Could not read exam id for this row.");
      return;
    }
    if (!id) return;
    setSaving(true);
    try {
      // Angular: mark isActive=false on the line, POST full array to saveUnivExamGroupDetails,
      // then reload via domain/list/UnivExamGroupDetails?...isActive==true
      await deactivateExamGroupExamLine({ lines, examId });
  ...
  return (
    <PageContainer className="space-y-5 p-4 sm:p-6">
      <PageHeader
        ...
        subtitle={
          titleSuffix
            ? `Exam papers delivery · ${titleSuffix}`
            : "Exam papers delivery · Exam group"
        }
      />

      <div className="app-card p-3 border-t-[3px] border-t-amber-300">
        <div className="flex items-center gap-2 min-w-0 border-b border-border pb-3">
          ...
          <h2 className="text-[15px] font-semibold leading-tight truncate">
            ...
            {titleParts.name && (
              <span className="text-primary font-bold">{titleParts.name}</span>
            )}
          </h2>
        </div>
          ...
          <div className="mt-4 flex flex-wrap items-end gap-4">
            ...
            <div className="space-y-1 min-w-[220px] flex-[2]">
              ...
              <Select
                ...
                placeholder={
                  effectiveUniversityId && academicYearId
                    ? "Select exam"
