...
/**
 * Add Room Seating Plan — University Exam Center variant.
 *
 * Faithful port of Angular
 * `exam-papers-delivery-process/exam-center-room-allotment/center-room-allotment`.
 * Differs from the college variant (admin-exam-masters/seating-plan-setup/
 * room-allotment) only in that the Building list is scoped to the selected
 * exam center (univExamcenterId) instead of the college's active buildings,
 * and Back returns to the exam-center seating-plan page.
 */
...
import { listBuildingsByUnivExamCenter } from '@/services/exam-papers-delivery'
...
export default function CenterRoomAllotmentPage() {
	...
	const params = useMemo(
		() => ({
			...
			univExamcenterId: Number(searchParams?.get('univExamcenterId') ?? 0),
	...
	useEffect(() => {
		void (async () => {
			const [bldgs, status, timetables, examRow] = await Promise.all([
				// Angular: buildings filtered by univExamCenterId.
				params.univExamcenterId
					? listBuildingsByUnivExamCenter(params.univExamcenterId).catch(() => [])
					: Promise.resolve([]),
	...
	}, [params.examId, params.univExamcenterId])
	...
	// Re-query rooms whenever location filters / timetable change (Angular getRooms()).
	useEffect(() => {
		if (!examTimetableId) {
			setVacancyRooms([])
			return
	...
	function handleGlobalRowsCols(rows: number, cols: number) {
		...
		setVacancyRooms((prev) =>
			prev.map((r) =>
				r.checked && !r.disabled ? { ...r, total_rows: rows, total_columns: cols, room_strength: rows * cols } : r,
			),
		)
	}
	...
	async function handleSave() {
		...
		const session = examTimetables.find((t: AnyRow) => Number(t.examTimetableId) === Number(examTimetableId))
	...
	function navigateBack() {
		...
		if (params.univExamcenterId) qp.set('univExamcenterId', String(params.univExamcenterId))
