...
/**
 * Copy Existing Seating — University Exam Center variant.
 *
 * Faithful port of Angular
 * `exam-papers-delivery-process/exam-center-room-allotment/exam-center-existing-allotment`.
 * The copy logic is shared with the college variant (copyExamRoomAllotmentSessions
 * copies a source session's room layout onto the chosen target sessions); this
 * page only carries univExamcenterId through and returns to the exam-center
 * seating-plan page on Back / Save.
 */
...
const INDEX =
  "/admin-examination-management/exam-papers-delivery-process/exam-center-room-allotment";
...
export default function CenterCopyExistingSeatingPage() {
  ...
  function navigateBack() {
    ...
    if (params.univExamcenterId)
      qp.set("univExamcenterId", String(params.univExamcenterId));
  ...
  return (
    <PageContainer className="space-y-4">
      <PageHeader
        ...
        subtitle="Reuse an earlier session's room layout"
      />
      ...
      <div className="rounded-md border border-border bg-card p-4 space-y-2 text-[13px]">
        <div>
          ...
          {params.courseCode
            ? `${params.courseCode} / ${params.academicYear}`
            : "—"}
        </div>
        <div>
      ...
      <div className="rounded-md border border-border bg-card p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          {sourceExamId ? (
            <div>
              <Label className="text-[12px]">Choose a exam date</Label>
              <Input
                type="date"
