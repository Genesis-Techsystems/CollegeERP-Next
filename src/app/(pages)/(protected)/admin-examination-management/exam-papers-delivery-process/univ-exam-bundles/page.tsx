...
export default function UnivExamBundlesPage() {
  ...
  const courses = useMemo(
    () => dedupeBy(filterRows, (r) => num(r.fk_course_id)),
    [filterRows],
  );
  ...
  async function onGetList() {
    ...
    try {
      ...
      filtered = filtered.filter(
        (r) =>
          num(r.univExamcenterId ?? r.univExamCenterId) ===
          Number(form.univExamcenterId),
      );
      if (Number(form.univExamBagId) > 0) {
        filtered = filtered.filter(
          (r) =>
            num(r.univExamBagId ?? r.univ_exam_bag_id) ===
          ...
          <div className="space-y-1 md:col-span-6">
            ...
            <Select
              options={exams.map((r) => ({
                ...
                label: `${txt(r.exam_name)} (${txt(r.from_date)} - ${txt(r.to_date)})`,
              }))}
              value={form.examId}
