...
export default function UnivExamBagTransportationPage() {
  ...
  const regionalOptions: SelectOption[] = useMemo(
    () =>
      regionalCenters.map((r) => ({
        value: String(
          num(r.univExamReionalCenterId ?? r.univExamRegionalCenterId),
        ),
  ...
  const headerText = useMemo(() => {
    const r = regionalCenters.find(
      (x) =>
        num(x.univExamReionalCenterId ?? x.univExamRegionalCenterId) ===
        Number(form.univExamReionalCenterId),
    );
    const c = centers.find(
      (x) =>
        num(x.univExamcenterId ?? x.univExamCenterId) ===
        Number(form.univExamcenterId),
    );
    const b = bags.find(
      (x) =>
        num(x.univExamBagId ?? x.univ_exam_bag_id) ===
  ...
  useEffect(() => {
    if (!form.univExamReionalCenterId && regionalOptions.length) {
      setForm((f) => ({
        ...f,
        univExamReionalCenterId: regionalOptions[0].value,
      }));
    }
  ...
  return (
    <PageContainer className="space-y-4">
      <FilterCard title="Exam Bag Transportation">
        ...
        <>
          <div className="app-card px-3 py-2 border-t-[3px] border-t-amber-300 border-b border-border">
            <h3 className="text-[13px] font-semibold text-[hsl(var(--card-title))]">
              Exam Bag Transportation - {headerText}
            </h3>
          </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="space-y-1 md:col-span-1">
            ...
            <Input
              type="date"
              value={formModal.receiveDate}
              onChange={(e) =>
                setFormModal((f) => ({ ...f, receiveDate: e.target.value }))
              }
            />
          </div>
          <div className="space-y-1 md:col-span-1">
            ...
            <Input
              type="date"
              value={formModal.dispatchDate}
              onChange={(e) =>
                setFormModal((f) => ({ ...f, dispatchDate: e.target.value }))
              }
            />
          </div>
        </div>
