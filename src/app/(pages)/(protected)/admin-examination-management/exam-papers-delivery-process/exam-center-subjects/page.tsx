...
export default function ExamCenterSubjectsPage() {
  ...
  const regulations = useMemo(
    () =>
      dedupeBy(
        baseRows.filter((r) => num(r.fk_course_id) === Number(form.courseId) && num(r.fk_regulation_id) > 0),
  ...
  const centerOptions: SelectOption[] = useMemo(
    () => centers.map((c) => ({ value: String(num(c.fk_univ_examcenter_id)), label: txt(c.examcenter_code) })),
    [centers],
  )
  const collegeOptions: SelectOption[] = useMemo(
    () => centerColleges.map((c) => ({ value: String(num(c.fk_college_id)), label: txt(c.college_code) })),
    [centerColleges],
  )
