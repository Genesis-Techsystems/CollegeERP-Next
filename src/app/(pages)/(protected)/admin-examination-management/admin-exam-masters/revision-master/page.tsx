...
/** Angular displayedColumns: id, examRevisionTypeName, fromDate, toDate, amount, isActive, actions */
const COL_DEFS = {
  siNo: {
    colId: "siNo",
    headerName: "SI.No",
    valueGetter: (p: { node?: { rowIndex?: number } }) =>
      (p.node?.rowIndex ?? 0) + 1,
    width: 72,
    flex: 0,
  } as ColDef<RevisionRow>,
  type: {
    field: "examRevisionTypeName",
    headerName: "Exam Revision Type",
    minWidth: 180,
  } as ColDef<RevisionRow>,
  fromDate: {
    field: "fromDate",
...
export default function RevisionMasterPage() {
  ...
  const [examRevisionTypeId, setExamRevisionTypeId] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    async function loadBase() {
      ...
      const firstCollegeId = Number(
        firstCollege?.collegeId ?? firstCollege?.fk_college_id ?? 0,
      );
  ...
  useEffect(() => {
    async function loadCourses() {
      ...
      const college = colleges.find(
        (c) => Number(c.collegeId ?? c.fk_college_id) === Number(collegeId),
      );
      ...
      const list = await listCoursesForRevisionFilters({
        collegeId,
        universityId,
      }).catch(() => []);
  ...
  useEffect(() => {
    async function loadRows() {
      ...
      try {
        if (!courseId) return;
        const list = await listRevisionMastersByCourse(courseId);
  ...
  async function save() {
    ...
    if (!examRevisionTypeId)
      nextErrors.examRevisionTypeId = "Exam Revision Type is required.";
    ...
    try {
      if (editing?.revisionMasterId) {
        ...
        await updateRevisionMaster(
          Number(editing.revisionMasterId),
          updatePayload,
        );
        toast.success("Revision master updated successfully.");
      } else {
        await createRevisionMaster(payload);
        toast.success("Revision master added successfully.");
      }
      closeModal();
      try {
        const list = await listRevisionMastersByCourse(courseId);
        ...
        toast.error(getErrorMessage(listErr));
        setRows([]);
      }
    } catch (err) {
      ...
      filters={
        <GlobalFilterBarRow>
          <GlobalFilterField
            label="College"
            icon={Building2}
            className="global-filter-field--shrink w-[22%] min-w-[12rem] max-w-[18rem]"
          >
            <CommonSelect
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="space-y-1 md:col-span-2">
              <CommonSelect
                ...
                value={
                  examRevisionTypeId != null ? String(examRevisionTypeId) : null
                }
                onChange={(v) => {
          ...
          <DialogFooter>
            {saveError ? (
              <p className="mr-auto text-[12px] text-red-600">{saveError}</p>
            ) : null}
