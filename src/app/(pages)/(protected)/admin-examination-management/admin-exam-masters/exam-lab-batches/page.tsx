...
export default function ExamLabBatchesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (courseList[0]?.fk_course_id)
        setCourseId(Number(courseList[0].fk_course_id));
      setLoading(false);
    }
    load();
  }, []);
  ...
  useEffect(() => {
    async function loadRest() {
      ...
      const data = await getUnivExamRestNoTimetable({
        courseId,
        examId,
        academicYearId,
        empId,
      ...
      const rest = data.filter(
        (x) => x.flag === "univ_exam_rest_filters" || x.flag === "regulations",
      );
      setRestFilters(rest);
    }
    loadRest();
  ...
  const colleges = useMemo(
    () =>
      dedupeBy(
        restFilters.filter((x) => x.fk_college_id),
        "fk_college_id",
      ),
    [restFilters],
  );
  ...
  const regulations = useMemo(
    () =>
      dedupeBy(
        restFilters.filter((x) => x.flag === "regulations"),
        "fk_regulation_id",
      ),
    [restFilters],
  );
  ...
  async function getList() {
    ...
    if (
      ...
      return;
    const data = await listExamLabBatches({
      collegeId,
      examId,
      courseYearId,
      courseGroupId,
  ...
  const openAdd = useCallback(() => {
    ...
    setFieldErrors({});
    setExamTypeId(
      availableExamTypes[0]?.generalDetailId
        ? Number(availableExamTypes[0].generalDetailId)
        : null,
    );
    setBatchName("");
    setCapacity("");
  ...
  const openEdit = useCallback(
    (r: Row) => {
      ...
      const rowTypeId = Number(
        r.examtypeCatdetId ?? r.fk_examtype_catdet_id ?? 0,
      );
      if (rowTypeId > 0) {
        setExamTypeId(rowTypeId);
      } else {
        const rowCode = String(
          r.examtypeCatdetCode ?? r.examTypeCatCode ?? "",
        ).toLowerCase();
        const matched = availableExamTypes.find(
          (t) => String(t.generalDetailCode ?? "").toLowerCase() === rowCode,
        );
        setExamTypeId(
          matched?.generalDetailId ? Number(matched.generalDetailId) : null,
        );
      }
  ...
  async function saveBatch() {
    ...
    setIsSaving(true);
    const selectedExamType = availableExamTypes.find(
      (t) => Number(t.generalDetailId) === Number(examTypeId),
    );
    ...
    try {
      const editId = Number(
        editing?.eaxmLabBatchId ?? editing?.examLabBatchId ?? 0,
      );
      if (editId > 0) {
        await updateExamLabBatch(editId, {
          ...payload,
          eaxmLabBatchId: editId,
        });
      } else {
      ...
      setSaveError(
        err?.message || "Unable to save exam batch. Please try again.",
      );
    } finally {
      setIsSaving(false);
    }
