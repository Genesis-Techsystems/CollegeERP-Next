...
export default function GradeSetupPage() {
    ...
    async function loadFilters() {
      ...
      try {
        ...
        for (const r of filtersData) {
          if (!uniMap.has(r.fk_university_id))
            uniMap.set(r.fk_university_id, r);
        }
  ...
  const openEdit = useCallback((row: any) => {
    ...
    setForm({
      ...
      minPoints: String(
        row?.minPoints ?? row?.fromPoints ?? row?.minGradePoint ?? "",
      ),
      maxPoints: String(
        row?.maxPoints ?? row?.toPoints ?? row?.maxGradePoint ?? "",
      ),
      fromPercentage: String(
        row?.fromPercentage ?? row?.minPercentage ?? row?.minScorePercent ?? "",
      ),
      toPercentage: String(
        row?.toPercentage ?? row?.maxPercentage ?? row?.maxScorePercent ?? "",
      ),
      creditPoints: String(row?.creditPoints ?? row?.gradePoint ?? ""),
      description: String(
        row?.description ?? row?.remarks ?? row?.gradeDesc ?? " ",
      ),
  ...
  async function saveGrade(e: any) {
    e.preventDefault();
    if (!selectedCourseId || !selectedRegulationId) return;
    const nextErrors: { gradeName?: string; gradeCode?: string } = {};
    if (!form.gradeName.trim())
      nextErrors.gradeName = "Grade name is required.";
    if (!form.gradeCode.trim())
      nextErrors.gradeCode = "Grade code is required.";
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        minScorePercent:
          form.fromPercentage === "" ? null : Number(form.fromPercentage),
        maxScorePercent:
      ...
      if (id != null) {
        await updateExamGrade(Number(id), {
          ...payload,
          examGradesId: Number(id),
        });
      } else {
    ...
    () => ({
      ...
      grade: {
        ...
        cellRenderer: (p: ICellRendererParams<any>) => {
          ...
          return (
            <span
              className={`inline-flex items-center rounded-lg px-2 py-0 text-[10px] font-semibold ${gradePillClass(gradeCode)}`}
            >
              {gradeCode}
            </span>
          );
      ...
      points: {
        ...
        valueGetter: (p) =>
          rangeText(
            p.data?.minPoints ?? p.data?.fromPoints ?? p.data?.minGradePoint,
            p.data?.maxPoints ?? p.data?.toPoints ?? p.data?.maxGradePoint,
          ),
      },
      score: {
        colId: "score",
        ...
        valueGetter: (p) =>
          rangeText(
            p.data?.fromPercentage ??
              p.data?.minPercentage ??
              p.data?.minScorePercent,
            p.data?.toPercentage ??
              p.data?.maxPercentage ??
              p.data?.maxScorePercent,
          ),
      },
      credit: {
        colId: "credit",
    ...
    <PageContainer className="space-y-4">
      ...
      >
        ...
        >
          ...
          <form
            onSubmit={saveGrade}
            className="px-4 py-2.5 space-y-2.5 bg-card"
          >
            {/* Course context bar */}
                ...
                <span className="text-[12px] text-[hsl(var(--primary))] font-semibold">
                  {(() => {
                    ...
                    const reg = regulations.find(
                      (r) => r.regulationId === selectedRegulationId,
                    );
                    const courseLabel =
                      course?.course_code ?? course?.course_name ?? "";
                    const regLabel = reg?.regulationCode ?? "";
                    const disLabel = isForDisabled
                      ? "For Disability"
                      : "Not For Disability";
                    const parts = [courseLabel, regLabel, disLabel].filter(
                      Boolean,
                    );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-2.5 pt-0.5">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setForm((s) => ({ ...s, gradeName: v }));
                    if (formErrors.gradeName)
                      setFormErrors((prev) => ({
                        ...prev,
                        gradeName: undefined,
                      }));
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    const v = e.target.value;
                    setForm((s) => ({ ...s, gradeCode: v }));
                    if (formErrors.gradeCode)
                      setFormErrors((prev) => ({
                        ...prev,
                        gradeCode: undefined,
                      }));
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-x-3 gap-y-2.5">
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((s) => ({ ...s, minPoints: e.target.value }))
                    }
                    placeholder="Min"
                  />
                </div>
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((s) => ({ ...s, maxPoints: e.target.value }))
                    }
                    placeholder="Max"
                  />
                </div>
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((s) => ({ ...s, fromPercentage: e.target.value }))
                    }
                    placeholder="Min %"
                  />
                </div>
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((s) => ({ ...s, toPercentage: e.target.value }))
                    }
                    placeholder="Max %"
                  />
                </div>
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((s) => ({ ...s, creditPoints: e.target.value }))
                    }
                    placeholder="Credit"
                  />
                </div>
