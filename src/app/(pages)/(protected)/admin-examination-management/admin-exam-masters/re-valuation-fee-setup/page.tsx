...
export default function RevaluationFeeSetupPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const list = distinct(Array.isArray(raw) ? raw : [], (r: any) =>
  ...
  const cols = useMemo<ColDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        headerName: "Re-Valuation Fee Structure",
        minWidth: 260,
      },
      {
      ...
      },
      {
        ...
        valueGetter: (p) => {
          const v =
            p.data?.collectionStartDate ??
            p.data?.collectionStartOn ??
            p.data?.fromDate;
          if (!v) return "—";
          const d = new Date(v);
      ...
      },
      {
        ...
        valueGetter: (p) => {
          const v =
            p.data?.collectionEndDate ??
            p.data?.collectionEndOn ??
            p.data?.toDate;
          if (!v) return "—";
          const d = new Date(v);
          return Number.isNaN(d.getTime())
  ...
  function openEdit(row: any) {
    ...
    const selectedCourse = courses.find(
      (c: any) => Number(c.fk_course_id) === Number(selectedCourseId),
    );
    const selectedYear = academicYears.find(
      (a: any) =>
        Number(a.fk_academic_year_id) === Number(selectedAcademicYearId),
    );
    ...
    router.push(
      `/admin-examination-management/admin-exam-masters/re-valuation-fee-setup/create?${params.toString()}`,
    );
  }
      ...
      {selectedCollegeId != null && titleLine ? (
        <div className="flex flex-wrap items-center gap-2 pt-1">
          <span className="text-[13px] font-medium text-slate-800">
            Re-Valuation Fee Structure :
          </span>
          <span className="text-[13px] text-[hsl(var(--primary))] font-semibold">
            {titleLine}
          </span>
        </div>
      ) : null}
  ...
  return (
    <FilteredListPage
      ...
      columnDefs={filtersComplete ? cols : []}
      loading={filtersComplete ? loadingList : false}
      pagination={filtersComplete}
