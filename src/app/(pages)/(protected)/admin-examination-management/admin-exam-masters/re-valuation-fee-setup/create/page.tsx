...
export default function CreateRevaluationFeeStructurePage() {
  ...
  useBreadcrumbLabel(
    editId ? "Edit Re Valuation Fee Setup" : "Create Re Valuation Fee Setup",
  );
  ...
  const [revalSubjectFees, setRevalSubjectFees] = useState<{
    one?: string;
    two?: string;
    three?: string;
    four?: string;
  ...
  function handleCourseChange(
    ...
    if (
      paramYearId &&
      years.some((a: any) => a.fk_academic_year_id === paramYearId)
    ) {
      ...
      for (const g of groupList) {
        ...
        for (const yr of yearList) {
          ...
          const courseYearName = String(
            yr.courseYearName ?? yr.course_year_name ?? yr.yearName ?? "",
          );
          const courseYearCode = String(
            yr.courseYearCode ?? yr.course_year_code ?? "",
          );
  ...
  useEffect(() => {
    async function loadExamMasters() {
      ...
      if (
        paramExamId &&
        list.some((e: any) => (e.examId ?? e.id) === paramExamId)
      ) {
  ...
  useEffect(() => {
    if (!loadingFilters && courses.length > 0) {
      if (
        paramCourseId &&
        courses.some((c: any) => c.fk_course_id === paramCourseId)
      ) {
    ...
    async function loadExisting() {
      ...
      setForm({
        ...
        collectionStartDate: String(
          row.collectionStartDate ?? row.collectionStartOn ?? "",
        ),
        collectionEndDate: String(
          row.collectionEndDate ?? row.collectionEndOn ?? "",
        ),
      ...
      const adt = Array.isArray(row.examFeeAdditionalStructure)
        ? row.examFeeAdditionalStructure
        : [];
      setAdditionalFees(
        adt.map((f: any) => ({
      ...
      const structureCourseId = Number(
        (Array.isArray(row.examFeeStructureCourseyr)
          ? row.examFeeStructureCourseyr
          : [])[0]?.courseId ??
          paramCourseId ??
          0,
      );
  ...
  function saveLateFeeRow() {
    ...
    setLateFeeFines((prev) => [
      ...prev,
      {
        name: lateFeeName,
        startDate: lateFeeStart,
        endDate: lateFeeEnd,
        regFeeFine: lateFeeReg,
        suppleFeeFine: lateFeeSupple,
      },
    ]);
    setLateFeeName("");
    setLateFeeStart("");
  ...
  function addAdditionalRow() {
    ...
    setAdditionalFees((s) => [
      ...s,
      { name: addFeeName, type: addFeeType, amount: addFeeAmount },
    ]);
    setAddFeeName("");
    setAddFeeAmount("");
  }
  ...
  const selectedCourseRow = courses.find(
    (c) => c.fk_course_id === selectedCourseId,
  );
  ...
  return (
    <PageContainer className="space-y-4">
      <PageHeader
        title={editId ? "Edit Re-Evaluation Fee" : ""}
        subtitle={editId ? "Update re-evaluation fee structure" : ""}
      />
      ...
      <div className="app-card overflow-hidden">
        <div className="px-4 py-3 border-b border-border bg-muted/40 flex items-center justify-between gap-2">
          <h2 className="app-card-title">
            {editId
              ? "Edit Re-Evaluation Fee Structure"
              : "Add Re-Evaluation Fee Structure"}
          </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-[0.9fr_1.1fr] gap-2">
                ...
                <div className="min-w-0 flex items-center gap-1 md:-ml-2">
                  ...
                  <span className="text-[hsl(var(--primary))]">
                    {examMasters.find(
                      (e) => (e.examId ?? e.id) === selectedExamId,
                    )?.examName ??
                      paramExamName ??
                      "—"}
                    {(() => {
                      ...
                      const from = formatDisplayDate(
                        selectedExam?.fromDate ??
                          selectedExam?.examFromDate ??
                          paramExamFromDate,
                      );
                      const to = formatDisplayDate(
                        selectedExam?.toDate ??
                          selectedExam?.examToDate ??
                          paramExamToDate,
                      );
                      return from || to
                        ? ` (${from || "—"} - ${to || "—"})`
              ...
              <div className="p-3">
                <div className="space-y-0.5">
                  ...
                  <div className="grid grid-cols-2 items-center gap-x-2 gap-y-1.5 max-w-md">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm((s) => ({ ...s, regularFee: e.target.value }))
                      }
                    />
                  </div>
                </div>
                ...
                <div className="mt-2.5 space-y-1.5 max-w-xl">
                  ...
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 items-center">
                    ...
                    <Input
                      inputMode="numeric"
                      value={revalSubjectFees.one ?? ""}
                      onChange={(e) =>
                        setRevalSubjectFees((s) => ({
                          ...s,
                          one: e.target.value,
                        }))
                      }
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                  <div className="md:col-span-3 space-y-1.5">
                    ...
                    <Input
                      className="h-8 text-[12px]"
                      value={lateFeeName}
                      onChange={(e) => setLateFeeName(e.target.value)}
                    />
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-1.5">
                    ...
                    <Input
                      ...
                      className="h-8 text-[12px]"
                      value={lateFeeReg}
                      onChange={(e) => setLateFeeReg(e.target.value)}
                    />
                  </div>
                  <div className="md:col-span-2 space-y-1.5">
                    <Label>Supple</Label>
                    <Input
                      ...
                      className="h-8 text-[12px]"
                      value={lateFeeSupple}
                      onChange={(e) => setLateFeeSupple(e.target.value)}
                    />
                  </div>
                  <div className="space-y-1.5 md:col-span-1">
                ...
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <div className="flex items-center gap-6 h-8">
                    <label className="inline-flex items-center gap-2 text-[12px]">
                      <input
                        type="radio"
                        checked={addFeeType === "regular"}
                        onChange={() => setAddFeeType("regular")}
                      />
                      <span>Regular</span>
                    </label>
                    <label className="inline-flex items-center gap-2 text-[12px]">
                      <input
                        type="radio"
                        checked={addFeeType === "supple"}
                        onChange={() => setAddFeeType("supple")}
                      />
                      <span>Supple</span>
                    </label>
                  </div>
