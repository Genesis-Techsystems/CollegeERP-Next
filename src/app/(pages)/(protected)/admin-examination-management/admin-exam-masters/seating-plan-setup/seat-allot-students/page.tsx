...
import {
  printSeatAllotmentDocument,
  type SeatAllotmentPrintMode,
} from "@/app/(pages)/(protected)/admin-examination-management/admin-exam-masters/seating-plan-setup/seat-allot-students/_print/printSeatAllotment";
import { useCollegeLogo } from "@/hooks/useCollegeLogo";
import {
  SeatAllotmentModal,
  type SeatAllotmentResult,
} from "@/app/(pages)/(protected)/admin-examination-management/_components/SeatAllotmentModal";
import {
...
function unwrapExamRoomAllotRecord(raw: any): any | null {
  ...
  if (
    typeof raw === "object" &&
    raw.examRoomAllotment != null &&
    Array.isArray(raw.examRoomAllotment)
  ) {
...
function maxSeatRowCol(rows: any[]): { maxRow: number; maxCol: number } {
  ...
  for (const r of rows) {
    ...
    const cn = num(
      pick(
        r,
        ["columnNo", "column_no", "columnNO", "colNo", "col_no", "column"],
        0,
      ),
    );
...
function buildSeatCells(
  ...
  const defaultStatusId = availableStatusId(examSeatStatuses);
  const defaultStatus = examSeatStatuses.find(
    (s) => Number(s.generalDetailId) === defaultStatusId,
  );
  const defaultCode = String(defaultStatus?.generalDetailCode ?? "Available");
  const defaultLabel = String(
    defaultStatus?.generalDetailDisplayName ??
      defaultStatus?.generalDetailName ??
      "Available",
  );
    ...
    for (let columnNo = 1; columnNo <= cols; columnNo++) {
      ...
      const statusCode = String(
        pick(
          ...
          [
            "examSeatStatusCode",
            "examseatstatusCatCode",
            "seat_status",
            "exam_display_seat_status_code",
          ],
          defaultCode,
        ),
      );
      const statusRow =
        examSeatStatuses.find(
          (s) => String(s.generalDetailCode) === statusCode,
        ) ??
        examSeatStatuses.find(
          (s) =>
            Number(s.generalDetailId) ===
            num(
              pick(seat, ["examseatstatusCatId", "examseatstatus_cat_id"], 0),
            ),
        );
      cells.push({
        key,
        ...
        examRoomStdAllotId:
          num(
            pick(
              ...
              [
                "examRoomStdAllotId",
                "exam_room_std_allot_id",
                "pk_exam_room_std_allot_id",
              ],
              0,
            ),
          ) || null,
        ...
        examTimetableId: ids.examTimetableId,
        roomId: ids.roomId,
        examseatstatusCatId: num(
          pick(
            seat,
            ["examseatstatusCatId", "examseatstatus_cat_id"],
            statusRow?.generalDetailId ?? defaultStatusId,
          ),
        ),
        studentId:
          num(pick(seat, ["studentId", "student_id", "fk_student_id"], 0)) ||
          null,
        subjectId:
          num(pick(seat, ["subjectId", "subject_id", "fk_subject_id"], 0)) ||
          null,
        stdName: String(
          pick(
            seat,
            ["stdName", "student_name", "firstName", "first_name"],
            "",
          ),
        ),
        hallticketNumber: String(
        ...
        subjectCode: String(
          pick(
            seat,
            ["subjectCode", "subject_code", "shortName", "short_name"],
            "",
          ),
        ),
        ...
        examDisplaySeatStatusCode: String(
          pick(
            ...
            [
              ...
              "examseatstatusCatDisplayName",
              "exam_display_seat_status_code",
            ],
            statusRow?.generalDetailDisplayName ?? defaultLabel,
          ),
        ),
        createdDt: pick(seat, ["createdDt", "created_dt"], null) as
          | string
          | null,
...
export default function SeatAllotStudentsPage() {
  ...
  async function reloadRoomAllotment() {
    ...
    try {
      const allotmentData =
        await fetchExamRoomAllotmentById(examRoomAllotmentId);
      const allotment = unwrapExamRoomAllotRecord(allotmentData);
      const examId =
        routeExamId ||
        num(pick(allotment, ["examId", "fk_exam_id", "exam_id"], 0));
      const courseId =
        routeCourseId ||
        num(
          pick(
      ...
      const examDate =
        details.examDate ||
        String(pick(allotment, ["examDate", "exam_date"], ""));
      const collegeId =
      ...
      if (filterRow) {
        setExamFilterIds({
          collegeId:
            collegeId ||
            num(
              filterRow.fk_college_id ??
                filterRow.college_id ??
                filterRow.collegeId,
            ),
          courseId:
            courseId ||
            num(
              filterRow.fk_course_id ??
                filterRow.course_id ??
                filterRow.courseId,
            ),
          examId:
            examId ||
            num(filterRow.fk_exam_id ?? filterRow.exam_id ?? filterRow.examId),
        });
        setScheduleMeta({
          collegeCode: String(
            filterRow.college_code ??
              ...
              pick(
                allotment,
                ["college_code", "collegeCode", "College.collegeCode"],
                "",
              ),
          ),
          academicYear: String(
            filterRow.academic_year ?? filterRow.academicYear ?? "",
          ).replace(/\s*-\s*/g, "-"),
          courseCode: String(
            filterRow.course_code ?? filterRow.courseCode ?? "",
          ),
        });
      }
    } finally {
  ...
  useEffect(() => {
    ...
    details.examId,
    details.courseId,
    details.examTimetableId,
    details.examDate,
    details.sessionId,
    details.examRoomAllotmentId,
    examSeatStatuses.length,
  ]);
  ...
  const bulkSubjectId = useMemo(() => {
    const raw = pick<unknown>(
      roomAllotment,
      ["subjectIds", "subjectId", "subject_ids"],
      0,
    );
  ...
  const filteredBulkStudents = useMemo(() => {
    ...
    return bulkStudents.filter((s) => {
      const name = String(
        s.firstName ?? s.first_name ?? s.stdName ?? "",
      ).toLowerCase();
      const roll = String(
        s.rollNumber ??
          s.roll_number ??
  ...
  const bulkCourseGroupOptions = useMemo(
    () =>
      bulkCourseGroups
        .map((g) => ({
          value: String(
            g.courseGroupId ?? g.fk_course_group_id ?? g.course_group_id ?? "",
          ),
          label: String(
            g.groupCode ?? g.group_code ?? g.groupName ?? g.group_name ?? "",
          ),
        }))
  ...
  const bulkCourseYearOptions = useMemo(
    () =>
      bulkCourseYears
        .map((y) => ({
          value: String(
            y.courseYearId ?? y.fk_course_year_id ?? y.course_year_id ?? "",
          ),
          label: String(
            y.courseYearName ??
              y.course_year_name ??
  ...
  useEffect(() => {
    if (mode !== "bulk" || bulkCourseGroupId || bulkCourseGroups.length === 0)
      return;
    const first = bulkCourseGroups[0];
    const id =
      first?.courseGroupId ??
      first?.fk_course_group_id ??
      first?.course_group_id;
    if (id) setBulkCourseGroupId(String(id));
  }, [mode, bulkCourseGroups, bulkCourseGroupId]);

  useEffect(() => {
    if (mode !== "bulk" || bulkCourseYearId || bulkCourseYears.length === 0)
      return;
    const first = bulkCourseYears[0];
    const id =
      first?.courseYearId ?? first?.fk_course_year_id ?? first?.course_year_id;
  ...
  useEffect(() => {
    ...
    async function resolveLabels() {
      ...
      const collegeId =
        Number(details.collegeId || 0) || allotmentContext.collegeId;
      const ayId = Number(searchParams?.get("academicYearId") || 0);
      if (!cId && !collegeId) {
        setResolvedCourse("");
      ...
      const [course, ay, college] = await Promise.all([
        ...
        ayId
          ? fetchSingleDomain("AcademicYear", "academicYearId", ayId).catch(
              () => null,
            )
          : Promise.resolve(null),
        collegeId
          ? fetchSingleDomain("College", "collegeId", collegeId).catch(
              () => null,
            )
          : Promise.resolve(null),
      ]);
      ...
      setResolvedAcademicYear(
        String(ay?.academicYear ?? ay?.academic_year ?? "").replace(
          /\s*-\s*/g,
          "-",
        ),
      );
  ...
  const courseLine = useMemo(() => {
    ...
    const college =
      ...
      "";
    const ay =
      details.academicYear || resolvedAcademicYear || scheduleMeta.academicYear;
    const course =
      details.courseCode ||
      details.courseName ||
      resolvedCourse ||
  ...
  useEffect(() => {
    ...
    async function resolveFromFilters() {
      ...
      if (!details.academicYear && !resolvedAcademicYear) {
        const ay = String(row.academic_year ?? row.academicYear ?? "").replace(
          /\s*-\s*/g,
          "-",
        );
      ...
      if (!Number(details.collegeId || 0) && !resolvedAllotmentIds.collegeId) {
        const collegeId = num(
          row.fk_college_id ?? row.college_id ?? row.collegeId,
        );
        if (collegeId > 0) {
          setResolvedAllotmentIds((prev) => ({ ...prev, collegeId }));
          setExamFilterIds((prev) => ({
            ...prev,
            collegeId: prev.collegeId || collegeId,
          }));
        }
      }
  ...
  const roomMeta = useMemo(() => {
    ...
    const coordsUnion =
      allotSeats.length > 0
        ? [...allotSeats, ...seatRows].filter(Boolean)
        : [...seatRows];
    ...
    const roomStrength = num(
      pick(metaSource, ["roomStrength", "room_strength"], 0),
    );
      ...
      : seatsSource.filter(
          (r) =>
            String(
              pick(
                r,
                ["examSeatStatusCode", "examseatstatusCatCode", "seat_status"],
                "",
              ),
            ).toLowerCase() === "blocked",
        ).length;
    const bookedFromGrid = seatCells.length
      ...
      : seatsSource.filter((r) => {
          ...
          return (
            status === "booked" ||
            num(pick(r, ["studentId", "student_id", "fk_std_id"], 0)) > 0
          );
        }).length;
    const availableFromGrid = seatCells.length
      ? seatCounts(seatCells).available
      : Math.max(
          (roomStrength || totalSeats || seatsSource.length) -
            bookedFromGrid -
            blockedFromGrid,
          0,
        );
    const blockedSeats = seatCells.length
      ? blockedFromGrid
      : num(
          pick(metaSource, ["blockedSeats", "blocked_seats"], blockedFromGrid),
        );
    const bookedSeats = seatCells.length
      ? bookedFromGrid
      : num(
          pick(
            metaSource,
            ["bookedSeats", "booked_seats", "noOfSeats"],
            bookedFromGrid,
          ),
        );
    ...
    return {
      ...
      roomLabel:
        ...
        String(
          pick(
            metaSource,
            ["room_name", "roomName", "roomCode", "room_code"],
            "",
          ),
        ),
    };
  ...
  const seatingGrid = useMemo(() => {
    ...
    if (seatCells.length > 0) {
      ...
      return Array.from({ length: totalRows }, (_, rIdx) => {
        const rowNo = rIdx + 1;
        const rowCells = (byRow.get(rowNo) ?? []).sort(
          (a, b) => a.columnNo - b.columnNo,
        );
    ...
    const allotSeatsFromAllotment = examRoomSeatDto(roomAllotment ?? null);
    const seatsSource =
      allotSeatsFromAllotment.length > 0 ? allotSeatsFromAllotment : seatRows;
    const seatByCell = new Map<string, any>();
    ...
    return Array.from({ length: totalRows }, (_, rIdx) => {
      ...
      return Array.from({ length: totalCols }, (_, cIdx) => {
        ...
        return {
          ...
          hallticket: String(
            pick(seat, ["hallticketNumber", "hallticket_number"], ""),
          ),
  ...
  const handlePrint = useCallback(
    async (mode: SeatAllotmentPrintMode) => {
      let printSeatRows = seatRows;
      if (
        mode === "attendance" ||
        mode === "stickers" ||
        ...
        const { collegeId, courseId, examId, examDate } = allotmentContext;
        const sessionId =
          Number(details.sessionId || 0) ||
          num(
            pick(
              roomAllotment,
              ["examSessionId", "fk_exam_session_id", "sessionId"],
              0,
            ),
          );
        const roomId = num(
        ...
        if (examId > 0) {
          setLoading(true);
          try {
            const data = await fetchRoomwiseOmrStudents({
              examId,
              courseId,
              examDate,
              sessionId,
            ...
            printSeatRows = Array.isArray(data) ? data : [];
            setSeatRows(printSeatRows);
          } finally {
            setLoading(false);
          }
        }
      ...
      printSeatAllotmentDocument({
        ...
        roomMeta: {
          ...
          totalCols: roomMeta.totalCols,
          capacity: roomMeta.capacity,
          bookedSeats: roomMeta.bookedSeats,
          availableSeats: roomMeta.availableSeats,
        },
        seatingGrid,
        seatRows: printSeatRows,
        attendanceRows,
        collegeLogo,
      });
    },
    [
      ...
      details.sessionId,
      roomAllotment,
      roomMeta.availableSeats,
      roomMeta.bookedSeats,
      roomMeta.capacity,
      roomMeta.roomLabel,
      roomMeta.totalCols,
      roomMeta.totalRows,
  ...
  async function enrichStudentParamsFromFilters(params: {
    ...
    const row =
      ...
      filters.find(
        (r) => num(r.fk_course_id) === Number(details.courseId || 0),
      ) ??
      filters[0];
    ...
    return {
      ...
      examId: params.examId || num(row.fk_exam_id ?? row.exam_id ?? row.examId),
      examDate:
        params.examDate ||
        toExamApiDate(details.examDate) ||
  ...
  function toggleBulkStudent(studentId: number, checked: boolean) {
    setBulkStudents((rows) =>
      rows.map((row) => {
        const id = Number(
          row.studentId ?? row.student_id ?? row.pk_student_id ?? 0,
        );
  ...
  async function handleBulkSave() {
    ...
    const bookedStatus = examSeatStatuses.find(
      (s) => String(s.generalDetailCode) === "Booked",
    );
    ...
    const bookedLabel = String(
      bookedStatus?.generalDetailDisplayName ??
        bookedStatus?.generalDetailName ??
        "Booked",
    );
    if (!bookedId) {
    ...
    const nextCells = seatCells.map((cell) => {
      ...
      const isAvailable =
        cell.examSeatStatusCode.toLowerCase() === "available" &&
        !cell.studentId;
      if (!isAvailable) return cell;
      const stu = selectedBulkStudents[studentIdx++];
      const studentId =
        Number(stu.studentId ?? stu.student_id ?? stu.pk_student_id ?? 0) ||
        null;
      const roll = String(
        stu.rollNumber ??
          stu.roll_number ??
          stu.hallticketNumber ??
          stu.hallticket_number ??
          "",
      );
      return {
        ...cell,
        ...
        subjectCode: String(
          stu.shortName ?? stu.subjectCode ?? stu.subject_code ?? "",
        ),
    ...
    if (studentIdx < selectedBulkStudents.length) {
      toast.warning(
        `Only ${studentIdx} of ${selectedBulkStudents.length} students were allotted (not enough available seats).`,
      );
    }
    ...
    const examRoomAllotmentId = num(
      pick(
        ...
        [
          ...
          "pk_exam_room_allotment_id",
        ],
        0,
      ),
    );
    const examDate =
      toDateStr(details.examDate) ||
      toDateStr(String(pick(roomAllotment, ["examDate", "exam_date"], "")));
    const payload = [
      {
          ...
          <div className="rounded-lg bg-white px-6 py-4 shadow-lg flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-slate-500">
                Fetching student allotment from server
              </span>
            </div>
          </div>
        </div>
          ...
          <>
            <div className="mb-3 rounded-md border border-cyan-200 bg-cyan-50/30 px-3 py-2 text-[12px]">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span>:</span>
                <span className="font-semibold text-blue-700">
                  {details.examName || details.examId || "-"}
                </span>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-semibold text-slate-700 min-w-16">
                ...
                <span>:</span>
                <span className="font-semibold text-blue-700">
                  {details.examType || details.examSession || "-"}
                </span>
              </div>
            </div>
            <GlobalFilterBarRow columns={2}>
              ...
              <GlobalFilterField label="Exam Timetable">
                <Input
                  value={`${details.examDate || ""} / ${details.examSession || ""}`}
                  readOnly
                  className="h-9 text-[13px]"
                />
              </GlobalFilterField>
              ...
              <GlobalFilterField label="Room Strength">
                <Input
                  value={String(roomMeta.roomStrength || 0)}
                  readOnly
                  className="h-9 text-[13px]"
                />
              </GlobalFilterField>
          ...
          <div className="space-y-2">
            ...
            {mode === "bulk" &&
              ...
              roomMeta.totalCols > 0 && (
                <div className="rounded border border-blue-200 bg-blue-50/20 p-3 space-y-3 mt-2">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                                ...
                                >
                                  <td className="px-2 py-1 align-top">
                                    <Checkbox
                                      checked={stu.checked}
                                      onCheckedChange={(v) =>
                                        toggleBulkStudent(studentId, v === true)
                                      }
                                    />
                                  </td>
                      ...
                      <div className="rounded border border-border bg-card p-2 space-y-2">
                        <Input
                          ...
                          onChange={(e) =>
                            setBulkSelectedSearch(e.target.value)
                          }
                          placeholder="Search..."
                          className="h-8 text-[12px]"
                        />
                  ...
                  bulkStudents.length === 0 ? (
                    <p className="text-[11px] text-muted-foreground px-1">
                      No students found for the selected group and year.
                    </p>
                  ) : null}
            ...
            {mode === "student" && (
              <div className="rounded border border-blue-200 p-2 space-y-2">
                <h3 className="text-[13px] font-semibold leading-tight text-blue-700 px-1">
                  Seating Order
                </h3>
                <p className="text-[11px] text-muted-foreground px-1">
                  Click a seat to allot or change student assignment.
                </p>
                              ...
                              {row.map((seat) => {
                                ...
                                return (
                                  <td
                                    ...
                                    title={
                                      seat.stdName
                                        ? `Student: ${seat.stdName}`
                                        : "Click to allot student"
                                    }
                                    onClick={() => {
                                      if (seat.cell)
                                        void openSeatModal(seat.cell);
                                    }}
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2 print-hide">
              <Button
                ...
                onClick={() => {
                  ...
                  if (details.examTimetableId)
                    qp.set("examTimetableId", details.examTimetableId);
              ...
              <Button
                type="button"
                className="h-8 px-6 text-[12px]"
                onClick={() => void handlePrint("seating")}
              >
                Print
              </Button>
              <Button
                type="button"
                className="h-8 px-6 text-[12px]"
                onClick={() => void handlePrint("attendance")}
              >
              ...
              <Button
                type="button"
                className="h-8 px-6 text-[12px]"
                onClick={() => void handlePrint("stickers")}
              >
                Print Stickers
              </Button>
              <Button
                type="button"
                className="h-8 px-6 text-[12px]"
                onClick={() => void handlePrint("groupwise-stickers")}
              >
                Group-Wise Stickers
              </Button>
            </div>
            <div className="text-[11px] text-muted-foreground px-1">
              {loading
                ? "Loading..."
                : `Students: ${attendanceRows.length} | Seats: ${seatRows.length}`}
            </div>
          </div>
        }
      >
