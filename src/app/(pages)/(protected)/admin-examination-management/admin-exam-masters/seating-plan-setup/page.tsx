...
function formatAcademicYearLabel(row: any): string {
  const raw = String(
    row?.academic_year ?? row?.academicYear ?? row?.label ?? "",
  ).trim();
  if (!raw) return "";
...
function extractSessionOptions(rawList: any[]): SessionOption[] {
  ...
  function addCandidate(obj: any) {
    ...
    const id =
      explicitId > 0 ? explicitId : toOptionId(parts.examDate, session, seq++);
    options.set(key, {
      id,
  ...
  for (const r of rows ?? []) {
    ...
    const roomCode =
      [
        ...
        r.blockCode,
        r.floorName,
        r.roomCode ?? r.room_code ?? r.room ?? r.block_room,
      ]
        .filter(Boolean)
        .join(" / ") || "-";
    const allotmentId = Number(
      r.examRoomAllotmentId ?? r.exam_room_allotment_id ?? r.id ?? 0,
    );
    const key =
      allotmentId > 0
        ? `id:${allotmentId}`
        : `${toDateStr(examDate)}|${session}|${roomCode}`;
    ...
    out.set(key, {
      ...
      roomCode,
      bookedSeats: Number(
        r.bookedSeats ?? r.booked_seats ?? r.noOfSeats ?? r.no_of_seats ?? 0,
      ),
...
function scopeRegSupUnivRows(
  ...
  return (rows ?? []).filter((r: any) => {
    ...
    if (Number(academicYearId) > 0 && ay > 0 && ay !== Number(academicYearId))
      return false;
    return true;
  });
}
...
export default function SeatingPlanSetupPage() {
  ...
  const fetchFilters = useCallback(async () => {
    ...
    try {
      ...
      if (distinctCourses.length > 0) {
        ...
        const targetCourse =
          urlCourseId > 0 &&
          distinctCourses.some(
            (c: any) => Number(c.fk_course_id) === urlCourseId,
          )
            ? urlCourseId
            : distinctCourses[0].fk_course_id;
        await handleCourseChange(
          targetCourse,
          Array.isArray(univRows) ? univRows : [],
        );
      }
    } finally {
  ...
  useEffect(() => {
    ...
    if (
      ...
      academicYears.some(
        (a: any) =>
          Number(a.fk_academic_year_id ?? a.academicYearId ?? a.id) === id,
      )
    ) {
  ...
  useEffect(() => {
    ...
    if (
      id > 0 &&
      examMasters.some(
        (e: any) => Number(e.fk_exam_id ?? e.examId ?? e.id) === id,
      )
    ) {
  ...
  async function handleCourseChange(courseId: number, regSupRows?: any[]) {
    ...
    const yearRowsForCourse = rowSource.filter(
      (a: any) =>
        Number(a.fk_course_id ?? a.course_id ?? 0) === Number(courseId),
    );
    ...
    setSelectedAcademicYearId((prev) => {
      const hasPrev = years.some(
        (y: any) => Number(y.fk_academic_year_id) === Number(prev),
      );
      if (hasPrev) return prev;
      const firstAy =
        years.find((y: any) => Number(y.is_curr_ay ?? 0) === 1) ?? years[0];
  ...
  useEffect(() => {
    ...
    setSelectedExamId((prev) => {
      ...
      return list.some((x: any) => Number(x.examId) === prevId)
        ? prevId
        : (list[0]?.examId ?? null);
    });
  ...
  const colleges = useMemo(
    () =>
      distinct(
        (restRows ?? []).filter((x: any) => x.fk_college_id),
        (x: any) => x.fk_college_id,
      ),
    [restRows],
  );
  ...
  const academicYearOptions = useMemo(() => {
    ...
    const fromUniv = (univExamFilterRows ?? [])
      .map((r: any, i: number) => {
        ...
        const id =
          Number(r.fk_academic_year_id ?? r.academic_year_id ?? 0) ||
          100000 + i;
        return { id, label };
      })
  ...
  useEffect(() => {
    ...
    async function loadExamTimetables() {
      ...
      const distinctColleges = distinct(
        restList.filter((x: any) => x.fk_college_id),
        (x: any) => x.fk_college_id,
      );
      setSelectedCollegeId((prev) => {
        const prevId = Number(prev ?? 0);
        return distinctColleges.some(
          (c: any) => Number(c.fk_college_id) === prevId,
        )
          ? prevId
      ...
      if (list.length === 0 && selectedCourseYearId) {
        const rows = await getExamTimetableDetails(
      ...
      if (list.length > 0) {
        ...
        if (effectiveUnivRows.length > 0) {
          const scoped = effectiveUnivRows.filter((r: any) => {
            const examId = Number(
              r.fk_exam_id ?? r.exam_id ?? r.pk_exam_id ?? 0,
            );
          ...
          for (const s of [...opts, ...fallbackOpts])
            uniq.set(`${s.examDate}|${s.session}`, s);
        ...
        if (mergedOpts.length > 0) {
          ...
          const allotmentOpts = extractSessionOptions(
            Array.isArray(allotmentRows) ? allotmentRows : [],
          );
        ...
        if (fallbackOpts.length === 0) {
          ...
          fallbackOpts = extractSessionOptions(
            Array.isArray(allotmentRows) ? allotmentRows : [],
          );
        }
  ...
  useEffect(() => {
    function mapSelectedSessionRows() {
      ...
      const rows = examTimetables.filter(
        (t: any) =>
          Number(t.examTimetableId) === Number(selectedExamTimetableId),
      );
      ...
      const mapped = rows.map((r: any, i: number) => ({
        ...
        roomCode:
          [
            ...
            r.blockCode,
            r.floorName,
            r.roomCode ?? r.room ?? r.block_room,
          ]
            .filter(Boolean)
            .join(" / ") || "-",
  ...
  function handleCopyExistingSeating() {
    ...
    const qp = new URLSearchParams({
      ...
      courseCode: String(
        courses.find(
          (c: any) =>
            Number(c.courseId ?? c.id ?? c.fk_course_id) ===
      ...
      academicYear: String(
        academicYearOptions.find((a) => a.id === selectedAcademicYearId)
          ?.label ?? "",
      ),
      examName: String(
        examMasters.find(
          (e: any) =>
            Number(e.examId ?? e.id ?? e.fk_exam_id) === Number(selectedExamId),
        )?.examName ?? "",
      ),
    });
    router.push(
  ...
  async function confirmAssignSeating() {
    ...
    try {
      ...
      const rows = await assignSeatingAllSession({
        examId: selectedExamId,
        examDate,
        sessionId,
      });
  ...
  function openSeatAllotStudents(row: AllocationRow) {
    ...
    const normalizedRoomCode =
      ...
      String(row.roomCode ?? "").trim() !== "-"
        ...
        : [
            ...
            raw.blockCode,
            raw.floorName,
            raw.roomCode ?? raw.room ?? raw.block_room,
          ]
            .map((x: unknown) => String(x ?? "").trim())
            .filter(Boolean)
            .join(" / ");

    const resolvedCourseId = pickNum(
      selectedCourseId,
      raw.fk_course_id,
      raw.courseId,
      raw.course_id,
    ...
    const resolvedExamId = pickNum(
      selectedExamId,
      raw.fk_exam_id,
      raw.examId,
      raw.exam_id,
    );
    const resolvedCollegeId = pickNum(
      selectedCollegeId,
      raw.fk_college_id,
      raw.collegeId,
      raw.college_id,
    ...
    const collegeRow = colleges.find(
      (c: any) =>
        Number(c.fk_college_id ?? c.collegeId) === Number(resolvedCollegeId),
    );
    const courseRow = courses.find(
      (c: any) =>
        Number(c.fk_course_id ?? c.courseId ?? c.id) ===
    ...
    const params = new URLSearchParams({
      ...
      examName: String(
        examMasters.find(
          (e: any) =>
            Number(e.examId ?? e.id ?? e.fk_exam_id) === Number(resolvedExamId),
        )?.examName ??
          raw.examName ??
          raw.exam_name ??
          "",
      ...
      examDate: String(
        normalizeDate(row.examDate ?? raw.examDate ?? raw.exam_date),
      ),
  ...
  const columnDefs = useMemo<ColDef[]>(
    () => [
      ...
      { field: "session", headerName: "Session", minWidth: 110 },
      {
        ...
        minWidth: 110,
        valueFormatter: (p: any) => formatRoomCode(p.value),
      },
      { field: "bookedSeats", headerName: "Booked Seats", width: 120, flex: 0 },
      {
        field: "blockedSeats",
        headerName: "Blocked Seats",
        width: 130,
        flex: 0,
      },
      {
  ...
  if (printMode) {
    const selectedExam = examMasters.find(
      (e: any) => Number(e.examId ?? e.id) === Number(selectedExamId),
    );
    const selectedCourse = courses.find(
      (c: any) => Number(c.fk_course_id ?? c.id) === Number(selectedCourseId),
    );
    const selectedAy = academicYears.find(
      (a: any) =>
        Number(a.fk_academic_year_id ?? a.id) ===
        Number(selectedAcademicYearId),
    );
    const examName =
      String(selectedExam?.examName ?? selectedExam?.exam_name ?? "").trim() ||
      "Exam";
    const courseLabel = String(
      selectedCourse?.course_code ?? selectedCourse?.course_name ?? "",
    ).trim();
    const ayLabel = String(
      selectedAy?.academic_year ?? selectedAy?.academicYear ?? "",
    ).trim();
    ...
    const totalStudents = filteredRows.reduce(
      (sum, r) => sum + (r.bookedSeats || 0),
      0,
    );
    ...
    const dateSessionGroups = filteredRows.reduce<
    ...
    if (printMode === "room-wise-seating") {
      ...
      const grouped = roomWiseAllocations.reduce<Record<string, any[]>>(
        (acc, curr) => {
      ...
      const groups = Object.entries(grouped).map(([room_name, records]) => ({
        room_name,
        records,
      }));
      return (
        <PrintShell title="Seating Arrangement">
          {groups.length === 0 ? (
            <p className="text-[11px] text-center py-6">
              No room-wise allotment data for this exam date / session.
            </p>
          ) : (
            groups.map(({ room_name, records }, gi) => (
              <table
                key={`rws-${room_name}-${gi}`}
                  ...
                  {records.map((alloc: any, i: number) => (
                    <tr key={`rws-row-${room_name}-${i}`}>
                      ...
                      <td
                        style={{ border: "1px solid #000", padding: "4px 6px" }}
                      >
                        {alloc["min(tssd.hallticket_number)"] ?? "—"} to{" "}
                        {alloc["max(tssd.hallticket_number)"] ?? "—"}
                      </td>
                      <td
                        style={{ border: "1px solid #000", padding: "4px 6px" }}
                      >
    ...
    if (printMode === "room-subject-counts") {
      ...
      const grouped = roomSubjectAllocations.reduce<Record<string, any[]>>(
        (acc, curr) => {
          const key = String(curr?.room_name ?? "—");
          if (!acc[key]) acc[key] = [];
          acc[key].push(curr);
      ...
      return (
        <PrintShell title="Seating Arrangement — Subject Counts">
          {groups.length === 0 ? (
            <p className="text-[11px] text-center py-6">
              No room-subject allotment data for this exam date / session.
            </p>
          ) : (
            groups.map(({ room_name, records }, gi) => (
              <table
                key={`rsc-${room_name}-${gi}`}
                className="w-full border-collapse text-[11px] mb-4"
                style={{ border: "1px solid #000" }}
              >
                <thead>
    ...
    if (printMode === "group-wise-seating") {
      ...
      let sno = 1;
      const groupedSubjects = Array.from(bySubject.entries()).map(
        ([subject_name, rows]) => {
          ...
          const branches = Array.from(byBranch.entries()).map(
            ([branch, allocations]) => ({
              sno: sno++,
              branch,
              allocations,
      ...
      return (
        <PrintShell title="Seating Arrangement — Group Wise">
          {groupedSubjects.length === 0 ? (
            <p className="text-[11px] text-center py-6">
              No group-wise allotment data for this exam date / session.
            </p>
          ) : (
            groupedSubjects.map((subject) => (
              <div
                key={`gws-${subject.subject_name}`}
                style={{ marginBottom: "24px" }}
              >
                <h3 style={{ marginTop: "24px", fontSize: "14px" }}>
                  ...
                  <tbody>
                    {subject.branches.flatMap((group) =>
                      group.allocations.map((alloc: any, i: number) => (
                        <tr
                          key={`gws-${subject.subject_name}-${group.branch}-${i}`}
                        >
                          {i === 0 && (
                            <td
                              style={{
                              ...
                              rowSpan={group.allocations.length}
                            >
                              {group.sno}
                            </td>
                          )}
    ...
    if (printMode === "attendance") {
      ...
      for (const s of studentAllotmentDetails) {
        const key = [
          s.fk_course_group_id,
          s.fk_subject_id,
          s.room_id,
          s.fk_examtype_catdet_id,
        ].join("|");
      ...
      const attGroups = Array.from(byKey.values()).map((students) =>
        students
          .slice()
          .sort((a: any, b: any) =>
            String(a.hallticket_number).localeCompare(
              String(b.hallticket_number),
              undefined,
              { numeric: true },
            ),
          ),
      ...
      if (attGroups.length === 0) {
        return (
          <PrintShell title="Attendance Sheet">
            <p className="text-[11px] text-center py-6">
              No allotted students for this exam date / session.
            </p>
          </PrintShell>
        );
      }
          ...
          {attGroups.map((students, gi) => {
            ...
            return (
              <div key={`att-${gi}`} className={gi > 0 ? "page-break" : ""}>
                ...
                >
                  {s?.exam_label_name ?? examName}{" "}
                  {s?.exam_type_name ? `(${s.exam_type_name})` : ""}
                </h4>
                ...
                <table
                  className="w-full border-collapse text-[11px] mb-3"
                  style={{ border: "1px solid #000" }}
                >
                  <thead>
                    <tr>
    ...
    if (printMode === "student" || printMode === "groupwise-stickers") {
      ...
      if (rooms.length === 0) {
        return (
          <PrintShell
            title={
              isGroupwise ? "Group-Wise Seating Stickers" : "Seating Stickers"
            }
          >
            <p className="text-[11px] text-center py-6">
              No allotted students for this exam date / session.
          ...
          {rooms.map((roomStudents, ri) => {
            if (isGroupwise) {
              ...
              return Array.from(byGroup.entries()).map(
                ([groupKey, students], gi) => {
                  const head = students[0];
                  return (
                    <div
                      key={`gst-${ri}-${gi}`}
                      className={ri + gi > 0 ? "page-break" : ""}
                      style={{ marginBottom: "20px" }}
                    >
                      <StickerHeader
                        row={head}
                        extraGroup={head?.group_code ?? groupKey}
                      />
      ...
      return (
        <PrintShell title="Invigilators">
          {invigilatorRows.length === 0 ? (
            <p className="text-[11px] text-center py-6">
              No invigilator allotments for this exam timetable.
            </p>
          ) : (
            <table
              className="w-full border-collapse text-[11px]"
  ...
  return (
            ...
            <div className="rounded-lg bg-white px-6 py-4 shadow-lg flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[11px] text-slate-500">
                  Fetching student data from server (can take 10-15s)
                </span>
              </div>
            </div>
          </div>
    ...
    >
      ...
      >
        <p>
          If you have already created a seating plan, this action will erase the
          existing plan and generate a new one. You may also need to reprint all
          related summaries.
        </p>
