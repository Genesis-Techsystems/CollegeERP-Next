...
import { PageContainer, TableContextHeader } from "@/components/layout";
...
type AdditionalFeeRow = {
  ...
  typeName: string;
  /** Angular `generalDetailCode` stored on payload `type`. */
  typeCode: string;
  examType: "REG" | "SUPPLE";
...
function isRevisionFeeItem(item: Record<string, unknown>) {
  ...
  return (
    masterId === REVISION_GENERAL_MASTER_ID ||
    masterCode === GM_CODES.REVISION_TYPE
  );
}
...
export default function CreateExamFeeStructurePage() {
  ...
  const [deletedAdditionalFees, setDeletedAdditionalFees] = useState<
  ...
  const [deletedRevisionFees, setDeletedRevisionFees] = useState<
    Record<string, unknown>[]
  >([]);
  ...
  const pageParams = useMemo(
    () => ({
      ...
      collegeName: String(searchParams.get("collegeName") ?? ""),
    }),
    [searchParams],
  );
  ...
  function goBackToList() {
    const qp = new URLSearchParams({
      check: String(pageParams.check || 1),
      universityId: String(pageParams.universityId || 0),
      courseId: String(pageParams.courseId || 0),
      academicYearId: String(pageParams.academicYearId || 0),
      examId: String(pageParams.examId || 0),
      collegeId: String(pageParams.collegeId || 0),
    });
    router.push(
      `/admin-examination-management/admin-exam-masters/exam-fee-setup?${qp.toString()}`,
    );
  }
  ...
  const filteredCourseGroupYears = useMemo(() => {
    ...
    return courseGroupYears.filter((c) =>
      `${c.groupCode} ${c.courseYearCode} ${c.courseYearName}`
        .toLowerCase()
        .includes(lower),
    );
  }, [q, courseGroupYears]);
  ...
  useEffect(() => {
    async function loadTypes() {
      try {
        ...
        const regSupple = (Array.isArray(examTypes) ? examTypes : []).filter(
          (t: any) => {
        ...
        if (revisionList.length > 0) {
          ...
          setRevalDraft((s) => ({
            ...
            typeName: String(
              first.generalDetailDisplayName ??
                first.generalDetailName ??
                "Revaluation",
            ),
          }));
        }
  ...
  const loadCourseGroupYears = useCallback(
    async (courseId: number, structureRow: any | null) => {
      if (!courseId) {
        setCourseGroupYears([]);
        return;
      }
      ...
      const savedCourseyr: any[] = Array.isArray(
      ...
      for (const g of groupList) {
        ...
        for (const y of yearList) {
          ...
          if (
            built.some(
              (b) =>
                b.courseGroupId === courseGroupId &&
                b.courseYearId === courseYearId,
            )
          )
            continue;
          const courseYearName = String(
            y.courseYearName ?? y.course_year_name ?? y.yearName ?? "",
          );
          const courseYearCode = String(
            y.courseYearCode ?? y.course_year_code ?? "",
          );
          const match = savedCourseyr.find(
            (x) =>
              Number(x.courseGroupId ?? x.fk_course_group_id) ===
                courseGroupId &&
          ...
          if (match) {
            built.push({
              ...
              examFeeStructureId:
                Number(
                  match.examFeeStructureId ??
                    structureRow?.examFeeStructureId ??
                    0,
                ) || undefined,
              examFeeCourseyrId:
                Number(match.examFeeCourseyrId ?? 0) || undefined,
            });
          } else {
            built.push({
              courseGroupId,
  ...
  useEffect(() => {
    async function loadExisting() {
      ...
      setForm((s) => ({
        ...
        collectionStartDate: toDateString(
          parseDateValue(String(row.collectionStartDate ?? "")),
        ),
        collectionEndDate: toDateString(
          parseDateValue(String(row.collectionEndDate ?? "")),
        ),
        regFee: row.regFee != null ? String(row.regFee) : "",
        suppleFee:
          row.supplyFee != null
            ? String(row.supplyFee)
            : row.suppleFee != null
              ? String(row.suppleFee)
              : "",
      ...
      const structureCourseId = Number(
        asRecordArray(row.examFeeStructureCourseyr)[0]?.courseId ??
          pageParams.courseId ??
          0,
      );
      ...
      const fineRows = asRecordArray(row.examFeeFine)
        ...
        .map((x) => ({
          ...
          fineFromDate: toDateString(
            parseDateValue(String(x.fineFromDate ?? "")),
          ),
          ...
          examFeeFineId:
            x.examFeeFineId != null ? Number(x.examFeeFineId) : undefined,
          serverSnapshot: { ...x },
        }));
      setFines(fineRows);
      setDeletedFines([]);
      ...
      for (const x of asRecordArray(row.examFeeAdditionalStructure).filter(
        ...
        if (isRevisionFeeItem(x)) {
          revisionLoaded.push({
            ...
            typeName: String(
              x.adtExamfeetypeCatDisplayName ??
                x.adtExamfeetypeCatCode ??
                x.type ??
                "",
            ),
          ...
          additionalLoaded.push({
            ...
            examType: displayCode.toLowerCase().includes("supp")
              ? "SUPPLE"
              : "REG",
    ...
    isEditMode,
    pageParams.examFeeStructureId,
    pageParams.courseId,
    loadCourseGroupYears,
  ]);

  function toggleCourseGroupYear(courseGroupId: number, courseYearId: number) {
    setCourseGroupYears((prev) =>
      prev.map((c) =>
        c.courseGroupId === courseGroupId && c.courseYearId === courseYearId
          ? { ...c, check: !c.check }
          : c,
      ),
    );
  ...
  function addFineRow() {
    ...
    if (
      Number.isFinite(fineStart) &&
      Number.isFinite(fineEnd) &&
      fineStart > fineEnd
    ) {
    ...
    if (
      fineDraft.regFeeFine !== "" &&
      parseNumberOrNull(fineDraft.regFeeFine ?? "") == null
    ) {
      toastError("Reg fee fine must be a number");
      return;
    }
    if (
      fineDraft.suppleFeeFine !== "" &&
      parseNumberOrNull(fineDraft.suppleFeeFine ?? "") == null
    ) {
  ...
  function removeFineRow(i: number) {
    ...
    if (row?.examFeeFineId != null && row.serverSnapshot) {
      setDeletedFines((d) => [
        ...d,
        { ...row.serverSnapshot, isActive: false },
      ]);
    }
  ...
  function addAdditionalRow() {
    ...
    const selected = additionalTypeOptions.find(
      (t) => t.value === String(additionalDraft.typeId),
    );
    const typeObj = additionalTypes.find(
      (t) =>
        String(t.generalDetailId ?? t.id ?? t.general_detail_id ?? "") ===
        String(additionalDraft.typeId),
    );
    // Angular addAdditionalFees: type = generalDetailCode
    const typeCode = String(
      typeObj?.generalDetailCode ??
        typeObj?.general_detail_code ??
        additionalDraft.typeName ??
        selected?.label ??
        "",
    );
    ...
    const examTypeCat = examFeeTypes.find((t) => {
      ...
      return additionalDraft.examType === "REG"
        ? code === "Regular"
        : code === "Supple";
    });
    setAdditionalRows((s) => [
      ...s,
      {
        ...
        serverSnapshot: {
          ...
          examTypeCatDisplayCode:
            additionalDraft.examType === "REG" ? "Regular" : "Supple",
          includeInRev: false,
          isActive: true,
        },
      },
  ...
  function removeAdditionalRow(i: number) {
    // Keep delete push outside the rows updater — nested setState in an updater
    // can double-fire under React Strict Mode and duplicate soft-deleted payloads.
    const row = additionalRows[i];
    if (row?.feeAddtId && row.serverSnapshot) {
      setDeletedAdditionalFees((d) => [
        ...d,
        { ...row.serverSnapshot, isActive: false },
      ]);
    }
  ...
  function addRevalRow() {
    ...
    if (
      Number.isFinite(revalStart) &&
      Number.isFinite(revalEnd) &&
      revalStart > revalEnd
    ) {
  ...
  function removeRevalRow(i: number) {
    const row = revalRows[i];
    if (row?.feeAddtId && row.serverSnapshot) {
      setDeletedRevisionFees((d) => [
        ...d,
        { ...row.serverSnapshot, isActive: false },
      ]);
    }
  ...
  // Angular drives course/exam from URL query params only (no clg_filters on add/edit).
  const effectiveCourseId =
    pageParams.courseId > 0 ? pageParams.courseId : null;
  const effectiveExamId = pageParams.examId > 0 ? pageParams.examId : null;
  const effectiveCollegeId =
    pageParams.collegeId > 0 ? pageParams.collegeId : loadedCollegeId;
  const hasCheckedCourseYear = courseGroupYears.some((c) => c.check);

  const programContextLine = [
    [pageParams.universityCode, pageParams.collegeName]
      .filter(Boolean)
      .join(" "),
    pageParams.courseName,
    pageParams.academicYear ? `(${pageParams.academicYear})` : "",
  ]
    .filter(Boolean)
    .join(" / ");

  const examContextLine = [
    pageParams.examName,
  ...
  function validate(): string[] {
    ...
    if (form.collectionStartDate && form.collectionEndDate) {
      ...
      if (Number.isFinite(s) && Number.isFinite(e) && s > e) {
        ...
        next.collectionEndDate =
          "Collection End Date must be on or after Start Date";
      }
    }
    const numericFields = [
      ...
      ["suppleFee", "5 Subject Fee", form.suppleFee],
      ["subject5Fee", "Subject 5 Fee", form.subject5Fee],
  ...
  async function save() {
    ...
    const buildAdditionalFeePayload = (
      ...
      const typeOption = additionalTypes.find(
        (t) => String(t.generalDetailId ?? t.id) === String(row.typeId),
      );
      return {
        ...base,
        ...
        type:
          row.typeCode ||
          typeOption?.generalDetailCode ||
          row.typeName ||
          base.type ||
          typeOption?.generalDetailDisplayName ||
          typeOption?.generalDetailName,
        isActive: true,
        collegeId,
    ...
    const buildFinePayload = (row: FineRow): Record<string, unknown> => {
      ...
      return {
        ...
        regFeeFine:
          row.regFeeFine === "" || row.regFeeFine == null
            ? null
            : Number(row.regFeeFine),
        supplyFeeFine:
          row.suppleFeeFine === "" || row.suppleFeeFine == null
            ? null
            : Number(row.suppleFeeFine),
        isActive: true,
        collegeId,
    ...
    const buildRevisionFeePayload = (
      ...
      return {
        ...
        adtExamfeetypeCatCode:
          revisionType?.generalDetailDisplayName ??
          base.adtExamfeetypeCatCode ??
          row.typeName,
      };
    };
    ...
    try {
      ...
      toastError(
        err,
        `Failed to ${isEditMode ? "update" : "save"} exam fee structure`,
      );
    }
  }
        ...
        <div className="px-3 py-3 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>
                Exam Fee Structure <span className="text-red-500">*</span>
              </Label>
              <Input
                ...
                onChange={(e) => {
                  setForm((s) => ({
                    ...s,
                    examFeeStructureName: e.target.value,
                  }));
              ...
              <div className="space-y-1.5">
                ...
                <DatePicker
                  ...
                  onChange={(date) =>
                    setForm((s) => ({
                      ...s,
                      collectionStartDate: toDateString(date),
                    }))
                  }
              ...
              <div className="space-y-1.5">
                ...
                <DatePicker
                  ...
                  onChange={(date) => {
                    setForm((s) => ({
                      ...s,
                      collectionEndDate: toDateString(date),
                    }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-3 items-start">
            ...
            <div className="max-h-[360px] overflow-y-auto scrollbar-hidden space-y-1">
              {filteredCourseGroupYears.map((c) => {
                ...
                return (
                  <label
                    key={key}
                    className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-muted/40 text-[12px]"
                  >
                    <Checkbox
                      checked={c.check}
                      onCheckedChange={() => {
              ...
              {filteredCourseGroupYears.length === 0 && (
                <div className="text-[12px] text-muted-foreground px-2">
                  No items
                </div>
              )}
            </div>
        ...
        <div className="lg:col-span-3 space-y-3">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
                  <div className="space-y-1.5">
                    <Input
                      ...
                      value={form.regFee}
                      onChange={(e) =>
                        setForm((s) => ({ ...s, regFee: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Input
                      placeholder="1 Subject Fee"
                      inputMode="numeric"
                      value={form.subject1Fee}
                      onChange={(e) =>
                        setForm((s) => ({ ...s, subject1Fee: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Input
                      placeholder="2 Subject Fee"
                      inputMode="numeric"
                      value={form.subject2Fee}
                      onChange={(e) =>
                        setForm((s) => ({ ...s, subject2Fee: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Input
                      placeholder="3 Subject Fee"
                      inputMode="numeric"
                      value={form.subject3Fee}
                      onChange={(e) =>
                        setForm((s) => ({ ...s, subject3Fee: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Input
                      placeholder="4 Subject Fee"
                      inputMode="numeric"
                      value={form.subject4Fee}
                      onChange={(e) =>
                        setForm((s) => ({ ...s, subject4Fee: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Input
                      placeholder="5 Subject Fee"
                      inputMode="numeric"
                      value={form.suppleFee}
                      onChange={(e) =>
                        setForm((s) => ({ ...s, suppleFee: e.target.value }))
                      }
                    />
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFineDraft((s) => ({ ...s, fineName: e.target.value }))
                    }
                  />
                </div>
                <div className="md:col-span-2 space-y-1.5">
                  ...
                  <DatePicker
                    ...
                    onChange={(date) =>
                      setFineDraft((s) => ({
                        ...s,
                        fineFromDate: toDateString(date),
                      }))
                    }
                    placeholder="dd-mm-yyyy"
                    className="text-[12px]"
              ...
              <div className="rounded-md border overflow-auto">
                <table className="w-full text-[12px]">
                  ...
                  <tbody>
                    ...
                    {fines.map((r, i) => (
                      <tr
                        key={`${r.fineName}-${r.fineFromDate}-${i}`}
                        className="border-t"
                      >
                        ...
                        <td className="px-2 py-1">
                          {r.fineFromDate
                            ? new Date(r.fineFromDate).toLocaleDateString()
                            : ""}{" "}
                          {r.fineToDate
                            ? `- ${new Date(r.fineToDate).toLocaleDateString()}`
                            : ""}
                        </td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                <div className="md:col-span-2 space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      const selected = revisionTypes.find(
                        (t) => String(t.generalDetailId ?? t.id) === v,
                      );
                      setRevalDraft((s) => ({
                        ...s,
                        typeId: v,
                        typeName: String(
                          selected?.generalDetailDisplayName ??
                            selected?.generalDetailName ??
                            v,
                        ),
                      }));
                    }}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setRevalDraft((s) => ({ ...s, amount: e.target.value }))
                    }
                  />
                </div>
                <div className="space-y-1.5">
          ...
          <div className="app-card overflow-hidden">
            <div className="px-4 py-2.5 border-b border-border bg-muted/40">
              <h3 className="text-[14px] font-semibold">
                List of Additional Fees Applicable
              </h3>
            </div>
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-6 h-8">
                    <label className="inline-flex items-center gap-2 text-[12px]">
                      <input
                        type="radio"
                        checked={additionalDraft.examType === "REG"}
                        onChange={() =>
                          setAdditionalDraft((s) => ({ ...s, examType: "REG" }))
                        }
                      />
                      Regular
                    </label>
                    <label className="inline-flex items-center gap-2 text-[12px]">
                      <input
                        type="radio"
                        checked={additionalDraft.examType === "SUPPLE"}
                        onChange={() =>
                          setAdditionalDraft((s) => ({
                            ...s,
                            examType: "SUPPLE",
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="h-8 flex items-center">
                    <Checkbox
                      checked={additionalDraft.includeInReg}
                      onCheckedChange={(v) =>
                        setAdditionalDraft((s) => ({
                          ...s,
                          includeInReg: Boolean(v),
                        }))
                      }
                    />
                  </div>
                ...
                <table className="w-full text-[12px]">
                  ...
                  <tbody>
                    ...
                    {additionalRows.map((r, i) => {
                      const type = additionalTypes.find(
                        (t: any) =>
                          String(t.generalDetailId ?? t.id) ===
                          String(r.typeId),
                      );
                      ...
                      return (
                        <tr key={`${r.typeId}-${i}`} className="border-t">
                          ...
                          <td className="px-2 py-1">
                            {r.examType === "REG" ? "Regular" : "Supple"}
                          </td>
                          <td className="px-2 py-1">{r.amount || "—"}</td>
                          <td className="px-2 py-1">
                            {r.includeInReg ? "Yes" : "No"}
                          </td>
                          <td className="px-2 py-1">
                            <Button
                              type="button"
