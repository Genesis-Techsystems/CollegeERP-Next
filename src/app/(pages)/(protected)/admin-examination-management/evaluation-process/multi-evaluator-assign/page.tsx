...
export default function MultiEvaluatorAssignPage() {
  ...
  useEffect(() => {
    if (academicYears[0])
      setAcademicYearId(num(academicYears[0].fk_academic_year_id));
  }, [academicYears]);
  useEffect(() => {
  ...
  // Angular radioChange(): exclude list is comma-separated profile ids (string match).
  ...
  // Angular radioChange(): only disable when disable_omr === 1 (strict) OR excluded.
  // Do not coerce with Number()/num() — "1"/true would wrongly disable rows Angular leaves selectable.
  const isOmrDisabledFor = (s: AnyRow, evaluatorId: number | null) =>
    isExcludedFor(s, evaluatorId) || s.disable_omr === 1;
