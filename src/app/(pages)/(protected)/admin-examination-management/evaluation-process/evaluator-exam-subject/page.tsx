...
const COL_DEFS = {
  ...
  evaluatorName: {
    ...
    flex: 1,
    valueGetter: (p) =>
      txt(p.data?.evaluator_name ?? p.data?.evaluatorName) || "-",
  } as ColDef<AnyRow>,
  subject: {
    headerName: "Subjects",
    minWidth: 140,
  ...
  examMonthYear: {
    ...
    flex: 1,
    valueGetter: (p) =>
      txt(p.data?.exam_month_yr ?? p.data?.examMonthYear) || "-",
  } as ColDef<AnyRow>,
};
...
export default function EvaluatorExamSubjectPage() {
  ...
  useEffect(() => {
    async function loadSubjects() {
      ...
      const groups = data?.result ?? [];
      const sub =
        groups.find((g) => txt(g?.[0]?.flag) === "univ_exam_sub_regexamstd") ??
        [];
      setSubjectRows(sub);
  ...
  async function onAssign() {
    const chosenEvaluators = evaluators.filter((r) =>
      selectedEvaluatorIds.includes(num(r.pk_exam_evaluator_profile_id)),
    );
    const chosenColleges = colleges.filter((r) =>
      selectedCollegeIds.includes(num(r.fk_college_id)),
    );
    const payload: AnyRow[] = [];

    for (const evaluator of chosenEvaluators) {
      for (const college of chosenColleges) {
        const timetableIds = txt(college.pk_exam_timetable_det_ids)
          .split(",")
          .map((x) => num(x))
          .filter((x) => x > 0);
  ...
  return (
    <FilteredListPage
      ...
      columnDefs={hasFetched ? columnDefs : undefined}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded border overflow-hidden bg-card">
              <div className="border-b bg-[#C3D9FF] px-2 py-1.5 text-[12px] font-semibold">
                Evaluators List:{" "}
                <span className="text-blue-700">{evaluators.length}</span>
              </div>
              <div className="max-h-[260px] overflow-auto p-2 space-y-1">
                ...
                {evaluators.length === 0 && (
                  <p className="px-1 py-4 text-center text-muted-foreground text-[12px]">
                    No evaluators.
                  </p>
                )}
              </div>
            ...
            <div className="rounded border overflow-hidden bg-card">
              <div className="border-b bg-[#C3D9FF] px-2 py-1.5 text-[12px] font-semibold">
                Colleges:{" "}
                <span className="text-blue-700">{colleges.length}</span>
              </div>
              <div className="max-h-[260px] overflow-auto p-2 space-y-1">
                {colleges.map((c) => {
