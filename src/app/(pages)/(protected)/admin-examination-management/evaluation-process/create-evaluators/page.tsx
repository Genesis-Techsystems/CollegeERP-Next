...
function isPersistedPrefRow(row: AnyRow) {
  return (
    pickNum(row, [
      "examEvaluatorPreferenceId",
      "examEvaluatorPreferencesId",
      "pk_exam_evaluator_preference_id",
    ]) > 0
  );
}
  ...
  return (p: { data?: AnyRow }) => (
    <div className="text-[12px]">
      ...
      <button
        type="button"
        className="text-blue-700 hover:underline"
        onClick={() => openPreferences(p.data ?? {})}
      >
        Preferences
      </button>
    </div>
...
function makePrefActionsRenderer(onDelete: (row: AnyRow) => void) {
  return (p: ICellRendererParams<AnyRow>) => (
    <TableRowActions
      onDelete={() => onDelete(p.data ?? {})}
      deleteLabel="Remove preference"
    />
  );
}
...
function emptyForm(): FormState {
  // Angular: profileValidFromDate defaults to new Date(); profileValidToDate starts empty.
  return {
    collegeId: "",
    isEmp: false,
    evaluatorEmpId: "",
    ...
    profileValidFromDate: toDateOnlyISO(new Date()),
    profileValidToDate: "",
    isActive: true,
    reason: "",
  };
...
// Angular CONSTANTS.patterns + validations (create-evaluator)
...
const MSG = {
  ...
  collegeId: "University College is required",
  titleId: "Title is required",
  evaluatorName: "Name is required",
  profileValidFromDate: "Start Date is required",
  profileValidToDate: "End Date is required",
} as const;

/** Mirrors Angular addevaluatorform FormBuilder validators used by submit(). */
function validateEvaluatorFields(form: FormState): Record<string, string> {
  const errors: Record<string, string> = {};
  if (!form.collegeId) errors.collegeId = MSG.collegeId;
  if (!form.titleId) errors.titleId = MSG.titleId;
  if (!form.evaluatorName.trim()) errors.evaluatorName = MSG.evaluatorName;
  // email: required (template) + pattern
  if (!form.email.trim() || !RE_EMAIL.test(form.email.trim()))
    errors.email = MSG.email;
  // phone / alternate: required + phNo pattern
  if (!RE_PHONE.test(form.phoneNumber.trim())) errors.phoneNumber = MSG.phNo;
  if (!RE_PHONE.test(form.alternatePhoneNumber.trim()))
    errors.alternatePhoneNumber = MSG.phNo;
  // aadhar: required + aadharNo pattern
  if (!RE_AADHAR.test(form.aadhar.trim())) errors.aadhar = MSG.aadharNo;
  // panCardNo: Validators.pattern only (empty is valid)
  if (form.panCardNo.trim() && !RE_PAN.test(form.panCardNo.trim()))
    errors.panCardNo = MSG.panNo;
  if (!form.profileValidFromDate)
    errors.profileValidFromDate = MSG.profileValidFromDate;
  if (!form.profileValidToDate)
    errors.profileValidToDate = MSG.profileValidToDate;
  return errors;
}

/** Single-field error — same rules Angular uses for dirty/touched mat-error. */
function errorForField(key: string, form: FormState): string | undefined {
  return validateEvaluatorFields(form)[key];
}

function patchFieldError(
...
export default function CreateEvaluatorsPage() {
  ...
  const [prefFieldErrors, setPrefFieldErrors] = useState<
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const onlyUniversity = all.filter(
        (c) => c?.isUniversity === true || c?.is_university === true,
      );
  ...
  async function saveBank() {
    ...
    try {
      ...
      if (bankEditId) {
        await updateEvaluatorBankDetails(bankEditId, {
          ...payload,
          evaluatorBankDetailId: bankEditId,
        });
      } else {
        await createEvaluatorBankDetails({
          ...payload,
  ...
  function onSelectEmployeeId(employeeId: string | null) {
    ...
    const next: FormState = {
      ...
      collegeId:
        emp?.collegeId != null ? String(emp.collegeId) : form.collegeId,
      email: emp?.email != null ? String(emp.email) : form.email,
      evaluatorName:
        emp?.firstName != null ? String(emp.firstName) : form.evaluatorName,
      phoneNumber: emp?.mobile != null ? String(emp.mobile) : form.phoneNumber,
    };
    setForm(next);
    // Live-clear/recheck autofilled fields (Angular dirty updates).
    for (const key of [
      "collegeId",
      "email",
      "evaluatorName",
      ...
      patchFieldError(setFieldErrors, key, errorForField(key, next));
    }
  }

  /** Update one form field and show Angular mat-error immediately while typing (dirty). */
  ...
  function openEdit(row: AnyRow) {
    ...
    setForm({
      ...
      isEmp: false,
      evaluatorEmpId:
        row?.evaluatorEmpId != null ? String(row.evaluatorEmpId) : "",
      ...
      travelToEvaluationCenter:
        row?.travelToEvaluationCenter != null
          ? String(row.travelToEvaluationCenter)
          : "",
  ...
  async function onSave() {
    ...
    const payload: AnyRow = {
      ...
      travelToEvaluationCenter:
        (form.travelToEvaluationCenter ?? "").trim() === ""
          ? ""
          : Number(form.travelToEvaluationCenter),
      ...
      examEvaluatorProfileDetailsDTOS:
        editRow?.examEvaluatorProfileDetailsDTOS ?? [],
    };
    setLoading(true);
    try {
      if (editRow?.examEvaluatorProfileId) {
        await updateEvaluatorProfile({
          ...payload,
          examEvaluatorProfileId: editRow.examEvaluatorProfileId,
        });
      } else {
  ...
  const openSubjects = useCallback(
    (row: AnyRow) => {
      ...
      if (profileId > 0) {
        ...
        router.push(
          `/admin-examination-management/evaluation-process/create-evaluators/evaluator-subject-roles?examEvaluatorProfileId=${profileId}`,
        );
        return;
      }
      toastError("Invalid evaluator profile.");
  ...
  const openPreferences = useCallback((row: AnyRow) => {
    setPrefRow(row);
    setPrefFieldErrors({});
    setPrefOpen(true);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const sorted = [...s].sort(
        (a, b) => pickNum(b, ["subjectId"]) - pickNum(a, ["subjectId"]),
      );
  ...
  function addPreferenceRow() {
    ...
    const courseObj = prefCourses.find(
      (c) => pickNum(c, ["courseId"]) === prefCourseId,
    );
    const regulationObj = prefRegulations.find(
      (r) => pickNum(r, ["regulationId"]) === prefRegulationId,
    );
    let duplicate = false;
    const next = [...prefAll];
    for (const sid of prefSubjectIds.map(Number)) {
      const subjectObj = prefSubjects.find(
        (s) => pickNum(s, ["subjectId"]) === sid,
      );
      ...
      if (!existing) {
        next.push({
          ...
          regulationCode: pickText(regulationObj, [
            "regulationCode",
            "regulationName",
          ]),
          subjectId: sid,
    ...
    if (duplicate) {
      toastError(
        "One or more subjects already exist for this course and regulation.",
      );
    }
    setPrefAll(next);
    setPrefSubjectIds([]);
  ...
  const deletePrefRow = useCallback((row: AnyRow) => {
    setPrefAll((prev) => {
      ...
      return prev.map((p) =>
        prefRowsMatch(p, row) ? { ...p, isActive: false } : p,
      );
    });
  }, []);

  const prefTableRows = useMemo(
    () => prefAll.filter((p) => p.isActive !== false),
    [prefAll],
  );
  ...
  const prefColumnDefs = useMemo<ColDef<AnyRow>[]>(
    () => [
      ...
      {
        ...PREF_COL_DEFS.actions,
        cellRenderer: makePrefActionsRenderer(deletePrefRow),
      },
    ],
    [deletePrefRow],
  );
  ...
  async function savePreferences() {
    ...
    if (prefTableRows.length === 0 && !hasPendingDeletes) {
      if (!validatePrefAdd()) return;
      setPrefFieldErrors({
        subjectIds: "Add at least one preference before saving.",
      });
      return;
    }
    setLoading(true);
  ...
  const cols = useMemo<ColDef[]>(
    () => [
      ...
      { field: "aadhar", headerName: "AadharCard", minWidth: 150 },
      {
        headerName: "Details",
        minWidth: 170,
        cellRenderer: makeDetailsRenderer(openSubjects, openPreferences),
      },
      {
        field: "isActive",
        headerName: "Status",
    ...
    >
      ...
      >
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              {editRow ? "Edit Evaluator Profile" : "Create Evaluator Profile"}
            </DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            {form.isEmp ? (
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onChange={(v) => updateFormField("collegeId", v ?? "")}
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onChange={(v) => updateFormField("titleId", v ?? "")}
                options={titleOptions}
                placeholder="Title"
                error={fieldErrors.titleId}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-[12px]">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateFormField("evaluatorName", e.target.value)
                }
                placeholder="Name"
              />
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateFormField("alternatePhoneNumber", e.target.value)
                }
            ...
            <div className="space-y-1">
              ...
              <Input
                type="number"
                value={form.aadhar ?? ""}
                onChange={(e) => updateFormField("aadhar", e.target.value)}
                placeholder="Aadhar"
              />
              {fieldErrors.aadhar ? (
                <p className="text-[11px] text-destructive">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateFormField("travelToEvaluationCenter", e.target.value)
                }
                placeholder="Travel to Evaluation center"
              />
            </div>
            <div className="space-y-1">
              <Label className="text-[12px]">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateFormField("profileValidFromDate", e.target.value)
                }
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateFormField("profileValidToDate", e.target.value)
                }
              ...
              <ActiveStatusField
                ...
                onActiveChange={(v) =>
                  setForm((p) => ({
                    ...p,
                    isActive: v === true,
                    reason: v === true ? "" : p.reason,
                  }))
                }
                onReasonChange={(v) => setForm((p) => ({ ...p, reason: v }))}
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
              <div className="md:col-span-3 space-y-1">
                ...
                <Select
                  ...
                  options={prefCourses.map(
                    (c) =>
                      ({
                        value: String(pickNum(c, ["courseId"])),
                        label: pickText(c, ["courseCode"]),
                      }) as SelectOption,
                  )}
                  placeholder="Course"
              ...
              <div className="md:col-span-3 space-y-1">
                ...
                <Select
                  ...
                  options={prefRegulations.map(
                    (r) =>
                      ({
                        value: String(pickNum(r, ["regulationId"])),
                        label: pickText(r, [
                          "regulationName",
                          "regulationCode",
                        ]),
        ...
        <DialogContent className="max-w-3xl">
          ...
          <div className="space-y-4">
            <div className="rounded-md border-2 border-[#B2EBF2] p-2">
              <BankSummaryRow
                label="Evaluator :"
                value={pickText(bankProfile, ["evaluatorName"])}
              />
              <BankSummaryRow
                label="Role :"
                value={pickText(bankProfile, ["roleName", "role_name"])}
              />
              <BankSummaryRow
                label="Mobile :"
                value={pickText(bankProfile, ["phoneNumber", "phone"])}
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setBankForm((p) => ({ ...p, branchName: e.target.value }))
                  }
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setBankForm((p) => ({ ...p, bankAddress: e.target.value }))
                  }
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setBankForm((p) => ({ ...p, phone: e.target.value }))
                  }
              ...
              <div>
                ...
                <Input
                  value={bankForm.upi}
                  onChange={(e) =>
                    setBankForm((p) => ({ ...p, upi: e.target.value }))
                  }
                  placeholder="Upi"
                />
              </div>
              ...
              {!bankForm.isActive && (
                <div className="md:col-span-3">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setBankForm((p) => ({ ...p, reason: e.target.value }))
                    }
                    placeholder="Reason"
                  />
                </div>
