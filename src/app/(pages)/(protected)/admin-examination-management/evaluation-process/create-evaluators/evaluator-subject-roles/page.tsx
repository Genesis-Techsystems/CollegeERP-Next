...
export default function EvaluatorSubjectRolesPage() {
  ...
  const subjectsFiltered = useMemo(() => {
    ...
    return subjectsAll.filter((s) => {
      ...
      const groups = pickText(s, ["groupNames"]).toLowerCase();
      return name.includes(t) || code.includes(t) || groups.includes(t);
    });
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // Roles load once on page open (v_get_exam_eval_roles) — not on every exam change.
  ...
  useEffect(() => {
    ...
    void (async () => {
      const subjects = await getEvaluatorSubjectRolesSubjects({
        courseId,
        examId,
        academicYearId,
        regulationId,
      ...
      setSubjectsAll(
        withSubjectGroupNames(Array.isArray(subjects) ? subjects : []),
      );
    })();
  ...
  function addToTable() {
    ...
    if (
      ...
      toastError(
        "Please select course, academic year, exam, regulation, subject, and role.",
      );
      return;
    }
    ...
    const baseDetail =
      dialogTitle === "Add Evaluator Details"
        ? {
            ...
            isReEvaluation,
            maxNoOfEvaluationsAssign: isReEvaluation
              ? undefined
              : Number(maxNoOfEvaluationsAssign || 0),
            maxNoOfReevaluationsAssign: isReEvaluation
              ? Number(maxNoOfReevaluationsAssign || 0)
              : undefined,
            isActive: true,
            reason: null,
    ...
    <FilteredPage
      ...
      filters={
        <div className="space-y-4 text-[13px]">
          {/* Angular row 1: Course (20) + Academic Year (20) + Exam (60) */}
          ...
          {/* Angular row 2: Role (18) + Regulation (20) + Subject (50) + Is Re-Evaluation (12) */}
          <GlobalFilterBarRow className="global-filter-bar__row--aesr-r2">
            <GlobalFilterField
              label="Role"
