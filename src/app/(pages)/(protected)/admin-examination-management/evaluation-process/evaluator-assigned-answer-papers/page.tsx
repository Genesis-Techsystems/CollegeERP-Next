...
const MARKING_PATH =
  "/admin-examination-management/evaluation-process/assign-answerpapers-dynamic";
...
export default function EvaluatorAssignedAnswerPapersPage() {
  ...
  const maxNoOfEvaluationsAssign = Number(
    searchParams.get("maxNoOfEvaluationsAssign") ?? 0,
  );
  const maxNoOfReevaluationsAssign = Number(
    searchParams.get("maxNoOfReevaluationsAssign") ?? 0,
  );
  const isReEvaluation =
    (searchParams.get("isReEvaluation") ?? "false") === "true";
  ...
  const [noOfStudentsAssigned, setNoOfStudentsAssigned] = useState(
    Number(searchParams.get("noOfStudentsAssigned") ?? 0),
  );
  const [noOfEvaluationsCompleted, setNoOfEvaluationsCompleted] = useState(
    Number(searchParams.get("noOfEvaluationsCompleted") ?? 0),
  );
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      setNoOfEvaluationsCompleted(
        answerRows.filter(
          (r) =>
            r.evaluationStatusCatDetId === EVAL_STATUS.EVALUATED ||
            r.evaluationStatusCatDetId === EVAL_STATUS.FINALIZED,
        ).length,
      );
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
          : "Failed to load assigned answer papers.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function assignNext() {
    if (isReEvaluation) {
      if (
        noOfStudentsAssigned >= maxNoOfReevaluationsAssign &&
        maxNoOfReevaluationsAssign > 0
      ) {
        setError("You have been assigned the maximum number of papers.");
        return;
      }
    } else if (
      noOfStudentsAssigned >= maxNoOfEvaluationsAssign &&
      maxNoOfEvaluationsAssign > 0
    ) {
      setError("You have been assigned the maximum number of papers.");
      return;
    }
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Failed to assign next answer paper.",
      );
    } finally {
      setAssigning(false);
    }
  ...
  function openPaper(row: StudentAnswerPaper) {
    ...
    const status = row.evaluationStatusCatDetId;
    if (
      status === EVAL_STATUS.EVALUATED ||
      status === EVAL_STATUS.REJECTED ||
      status === EVAL_STATUS.UFM
    )
      return;
  ...
  return (
    <PageContainer className="space-y-4">
      <PageHeader
        title="Evaluator Assigned Answer Papers"
        subtitle={
          subjectName
            ? `Subject: ${subjectName}`
            : "Assigned answer papers list"
        }
      />
          ...
          <table className="w-full text-[12px]">
            ...
            <tbody>
              {filteredRows.map((row, i) => {
                ...
                const isDone =
                  status === EVAL_STATUS.EVALUATED ||
                  status === EVAL_STATUS.FINALIZED;
                const canCheck =
                  row.studentAnswerPath &&
                  (status === EVAL_STATUS.NEW ||
                    status === EVAL_STATUS.ASSIGNED ||
                    status === EVAL_STATUS.IN_PROGRESS ||
                    status === 0);
                return (
                  <tr
                    key={`${row.examEvaluationAssignmentId}-${row.omrSerialNo}-${i}`}
                    className="border-t"
                  >
                    ...
                    <td className="px-2 py-1">
                      {canCheck ? (
                        <Button
                          type="button"
                          className={`h-7 px-2 text-[11px] text-white ${statusClassName(row)}`}
                        ...
                        <span
                          className={`inline-flex h-7 items-center rounded px-2 text-[11px] text-white ${statusClassName(row)}`}
                        >
                          {getStatusLabel(row)}
                        </span>
                      )}
                    ...
                    <td className="px-2 py-1">
                      {isDone ? (row.evaluatedTotalMarks ?? "-") : "-"}
                    </td>
                    <td className="px-2 py-1">
                      {isDone
                        ? formatDate(row.answerSheetCheckDate) || "-"
                        : "-"}
                    </td>
                  </tr>
                );
      ...
      <div className="text-[12px] text-slate-600">
        Total Assigned:{" "}
        <span className="font-semibold">{noOfStudentsAssigned}</span> |
        Evaluated:{" "}
        <span className="font-semibold">{noOfEvaluationsCompleted}</span>
        {examId > 0 ? (
          <>
            {" "}
            | Exam Id: <span className="font-semibold">{examId}</span>
          </>
        ) : null}
      </div>
    </PageContainer>
