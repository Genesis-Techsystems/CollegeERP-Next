...
function getTemplateTitle(row: AnyRow): string {
  return txt(row.templateTitle || row.template_title || row.questionpaper_template_title || row.title)
}
...
export default function AssignQuestionTemplatePage() {
  ...
  const existingTemplateAssignId = num(searchParams.get('existingTemplateAssignId'))
  ...
  useEffect(() => {
    async function loadTemplate() {
      ...
      const rows = await getQuestionPaperTemplateViewRows(selectedTemplateId).catch(() => [])
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return templates.filter((t) => getTemplateTitle(t).toLowerCase().includes(q))
  }, [templates, search])

  async function saveAssignment() {
    if (!examId || !regulationId || !subjectId || !courseYearId || !selectedTemplateId) return
    setLoading(true)
    try {
      if (isEdit) {
      ...
      router.push(`/admin-examination-management/evaluation-process/assign-questionpaper-template?courseId=${searchParams.get('courseId') ?? ''}&academicYearId=${searchParams.get('academicYearId') ?? ''}&examId=${examId}&courseYearId=${courseYearId}&regulationId=${regulationId}`)
    } finally {
      setLoading(false)
    }
  }
    ...
    <PageContainer className="space-y-5">
      ...
      <div className="app-card p-3 space-y-3">
        <div className="flex items-center justify-end gap-2">
          <Button type="button" onClick={() => void saveAssignment()} disabled={!selectedTemplateId || loading}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
              ...
              {filteredTemplates.map((t, i) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={`w-full px-3 py-2 text-left text-[13px] border-b ${active ? 'bg-blue-700 text-white' : 'hover:bg-slate-50'}`}
                    onClick={() => {
          ...
          <div className="md:col-span-8 border rounded p-2 max-h-[560px] overflow-auto">
            ...
            {templateRows.map((row, i) => {
              ...
              return (
                <div key={`row-${i}-${groupNo}-${questionCode}`} className="grid grid-cols-12 gap-2 text-[14px]">
                  {!questionCode && (
                    <>
                      <div className="col-span-10 bg-slate-200 px-2 py-1 font-semibold">
                        <span>{groupNo}.</span> {title}
                      </div>
                      <div className="col-span-2 bg-slate-200 px-2 py-1 text-right font-semibold">{questionMarks}</div>
                    </>
                  )}
                  {questionCode && (
                    <>
                      ...
                      <div className="col-span-2 text-right">{individualMarks}</div>
                    </>
                  )}
