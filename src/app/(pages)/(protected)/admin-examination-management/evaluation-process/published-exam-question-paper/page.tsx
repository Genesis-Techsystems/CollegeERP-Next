      ...
      <div className="flex items-center gap-2">
        {hasPath && (
          <button
            ...
            onClick={() =>
              window.open(
                `${minio}${pickText(row, ["questionpaper_path"])}`,
                "_blank",
              )
            }
          >
        ...
        {!validSecret && (
          <Button
            ...
            disabled={loading}
            onClick={() => void onGeneratePassCode(row)}
          >
            Get PassCode
          </Button>
        )}
...
export default function PublishedExamQuestionPaperPage() {
  ...
  async function onGeneratePassCode(row: AnyRow) {
    ...
    try {
      await generateSecretCodeForPublishedQp({
        examQuestionPaperCollegeId: pickNum(row, [
          "pk_examquestionpaper_college_id",
        ]),
  ...
  async function onValidatePassCode() {
    ...
    try {
      const result = await validateSecretCodeForPublishedQp({
        ...
        examQuestionPaperCollegeId: pickNum(passcodeRow, [
          "pk_examquestionpaper_college_id",
        ]),
      ...
      if (returnedPath && returnedPath !== "Secret Code is Expired") {
        setRows((prev) =>
          prev.map((r) =>
            pickNum(r, ["pk_examquestionpaper_college_id"]) ===
            pickNum(passcodeRow, ["pk_examquestionpaper_college_id"])
  ...
  const cols = useMemo<ColDef[]>(
    () => [
      ...
      {
        field: "coursegroupcodes",
        headerName: "Course Group",
        minWidth: 110,
        maxWidth: 130,
        flex: 1,
        valueGetter: (p) =>
          p.data?.group_code ?? p.data?.coursegroupcodes ?? "-",
      },
      {
        ...
        flex: 1,
        valueGetter: (p) =>
          p.data?.course_year_code ?? p.data?.courseyearcode ?? "-",
      },
      {
      ...
      },
      {
        ...
        minWidth: 230,
        flex: 2.2,
        valueGetter: (p) => p.data?.questionpaper_title ?? "-",
      },
      {
        headerName: "Actions",
        minWidth: 230,
  ...
  return (
    <FilteredListPage
      title="Published Exam Question Paper"
      filters={(
    ...
    >
      <Dialog open={passcodeOpen} onOpenChange={setPasscodeOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          />
          <DialogFooter>
            <Button onClick={() => void onValidatePassCode()} disabled={loading}>
              Ok
            </Button>
            <Button variant="outline" onClick={() => setPasscodeOpen(false)} disabled={loading}>
              Cancel
            </Button>
          </DialogFooter>
        </DialogContent>
