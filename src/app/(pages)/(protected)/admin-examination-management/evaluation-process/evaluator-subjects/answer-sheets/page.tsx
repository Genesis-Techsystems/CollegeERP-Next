...
function matchesTab(row: StudentAnswerPaper, tab: TabKey): boolean {
  ...
  switch (tab) {
    case "all":
      return true;
    case "todo":
      return s === EVAL_STATUS.NEW || s === EVAL_STATUS.ASSIGNED;
    case "in_progress":
      return s === EVAL_STATUS.IN_PROGRESS;
    case "done":
      return s === EVAL_STATUS.EVALUATED || s === EVAL_STATUS.FINALIZED;
    case "rejected":
      return s === EVAL_STATUS.REJECTED || s === EVAL_STATUS.UFM;
    default:
      return true;
  }
}
...
function makeActionRenderer(navigate: (row: StudentAnswerPaper) => void) {
  return function actionRenderer(row: StudentAnswerPaper) {
    ...
    const isDone =
      row.evaluationStatusCatDetId === EVAL_STATUS.EVALUATED ||
      row.evaluationStatusCatDetId === EVAL_STATUS.FINALIZED;
    const isRejected =
      row.evaluationStatusCatDetId === EVAL_STATUS.REJECTED ||
      row.evaluationStatusCatDetId === EVAL_STATUS.UFM;
    ...
    if (actionable) {
      return (
        ...
        >
          {row.evaluationStatusCatDetId === EVAL_STATUS.IN_PROGRESS
            ? "Continue"
            : "Evaluate"}
...
export default function AnswerSheetsPage() {
  ...
  // Prefer explicit returnTo so Back goes to the page that opened this list
  // (My Subjects, Evaluation Dashboard, etc.).
  const returnTo = searchParams.get("returnTo") || DEFAULT_BACK_PATH;
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getStudentAnswerPapers(
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load answer papers.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const stats = useMemo(() => {
    ...
    const done = rawData.filter(
      (r) =>
        r.evaluationStatusCatDetId === EVAL_STATUS.EVALUATED ||
        r.evaluationStatusCatDetId === EVAL_STATUS.FINALIZED,
    ).length;
    const inProgress = rawData.filter(
      (r) => r.evaluationStatusCatDetId === EVAL_STATUS.IN_PROGRESS,
    ).length;
    const pending = rawData.filter(
      (r) =>
        r.evaluationStatusCatDetId === EVAL_STATUS.NEW ||
        r.evaluationStatusCatDetId === EVAL_STATUS.ASSIGNED,
    ).length;
    const rejected = rawData.filter(
      (r) =>
        r.evaluationStatusCatDetId === EVAL_STATUS.REJECTED ||
        r.evaluationStatusCatDetId === EVAL_STATUS.UFM,
    ).length;
  ...
  const columnDefs = useMemo<TableColumn<StudentAnswerPaper>[]>(
    () => [
      { id: "#", label: "#", width: 5, type: "id" },
      {
        ...
        label: "Serial No.",
        width: 30,
        render: serialRenderer,
      },
      {
        id: "evaluationStatusCatDetId",
        label: "Status",
        width: 20,
        render: statusRenderer,
      },
      {
        id: "evaluatedTotalMarks",
        label: "Marks",
        width: 15,
        render: marksRenderer,
      },
      {
        id: "actions",
        label: "",
        width: 15,
        render: makeActionRenderer(handleNavigate),
      },
    ],
    [handleNavigate],
  );
  ...
  const subtitle = [
    subjectCode,
    `${stats.total} paper${stats.total !== 1 ? "s" : ""} assigned`,
  ]
    .filter(Boolean)
    .join(" · ");
