...
const PdfCanvasViewer = forwardRef<PdfCanvasViewerHandle, PdfCanvasViewerProps>(
  function PdfCanvasViewer(
    ...
    const renderPage = useCallback(async (pageNum: number, zoom: number) => {
      ...
      const myGen = (genRef.current[pageNum] =
        (genRef.current[pageNum] ?? 0) + 1);
    ...
    useEffect(() => {
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          ...
          for (const e of entries) {
            ...
            if (
              e.isIntersecting &&
              (!best || e.intersectionRatio > best.ratio)
            ) {
      ...
      Object.entries(pageWrapRefs.current).forEach(([, el]) => {
    ...
    const goTo = useCallback(
      (p: number) => {
        ...
        pageWrapRefs.current[clamped]?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      },
    ...
    return (
      <div className="flex flex-col h-full">
        {/* ── Toolbar ── */}
        <div className="shrink-0 flex items-center gap-1 bg-white border-b border-border px-3 py-1.5 text-xs select-none">
          <button
            onClick={() =>
              setZoomMultiplier((z) =>
          ...
          <span className="min-w-[44px] text-center font-mono text-slate-600">
                ...
                <div className="relative inline-block shadow-xl">
                  <canvas
                    ...
                    onClick={(e) => {
                      ...
                      onCanvasClick(
                        pageNum,
                        canvasX / zoomMultiplier,
                        canvasY / zoomMultiplier,
                      );
                    }}
                  />
                        ...
                        return (
                          ...
                          >
                            <span
                              ...
                              style={{
                                fontSize: Math.max(11, Math.round(SW * 0.3)),
                                marginRight: 2,
                                marginLeft: -Math.round(SW * 0.9),
                              }}
                            >
                              {stamp.label}
                            </span>
                            ...
                            <span
                              ...
                              style={{
                                ...
                                fontSize: Math.max(16, Math.round(SW * 0.65)),
                              }}
                              aria-hidden
                            >
                              ✓
        ...
        {showThumbnails && numPages > 0 && (
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) onCloseThumbnails();
            }}
          >
                    ...
                    {Array.from({ length: numPages }, (_, i) => i + 1).map(
                      (pageNum) => (
                        ...
                        >
                          ...
                          />
                          <div className="bg-muted/40 text-center text-[11px] text-slate-500 py-1 border-t border-border">
                            Page {pageNum}
                          </div>
                        </button>
                      ),
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg mx-3 mt-3 shrink-0">
        <div className="text-xs font-semibold mb-1 text-center opacity-80">
        ...
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-card z-10 border-b border-border">
            <tr>
              ...
              <th className="px-3 py-2 text-center font-bold text-primary">
                Marks
              </th>
              <th className="px-3 py-2 text-center font-bold text-primary">
                Max
            ...
            {parts.map(([level, qs]) => (
              <React.Fragment key={`part-${level}`}>
                <tr>
                  <td
                    colSpan={3}
                    className="px-3 pt-3 pb-1 font-black text-[10px] text-foreground uppercase tracking-wider"
                  >
                    {partLabel(level)}
                  </td>
                </tr>
...
function NotAnsweredDialog({
  open,
  onOpenChange,
  questions,
  onMarkNotAnswered,
}: NotAnsweredDialogProps) {
  ...
  const toggleAll = () => {
    setSelected((prev) =>
      prev.size === questions.length
        ? new Set()
        : new Set(questions.map((q) => q.questionPaperMarksId)),
    );
  };
  ...
  const markedSelected = questions.filter(
    (q) =>
      selected.has(q.questionPaperMarksId) &&
      q.answeredMarks !== null &&
      !q.isNotAnswered,
  );
  ...
  const unevaluatedCount = questions.filter(
    (q) => q.no_action_yet === 1 && !q.isNotAnswered,
  ).length;
        ...
        {confirmStep ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                <span className="font-semibold">{markedSelected.length}</span>{" "}
                selected question(s) already have marks assigned. Marking as Not
                Answered will reset their marks to 0 and remove any stamps.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="gap-2 mt-4">
              <button
                onClick={() => setConfirmStep(false)}
                className="px-4 py-2 rounded border border-border text-sm font-medium hover:bg-muted transition-colors"
              >
                Go Back
              </button>
              <button
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <span className="font-semibold">0</span> marks. Questions with
                existing marks will require confirmation.
              </DialogDescription>
            </DialogHeader>
                ...
                return (
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        toggleQuestion(q.questionPaperMarksId)
                      }
                      onClick={(e) => e.stopPropagation()}
                    />
...
export default function MarkingPage() {
  ...
  const examEvaluationAssignmentId = Number(
    searchParams.get("examEvaluationAssignmentId") ?? 0,
  );
  const studentAnswerPaperId = Number(
    searchParams.get("studentAnswerPaperId") ?? 0,
  );
  const examEvaluatorProfileId = Number(
    searchParams.get("examEvaluatorProfileId") ?? 0,
  );
  const examEvaluatorProfileDetId = Number(
    searchParams.get("examEvaluatorProfileDetId") ?? 0,
  );
  ...
  const [assignmentDetail, setAssignmentDetail] =
    useState<EvalAssignmentDetail | null>(null);
  ...
  const totalMax =
    assignmentDetail?.qpTotalMarks ??
    questions.reduce((s, q) => s + (q.questionMarks ?? 0), 0);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const [qpResult, base64Result, intervalResult] =
        ...
        if (qpResult.status === "fulfilled") {
          ...
          setLoadError(
            qpResult.reason instanceof Error
              ? qpResult.reason.message
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (
        ...
        const idx = questions.findIndex(
          (q) => q.questionPaperMarksId === activeQId,
        );
        if (idx > 0) {
          setActiveQId(questions[idx - 1].questionPaperMarksId);
          setSelectedMark(null);
        }
        return;
      }
  ...
  const handleMarksChange = useCallback(
    (questionPaperMarksId: number, marks: number | null) => {
      setQuestions((prev) => {
        pushUndo(prev);
        return prev.map((q) =>
  ...
  const handleAddStamp = useCallback(
    (pageNum: number, x: number, y: number) => {
      ...
      setStamps((prev) => [
        ...prev.filter((s) => s.questionId !== activeQId),
        stamp,
      ]);
      ...
      const payload = buildMarkStampPayload(
        activeQuestion,
        pageNum,
        x,
        y,
      ...
      saveStudentEvalPages([payload])
        .then(() => {
          setHasUnsavedMarkFailure(false);
  ...
  const handleMarkNotAnswered = useCallback(
    (ids: number[]) => {
      ...
      const payloads: NotAnsweredPayload[] = affected.map(
        buildNotAnsweredPayload,
      );
  ...
  const handleSaveExit = useCallback(async () => {
    if (!window.confirm("Are you sure you want to exit the evaluation?"))
      return;
    if (locked) {
      navigateBack();
      return;
  ...
  const handleSubmit = useCallback(async () => {
    if (hasUnsavedMarkFailure) {
      toastError(
        "One or more marks failed to save. Please re-mark the highlighted question(s) before finishing.",
      );
      return;
    }
    if (pendingQuestions.length > 0) {
      alert(
        `Please evaluate the following question(s): ${pendingQuestions.map((q) => q.qvalue).join(", ")}`,
      );
      return;
    }
    if (
      !window.confirm(
        "Are you sure you want to complete the evaluation? This cannot be undone.",
      )
    )
      return;
    ...
    try {
      ...
      const filename =
        ...
        `evaluation-${examEvaluationAssignmentId}.pdf`;
    ...
    return (
      <div className="flex h-full items-center justify-center bg-muted/40">
        <div className="text-center space-y-4">
          <div className="h-10 w-10 animate-spin rounded-full border-4 border-border border-t-primary mx-auto" />
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex shrink-0 border-r border-border bg-card">
          ...
          <div className="w-28 border-r border-border flex flex-col">
            <div className="px-2 py-2 border-b border-border bg-muted/50">
              <h3 className="text-[10px] font-bold text-center text-muted-foreground uppercase tracking-wider">
                Questions
              </h3>
            </div>
          ...
          <div className="w-28 flex flex-col">
            <div className="px-2 py-2 border-b border-border bg-primary/10">
              <h3 className="text-[10px] font-bold text-center text-primary uppercase tracking-wider">
                Marks
              </h3>
            </div>
              ...
              <div className="shrink-0 border-t border-border p-2 bg-muted/20">
                <div className="grid grid-cols-2 gap-1.5 place-items-center">
                  ...
                  <button
                    ...
                    className="col-span-2 flex items-center justify-center gap-1.5 w-full h-8 rounded-lg border-2 bg-white border-slate-300 text-slate-600 text-[10px] font-medium hover:bg-muted/40 hover:border-slate-400 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-150 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="col-span-2 flex items-center justify-center gap-1 w-full h-8 rounded-lg border-2 bg-white border-slate-300 text-slate-500 text-[10px] font-medium hover:bg-muted/40"
                  >
                    Question Paper
                  </button>
                  */}
                  ...
                  <div className="col-span-2 flex items-center justify-between w-full mt-0.5 gap-1">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center w-6 h-6 rounded border bg-white border-slate-300 text-slate-500 hover:bg-muted/40 text-sm font-bold leading-none transition-colors"
                    >
                      −
                    </button>
                    <span className="text-[10px] font-mono text-slate-600 min-w-[24px] text-center">
                      {stampSize}
                    </span>
                    <button
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          ...
          {activeQuestion && (
            <div className="px-5 py-3 border-b border-border bg-card flex items-start gap-3 shrink-0">
            ...
            {pdfError ? (
              <div className="flex h-full flex-col items-center justify-center gap-3 bg-white text-slate-400 text-sm">
                ...
                <p className="text-xs text-slate-400">
                  Please evaluate using the sidebar.
                </p>
              </div>
            ) : pdfUrl ? (
              <PdfCanvasViewer
