...
export default function ViewAnswerSheetsPage() {
  ...
  const examOptions = useMemo(
    () =>
      exams.map((e) => ({
        ...
        label: `${txt(e.exam_name)} ${txt(e.exam_date)}`,
      })),
    [exams],
  );
  const examDateOptions = useMemo(
    () => examDates.map((d) => ({ value: d, label: formatDate(d) })),
    [examDates],
  );
  ...
  useEffect(() => {
    async function loadBase() {
      ...
      try {
        ...
        setBaseRows(
          Array.isArray(rows)
            ? rows.filter((r) => txt(r.flag) === "univ_exam_filters" || !r.flag)
            : [],
        );
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    async function loadRestForExam() {
      ...
      try {
        ...
        const rest =
          groups.find((g) => txt(g?.[0]?.flag) === "univ_exam_rest_filters") ??
          groups.flatMap((g) => g || []);
        setRestRows(rest);
      } finally {
        setLoading(false);
  ...
  const examName = txt(
    exams.find((e) => num(e.fk_exam_id) === num(examId))?.exam_name,
  );
  const totalStudents = filteredSummary.reduce(
    (acc, r) => acc + num(r.total_students),
    0,
  );
  const attendanceMarked = filteredSummary.reduce(
    (acc, r) => acc + num(r.attendance_marked),
    0,
  );
  const attendanceNotMarked = filteredSummary.reduce(
    (acc, r) => acc + num(r.attendance_not_marked),
    0,
  );
  const presented = filteredSummary.reduce(
    (acc, r) => acc + num(r.presented_Students),
    0,
  );
  const uploaded = filteredSummary.reduce(
    (acc, r) => acc + num(r.no_oof_answerpaper_uploaded),
    0,
  );
        ...
        <GlobalFilterBarRow>
          ...
          >
            <Button
              ...
              onClick={getList}
              disabled={loading || !examTimetableId}
            >
              Get List
            </Button>
          </GlobalFilterField>
      ...
      {showList && (
        <>
          <div className="app-card p-3 space-y-2">
            <h3 className="app-card-title border-b border-amber-200 pb-2">
              Summary List ({examName} / {formatDate(examDate || "1990-01-01")})
            </h3>
            ...
            <p className="text-[12px] font-medium">
              Total Students:{" "}
              <span className="text-red-600">{totalStudents}</span> | Attendance
              Marked: <span className="text-red-600">{attendanceMarked}</span> |
              Attendance Not Marked:{" "}
              <span className="text-red-600">{attendanceNotMarked}</span> |
              Presented Students:{" "}
              <span className="text-red-600">{presented}</span> | Answerpaper
              Uploaded: <span className="text-red-600">{uploaded}</span>
            </p>
              ...
              <table className="w-full text-[12px]">
                <thead className="bg-muted/40">
                  <tr>
                    ...
                    <th className="px-2 py-1 text-left">Invigilator</th>
                    <th className="px-2 py-1 text-left">Room</th>
                    <th className="px-2 py-1 text-left">Total Students</th>
                    <th className="px-2 py-1 text-left">Attendance Marked</th>
                    <th className="px-2 py-1 text-left">
                      Attendance Not Marked
                    </th>
                    <th className="px-2 py-1 text-left">Presented Students</th>
                    <th className="px-2 py-1 text-left">
                      No.of.Answerpaper Uploaded
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {filteredSummary.map((r, i) => (
                    <tr
                      key={`s-${i}-${txt(r.invigilator_emp_name)}-${txt(r.room_name)}`}
                      className="border-t"
                    >
                      <td className="px-2 py-1">{i + 1}</td>
                      <td className="px-2 py-1">
                        {txt(r.invigilator_emp_name)}
                      </td>
                      <td className="px-2 py-1">{txt(r.room_name)}</td>
                      <td className="px-2 py-1">{num(r.total_students)}</td>
                      <td className="px-2 py-1">{num(r.attendance_marked)}</td>
                      <td className="px-2 py-1">
                        {num(r.attendance_not_marked)}
                      </td>
                      <td className="px-2 py-1">{num(r.presented_Students)}</td>
                      <td className="px-2 py-1">
                        {num(r.no_oof_answerpaper_uploaded)}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="app-card p-3 space-y-2">
            <h3 className="app-card-title border-b border-amber-200 pb-2">
              Group Wise List ({examName} /{" "}
              {formatDate(examDate || "1990-01-01")})
            </h3>
            <div className="w-full max-w-sm">
              <Input
                placeholder="Search"
            ...
            <div className="overflow-auto rounded border max-h-[300px]">
              <table className="w-full text-[12px]">
                ...
                <tbody>
                  {filteredGroup.map((r, i) => (
                    <tr
                      key={`g-${i}-${txt(r.course_year_code)}-${txt(r.group_code)}`}
                      className="border-t"
                    >
                      <td className="px-2 py-1">{i + 1}</td>
                      <td className="px-2 py-1">{txt(r.course_year_code)}</td>
                      <td className="px-2 py-1">{txt(r.group_code)}</td>
                      <td className="px-2 py-1">{num(r.total_students)}</td>
                      <td className="px-2 py-1">{num(r.attendance_marked)}</td>
                      <td className="px-2 py-1">
                        {num(r.attendance_not_marked)}
          ...
          <div className="app-card p-3 space-y-2">
            <h3 className="app-card-title border-b border-amber-200 pb-2">
              Detail List ({examName} / {formatDate(examDate || "1990-01-01")})
            </h3>
            <div className="w-full max-w-sm">
              <Input
                placeholder="Search"
              ...
              <table className="w-full text-[12px]">
                <thead className="bg-muted/40">
                  <tr>
                    <th className="px-2 py-1 text-left">SI.No</th>
                    <th className="px-2 py-1 text-left">Course Year</th>
                    <th className="px-2 py-1 text-left">Hallticket Number</th>
                ...
                <tbody>
                  {filteredDetail.map((r, i) => (
                    <tr
                      key={`d-${i}-${txt(r.hallticket_number)}-${txt(r.subject_name)}-${txt(r.room_name)}`}
                      className="border-t"
                    >
                      <td className="px-2 py-1">{i + 1}</td>
                      <td className="px-2 py-1">{txt(r.course_year_code)}</td>
                      <td className="px-2 py-1">{txt(r.hallticket_number)}</td>
                      <td className="px-2 py-1">{txt(r.student_name)}</td>
                      <td className="px-2 py-1">{txt(r.subject_name)}</td>
                      <td className="px-2 py-1">
                        {txt(r.invigilator_emp_name)}
                      </td>
                      <td className="px-2 py-1">{txt(r.room_name)}</td>
                      <td className="px-2 py-1">{txt(r.is_present_lbl)}</td>
                      <td className="px-2 py-1">
                        {num(r.is_answerpaper_uploaded) === 1
                          ? "Completed"
                          : "Pending"}
                      </td>
                    </tr>
