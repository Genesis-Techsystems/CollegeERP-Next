...
const COL_DEFS = {
  ...
  group: {
    ...
    valueGetter: (p) =>
      txt(
        p.data?.group_code ?? p.data?.groupCode ?? p.data?.course_group_code,
      ) || "—",
  } as ColDef<Row>,
  courseYear: {
    ...
    valueGetter: (p) =>
      txt(
        p.data?.course_year_code ??
          p.data?.courseYearCode ??
          p.data?.course_year_name,
      ) || "—",
  } as ColDef<Row>,
  subject: {
    headerName: "Subject",
...
export default function ExamTimetableReportPage() {
  ...
  useEffect(() => {
    ...
    if (!ok) {
      setRegulationId(
        String(
          num(regulations[0].fk_regulation_id ?? regulations[0].regulationId),
        ),
      );
    }
  ...
  const getRowId = useCallback((p: { data?: Row }) => {
    ...
    if (!d) return "";
    const id = num(
      d.exam_timetable_id ?? d.fk_exam_timetable_id ?? d.examTimetableId,
    );
