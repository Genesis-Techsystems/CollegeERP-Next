...
import { printInvigilatorAllotmentReport } from "@/app/(pages)/(protected)/admin-examination-management/admin-exam-reports/_components/printInvigilatorAllotmentReport";
import {
...
const COL_DEFS = {
  ...
  examDate: {
    ...
    flex: 0,
    valueGetter: (p) =>
      parseMaybeDate(p.data?.exam_date ?? p.data?.examDate) || "—",
  } as ColDef<Row>,
  session: {
    headerName: "Session",
    minWidth: 110,
    ...
    valueGetter: (p) =>
      txt(
        p.data?.exam_session_name ??
          p.data?.session_name ??
          p.data?.examSessionName,
      ) || "—",
  } as ColDef<Row>,
  timings: {
    ...
    flex: 0,
    valueGetter: (p) => {
      const start = tConvert(
        p.data?.session_start_time ?? p.data?.sessionStartTime,
      );
  ...
  room: {
    ...
    valueGetter: (p) =>
      txt(
        p.data?.search_String ?? p.data?.search_string ?? p.data?.room_details,
      ) || "—",
  } as ColDef<Row>,
};
...
export default function ExamInvigilatorAllotmentReportPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        const univ = list.filter((r) => txt(r.flag) === "univ_exam_filters")
          .length
          ? list.filter(
              (r) =>
                txt(r.flag) === "univ_exam_filters" || num(r.fk_course_id) > 0,
            )
          : list;
        setBaseRows(univ);
  ...
  const courseOptions: SelectOption[] = useMemo(
    () =>
      courses.map((r) => ({
        ...
        label:
          txt(r.course_code ?? r.courseCode) || String(num(r.fk_course_id)),
      })),
    [courses],
  );
  ...
  const collegeOptions: SelectOption[] = useMemo(
    () =>
      colleges.map((r) => ({
        ...
        label:
          txt(r.college_code ?? r.collegeCode) || String(num(r.fk_college_id)),
      })),
    [colleges],
  );
  ...
  const getRowId = useCallback(
    (p: { data?: Row; node?: { rowIndex?: number | null } }) => {
      ...
      if (!d) return "";
      const id = num(
        d.exam_invigilation_allotment_id ?? d.fk_exam_invigilation_allotment_id,
      );
  ...
  function handlePrintReport() {
    ...
    printInvigilatorAllotmentReport({
      collegeName: txt(
