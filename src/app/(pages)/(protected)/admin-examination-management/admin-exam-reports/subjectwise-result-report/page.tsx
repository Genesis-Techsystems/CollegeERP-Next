...
function makeColDefs(isInternalExam: boolean): ColDef<Row>[] {
  return [
    ...
    },
    {
      ...
      flex: 1,
      valueGetter: (p) =>
        dash(p.data?.firstName ?? p.data?.student_name ?? p.data?.studentName ?? p.data?.Student),
    },
    {
...
export default function SubjectwiseResultReportPage() {
  ...
  const subjectsForType = useMemo(() => {
    const filtered = subjectTypeId
      ? subjectRows.filter((r) => num(r.fk_subjecttype_catdet_id) === Number(subjectTypeId))
      : subjectRows
    return dedupeBy(filtered, (r) => num(r.fk_subject_id))
  }, [subjectRows, subjectTypeId])

  const selectedSubject = useMemo(
    () => subjectsForType.find((r) => num(r.fk_subject_id) === Number(subjectId)) ?? null,
  ...
  const columnDefs = useMemo(() => makeColDefs(isInternalExam), [isInternalExam])

  const summary = useMemo(() => {
    const college =
      colleges.find((r) => num(r.fk_college_id) === Number(collegeId)) ?? null
    const group =
      courseGroups.find((r) => num(r.fk_course_group_id) === Number(courseGroupId)) ?? null
    const year =
      courseYears.find((r) => num(r.fk_course_year_id) === Number(courseYearId)) ?? null
    const course =
      courses.find((r) => num(r.fk_course_id) === Number(courseId)) ?? null
    const ay =
      academicYears.find((r) => num(r.fk_academic_year_id) === Number(academicYearId)) ?? null
    const regCode =
    ...
    const subjectDetails = selectedSubject
      ...
      : ''
    const subjectTypCode = txt(selectedSubject?.subject_type) ||
      txt(subjectTypes.find((r) => num(r.fk_subjecttype_catdet_id) === Number(subjectTypeId))?.subject_type)
    const examTypeCatCode = txt(
      selectedSubject?.ttd_exam_type ?? selectedSubject?.exam_type ?? selectedSubject?.examType,
    )
    return {
  ...
  useEffect(() => {
    ...
    if (
      courseGroupId === '0' ||
      courseGroups.some((r) => num(r.fk_course_group_id) === Number(courseGroupId))
    ) {
      return
    }
  ...
  useEffect(() => {
    ...
    if (
      regulationId === '0' ||
      opts.some((o) => o.value === regulationId)
    ) {
      return
    }
  ...
  useEffect(() => {
    ...
    if (!subjectTypes.some((r) => num(r.fk_subjecttype_catdet_id) === Number(subjectTypeId))) {
  ...
  useEffect(() => {
    ...
    if (!subjectsForType.some((r) => num(r.fk_subject_id) === Number(subjectId))) {
  ...
  useEffect(() => {
    ...
    if (!raw) return
    try {
      const d = /^\d{4}-\d{2}-\d{2}/.test(raw) ? parseISO(raw.slice(0, 10)) : new Date(raw)
    ...
    async function loadReport() {
      ...
      try {
        ...
        for (const r of Array.isArray(list) ? list : []) {
          const roll = txt(
            r.rollNumber ?? r.hallticketNumber ?? r.hallticket_number ?? r.hallticket_no,
          )
          ...
          deduped.push({
            ...r,
            marks: marks === null || marks === undefined || marks === '' ? 0 : marks,
          })
        }
        setRows(deduped)
      } catch (e) {
            ...
            <div className="space-y-1 md:col-span-2">
              ...
              <Select
                ...
                options={subjectTypes.map((r) => ({
                  ...
                  label: txt(r.subject_type) || String(num(r.fk_subjecttype_catdet_id)),
                }))}
            ...
            <>
              ...
              <div className="flex gap-4 rounded-md border border-border bg-muted/30 p-3">
                ...
                <div className="min-w-0 space-y-1 text-sm leading-snug">
                  <p>
                    {summary.exam || '—'}{' '}
                    <span className="text-blue-600">({summary.examDateLabel || '—'})</span>
                  </p>
                  <p className="text-muted-foreground">
                    {summary.collegeCode || '—'} / {summary.course || '—'} /{' '}
                    {summary.courseGroup || '—'} / {summary.courseyear || '—'}
                    {summary.academicYear ? (
                      <span className="text-blue-600"> ({summary.academicYear})</span>
                    ) : null}
                  </p>
                  <p className="font-semibold text-foreground">
                    ...
                    {summary.examTypeCatCode ? (
                      <span className="font-normal"> ({summary.examTypeCatCode})</span>
                    ) : null}
                  </p>
                </div>
              </div>
