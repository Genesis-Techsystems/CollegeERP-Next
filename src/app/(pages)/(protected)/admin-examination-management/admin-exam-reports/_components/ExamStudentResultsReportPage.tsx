...
const MODE_LABELS: Record<
  StudentResultsReportKind,
  { course: string; student: string }
> = {
  summary: { course: "Summary By Course", student: "Summary By Student" },
  details: {
    course: "Detailed Result By Course",
    student: "Detailed Result By Student",
  },
...
export function ExamStudentResultsReportPage({
  kind,
}: {
  kind: StudentResultsReportKind;
}) {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        if (courses[0] && mode === "course")
          setCourseId(String(num(courses[0].fk_course_id)));
      } catch (e) {
  ...
  const academicYears = useMemo(
    () =>
      dedupeBy(
        baseRows.filter(
          (r) => !courseId || num(r.fk_course_id) === Number(courseId),
        ),
  ...
  const allowsAllCascade =
    kind === "backlog" || kind === "details" || kind === "credits";
  ...
  function buildSummary(): string {
    ...
    const reg = regulations.find(
      (r) => num(r.fk_regulation_id ?? r.regulationId) === Number(regulationId),
    );
    return [
  ...
  async function onGetList() {
    ...
    try {
      const flag =
        kind === "details" ? "exam_std_result_detail" : "std_summary";
