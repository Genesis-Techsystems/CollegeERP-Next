...
function examFormRenderer(p: ICellRendererParams<Row>) {
  const path = txt(
    p.data?.application_file_path ?? p.data?.applicationFilePath,
  );
  if (!path) return "—";
  return (
    <Button
...
const COL_DEFS = {
  ...
  hallticket: {
    ...
    flex: 0,
    valueGetter: (p) =>
      txt(p.data?.hallticket_no ?? p.data?.hallticketNo) || "—",
  } as ColDef<Row>,
  studentName: {
  ...
  htIssued: {
    ...
    flex: 0,
    valueGetter: (p) =>
      yesNo(p.data?.is_hallticket_issued ?? p.data?.isHallticketIssued),
  } as ColDef<Row>,
};
        ...
        <>
          <div className="inv-allot-report-filters space-y-2">
            ...
            <div className="inv-allot-report-filters__row">
              ...
              <div className="inv-allot-report-filters__fx13 flex items-end gap-2 md:col-span-2">
                <Button
                  type="button"
                  onClick={() => void onGetList()}
                  disabled={loading || loadingFilters}
                  className="h-8 shrink-0 px-2.5 text-[12px] w-full"
                >
                  Get List
                </Button>
                <Button
