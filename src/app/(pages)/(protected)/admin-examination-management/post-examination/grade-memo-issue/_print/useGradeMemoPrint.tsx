...
function examTypeSuffix(courseCode: string, isRegular: boolean, isSupply: boolean): string {
  const isDPharm = courseCode === 'D.Pharm' || courseCode.toUpperCase() === 'DPHARM'
  if (isSupply && !isRegular) {
    return isDPharm ? 'Annual Examination Supplementary' : 'Make-Up/Supplementary SEE'
  }
  if (isRegular) {
...
function filteredGrades(grades: AnyRow[], regulationId: unknown): AnyRow[] {
  ...
  return grades.filter((g) => Number(g.fk_regulation_id ?? g.regulationId ?? 0) === rid)
}
...
const infoTable: CSSProperties = { width: '100%', borderCollapse: 'collapse', fontSize: 13 }
const infoTh: CSSProperties = { textAlign: 'left', fontWeight: 700, padding: '2px 0', width: '38%' }
const infoTd: CSSProperties = { textAlign: 'left', padding: '2px 0', textTransform: 'uppercase' }
...
const photoWrap: CSSProperties = { width: 110, textAlign: 'center', flexShrink: 0 }
const photo: CSSProperties = { height: 96, width: 100, objectFit: 'cover', border: '1px solid #000' }
const usn: CSSProperties = { fontSize: 12, fontWeight: 700, displayBlock: 4 }
...
const note: CSSProperties = { fontSize: 11, textAlign: 'justify', margin: '2px 0' }
        ...
        <table style={infoTable}>
          <tbody>
            <tr>
              ...
              <td style={{ padding: '0 3px', textAlign: 'center', width: 12 }}>:</td>
            ...
            <tr>
              ...
              <td style={{ padding: '0 3px', textAlign: 'center' }}>:</td>
              <td style={{ ...infoTd, textTransform: 'none' }}>{txt(row.clg_name)}</td>
            </tr>
            <tr>
...
function GradeCardSubjects({ rows, padTo }: { rows: AnyRow[]; padTo?: number }) {
      ...
      <tbody>
        {rows.map((s, i) => (
          <tr key={`${txt(s.subject_code)}-${i}`}>
            ...
            <td style={gridCell}>{txt(s.subject_type) === 'LAB' ? 'Pr.' : 'Th.'}</td>
...
function SgpaSummaryFromList({ rows }: { rows: AnyRow[] }) {
  const head = rows[0] ?? {}
  const creditsReg = head.total_credits_registered ?? sumField(rows, 'credits_registered')
    ...
    <table style={{ ...gridTable, marginTop: 10 }}>
      ...
      <tbody>
        <tr>
          ...
          <td style={gridCell}>
            {head.sgpa != null ? txt(head.sgpa) : <span style={{ color: 'red', fontWeight: 700 }}>-</span>}
          </td>
          <td style={gridCell}>
            {head.cgpa != null ? txt(head.cgpa) : <span style={{ color: 'red', fontWeight: 700 }}>-</span>}
          </td>
        </tr>
      </tbody>
    </table>
  ...
  return (
    <div style={{ padding: '5px 0' }}>
      <p style={{ margin: 0, textAlign: 'left' }}>
        Medium of Instruction : <span style={{ fontWeight: 500 }}>{medium}</span>
      </p>
      ...
      {withNote && (
        <p style={note}>
          Note :{' '}
          <span>
            Sharnbasva University is not responsible for any inadvertent error that may have crept into
            results being published on internet or given in provisional result sheet. The results published
            on internet / provisional result sheet are for immediate reference to the examinees,{' '}
            <u>which shall not be the final grade card</u>.
          </span>
        </p>
      )}
      {withSignature && (
...
function GradeScaleTable({ grades, regulationId }: { grades: AnyRow[]; regulationId: unknown }) {
...
function MarkSheetSubjects({ rows, dataFlag }: { rows: AnyRow[]; dataFlag: boolean }) {
  const head = rows[0] ?? {}
  return (
    <table style={gridTable}>
      <thead>
        <tr>
          ...
          <th style={gridHead}>{dataFlag ? 'Course Title' : 'Subject Title'}</th>
          ...
          <th style={gridHead}>{dataFlag ? 'Marks Obtained' : 'Total Marks Obtained'}</th>
          {dataFlag && (
            <th style={gridHead}>
              Credits Earned
              <br />
...
function Banner({ show }: { show: boolean }) {
  ...
  return (
    // eslint-disable-next-line @next/next/no-img-element
    <img src={BANNER} alt="" style={{ width: '100%', height: 'auto', display: 'block' }} />
  )
}
  ...
  return (
    <div style={{ ...page, position: 'relative' }}>
      <div style={sample ? { marginTop: 170 } : undefined}>
        {!sample && <Banner show />}
        <p style={sample ? titleBlack : { ...titleRed, marginTop: sample ? 0 : -10 }}>GRADE CARD</p>
        <hr />
        <ExamSubtitle row={head} isRegular={isRegular} isSupply={isSupply} sample={sample} />
...
function MarkSheetPage({
  ...
  return (
    <div style={{ ...page, position: 'relative' }}>
      ...
      {dataFlag && <GradeScaleTable grades={grades} regulationId={head.fk_regulation_id} />}
