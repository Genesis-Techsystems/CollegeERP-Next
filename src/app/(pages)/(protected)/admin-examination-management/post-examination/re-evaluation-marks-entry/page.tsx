...
function passBadge(value: boolean | null) {
  if (value === null)
    return <span className="text-muted-foreground">Not Posted</span>;
...
export default function ReEvaluationMarksEntryPage() {
  ...
  const [examTimetableDetId, setExamTimetableDetId] = useState<number | null>(
    null,
  );
  ...
  const academicYears = useMemo(
    () =>
      dedupeBy(
        allFilters.filter(
          (r) => numFrom(r, ["fk_college_id"]) === Number(collegeId),
        ),
  ...
  useEffect(() => {
    async function init() {
      setLoading(true);
      try {
        const rows = await getReevaluationMarksFilters({
          organizationId,
          employeeId,
        });
  ...
  useEffect(() => {
    if (academicYears[0])
      setAcademicYearId(numFrom(academicYears[0], ["fk_academic_year_id"]));
  }, [academicYears]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (courseGroups[0])
      setCourseGroupId(numFrom(courseGroups[0], ["fk_course_group_id"]));
  }, [courseGroups]);
  useEffect(() => {
    if (courseYears[0])
      setCourseYearId(numFrom(courseYears[0], ["fk_course_year_id"]));
  }, [courseYears]);
  useEffect(() => {
    if (subjectTypes[0])
      setSubjectTypeId(numFrom(subjectTypes[0], ["fk_subjecttype_catdet_id"]));
  }, [subjectTypes]);
  useEffect(() => {
    if (subjects[0])
      setExamTimetableDetId(numFrom(subjects[0], ["fk_exam_timetable_det_id"]));
  }, [subjects]);
  ...
  const selectedSubject = useMemo(
    () =>
      subjects.find(
        (r) =>
          numFrom(r, ["fk_exam_timetable_det_id"]) ===
          Number(examTimetableDetId),
      ) ?? null,
  ...
  async function getList() {
    ...
    setLoading(true);
    try {
      const bundle = await getExamRevisionMarksBundle({
        examId: selectedExamId,
        subjectId: selectedSubjectId,
      });
      const setup = Array.isArray(bundle.marksSetupRows)
        ? bundle.marksSetupRows
        : [];
      const students = Array.isArray(bundle.studentRows)
        ? bundle.studentRows
        : [];
      ...
      toolbarLeading={
        studentRows.length > 0 ? (
          <div className="text-[12px] text-slate-600 whitespace-nowrap shrink-0">
            Entered marks should be less than:{" "}
            <span className="font-semibold">{maxMarks || "-"}</span>
          </div>
        ) : undefined
      }
      toolbarTrailing={
