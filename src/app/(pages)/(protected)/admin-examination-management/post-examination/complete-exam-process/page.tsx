...
function buildYearOptions(rows: AnyRow[], courseId: number | null): SelectOption[] {
  if (!courseId) return []
  return dedupeBy(
  ...
  return dedupeBy(
    rows.filter(
      (x) =>
        ...
        numFrom(x, ['fk_academic_year_id', 'academicYearId']) === academicYearId,
    ),
    ['fk_exam_id', 'examId'],
  )
    .map((x) => {
    ...
    <div className="app-data-table app-data-table-card flex flex-col">
      ...
      >
        <div className={cn('min-h-0', open ? 'overflow-visible' : 'overflow-hidden')}>
          ...
          <div className={cn('px-5 py-4', hasFilters && 'border-t border-border')}>{body}</div>
        </div>
      </div>
    </div>
  )
...
function ActionCard({
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-border p-3">
      ...
      <Button className="h-8 shrink-0 text-[12px]" onClick={onClick} disabled={disabled}>
        {button}
      </Button>
    </div>
  )
  ...
  return (
    <GlobalFilterBarRow>
      ...
      <GlobalFilterField label="Exam Year *">
        <CommonSelect
          value={selection.academicYearId ? String(selection.academicYearId) : null}
...
function useCardFilters(filterRows: AnyRow[]) {
  ...
  const courseOptions = useMemo(() => buildCourseOptions(filterRows), [filterRows])
  const yearOptions = useMemo(
    () => buildYearOptions(filterRows, selection.courseId),
    [filterRows, selection.courseId],
  )
  const examOptions = useMemo(
    () => buildExamOptions(filterRows, selection.courseId, selection.academicYearId),
  ...
  return {
    ...
    setExamId: (examId: number | null) => setSelection((prev) => ({ ...prev, examId })),
  }
}
...
export default function CompleteExamProcessPage() {
  ...
  function requireExam(examId: number | null): examId is number {
        ...
        body={
          <div className="space-y-3">
            <ActionCard
              ...
              onClick={() =>
                confirmAction(
                  processFilters.selection.examId,
                  'Setup Assignments?',
                  () =>
                    runCompleteExamSetupAssignments(processFilters.selection.examId as number),
            ...
            <ActionCard
              ...
              disabled={loading}
              onClick={() =>
                router.push('/admin-examination-management/post-examination/verify-exam-marks')
              }
            />
          </div>
        }
        ...
        body={
          <div className="space-y-3">
            <ActionCard
              ...
              onClick={() =>
                confirmAction(
                  reEvalFilters.selection.examId,
        ...
        body={
          <div className="flex flex-wrap justify-end gap-2">
            <Button
              ...
              disabled={loading}
              onClick={() => {
                if (!requireExam(resultFilters.selection.examId)) return
                void runAction(
                  () =>
                    runCompleteExamResultProcessing(resultFilters.selection.examId as number),
