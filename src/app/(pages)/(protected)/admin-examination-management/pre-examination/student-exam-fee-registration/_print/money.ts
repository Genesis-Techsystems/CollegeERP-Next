...
/** Angular exam-form SemisterList match only — blank when code is unknown. */
export function examFormSemesterLabel(courseYearCode: unknown): string {
  const code = String(courseYearCode ?? "").trim();
  return SEMESTER_LABELS.find((x) => x.id === code)?.value ?? "";
}
