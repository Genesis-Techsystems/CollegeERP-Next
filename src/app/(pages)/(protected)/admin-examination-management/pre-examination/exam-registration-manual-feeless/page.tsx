...
const subId = (s: AnyRow) => Number(s?.subjectId ?? s?.fk_subject_id ?? s?.subject_id ?? 0)
...
export default function ExamRegistrationManualFeelessPage() {
  ...
  const studentRegSubject = useMemo(
    () => regSubjects.map((s) => `${g(s, ['shortName', 'subjectCode'])} - ${g(s, ['subjectCode'])}`).join(', '),
    [regSubjects],
  )
  ...
  function supplyCourseYears(type: 1 | 2, eid: number) {
    ...
    if (type === 1) {
      const cys = cyList.filter((x) => Number(x.fromCourseYearId) === Number(stu.courseYearId))
      setCourseYears(cys)
      if (cys.length > 0) {
  ...
  // ===== SUBJECTS =====
  function normalize(rows: AnyRow[], cyId: number, examType: 'Regular' | 'Supple'): AnyRow[] {
    return rows.map((r) => ({
      ...r,
      subjectId: subId(r),
      courseYearId: cyId,
  ...
  async function getStudentSubjects(cyId: number, eid: number, failAbsentOnly: boolean) {
    const stu = studentRef.current
    let rows: AnyRow[] = []
    ...
    if (Number(stu.courseYearId) === Number(cyId)) {
      ...
      examType = 'Supple'
      const cyRow = courseYearsListRef.current.find((x) => Number(x.fromCourseYearId) === Number(cyId))
      ...
      if (failAbsentOnly) rows = rows.filter((r) => r.examresultCatCode === 'FAIL' || r.examresultCatCode === 'ABSENT')
      void cyRow
    }
  ...
  function getRelevantExamSubjects(cyId: number) {
    if (cyId == null) return
    void getStudentSubjects(cyId, Number(examIdRef.current), Number(studentRef.current.courseYearId) !== Number(cyId))
  }
  ...
  function checkedSubjects(check: boolean, item: AnyRow) {
    setStudentSubjects((prev) =>
      prev.map((s) => (subId(s) === subId(item) && s.courseYearId === item.courseYearId ? { ...s, checked: check, isSelected: check } : s)),
    )
  }
  ...
  function addExamSubjects() {
    ...
    for (const sub of checked) {
      ...
      } else if (!existing.subjects.some((x: AnyRow) => subId(x) === subId(sub))) {
  ...
  async function confirmPay() {
    ...
    try {
      ...
      if (courseYearIdRef.current) await getStudentSubjects(Number(courseYearIdRef.current), Number(examIdRef.current), false)
    } catch (e) {
      toastError(e instanceof Error ? e.message : 'Failed to register subjects.')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function submitUpload() {
    ...
    try {
      ...
      toastError(e instanceof Error ? e.message : 'Failed to upload exam form.')
    } finally {
      setUploading(false)
    }
  }
  ...
  async function viewExamForm() {
    ...
    const path = g(row, ['applicationFilePath', 'application_file_path', 'filePath', 'file_path'])
    if (!path) {
  ...
  return (
    <PageContainer className="space-y-3">
      <div className="text-[13px] text-muted-foreground">
        ...
        <span className="text-foreground"> Exam Registration Manual Feeless</span>
      </div>

      <div className="rounded border bg-white shadow-sm">
        <div className="flex items-center gap-2 border-b bg-[#eaf2ff] px-4 py-2">
          <span className="material-icons text-[18px]">📝</span>
          <span className="text-[14px] font-semibold">Exam Registration Manual Feeless</span>
        </div>
        ...
        <div className="p-4 space-y-3">
              ...
              <div className="flex gap-4">
                <div className="w-[120px] shrink-0">
                  {student.studentPhotoPath && !photoError ? (
                    ...
                    <div className="flex w-full items-center justify-center bg-[#c3d9ff] p-1.5 text-[28px] font-semibold text-white" style={{ height: 110 }}>
          ...
          {studentId && flag && (
            <div className="rounded border-2 border-[#89c5ff] p-2.5">
              <h2 className="mb-2 rounded bg-[#c3d9ff] px-3 py-1.5 text-[15px] font-medium">Select Exam Fee Subjects</h2>

              <div className="bg-white px-2 py-2">
                <div className="flex items-center gap-8 text-[13px]">
                  <label className="flex items-center gap-2">
                    <input type="radio" name="checkExam" checked={checkExam === 1} onChange={() => onChangeCheckExam(1)} /> Regular
                  </label>
                  <label className="flex items-center gap-2">
                    <input type="radio" name="checkExam" checked={checkExam === 2} onChange={() => onChangeCheckExam(2)} /> Supplementary
                  </label>
                </div>
              </div>
                  ...
                  <div className="md:col-span-2 bg-white p-2">
                    ...
                    {courseYearId && checkExam === 2 && (
                      <div className="mt-2 flex gap-4">
                        <span className="cursor-pointer text-[13px] text-blue-600 underline" onClick={() => void getStudentSubjects(Number(courseYearId), Number(examIdRef.current), false)}>
                          All
                        </span>
                        <span className="cursor-pointer text-[13px] text-blue-600 underline" onClick={() => getRelevantExamSubjects(Number(courseYearId))}>
                          Supple
                        </span>
                      </div>
                    )}
                ...
                {studentSubjects.length > 0 && (
                  <div className="md:col-span-6 border border-[#dedede] bg-white">
                    <div className="flex items-center justify-between gap-2 p-1.5">
                      <Input className="h-7 w-full max-w-sm text-[12px]" placeholder="Search subject…" value={searchText} onChange={(e) => setSearchText(e.target.value)} />
                      <span className="text-[13px] font-medium text-blue-600">Subjects: {selectedCount}</span>
                    </div>
                      ...
                      <table className="w-full text-[12px]">
                        <thead className="sticky top-0">
                          <tr className="bg-[#C3D9FF]">
                            <th className="w-[44px] px-2 py-1 text-left">
                              <input type="checkbox" checked={checksubject} onChange={(e) => markAll(e.target.checked)} />
                        ...
                        <tbody>
                          {filteredSubjects.map((obj, i) => (
                            <tr key={`sub-${subId(obj) || i}`} className={`border-t ${obj.subjAlreadyRegistered ? 'bg-[#f2f0f0]' : ''}`}>
                  ...
                  <div className="md:col-span-4 border border-[#dedede] bg-white">
                    <div className="max-h-[260px] overflow-y-auto">
                      <table className="w-full text-[12px]">
                        <thead className="sticky top-0">
                          <tr className="bg-[#C3D9FF]">
                            <th className="px-2 py-1 text-left text-blue-700">Selected Subjects : {selectedCount}</th>
                          </tr>
                        </thead>
                        <tbody>
          ...
          {courseYearFee.length > 0 && (
            <div className="mx-1">
              <h2 className="mb-1 rounded bg-[#c3d9ff] px-3 py-1.5 text-[15px] font-medium">Exam Registration</h2>
                ...
                <table className="w-full bg-white text-[13px]">
                  <thead>
                    <tr className="bg-white">
                      ...
                      <th className="border-b-4 border-[#c3d9ff] px-2 py-1 text-left">Course Year</th>
                      ...
                      <th className="border-b-4 border-[#c3d9ff] px-2 py-1 text-center">View Subjects</th>
                      <th className="border-b-4 border-[#c3d9ff] px-2 py-1 text-center">Exam Form</th>
                    ...
                    {courseYearFee.map((cy, i) => (
                      <tr key={`cy-${i}`} className={i % 2 ? 'bg-[#f1f6ff]' : 'bg-white'}>
                        ...
                        <td className="px-2 py-1 text-center">
                          <button className="text-[#9E9E9E]" title="View Subjects" onClick={() => viewCourseYearSubjects(cy)}>
                            👁
                          </button>
                        </td>
                        <td className="px-2 py-1 text-center">
                          <Button variant="outline" className="h-7 text-[12px]" onClick={() => void viewExamForm()}>
                            View
                          </Button>
                        </td>
                        <td className="px-2 py-1 text-center">
                          <Button className="h-7 text-[12px]" onClick={onRegister} disabled={saving}>
                            Register
                          </Button>
                        </td>
                      </tr>
          ...
          {regSubjects.length > 0 && (
            <div className="mx-1">
              <h2 className="mb-1 rounded bg-[#c3d9ff] px-3 py-1.5 text-[15px] font-medium">Registrated Subjects</h2>
              <div className="rounded bg-[#C3D9FF] p-1">
                <table className="w-full bg-white text-[13px]">
                  <thead>
                    <tr className="bg-white">
                      <th className="border-b-4 border-[#c3d9ff] px-2 py-1 text-left">SI No</th>
                      <th className="border-b-4 border-[#c3d9ff] px-2 py-1 text-left">Course Year</th>
                      <th className="border-b-4 border-[#c3d9ff] px-2 py-1 text-left">Subject</th>
                      <th className="border-b-4 border-[#c3d9ff] px-2 py-1 text-center">Exam Form</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="bg-white">
                      ...
                      <td className="px-2 py-1">{g(regSubjects[0], ['courseYearCode', 'course_year_code', 'courseYearName'])}</td>
                      ...
                      <td className="px-2 py-1 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          <Button variant="outline" className="h-7 text-[12px]" onClick={openUpload}>
                            Upload
                          </Button>
                        </div>
                      </td>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-2 text-[13px]">
            ...
            <div>
              ...
              {g(examsList.find((e) => Number(e.examId) === Number(examId)), ['examName', 'exam_name'])}
            </div>
                ...
                <tbody>
                  {payReceiptsRef.current.map((r, i) => (
                    <tr key={`pr-${i}`} className="border-t">
                      ...
                      <td className="px-2 py-1 text-right">{(r.examStudentDetailDTOs ?? []).length}</td>
            ...
            <div className="space-y-1">
              <label className="text-[12px] text-muted-foreground">Upload Exam Form (optional)</label>
              <Input type="file" className="text-[12px]" onChange={(e) => setPayFile(e.target.files?.[0] ?? null)} />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-2">
            <Input type="file" className="text-[12px]" onChange={(e) => setUploadFile2(e.target.files?.[0] ?? null)} />
          </div>
          <DialogFooter>
              ...
              <tbody>
                {viewSubjRows.map((s, i) => (
                  <tr key={`vs-${i}`} className="border-t">
                    <td className="px-2 py-1">{i + 1}</td>
                    <td className="px-2 py-1">{g(s, ['subjectCode', 'subject_code']) || '-'}</td>
                    <td className="px-2 py-1">{g(s, ['subjectName', 'subject_name', 'shortName']) || '-'}</td>
                  </tr>
                ))}
