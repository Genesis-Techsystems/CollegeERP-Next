...
import {
  useSchedulingFormsPrint,
  type PrintAllocationRow,
} from "./_print/useSchedulingFormsPrint";
  ...
  const picked = rows
    .map((r) => ({
      examTimetableId: Number(
        r.examTimetableId ?? r.fk_exam_timetable_id ?? r.exam_timetable_id ?? 0,
      ),
      examDate: r.examDate ?? r.exam_date ?? "",
      examSessionName:
        r.examSessionName ?? r.exam_session_name ?? r.session_name ?? "",
    }))
...
export default function ExamSchedulingFormsPage() {
  ...
  const courses = useMemo(
    () => dedupeBy(baseRows, (r) => Number(r.fk_course_id)),
    [baseRows],
  );
  ...
  const ymdOf = (v: unknown) =>
    String(v ?? "").match(/\d{4}-\d{2}-\d{2}/)?.[0] ?? "";
  const printExamDate =
  ...
  const printCourseLabel = String(
    courses.find((c) => Number(c.fk_course_id) === Number(courseId))
      ?.course_code ?? "",
  ).trim();
  const printAyLabel = String(
    academicYears.find(
      (a) => Number(a.fk_academic_year_id) === Number(academicYearId),
    )?.academic_year ?? "",
  ).trim();
  const printHeaderSubtitle = [printCourseLabel, printAyLabel]
    .filter(Boolean)
    .join(" / ");
  ...
  const printAllocationRows = useMemo<PrintAllocationRow[]>(
    () =>
      filteredRows.map((r) => ({
        ...
        roomCode:
          [r.buildingCode, r.blockCode, r.floorName, r.roomCode]
            .filter(Boolean)
            .join(" / ") || "-",
  ...
  const { printMode, loadingOverlay, printButtons, printView } =
    useSchedulingFormsPrint({
      courseId,
      examId,
      examTimetableId,
  ...
  async function onExamChange(nextExamId: number) {
    ...
    try {
      ...
      if ((!ttList || ttList.length === 0) && firstCourseYearId) {
        const details = await getExamTimetableDetails(
          firstCourseYearId,
          courseId,
          nextExamId,
        ).catch(() => []);
      ...
      setTimetables(
        ttList.length > 0 ? ttList : mapTimetablesFromRestRows(rest),
      );
      setCollegeId(null);
  ...
  async function getList() {
    ...
    setLoading(true);
    try {
      const rows = await listExamRoomAllotments(
        collegeId,
        examId,
        examTimetableId,
      ).catch(() => []);
  ...
  function openSeatAllotStudents(row: AnyRow) {
    ...
    const courseLabel =
      String(
        courseRow?.courseCode ??
          ...
          "",
      ).trim() || String(courseRow?.fk_course_id ?? "");
    const params = new URLSearchParams({
      ...
      roomCode: String(
        [row.buildingCode, row.blockCode, row.floorName, row.roomCode]
          .filter(Boolean)
          .join(" / "),
      ),
    ...
    router.push(
      `/admin-examination-management/pre-examination/exam-scheduling-forms/add-exam-scheduling-forms?${params.toString()}`,
    );
  }
            ...
            <GlobalFilterField label="Exam Timetable">
              <Select
                ...
                options={timetables.map((t) => ({
                  ...
                  label: `${toDateStr(t.examDate)} (${t.examSessionName ?? "-"})`,
                }))}
                ...
                {filteredRows.map((row, i) => (
                  <tr key={`rs-${i}`} className="border-t">
                    ...
                    <td className="px-2 py-1">
                      {toDateStr(row.examDate) || "-"}
                    </td>
                    <td className="px-2 py-1">
                      {row.examSessionName ?? "-"} (
                      {toTime12H(row.sessionStartTime)} -{" "}
                      {toTime12H(row.sessionEndTime)})
                    </td>
                    <td className="px-2 py-1">
                      {[
                        row.buildingCode,
