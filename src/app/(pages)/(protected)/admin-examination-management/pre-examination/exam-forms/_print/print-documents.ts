/**
 * Exam Forms print HTML builders (Angular print-form / print-formA / print-Dform).
 * Screen preview uses dedicated route pages; Print button uses iframe isolation.
 */
...
export function buildFormADocument(
  students: AnyRow[],
...
export function buildDFormDocument(
  students: AnyRow[],
  meta: ExamFormsPrintMeta,
): string {
  const head = students[0] ?? {};
...
export function buildExamFormDocument(
  students: AnyRow[],
  meta: ExamFormsPrintMeta,
): string {
  const head = students[0] ?? {};
...
/** Extract inner layout HTML for on-screen HTML preview pages. */
export function extractPrintLayoutBody(fullDocument: string): string {
  const match = fullDocument.match(
    /<div class="layout">([\s\S]*?)<\/div>\s*<\/body>/i,
  );
  return match?.[1]?.trim() ?? "";
}

export const EXAM_FORMS_PRINT_CSS = PRINT_CSS;
