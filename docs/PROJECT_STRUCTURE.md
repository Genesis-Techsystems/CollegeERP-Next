...
| File | What it holds |
|---|---|
| `api.ts` | Spring Boot endpoint paths — `AUTH_API`, `EXAM_API`, `EXAM_ONLINE_API`, `EXAM_EVAL_API`, `ORG_API`, `NEXT_API`, `MINIO_URL` |
...
| `generic-functions.ts` | Session storage helpers (`setSecuredValue`, `getSecuredValue`, etc.) + `isEmptyObject` |
...
| `Sidebar.tsx` | Navigation sidebar — renders NavItem tree, debug filters, collapse animations |
| `Topbar.tsx` | Header bar — collapsible live search, notifications stub, user avatar dropdown |
| `NavItem.tsx` | Individual sidebar item — icon, label, collapsible children, active state |
...
| `query-keys.ts` | `QK` — TanStack Query key factory for all entities (prevents string typos) |
...
| File | What it does |
|---|---|
| `crud.ts` | Generic HTTP client — `domainList`, `domainCreate`, `domainUpdate`, `domainSoftDelete`, `getAllRecords`, `uploadFile`. Parses Spring Boot `{ statusCode, success, data }` envelopes. |
...
| File | What it does |
|---|---|
| `navigation-store.ts` | Sidebar state — `navItems`, collapsed items, `isSidebarOpen`, `isSidebarCollapsed`, `autoCollapse`, `sidebarPosition` |
