...
@Component({
  selector: 'app-patym-redirect',
  templateUrl: './patym-redirect.component.html',
  styleUrls: ['./patym-redirect.component.scss']
})
export class PatymRedirectComponent implements OnInit {
  ...
  pay(item): void{
     ...
     this.crudService.upload(this.pgRedirectUrl, this.formData)
     .subscribe(result => {
         this.spinner.hide();
         if (result.statusCode === 200){
             if (result.success) {
               ...
               //  item.ORDER_ID = 'ORDER123';
                 item.CALLBACK_URL = 'http://localhost:9090/cms/pgpaytmresponse';
                 this.patym = item;
                 this.flag = true;
                 console.log(item);
