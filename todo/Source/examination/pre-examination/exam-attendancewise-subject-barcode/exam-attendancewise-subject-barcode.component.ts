...
@Component({
  selector: 'app-exam-attendancewise-subject-barcode',
  templateUrl: './exam-attendancewise-subject-barcode.component.html',
  styleUrls: ['./exam-attendancewise-subject-barcode.component.scss']
})
export class ExamAttendancewiseSubjectBarcodeComponent implements OnInit {
 ...
 getFiltersList(): void {
  ...
  this.courseYear = this.courseYears.filter(x=>x.fk_course_year_id == this.examFeeCollectionForm.value.courseYearId)[0]?.course_year_name
...
getDetails(): void{ 
    if (this.examFeeCollectionForm.valid){
        ...
        this.selectedData = this.selectedData + ' / ' + this.courseYears.filter(x => (x.fk_course_year_id === this.examFeeCollectionForm.value.courseYearId))[0]?.course_year_code;
        this.selectedData = this.selectedData + ' / ' + this.subjectsList.filter(x => (x.fk_subject_id === this.examFeeCollectionForm.value.subjectId))[0].subject_name;
                   ...
                   if (result.success) {
                       if (result.data.result[0].length > 0){
                            ...
                            for (let i = 0; i < result.data.result[0].length; i++){
                              if( result.data.result[0][i].is_present==true){
 ...
 isEmptyObject(obj) {
   ...
   generateBarcode(): void{
    ...
    this.studentdata=[]
    for(let i=0;i<this.subjectModerationStudents.length;i++){
      this.studentdata.push(this.subjectModerationStudents[i].examStdDetId)
    }
...
omrpage(){
    JSON.stringify(this.subjectModerationStudents)
    this.router.navigate(['admin-examination-management/admin-pre-examinations/omr-sheets-design'],
    {queryParams: {data:JSON.stringify(this.subjectModerationStudents),
  ...
  omrSinglePage(){
    JSON.stringify(this.subjectModerationStudents)
    this.router.navigate(['admin-examination-management/admin-pre-examinations/omr-single-page-design'],
    {queryParams: {data:JSON.stringify(this.subjectModerationStudents),
        CollegeName: this.CollegeName,
        ExamName:this.examname,
        collegeId:this.examFeeCollectionForm.value.collegeId,
  ...
  Stickers(){
    JSON.stringify(this.subjectModerationStudents)
    this.router.navigate(['admin-examination-management/admin-pre-examinations/print-barcode-stickers'],
    {queryParams: {data:JSON.stringify(this.subjectModerationStudents),
        collegeCode: this.collegeCode,
        academicyear:  this.academicyear,
        courseCode:  this.courseCode,
        courseGroupCode:  this.courseGroupCode,
...
Print(studentAtt){    
    ...
    {queryParams:{
        ...
        hallticketno:studentAtt.hallticket_number,
        omrbarcode:studentAtt.omr_barcode,
        omrserialno:studentAtt.omr_serial_no,
        CollegeName: this.CollegeName,
        ExamName:this.examname,
        fathername:studentAtt.father_name,
        caste:studentAtt.caste,
        DOB:studentAtt.date_of_birth,
        gender:studentAtt.gender,
        aadharNumber:studentAtt.aadhar_card_no,
        examName:studentAtt.exam_name,
        examCenter:studentAtt.examcenter,
        roomNumber:studentAtt.room_number,
        subjectCode:studentAtt.subject_code,
        subjectName:studentAtt.subject_name,
        examDate:studentAtt.exam_date,
        collegeId:this.examFeeCollectionForm.value.collegeId,
        academicYearId:this.examFeeCollectionForm.value.academicYearId,
        courseId:this.examFeeCollectionForm.value.courseId,
        courseGroupId:this.examFeeCollectionForm.value.courseGroupId,
  ...
  printDform(){ 
    this.router.navigate(['admin-examination-management/admin-pre-examinations/attendance-print-Dform'])
    let queryparams = [
        {
  ...
  printformA(){ 
    this.router.navigate(['admin-examination-management/admin-pre-examinations/attendance-print-formA'])
    let queryparams = [
        {
         data:this.subjectModerationStudents,
         groupName:this.groupName,
