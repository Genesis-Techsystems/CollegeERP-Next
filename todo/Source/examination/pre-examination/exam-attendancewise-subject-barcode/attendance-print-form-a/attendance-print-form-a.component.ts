...
@Component({
  selector: 'app-attendance-print-form-a',
  templateUrl: './attendance-print-form-a.component.html',
  styleUrls: ['./attendance-print-form-a.component.scss']
})
export class AttendancePrintFormAComponent implements OnInit {
...
ngOnInit(): void {
  if(this.paramaters.AFormData && this.paramaters.AFormData != null){
    this.data = this.paramaters.AFormData;
    this.details = this.data[0].data;
    this.groupName =this.data[0].groupName;
    this.Logo = this.data[0].Logo;
    if(this.details && this.details.length > 0){
      this.students = this.details.filter(x=>(x.isPresent === null))
      this.presentStudents = this.details.filter(x=>(x.isPresent === true))
      this.absentStudents = this.details.filter(x=>(x.isPresent === false))
    }
    console.log(this.paramaters.AFormData,"this.paramaters.AFormData")
  }else{
    this.goBack();
  }
