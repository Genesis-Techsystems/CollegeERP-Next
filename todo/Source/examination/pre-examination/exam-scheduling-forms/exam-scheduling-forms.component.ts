...
@Component({
  selector: 'app-exam-scheduling-forms',
  templateUrl: './exam-scheduling-forms.component.html',
  styleUrls: ['./exam-scheduling-forms.component.scss']
})
export class ExamSchedulingFormsComponent implements OnInit {
...
get isExamTimetableEmpty(): boolean {
  return !!this.examTimetable && Object.keys(this.examTimetable).length === 0;
    ...
    this.isPackingSlip = false;
    this.dataSource = new MatTableDataSource(this.roomAllotments);
    if (academicYearId !== null && academicYearId !== undefined && this.staffForm.value.academicYearId !== null && this.staffForm.value.academicYearId !== undefined){
  /*-----------Exams -----------*/      
