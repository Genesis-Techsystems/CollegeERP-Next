...
@Component({
  selector: 'app-re-evaluation-result-report',
  templateUrl: './re-evaluation-result-report.component.html',
  styleUrls: ['./re-evaluation-result-report.component.scss']
})
export class ReEvaluationResultReportComponent implements OnInit {

  panelOpenState = true;

  private examFiltersByCodeUrl = CONSTANTS.examFiltersByCodeUrl;
  private collegeCrudUrl = CONSTANTS.collegeCrudUrl;
  private getReEvaluationMarksReportUrl = CONSTANTS.getReEvaluationMarksReportUrl;
  ...
  trafoItem = "Re-Evaluation Result Report";
...
getExamTypes(examId){
  ...
  getDetails() {
    ...
    if (this.staffForm.valid) {
      ...
      this.selectedData();
      let request = [
        { paramName: 'in_flag', paramValue: 'reevaluation_marks_report' },
        { paramName: 'in_exam_id', paramValue: this.staffForm.value.examId },
      ...
      this.crudService.getDetailsByRequest(this.getReEvaluationMarksReportUrl, '', request, '&')
        .subscribe(result => {
          this.spinner.hide();
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              ...
              this.resultStats = result.data.result[1];
            } else {
              this.snotifyService.success(result.message, 'Success!');
            }
          } else {
