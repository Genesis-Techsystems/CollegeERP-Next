...
@Component({
  selector: 'app-student-backlog-report',
  templateUrl: './student-backlog-report.component.html',
  styleUrls: ['./student-backlog-report.component.scss']
})
export class StudentBacklogReportComponent implements OnInit {
  ...
  trafoItem = "Student Backlog Report";
  ...
  constructor(private formBuilder: FormBuilder, private snotifyService: SnotifyService, private crudService: CrudService,
    private spinner: NgxSpinnerService, private router: Router, private genericFunctions: GenericFunctions) {
  ...
  getDetails(): void {
    if (this.staffForm.valid) {
      ...
      if (this.staffForm.value.bacKlogCount === null) {
        this.staffForm.get('bacKlogCount').setValue(0);
      }
      this.selectedData();
      this.getColleges();
      /*----------- STUDENTS -----------*/
      ...
      this.crudService.listByTwelveIds(this.examStudentResultsUrl, 'std_summary', this.staffForm.value.examId, this.staffForm.value.courseId,
        ...
        this.staffForm.value.regulationId, '-1', 0, this.staffForm.value.bacKlogCount, '-1',
