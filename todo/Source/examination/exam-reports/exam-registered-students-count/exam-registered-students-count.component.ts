...
@Component({
  selector: 'app-exam-registered-students-count',
  templateUrl: './exam-registered-students-count.component.html',
  styleUrls: ['./exam-registered-students-count.component.scss']
})
export class ExamRegisteredStudentsCountComponent implements OnInit {

  displayedColumns: string[] = ['id', 'exam', 'examType', 'college', 'course', 'group', 'year', 'subject', 'registeredStudents'];
  ...
  private getExamStdRegistrationUrl = CONSTANTS.getExamStdRegistrationUrl;
...
getExamFiltersList(): void {
  ...
  selectedCourse(courseId) {
    ...
    this.examRegStdForm.get('regulationId').setValue('');
    this.examRegStdForm.get('subjectId').setValue('');
  ...
  selectedExam(examId): void {
    this.examRegStdForm.get('regulationId');
    this.examRegStdForm.get('subjectId').setValue('');
    this.collegesListDetails = [];
    this.colleges = [];
    this.groupList = [];
  ...
  getStudentsList(){
    ...
    if (this.examRegStdForm.valid) {
      this.collegeCode =this.colleges.filter(x=>x.fk_college_id === this.examRegStdForm.value.collegeId)[0]?.college_code, 
      ...
      this.courseGroupCode =this.courseGroups.filter(x=>x.fk_course_group_id === this.examRegStdForm.value.courseGroupId)[0]?.group_code;
      this.courseYearCode = this.courseYears.filter(x=>x.fk_course_year_id === this.examRegStdForm.value.courseYearId)[0]?.course_year_name;
      ...
      /*----------- SUBJECTS -----------*/
      let request = [
        { paramName: 'in_flag', paramValue: 'exam_std_subject_registered_count' },
        { paramName: 'in_exam_id', paramValue: this.examRegStdForm.value.examId },
        { paramName: 'in_course_group_id', paramValue: 0 },
        { paramName: 'in_course_year_id', paramValue: 0 },
        { paramName: 'in_course_group_id', paramValue: 0 },
      ...
      this.crudService.getDetailsByRequest(this.getExamStdRegistrationUrl, '', request, '&')
        .subscribe(result => {
          this.spinner.hide();
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
