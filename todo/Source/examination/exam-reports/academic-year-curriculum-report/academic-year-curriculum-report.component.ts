...
@Component({
  selector: 'app-academic-year-curriculum-report',
  templateUrl: './academic-year-curriculum-report.component.html',
  styleUrls: ['./academic-year-curriculum-report.component.scss']
})
export class AcademicYearCurriculumReportComponent implements OnInit {
 ...
 dataSource: MatTableDataSource<any>;
   ...
   panelOpenState = true;
   step = 0;  
   trafoItem=" Academic Year Curriculum Report";
   collegeCode: any;
   exam: any;
   columns: string[] = [];
   dataDetails;
...
academicyears = [];
  ...
  this.academicyears = [];
    ...
    this.dataSource = new MatTableDataSource<any>([]);
     let universityId = this.CollegesListDetails.filter(x => x.fk_college_id == collegeId)[0]?.fk_university_id;
 ...
 selectedGroup(courseGroupId): void {
   ...
   getColleges(): void{
    ...
    this.academicYearCode = this.academicYears.filter(x=>(x.fk_academic_year_id == academicYearId))[0].academic_year;
    // this.dataDetails =   this.dataDetails +' / '+ this.academicYearCode;
    this.studentDueList = [];
    this.dataSource = new MatTableDataSource<any>(this.studentDueList);
  }
   getDetails() {
     this.curriculumList=[];
     if(this.staffForm.valid){
       ...
       this.getColleges();
      let request = [
           {paramName:'in_flag', paramValue:'ay_univ_curriculum'},
