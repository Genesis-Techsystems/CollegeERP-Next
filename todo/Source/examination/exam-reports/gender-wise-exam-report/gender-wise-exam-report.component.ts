...
@Component({
  selector: 'app-gender-wise-exam-report',
  templateUrl: './gender-wise-exam-report.component.html',
  styleUrls: ['./gender-wise-exam-report.component.scss']
})
export class GenderWiseExamReportComponent implements OnInit {
    ...
    if (this.staffForm.valid) {
      ...
      this.crudService.getDetailsByRequest(this.ResultFinalAnalysisUrl, '', request, '&')
        .subscribe(result => {
          ...
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              this.subjectWiseresult = result.data.result[0];
              this.exam = this.subjectWiseresult[0]?.exam_label_name;
              this.selectedData();
              // this.snotifyService.success(result.message, 'Success!');
            } else {
              this.snotifyService.success(result.message, 'Success!');
