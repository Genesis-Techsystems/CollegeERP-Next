...
@Component({
  selector: 'app-exam-student-registration-tt-report',
  templateUrl: './exam-student-registration-tt-report.component.html',
  styleUrls: ['./exam-student-registration-tt-report.component.scss']
})
export class ExamStudentRegistrationTtReportComponent implements OnInit {

  displayedColumns: string[] = ['id', 'subject', 'college', 'course', 'group', 'examType', 'registeredStudents'];
  ...
  examFeeTypes = [];
  trafoItem="Exam Registered Students Count";
  searchText = '';
  ...
  getStudentsList(){
      ...
      if (this.examRegStdForm.valid) {
        ...
        if (this.examRegStdForm.value.isReevaluation === true) {
          flag = 're_exam_std_subject_registered_count';
        } else {
          flag = 'exam_std_subject_registered_count';
        }
        /*----------- SUBJECTS -----------*/
        let request = [
...
getGrandTotal(): number {
  return this.studentsList.reduce((sum, student) => sum + Number(student.students_registered_for_exam || 0), 0);
}
exportAsExcel()
  {
