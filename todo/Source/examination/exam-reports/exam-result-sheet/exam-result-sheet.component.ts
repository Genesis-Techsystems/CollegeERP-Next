...
@Component({
  selector: 'app-exam-result-sheet',
  templateUrl: './exam-result-sheet.component.html',
  styleUrls: ['./exam-result-sheet.component.scss']
})
export class ExamResultSheetComponent implements OnInit {
  ...
  reset(): void {
    ...
    this.staffForm.get('isReevaluation').setValue('');
    this.examStudentResult = [];
  }
  getDetails() {
    this.examStudentResult = [];
    ...
    if (this.staffForm.valid) {
      ...
      if (this.staffForm.value.isReevaluation == true) {
        ...
        this.dialogTitle = 'Re-Evaluation Exam Result Sheet'
      } else {
        this.flag = 'final_results_list';
        this.dialogTitle = 'Exam Result Sheet'
      }
      let request = [
              ...
              if (this.staffForm.value.courseGroupId == 0) {
                ...
                for (let i = 0; i < this.courseGroups.length; i++) {
                  ...
                  for (let j = 0; j < this.examStudentResult.length; j++) {
                    ...
                    else if (this.examStudentResult[j].fk_course_group_id == this.courseGroups[i].fk_course_group_id && this.examStudentResult[j].ResultStatus == 'Promoted'
                    ) {
                      this.failList.push(this.examStudentResult[j])
                    }
                    else if (this.examStudentResult[j].fk_course_group_id == this.courseGroups[i].fk_course_group_id && this.examStudentResult[j].ResultStatus == 'Detained'
                    ) {
                      this.detainList.push(this.examStudentResult[j])
                    }
                  }
                  this.PassmainList.push(this.passList);
                  this.FailmainList.push(this.failList);
                  this.detainMainList.push(this.detainList);
                  this.testArray.push(this.passList);
                  this.testArray.push(this.failList);
                  this.testArray.push(this.detainList);
                }
              }
              if (this.staffForm.value.courseGroupId != 0) {
  ...
  selectedStatus(resultstatus) {
    ...
    if (this.examStudentResultGroup.length > 0) {
      ...
      this.promotedList = this.examStudentResultGroup.filter(x => (x.ResultStatus == 'Promoted'));
      this.detainedList = this.examStudentResultGroup.filter(x => (x.ResultStatus == 'Detained'));
    }
  }
  getColleges(): void{
      this.collegeLogo =[];
