...
@Component({
  selector: 'app-branch-wise-failed-result-sheets',
  templateUrl: './branch-wise-failed-result-sheets.component.html',
  styleUrls: ['./branch-wise-failed-result-sheets.component.scss']
})
export class BranchWiseFailedResultSheetsComponent implements OnInit {
  ...
  testArray = [];
  flag;
  dialogTitle = 'Group Wise Failed Result Sheets';
  examName: any;
  collegeLists: any;
  examListDetails = [];
  ...
  failed = false;
  dataDetails = ' ';
  trafoItem = "Group Wise Failed Result Sheets";
  searchText = ''
  collegeLogo = [];
  orgCode = '';
  Logo:any;
  ...
  getDetails() {
    ...
    if (this.staffForm.valid) {
      ...
      if (this.staffForm.value.isReevaluation == true) {
        ...
        this.dialogTitle = 'Re-Evaluation Group Wise Failed Result Sheets'
      } else {
        this.flag = 'final_results_list';
        this.dialogTitle = 'Group Wise Failed Result Sheets'
      }
      let request = [
      ...
      this.crudService.getDetailsByRequest(this.ResultFinalAnalysisUrl, '', request, '&')
        .subscribe(result => {
          ...
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              this.examStudentResult = result.data.result[0].filter(x => (x.ResultStatus == 'Promoted'));
              ...
              if (this.staffForm.value.courseGroupId == 0) {
                ...
                for (let i = 0; i < this.courseGroups.length; i++) {
                  ...
                  for (let j = 0; j < this.examStudentResult.length; j++) {
                    if (this.examStudentResult[j].fk_course_group_id == this.courseGroups[i].fk_course_group_id && this.examStudentResult[j].ResultStatus == 'Promoted'
                    ) {
  ...
  selectedStatus(resultstatus) {
    ...
    if (this.examStudentResultGroup.length > 0) {
      this.passedList = this.examStudentResultGroup.filter(x => (x.ResultStatus == 'Promoted'));
    }
  }
  Print() {
    window.print();
