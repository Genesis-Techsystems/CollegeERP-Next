...
displayedColumns: string[] = ['sno','hallticket_number','student_name','group_code' ,'course_year_code','batch_name'];
  ...
  this.mainList = [];
   if (this.staffForm.valid) {
     ...
     const request = [
       { paramName: 'in_flag', paramValue: 'batch_wise_detention_report' },
     ...
     this.crudService
       .getDetailsByRequest('getAllRecords/s_get_detention_report', '', request, '&')
       .subscribe({
         next: (result) => {
           this.spinner.hide();
           ...
           if (result.statusCode === 200) {
             ...
             if (this.examSubjectStats.length === 0) {
