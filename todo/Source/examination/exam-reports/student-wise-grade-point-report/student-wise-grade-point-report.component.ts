...
@Component({
  selector: 'app-student-wise-grade-point-report',
  templateUrl: './student-wise-grade-point-report.component.html',
  styleUrls: ['./student-wise-grade-point-report.component.scss']
})
export class StudentWiseGradePointReportComponent implements OnInit {
  ...
  panelOpenState = true;
   ...
   trafoItem = "Student Wise Grade Point Report";
   collegeName;
   examName: any;
   newList = [];
   studentsList: any[];
...
getDetails() {
  ...
  if (this.staffForm.valid) {
    ...
    this.courseGroupCode = this.courseGroups.find(x => x.fk_course_group_id === this.staffForm.value.courseGroupId)?.group_code;
    this.courseYearCode = this.courseYears.find(x => x.fk_course_year_id === this.staffForm.value.courseYearId)?.course_year_name;
    this.exam = this.examsList.find(x => x.fk_exam_id === this.staffForm.value.examId)?.exam_name;

    this.selectedData();
    this.getColleges();

    const flag = this.staffForm.value.isReevaluation ? 'tabulation_register_re_evaluation' : 'tabulation_register_new';
          ...
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              if (this.universityCode === 'SUK') {
                ...
                if (this.tabulationRegisterList?.length > 0) {
                  this.mainList = [];
                  ...
                  for (let i = 0; i < this.studentsList.length; i++) {
                    const newList = this.tabulationRegisterList.filter(
                      x => x.hallticket_number === this.studentsList[i].hallticket_number
                    );
                    this.mainList.push(newList);
                ...
                if (this.examRegisteredStudents?.length > 0) {
                  this.subjects = this.examRegisteredStudents.map(x => ({ subject_code: x.subject_code }));
                  ...
                  this.mainList = this.studentsList.map(stud =>
                    this.examRegisteredStudents.filter(x => x.hallticket_number === stud.hallticket_number)
                  );
                }
              }

              // ✅ Call the new stored procedure after processing the main list
              this.getExamSubjectStats();
...
getExamSubjectStats() {
  const request = [
    { paramName: 'in_flag', paramValue: 'EXAM_SUBJECT_ANALYSIS' },
    { paramName: 'in_college_id', paramValue: this.staffForm.value.collegeId },
    { paramName: 'in_course_id', paramValue: this.staffForm.value.courseId },
    { paramName: 'in_course_group_id', paramValue: this.staffForm.value.courseGroupId },
    { paramName: 'in_course_year_id', paramValue: this.staffForm.value.courseYearId },
    { paramName: 'in_exam_id', paramValue: this.staffForm.value.examId },
    { paramName: 'in_batch_id', paramValue: this.staffForm.value.batchId || 0 }
  ];
        ...
        if (result.statusCode === 200) {
          if (result.data && result.data.result?.length > 0) {
            ...
            if (Array.isArray(resultData) && Array.isArray(resultData[0])) {
              this.examSubjectStats = resultData[0];
            } else {
              this.examSubjectStats = resultData || [];
            }

            // ✅ Remove duplicates based on subject name (ignoring [1], [2], etc.)
            this.examSubjectStats = this.examSubjectStats.filter(
              (obj, index, self) => {
                const cleanName = obj.subject_name.replace(/\s*\[.*?\]\s*/g, '').trim().toLowerCase();
                return (
                  index ===
                  self.findIndex(
                    o => o.subject_name.replace(/\s*\[.*?\]\s*/g, '').trim().toLowerCase() === cleanName
                  )
                );
              }
            );

            console.log('Filtered Subject Stats:', this.examSubjectStats);
            // ✅ Fix ends here
          } else {
            this.snotifyService.success(result.message, 'No subject stats found!');
          }
        } else {
          this.snotifyService.error(result.message, 'Error!');
        }
