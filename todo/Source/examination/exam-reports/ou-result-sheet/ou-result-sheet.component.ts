...
@Component({
  selector: 'app-ou-result-sheet',
  templateUrl: './ou-result-sheet.component.html',
  styleUrls: ['./ou-result-sheet.component.scss']
})
export class OuResultSheetComponent implements OnInit {
  ...
  getDetails() {
    ...
    if (this.staffForm.valid) {
      ...
      let request = [
        ...
        { paramName: 'in_flag', paramValue: 'ou_result_sheet' },
        { paramName: 'in_orgid', paramValue: 1 },
        { paramName: 'in_fdate', paramValue: '1990-01-01' },
        { paramName: 'in_tdate', paramValue: '1990-01-01' },

        { paramName: 'in_exam_id', paramValue: this.staffForm.value.examId },
        { paramName: 'in_clg_id', paramValue: this.staffForm.value.collegeId },
        { paramName: 'in_course_id', paramValue: this.staffForm.value.courseId },
        ...
        { paramName: 'in_student_id', paramValue: this.staffForm.value.studentId }

      ];

      this.crudService.getDetailsByRequest(
        this.ouresultsheetUrl,
        '',
        request,
        '&'
      ).subscribe(result => {
