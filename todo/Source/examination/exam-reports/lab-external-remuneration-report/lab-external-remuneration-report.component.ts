...
@Component({
  selector: 'app-lab-external-remuneration-report',
  templateUrl: './lab-external-remuneration-report.component.html',
  styleUrls: ['./lab-external-remuneration-report.component.scss']
})
export class LabExternalRemunerationReportComponent implements OnInit {

  displayedColumns: string[] = ['id', 'evaluatorName', 'subjectcode', 'evaluationCount', 'Amount', 'TotalAmount', 'AccNo', 'IfscCode'];
  ...
  selectedRegulation(regulationId): void {
    ...
    let request = [
      ...
      { paramName: 'in_flag_type', paramValue: 'LAB' },
      { paramName: 'in_university_id', paramValue: 0 },
  ...
  getList() {
    ...
    if (this.evaluatorForm.valid) {
      ...
      if (this.evaluatorForm.value.isReevaluation === true) {
        flag = 're_lab_evaluator_remuneration';
      } else {
        flag = 'lab_evaluator_remuneration';
      }
      let request = [
              ...
              if (this.evaluatedReport && this.evaluatedReport.length > 0) {
                this.evaluatedReportsByProfile = Object.values(
                  this.evaluatedReport.reduce((acc, item) => {
                    ...
                    const key = `${item.pk_exam_evaluator_profile_id}`;
  ...
  exportAsExcel() {
    ...
    const data = this.evaluatedReportsByProfile.map(report => ({
      ...
      'Evaluations Count': report.evaluation_count,
      'Amount Per Evaluation': report.amount,
      'Final Amount': report.total_final_amount
    }));
    ...
    // Export file
    XLSX.writeFile(wb, 'Lab_Remuneration_Report.xlsx');
  }
