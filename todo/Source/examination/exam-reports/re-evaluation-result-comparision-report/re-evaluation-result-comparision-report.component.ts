...
@Component({
  selector: 'app-re-evaluation-result-comparision-report',
  templateUrl: './re-evaluation-result-comparision-report.component.html',
  styleUrls: ['./re-evaluation-result-comparision-report.component.scss']
})
export class ReEvaluationResultComparisionReportComponent implements OnInit {

  panelOpenState = true;

  private examFiltersByCodeUrl = CONSTANTS.examFiltersByCodeUrl;
  private collegeCrudUrl = CONSTANTS.collegeCrudUrl;
  private getEvaluationAnalysisUrl = CONSTANTS.getEvaluationAnalysisUrl;
  ...
  trafoItem = "Re-Evaluation Comparision Result Report";
  ...
  getDetails() {
    ...
    if (this.staffForm.valid) {
      ...
      this.crudService.getDetailsByRequest(this.getEvaluationAnalysisUrl, '', request, '&')
        .subscribe(result => {
          this.spinner.hide();
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              ...
              this.reportSummary = result.data.result[1];
            } else {
              this.snotifyService.success(result.message, 'Success!');
            }
          } else {
