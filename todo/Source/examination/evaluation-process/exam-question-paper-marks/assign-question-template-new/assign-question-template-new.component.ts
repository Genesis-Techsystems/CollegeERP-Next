...
@Component({
  selector: 'app-assign-question-template-new',
  templateUrl: './assign-question-template-new.component.html',
  styleUrls: ['./assign-question-template-new.component.scss']
})
export class AssignQuestionTemplateNewComponent implements OnInit {
   ...
   displayedColumns: string[] = ['id', 'questionNo', 'subQuestion', 'question', 'totalMarks'];
    ...
    ngOnInit(): void {
      ...
      if(this.data.fk_exam_qp_template_id !== null){
    ...
    getTemplateList() {
      this.spinner.show();
       ...
       this.crudService.listDetailsById(this.examQpTemplateUrl, 'true', 'isActive')
                  .subscribe(result => {
                      this.spinner.hide();
                      ...
                      if (!this.isEmptyObject(this.data) && this.isEdit && this.TemplateList.length > 0) {
                          ...
                          const selectedIndex = this.TemplateList.findIndex(t => 
                              t.examQpTemplateId === this.data.fk_exam_qp_template_id
                          );
  
                          if (selectedIndex !== -1) {
                              this.selectedIndex = selectedIndex;
                              this.selectionTemplate(this.data.fk_exam_qp_template_id, selectedIndex);
                          }
                      } else if (this.TemplateList.length > 0) {
                          this.selectionTemplate(this.TemplateList[0].examQpTemplateId, 0);
                      }
                  } else {
      ...
      //                     // Find the index of the selected template
      //                     const selectedIndex = this.TemplateList.findIndex(t => 
      //                         t.examQuestionPaperTemplateId === this.data.fk_exam_qp_template_id
      //                     );
  ...
  selectionTemplate(event: any, index?: number) {
      ...
      if (this.TemplateDuplicateList.filter(x => (x.examQpTemplateId === this.templateId)).length > 0){
          this.paperTemplate = this.TemplateDuplicateList.filter(x => (x.examQpTemplateId === this.templateId))[0].examQpTemplateDetailsDTO;
           this.paperTemplate1 = this.TemplateDuplicateList.filter(x => (x.examQpTemplateId === this.templateId))[0].examQpTemplateDetailsDTO;
      } 
  }
  assigntemplate(){
      this.isShow = true;
      ...
      let reqest =
      {
        ...
        "examQpTemplateId": this.templateId,
    ...
    updateTemplate(): void {
      ...
      let details =
      {
        "examQptempAssignId":this.data.fk_exam_qp_template_id,
        "examMasterId": this.params.examId,
        "regulationId": this.params.regulationId,
        "subjectId": this.params.subjectId,
        "examQpTemplateId": this.templateId,
      ...
      this.spinner.show();
      this.crudService.updateDetails(this.ExamQPtempAssignUrl, details,this.data.fk_exam_qp_template_id,'examQptempAssignId')
          .subscribe(result => {
              this.spinner.hide();
              if (result.statusCode === 200) {
                      this.snotifyService.success(result.message, 'Success!');
