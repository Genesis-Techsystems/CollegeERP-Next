...
import { ViewTemplateModalComponent } from '../view-template-modal/view-template-modal.component';

@Component({
  selector: 'app-add-questionpaper-modal',
  templateUrl: './add-questionpaper-modal.component.html',
  styleUrls: ['./add-questionpaper-modal.component.scss']
})
export class AddQuestionpaperModalComponent implements OnInit {
  ...
  private getQuestionPaperAssignments = CONSTANTS.getQuestionPaperAssignments;
  ...
  constructor(private formBuilder: FormBuilder, @Inject(MAT_DIALOG_DATA) private data, private snotifyService: SnotifyService, private dialogRef: MatDialogRef<AddQuestionpaperModalComponent>,
  ...
  selectedCourse(courseId) {
    ...
    this.addquestionpaperForm.get('courseYearId').setValue(0)
  ...
  selectedregulationCode(value) {
      ...
      this.courseYears = [];
      this.subjectDetails = [];
      this.courseYearList = this.subjectListDetails.filter(x => (x.fk_regulation_id == this.addquestionpaperForm.value.regulationId))
      ...
      if (!this.isEmptyObject(this.data) && this.courseYears.length > 0) {
        this.addquestionpaperForm.get('courseYearId').setValue(+this.data[0].courseYearId);
        this.selectedYear(this.addquestionpaperForm.value.courseYearId);
  
      } else
        if (this.courseYears.length > 0) {
          this.addquestionpaperForm.get('courseYearId').setValue(this.courseYears[0].fk_course_year_id);
          this.selectedYear(this.addquestionpaperForm.value.courseYearId);
        }
    }
    ...
    getTemplateDetails(): void{
         this.spinner.show();
          let request = [
            ...
            {paramName: 'in_course_year_id', paramValue: this.data[0].courseYearId},
            {paramName: 'in_regulation_id', paramValue:this.data[0].regulationId},
            {paramName: 'in_subject_id', paramValue:this.data[0].subjectId},
          ];
          this.crudService.getDetailsByRequest(this.getQuestionPaperAssignments, '', request, '&')
          .subscribe(result => {
             this.spinner.hide();
             if (result.statusCode === 200){
                  if (result.data && result.data !== '' && result.data.result.length > 0) {
  ...
  submit(): void {
    const Obj = {
      ...
      examQuestionPaperTemplateId:this.addquestionpaperForm.value.examQuestionPaperTemplateId
    }
  ...
  viewTemplate(): void{
    let row = {
      fk_exam_questionpaper_template_id : this.addquestionpaperForm.value.examQuestionPaperTemplateId
    } 
    if (row.fk_exam_questionpaper_template_id != 0){
      const dialogRef = this.dialog.open(ViewTemplateModalComponent, {
          width: '900px',
          data: row
      });
