...
import { AddQuestionpaperModalComponent } from './add-questionpaper-modal/add-questionpaper-modal.component';
...
import { ViewTemplateQuestionsComponent } from './view-template-questions/view-template-questions.component';
import { ParametersService } from 'app/main/services/parameters.service';

@Component({
  selector: 'app-exam-question-paper-marks',
  templateUrl: './exam-question-paper-marks.component.html',
  styleUrls: ['./exam-question-paper-marks.component.scss'],
  providers: [ViewQuestionsComponent, ViewTemplateComponent]
})

export class ExamQuestionPaperMarksComponent implements OnInit {
  displayedColumns: string[] = ['id', 'questionPaperTitle', 'questionPaperCode', 'courseYearCode', 'setNumber', 'totalQuestions', 'totalMarks', 'passMarks', 'PrepareByEmp', 'questionPaperPath', 'modelAnswerSheetPath', 'paperTemplate', 'Status', 'Actions'];
  ...
  constructor(private route: ActivatedRoute, private formBuilder: FormBuilder, private router: Router, private dialog: MatDialog, private snotifyService: SnotifyService,
    private crudService: CrudService, private spinner: NgxSpinnerService, private genericFunctions: GenericFunctions, private com: ViewQuestionsComponent, private parameters: ParametersService) {
  ...
  selectedregulationCode(value) {
    ...
    this.QuestionPapers = []
    this.dataSource = new MatTableDataSource([])
    this.courseYearList = this.subjectListDetails.filter(x => (x.fk_regulation_id == this.questionPaperForm.value.regulationId))
  ...
  openDialog() {
    const dialogRef = this.dialog.open(AddQuestionpaperModalComponent, {
      width: '100%',
    ...
    dialogRef.afterClosed().subscribe(details => {
      if (details != null && details !== '') {
        ...
        if (details.examQuestionPaperTemplateId !== null) {
          this.spinner.show();
          /*---------- ADD EXAM QUESTION PAPER----------*/
          this.crudService.addDetails(this.ExamQuestionPaperCrudUrl, details)
            .subscribe(result => {
              this.spinner.hide();
              if (result.statusCode === 200) {
  ...
  manageQuestions(item): void {
    ...
    let queryParams = [{
      ...
      templateId: item.fk_exam_questionpaper_template_id,
  ...
  printQA(item) {
    ...
    let queryParams = [{
      ...
      regulationId: this.questionPaperForm.value.regulationId,
      subjectName: this.subjectName,
      pkEQPTid: item.fk_exam_questionpaper_template_id,
