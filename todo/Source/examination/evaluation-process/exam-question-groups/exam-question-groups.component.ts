...
import { AddExamQuestionGroupsModalComponent } from './add-exam-question-groups-modal/add-exam-question-groups-modal.component';
@Component({
  selector: 'app-exam-question-groups',
  templateUrl: './exam-question-groups.component.html',
  styleUrls: ['./exam-question-groups.component.scss']
})
export class ExamQuestionGroupsComponent implements OnInit {

 // dataSource:  MatTableDataSource<storeMaster>;; 
 displayedColumns: string[] = ['id', 'questionGroupName', 'bestOfAnswers', 'isSingleGroup','Status','actions'];
 dataSource:any;
 ...
 examQuestiongroupForm: FormGroup;
 colleges: College[] = [];
 ...
 constructor(private formBuilder: FormBuilder, private snotifyService: SnotifyService,
   ...
   this.examQuestiongroupForm = this.formBuilder.group({
     courseId: ['', ],  
...
filterExam(): void {
 ...
 selectedCollege(collegeId): void{
   this.examQuestiongroupForm.get('academicYearId').setValue('');
   this.examQuestiongroupForm.get('courseId').setValue('');
   this.examQuestiongroupForm.get('examId').setValue('');
   this.examQuestiongroupForm.get('courseGroupId').setValue('');
   this.examQuestiongroupForm.get('courseYearId').setValue('');
   this.examQuestiongroupForm.get('subjectTypeId').setValue('');
   this.examQuestiongroupForm.get('subjectId').setValue('');
   this.examQuestiongroupForm.get('examTimetableDetId').setValue('');
   this.courses = [];
   this.examsList = [];
   this.courseYears = [];
   this.courseGroups = [];
...
this.crudService.listDetailsByTwoIds(this.courseCrudUrl, this.examQuestiongroupForm.value.collegeId, 'true', this.getDetailsByCollegeIdUrl, this.isActive)
.subscribe(result => {
   if (result.statusCode === 200){
           if (result.data.resultList && !_.isEmpty(result.data.resultList)) {
...
selectedCourse(courseId): void{
 ...
 // tslint:disable-next-line:max-line-length
   this.crudService.listDetailsByFourIdsWithSortOrder(this.examMasterUrl, this.examQuestiongroupForm.value.collegeId, this.examQuestiongroupForm.value.academicYearId, courseId, 'true',
...
selectedExam(examId): void{
 ...
 this.examQuestiongroupForm.get('examDate').setValue(this.minDate);
...
/*----------- COURSES GROUPS -----------*/      
 ...
 this.exam = this.examsList.filter(x => (x.examId === this.examQuestiongroupForm.value.examId))[0].examName +
  '( ' + this.genericFunctions.momentWithDateFormatYMD(this.examsList.filter(x => (x.examId === this.examQuestiongroupForm.value.examId))[0].fromDate) + ' - ' + 
  this.genericFunctions.momentWithDateFormatYMD(this.examsList.filter(x => (x.examId === this.examQuestiongroupForm.value.examId))[0].toDate)
    + ')';
 this.crudService.listDetailsByTwoIds(this.courseGroupCrudUrl, this.examQuestiongroupForm.value.courseId, 'true', this.getDetailsByCourseIdUrl, this.isActive)
 .subscribe(result => {
     if (result.statusCode === 200) {
         if (result.data.resultList && !_.isEmpty(result.data.resultList)) {
...
selectedCourseGroup(courseGroupId): void{
 ...
 this.examTimetableSubjectsList = [];
 this.examQuestiongroupForm.get('courseYearId').setValue('');
 this.examQuestiongroupForm.get('groupSectionId').setValue('');  
 this.examQuestiongroupForm.get('subjectTypeId').setValue('');
 this.examQuestiongroupForm.get('subjectId').setValue('');
 this.examQuestiongroupForm.get('examTimetableDetId').setValue('');
 if (this.examQuestiongroupForm.value.collegeId != null && courseGroupId != null){
 this.courseGroup = this.courseGroups.filter(x => (x.courseGroupId === courseGroupId))[0].groupCode;
 /*----------- COURSES Years -----------*/      
 
 this.crudService.listDetailsByTwoIdsWithSortLtd(this.courseYearCrudUrl, this.examQuestiongroupForm.value.courseId, 'true', 'ASC',
...
selectedYear(courseYearId): void{
 ...
 this.examQuestiongroupForm.get('subjectId').setValue('');
 this.examQuestiongroupForm.get('examTimetableDetId').setValue('');
 if (this.examQuestiongroupForm.value.collegeId != null && courseYearId != null){
 this.courseyear = this.courseYears.filter(x => (x.courseYearId === courseYearId))[0].courseYearName;
 /*----------- COURSES YEARS -----------*/      
 
 // tslint:disable-next-line:max-line-length
 this.crudService.listDetailsByFourIds(this.groupSectionCrudUrl, courseYearId, this.examQuestiongroupForm.value.academicYearId, this.examQuestiongroupForm.value.courseGroupId, 'true', this.getDetailsByCourseYearIdUrl, 
...
selectedSubjectType(subjectTypeId): void{
 ...
 if (subjectTypeId !== null){  
   this.examTimetableSubjectsList = [];
   const dateConvert = this.genericFunctions.momentFormatYMD(this.examQuestiongroupForm.value.examDate);
     /*----------- EXAM TIMETABLES -----------*/
   this.crudService.listBySixIds(this.subjectsforexamUrl, this.examQuestiongroupForm.value.collegeId, 
                                                            this.examQuestiongroupForm.value.courseId, 
                                                            this.examQuestiongroupForm.value.examId, 
                                                            dateConvert, 
                                                            this.examQuestiongroupForm.value.courseGroupId, 
                                                            this.examQuestiongroupForm.value.courseYearId, 
...
selectedSubject(examTimetableDetId, list): void{
 ...
 if (examTimetableDetId !== null){
   ...
   if (this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.examQuestiongroupForm.value.examTimetableDetId)).length > 0){
         this.examTypeId = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.examQuestiongroupForm.value.examTimetableDetId))[0].examTypeCatId;
         // tslint:disable-next-line: max-line-length
         this.examQuestiongroupForm.get('subjectId').setValue(this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.examQuestiongroupForm.value.examTimetableDetId))[0].subjectId);
         this.regulationId = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.examQuestiongroupForm.value.examTimetableDetId))[0].regulationId;
         this.subjectTypeId = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.examQuestiongroupForm.value.examTimetableDetId))[0].subjectTypeId;
         this.subjectTypCode = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.examQuestiongroupForm.value.examTimetableDetId))[0].subjectTypeName;
         this.examTypeCatCode = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.examQuestiongroupForm.value.examTimetableDetId))[0].examTypeCatCode;
     }

   
     /*----------- EXAM TIMETABLES -----------*/
   this.crudService.listByEightIds(this.examMarksEntryStudentsUrl, this.examQuestiongroupForm.value.collegeId, 
       this.examQuestiongroupForm.value.courseId, 
       this.examQuestiongroupForm.value.examId, 
       dateConvert, 
       this.examQuestiongroupForm.value.courseGroupId, 
       this.examQuestiongroupForm.value.courseYearId, 
       this.examQuestiongroupForm.value.subjectId, 
       this.examTypeId,
       ...
       .subscribe(result => {
         if (result.statusCode === 200){
                 if (result.data && !_.isEmpty(result.data) && result.data.length > 0) {
                   ...
                   // this.getSubjectSyllabus(this.examQuestiongroupForm.value.subjectId);
...
getQuestionpaperList(){
  if(this.examQuestiongroupForm.valid){
    this.flag=true
...
openDialog(){
  const dialogRef = this.dialog.open(AddExamQuestionGroupsModalComponent, {
    width: '900px',
    data: {}
});
}
