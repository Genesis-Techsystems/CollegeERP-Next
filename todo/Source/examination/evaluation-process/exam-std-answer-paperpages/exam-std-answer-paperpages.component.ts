...
import { AddStdAnswerPaperpagesComponent } from './add-std-answer-paperpages/add-std-answer-paperpages.component';

@Component({
  selector: 'app-exam-std-answer-paperpages',
  templateUrl: './exam-std-answer-paperpages.component.html',
  styleUrls: ['./exam-std-answer-paperpages.component.scss']
})
export class ExamStdAnswerPaperpagesComponent implements OnInit {
 
  displayedColumns: string[] = ['id', 'AnswerPaperPage', 'AnswerPaperId', 'PageNumber', 'StudentAnswerPaperPage','StudentAnswerPage_Path','Status'];  
  ...
  StdAnswerPaperpages = [
    {
     AnswerPaperPage:'30',AnswerPaperId:'12',PageNumber:'50',StudentAnswerPaperPage:'Physics',StudentAnswerPage_Path:'pdf/path',Status:true
    },
    {
      AnswerPaperPage:'35',AnswerPaperId:'23',PageNumber:'50',StudentAnswerPaperPage:'Maths',StudentAnswerPage_Path:'pdf/path',Status:false
    },
  ]
...
selectedCollege(collegeId): void{
 ...
 this.exmstdanswerpaperpages.get('courseId').setValue('');
 this.exmstdanswerpaperpages.get('examId').setValue('');
 this.exmstdanswerpaperpages.get('courseGroupId').setValue('');
 this.exmstdanswerpaperpages.get('courseYearId').setValue('');
 this.exmstdanswerpaperpages.get('subjectTypeId').setValue('');
 this.exmstdanswerpaperpages.get('subjectId').setValue('');
 this.exmstdanswerpaperpages.get('examTimetableDetId').setValue('');
 this.courses = [];
 this.examsList = [];
 this.courseYears = [];
 this.courseGroups = [];
...
this.crudService.listDetailsByTwoIds(this.courseCrudUrl, this.exmstdanswerpaperpages.value.collegeId, 'true', this.getDetailsByCollegeIdUrl, this.isActive)
.subscribe(result => {
 if (result.statusCode === 200){
         if (result.data.resultList && !_.isEmpty(result.data.resultList)) {
...
// tslint:disable-next-line:max-line-length
 this.crudService.listDetailsByFourIdsWithSortOrder(this.examMasterUrl, this.exmstdanswerpaperpages.value.collegeId, this.exmstdanswerpaperpages.value.academicYearId, courseId, 'true',
...
this.exmstdanswerpaperpages.get('examDate').setValue(this.minDate);
...
this.exam = this.examsList.filter(x => (x.examId === this.exmstdanswerpaperpages.value.examId))[0].examName +
'( ' + this.genericFunctions.momentWithDateFormatYMD(this.examsList.filter(x => (x.examId === this.exmstdanswerpaperpages.value.examId))[0].fromDate) + ' - ' + 
this.genericFunctions.momentWithDateFormatYMD(this.examsList.filter(x => (x.examId === this.exmstdanswerpaperpages.value.examId))[0].toDate)
  + ')';
this.crudService.listDetailsByTwoIds(this.courseGroupCrudUrl, this.exmstdanswerpaperpages.value.courseId, 'true', this.getDetailsByCourseIdUrl, this.isActive)
.subscribe(result => {
   if (result.statusCode === 200) {
       if (result.data.resultList && !_.isEmpty(result.data.resultList)) {
...
this.examTimetableSubjectsList = [];
this.exmstdanswerpaperpages.get('courseYearId').setValue('');
this.exmstdanswerpaperpages.get('groupSectionId').setValue('');  
this.exmstdanswerpaperpages.get('subjectTypeId').setValue('');
this.exmstdanswerpaperpages.get('subjectId').setValue('');
this.exmstdanswerpaperpages.get('examTimetableDetId').setValue('');
if (this.exmstdanswerpaperpages.value.collegeId != null && courseGroupId != null){
this.courseGroup = this.courseGroups.filter(x => (x.courseGroupId === courseGroupId))[0].groupCode;
/*----------- COURSES Years -----------*/      

this.crudService.listDetailsByTwoIdsWithSortLtd(this.courseYearCrudUrl, this.exmstdanswerpaperpages.value.courseId, 'true', 'ASC',
...
this.exmstdanswerpaperpages.get('subjectId').setValue('');
this.exmstdanswerpaperpages.get('examTimetableDetId').setValue('');
if (this.exmstdanswerpaperpages.value.collegeId != null && courseYearId != null){
this.courseyear = this.courseYears.filter(x => (x.courseYearId === courseYearId))[0].courseYearName;
/*----------- COURSES YEARS -----------*/      

// tslint:disable-next-line:max-line-length
this.crudService.listDetailsByFourIds(this.groupSectionCrudUrl, courseYearId, this.exmstdanswerpaperpages.value.academicYearId, this.exmstdanswerpaperpages.value.courseGroupId, 'true', this.getDetailsByCourseYearIdUrl, 
...
if (subjectTypeId !== null){  
 this.examTimetableSubjectsList = [];
 const dateConvert = this.genericFunctions.momentFormatYMD(this.exmstdanswerpaperpages.value.examDate);
   /*----------- EXAM TIMETABLES -----------*/
 this.crudService.listBySixIds(this.subjectsforexamUrl, this.exmstdanswerpaperpages.value.collegeId, 
                                                          this.exmstdanswerpaperpages.value.courseId, 
                                                          this.exmstdanswerpaperpages.value.examId, 
                                                          dateConvert, 
                                                          this.exmstdanswerpaperpages.value.courseGroupId, 
                                                          this.exmstdanswerpaperpages.value.courseYearId, 
...
if (examTimetableDetId !== null){
 ...
 if (this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.exmstdanswerpaperpages.value.examTimetableDetId)).length > 0){
       this.examTypeId = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.exmstdanswerpaperpages.value.examTimetableDetId))[0].examTypeCatId;
       // tslint:disable-next-line: max-line-length
       this.exmstdanswerpaperpages.get('subjectId').setValue(this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.exmstdanswerpaperpages.value.examTimetableDetId))[0].subjectId);
       this.regulationId = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.exmstdanswerpaperpages.value.examTimetableDetId))[0].regulationId;
       this.subjectTypeId = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.exmstdanswerpaperpages.value.examTimetableDetId))[0].subjectTypeId;
       this.subjectTypCode = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.exmstdanswerpaperpages.value.examTimetableDetId))[0].subjectTypeName;
       this.examTypeCatCode = this.examTimetableSubjectsList.filter(x => (x.examTimetableDetId === this.exmstdanswerpaperpages.value.examTimetableDetId))[0].examTypeCatCode;
   }

 
   /*----------- EXAM TIMETABLES -----------*/
 this.crudService.listByEightIds(this.examMarksEntryStudentsUrl, this.exmstdanswerpaperpages.value.collegeId, 
     this.exmstdanswerpaperpages.value.courseId, 
     this.exmstdanswerpaperpages.value.examId, 
     dateConvert, 
     this.exmstdanswerpaperpages.value.courseGroupId, 
     this.exmstdanswerpaperpages.value.courseYearId, 
     this.exmstdanswerpaperpages.value.subjectId, 
     this.examTypeId,
     ...
     .subscribe(result => {
       if (result.statusCode === 200){
               if (result.data && !_.isEmpty(result.data) && result.data.length > 0) {
                 ...
                 // this.getSubjectSyllabus(this.exmstdanswerpaperpages.value.subjectId);
...
openDialog(): void {
  const dialogRef = this.dialog.open(AddStdAnswerPaperpagesComponent, {
      width: '750px',
      data: {}
  });
