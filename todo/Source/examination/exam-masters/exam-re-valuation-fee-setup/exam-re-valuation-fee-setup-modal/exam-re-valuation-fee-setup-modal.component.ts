...
@Component({
  selector: 'app-exam-re-valuation-fee-setup-modal',
  templateUrl: './exam-re-valuation-fee-setup-modal.component.html',
  styleUrls: ['./exam-re-valuation-fee-setup-modal.component.scss']
})
export class ExamReValuationFeeSetupModalComponent implements OnInit {

  displayedColumns: string[] = ['id', 'ApplicableFor', 'examType', 'fee', 'actions'];
  ...
  getExams(courseId): void {
      if (courseId != null) {
          this.flag = false;
          this.crudService.listDetailsByThreeIds(this.examMasterUrl, this.pageParams.collegeId, courseId, 'true', 'College.collegeId', 
  ...
  goBack(): void {
      this.router.navigate(['admin-examination-management/admin-exam-masters/revaluation-fee-setup'], {
          queryParams: {
          ...
          .subscribe(result => {
              ...
              if (result.statusCode === 200) {
                  if (result.success) {
                      ...
                      this.router.navigate(['admin-examination-management/exam-masters/exam-fee-setup'], {
                          queryParams: {
                              collegeId: this.pageParams.collegeId,
                              examId: this.pageParams.examId,
                              academicYearId : this.pageParams.academicYearId,
