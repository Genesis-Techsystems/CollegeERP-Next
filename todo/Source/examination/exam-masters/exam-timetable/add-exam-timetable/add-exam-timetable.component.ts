...
import { ExistingExamTimetableComponent } from './existing-exam-timetable/existing-exam-timetable.component';
...
@Component({
  selector: 'app-add-exam-timetable',
  templateUrl: './add-exam-timetable.component.html',
  styleUrls: ['./add-exam-timetable.component.scss']
})
...
export class AddExamTimetableComponent implements OnInit {
  ...
  .subscribe(result =>  {
          ...
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              ...
              if (this.filtersDetailsList.filter(x => (x.fk_course_id == this.pageParams.courseId 
  ...
  getExamTimetables(): void{
    this.spinner.show();  
    this.crudService.listByThreeIds(this.examtTimetableDetailsUrl, this.pageParams.courseYearId, 
        this.pageParams.courseId, this.pageParams.examId, 'courseYearId', 'courseId', 'examId')
       .subscribe(result => {
          this.spinner.hide();
          if (result.statusCode === 200){
...
createGroup(subjectObj,generalDetailId,generalDetailCode, subjectCode, subjectType){
              ...
              this.groupList.map(grp=>{
                //  if (this.examLabBatches[n].fk_subject_id  === grp.fk_subject_id){
                  if (this.courseGroupYears.filter(x => (x.courseGroupId === grp.fk_course_group_id 
                      && x.examTypeCatId === generalDetailId)).length === 0){
                 ...
                 if (result.statusCode === 200){
                     if (result.success) {
                         if (result.data && result.data.length > 0){
                            ...
                            this.examTimetable = [];                            
                            this.getExamTimetables();
                            const dialogRef = this.dialog.open(ExistingExamTimetableComponent, {
                                    width: '650px',
                                    data: result.data
                                });
                         }else{
  ...
  tConvert(time): any{
   ...
   goBack(): void{
        this.router.navigate(['admin-examination-management/admin-exam-masters/exam-timetable'], 
        { queryParams: { 
            // collegeId: this.pageParams.collegeId,
            courseId: this.pageParams.courseId ,
            courseYearId: this.pageParams.courseYearId,
