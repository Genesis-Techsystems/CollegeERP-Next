...
import { ExamCenterSeatingorderComponent } from './exam-center-seatingorder/exam-center-seatingorder.component';

@Component({
  selector: 'app-exam-center-room-allotment-modal',
  templateUrl: './exam-center-room-allotment-modal.component.html',
  styleUrls: ['./exam-center-room-allotment-modal.component.scss']
})
export class ExamCenterRoomAllotmentModalComponent implements OnInit {
...
selectedTimetable(examTimetableId){
    ...
    addSeat(col): void {
        col.generalDetails = this.examSeatStatuses;
        col.courseId = this.pageParams.courseId;
        col.academicYear = this.pageParams.academicYear;
        col.courseCode = this.pageParams.courseCode;
        col.roomName = this.room.blockName + '/' + this.room.floorName + '/' + this.room.roomName;
        if (this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId)).length > 0) {
            col.collegeCode = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].collegeCode;
            col.subjectName = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].subjectName;
            col.subjectId = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].subjectId;
            col.subjectCode = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].subjectCode;
            col.examTypeCatCode = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].examTypeCatCode;
            col.courseYear = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].courseYearName;
            col.examSessionName = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].examSessionName;
            col.examDate = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].examDate;
            this.selectedExamDate = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].examDate;
        }

        col.examName = this.examsList.examName;

        const dialogRef = this.dialog.open(ExamCenterSeatingorderComponent, {
            width: '900px',
            data: col
        });
        dialogRef.afterClosed().subscribe(details => {
            if (details != null && details !== '') {
               // col.comments = details.comments;
                col.examseatstatusCatId = details.examseatstatusCatId;
                col.studentId = details.studentId;
                col.subjectId = details.subjectId;
                col.examSeatStatusCode = details.examSeatStatusCode;
                col.examDisplaySeatStatusCode = details.examDisplaySeatStatusCode;
                col.hallticketNumber = details.hallticketNumber;
                col.subjectCode = details.subjectCode;
                this.addExamTable('seatOrder');
            }
        });
    }
    ...
    addExamTable(name): void {
        this.examStdRoomAllots = [];
        this.examRoomAllot = [];
        this.calAvailableSeats();
      //  this.examRoomAllotmentId = null;
        this.createdDt = null;
        if (this.examRoomAllotment.length > 0) {
            this.examRoomAllotmentId = this.examRoomAllotment[0].examRoomAllotmentId;
            this.createdDt = this.examRoomAllotment[0].createdDt;
            this.selectedExamDate = this.examRoomAllotment[0].examDate;
            if (this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId)).length > 0) {
                this.selectedExamDate = this.examTimetables.filter(x => (x.examTimetableId === this.staffForm.value.examTimetableId))[0].examDate;
            }
        } else {
        ...
        this.examRoomAllot.push({
            collegeId: this.pageParams.collegeId,
            examId: this.staffForm.value.examId,
            examRoomAllotmentId: this.examRoomAllotmentId,
            examTimetableId: this.staffForm.value.examTimetableId,
            roomId: this.staffForm.value.roomId,
            examDate: this.selectedExamDate,
            totalRows: this.staffForm.value.totalRows,
            totalColumns: this.staffForm.value.totalColumns,
            priority: this.staffForm.value.priority,
            roomStrength: this.staffForm.value.totalRows * this.staffForm.value.totalColumns,
            availableSeats: this.available,
            blockedSeats: this.blocked,
            bookedSeats: this.booked,
            isActive: true,
            createdDt: this.createdDt,
            examRoomStudentAllotmentDTO: this.examStdRoomAllots
        });
        if (this.staffForm.valid) {
                    ...
                    if (result.statusCode === 200) {
                        if (result.success) {
                            ...
                            if (name === 'update'){
                                ...
                                this.getExamRoomAllotment(result.data.examRoomAllotIds[0]);
                            }
                        } else {
                            this.snotifyService.info(result.message, 'Info!');
                        }
    ...
    addExamTableBulk(name): void {
      ...
      //  this.examRoomAllotmentId = null;
        ...
        for (let i = 0; i < this.selectedStudents.length; i++){
             for (let j = 0; j < this.examStdRoomAllots.length; j++){
                  if (this.examStdRoomAllots[j].studentId === null){
                      this.examStdRoomAllots[j].studentId = this.selectedStudents[i].studentId;
                    //   this.examStdRoomAllots[j].subjectId = this.pageParams.subjectId;
                      this.examStdRoomAllots[j].examseatstatusCatId = this.examSeatStatuses.filter(x => (x.generalDetailCode === 'Booked'))[0].generalDetailId;
                      break;
                  }
             }
        }

        this.available = this.examRoomAllotment[0].availableSeats - this.selectedStudents.length;
        this.booked = this.examRoomAllotment[0].bookedSeats + this.selectedStudents.length;
        this.blocked = this.examRoomAllotment[0].blockedSeats;
    ...
    printStickers(){
        ...
        this.router.navigate(['admin-examination-management/exam-papers-delivery-process/exam-center-seating-plan/add-exam-center-allotment/print-seating-stickers'],
        {
            queryParams: {
    ...
    printGroupStickers(){
        JSON.stringify(this.studentAllotmentDetails)
        this.router.navigate(['admin-examination-management/exam-papers-delivery-process/exam-center-seating-plan/add-exam-center-allotment/print-group-seating-stickers'],
        {
            queryParams: {
            data: JSON.stringify(this.studentAllotmentDetails),
            collegeCode: this.studentAllotmentDetails[0]?.college_code,
