...
import { ExamCenterAssignSeatingComponent } from './exam-center-assign-seating/exam-center-assign-seating.component';

@Component({
  selector: 'app-exam-center-room-allotment',
  templateUrl: './exam-center-room-allotment.component.html',
  styleUrls: ['./exam-center-room-allotment.component.scss']
})
export class ExamCenterRoomAllotmentComponent implements OnInit {
  ...
  searchExamList(value: string) {
    ...
    selectedCollege(examId): void{
        ...
        if (examId != null && this.staffForm.valid){
            this.spinner.show();          
            this.crudService.listDetailsByThreeIds(this.examRoomAllotmentCrudUrl,this.staffForm.value.univExamcenterId, this.staffForm.value.examId, this.staffForm.value.examTimetableId, 'room.block.building.univExamCenters.univExamcenterId', this.getExamMasterDetailsUrl, 'ExamTimetable.examTimetableId')
            .subscribe(result => {
                this.spinner.hide();
                this.flag = true;
                if (result.statusCode === 200){
    ...
    copyExistingSeating(): void{
        if (this.staffForm.valid){
            this.router.navigate(['admin-examination-management/exam-papers-delivery-process/exam-center-seating-plan/copy-existing-seating'], 
    ...
    addExamRoomAllotment(): void{
        if (this.staffForm.valid){
            this.router.navigate(['admin-examination-management/exam-papers-delivery-process/exam-center-seating-plan/center-room-allotment'], 
            { queryParams: { collegeId: this.staffForm.value.collegeId,
                             examRoomAllotmentId: null,
                            examId: this.staffForm.value.examId,
                            academicYear: this.academicYears.filter(x => (x.fk_academic_year_id === this.staffForm.value.academicYearId))[0].academic_year, 
    ...
    editExamRoomAllotment(row): void{
        if (this.staffForm.valid){
            this.router.navigate(['admin-examination-management/exam-papers-delivery-process/exam-center-seating-plan/add-exam-center-allotment'], 
            { queryParams: {
    ...
    AssignSeating(){
        const dialogRef = this.dialog.open(ExamCenterAssignSeatingComponent, {
            width: '400px',
            data: {}
            });
