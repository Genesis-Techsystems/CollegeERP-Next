...
export class PrintFormsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.Date = new Date();
    if(this.parameterservice.reEvaluationExamForm){
      this.studentExamFormDetails = this.parameterservice.reEvaluationExamForm;
  ...
  goBack(): void{
    this.router.navigate(['admin-examination-management/re-evaluation/re-evaluation-fee'],{
      queryParams:{
        studentId:this.studentExamFormDetails[0]?.fk_student_id,
        stdRollNumber:this.studentExamFormDetails[0]?.hallticket_number,
        examId:this.studentExamFormDetails[0]?.fk_exam_id,
        courseYearId:this.studentExamFormDetails[0]?.fk_course_year_id
      }
    }
    )
  }
