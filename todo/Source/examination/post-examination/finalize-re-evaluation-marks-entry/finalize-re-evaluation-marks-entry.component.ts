...
@Component({
  selector: 'app-finalize-re-evaluation-marks-entry',
  templateUrl: './finalize-re-evaluation-marks-entry.component.html',
  styleUrls: ['./finalize-re-evaluation-marks-entry.component.scss']
})
export class FinalizeReEvaluationMarksEntryComponent implements OnInit {
...
this.examFeeCollectionForm.get('subjectId').setValue(this.examTimetableSubjectsList.filter(x => (x.fk_exam_timetable_det_id === this.examFeeCollectionForm.value.examTimetableDetId))[0].fk_subject_id);
  ...
  this.spinner.show();
  let request = [
    {paramName: 'in_flag', paramValue:'examrevision_std_marks_status'},
...
.subscribe(result =>  {
        ...
        if (result.statusCode === 200) {
          if (result.data && result.data !== '' && result.data.result.length > 0) {
            ...
            //         if(this.DetailsList[i].length>0 && this.DetailsList[i][0].flag === 'examrevision_std_marks_status'){
