...
@Component({
  selector: 'app-re-evaluation-marks-entry',
  templateUrl: './re-evaluation-marks-entry.component.html',
  styleUrls: ['./re-evaluation-marks-entry.component.scss']
})
export class ReEvaluationMarksEntryComponent implements OnInit {
  ...
  private updateExamStudentDetailsForRevaluationMarksUrl=CONSTANTS.updateExamStudentDetailsForRevaluationMarksUrl;
  private updateRevisedMarksUrl=CONSTANTS.updateRevisedMarksUrl
  public formData;
  filtersDetailsList=[];
  CollegesListDetails=[];
...
this.examFeeCollectionForm.get('examDate').setValue(this.examTimetableSubjectsList.filter(x => (x.fk_exam_timetable_det_id === this.examFeeCollectionForm.value.examTimetableDetId))[0].exam_date);
  ...
  this.spinner.show();
  let request = [
    {paramName: 'in_flag', paramValue:'examrevision_std_marks_list'},
  ...
  this.crudService.getDetailsByRequest(this.examStudentRevisedDetailsUrl, '', request, '&')
.subscribe(result =>  {
        this.spinner.hide();
        if (result.statusCode === 200) {
          if (result.data && result.data !== '' && result.data.result.length > 0) {
            this.DetailsList = result.data.result;
             for(let i=0; i<this.DetailsList.length; i++){
                    if(this.DetailsList[i].length>0 && this.DetailsList[i][0].flag === 'examrevision_std_marks_list'){
                      this.StudentMarksList.push(this.DetailsList[i]);

                      }
                     else if(this.DetailsList[i].length>0 && this.DetailsList[i][0].flag === 'marks_setup'){
                        this.MarksSetupList.push(this.DetailsList[i]);
  
                        }
                    }
                    if(this.MarksSetupList.length>0){
                      if(this.isInternalExam){
                        this.maxValue=this.MarksSetupList[0][0].internal_marks
                      }
                      else{
                        this.maxValue=this.MarksSetupList[0][0].external_marks
                      }
                    }
                    for (let i = 0; i < this.StudentMarksList[0].length; i++){
                                          if (this.StudentMarksList[0][i].reevaluation_marks === null){
                                              this.StudentMarksList[0][i].reevaluation_marks = 0;
                                          }
                                          // if (this.examStudentsList[i].isMarksPublished === null){
                                          //   this.examStudentsList[i].isMarksPublished = false;
                                          // }
                                          // if (this.StudentMarksList[i].isPresent === false){
                                          //     this.StudentMarksList[i].isPass = false;
                                          // }
                                          this.enteredMarks(this.StudentMarksList[0][i]);
                                        }
                    
                // this.dataSource = new MatTableDataSource<any>(this.DetailsList);
...
enteredEmployee(event): void{
  if (event.target.value.length > 4){
    ...
    enteredMarks(item): void{
      ...
      if (item.reevaluation_marks != "" && item.reevaluation_marks != null) {
        if (parseInt(item.reevaluation_marks) < this.minvalue) {
          item.reevaluation_marks = this.minvalue;
        }
        if (parseInt(item.reevaluation_marks) > this.maxValue) {
          this.snotifyService.info('Entered Marks Should Less Than ' + this.maxValue + 'Marks', 'Info!');
            item.reevaluation_marks =this.maxValue
        }
      }
      
      const isMarks = (this.MarksSetupList[0][0].external_marks * this.MarksSetupList[0][0].external_pass_percentage / 100) > item.reevaluation_marks;
      if (isMarks ){
                    item.isPass = false; 
              }
              else{
    ...
    postExamMarks(): void{
      ...
      for (let i = 0; i < this.StudentMarksList[0].length; i++){
        this.postMarksList.push({
          // examStdDetId:this.StudentMarksList[0][i].pk_exam_std_det_id,
          //reevaluationMarks:this.StudentMarksList[0][i].reevaluation_marks,
          // reevaluationMarksComments:this.StudentMarksList[0][i].reevaluation_marks_comments
          examRevisionSubId:this.StudentMarksList[0][i].pk_exam_revision_sub_id,
          revisedMarks:this.StudentMarksList[0][i].reevaluation_marks,
          revisedByEmpId:+localStorage.getItem('employeeId'),
          revisedByEmpName:localStorage.getItem('uName')
      })
    }
      this.crudService.update(this.updateRevisedMarksUrl, this.postMarksList)
      .subscribe(result => {
          this.spinner.hide();
          if (result.statusCode === 200 && result.success==true){
              // if (result.data && result.data !== '') {
                  this.snotifyService.success(result.message, 'Success!');
                  this.selectedSubject(this.examFeeCollectionForm.value.examTimetableDetId,result);
              // }
          }else {
              this.snotifyService.error(result.message, 'Error!');
              this.selectedSubject(this.examFeeCollectionForm.value.examTimetableDetId,[]);
