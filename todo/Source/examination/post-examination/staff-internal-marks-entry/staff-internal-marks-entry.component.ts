...
@Component({
  selector: 'app-staff-internal-marks-entry',
  templateUrl: './staff-internal-marks-entry.component.html',
  styleUrls: ['./staff-internal-marks-entry.component.scss']
})
export class StaffInternalMarksEntryComponent implements OnInit {
  ...
  selectedRegulation(regulationId) {
    ...
    if (this.examFeeCollectionForm.value.regulationId != null && regulationId != null) {
      /*----------- SUBJECTS -----------*/
      this.subjectTypeList = this.CollegesListDetails.filter(x => (x.fk_college_id === this.examFeeCollectionForm.value.collegeId && x.fk_course_group_id === this.examFeeCollectionForm.value.courseGroupId
      && x.fk_course_year_id === this.examFeeCollectionForm.value.courseYearId && x.fk_regulation_id === this.examFeeCollectionForm.value.regulationId
    ));
  ...
  selectedSubject(examTimetableDetId): void {
    ...
    this.sections = this.CollegesListDetails.filter(x => (x.fk_college_id === this.examFeeCollectionForm.value.collegeId && x.fk_course_group_id === this.examFeeCollectionForm.value.courseGroupId
      && x.fk_course_year_id === this.examFeeCollectionForm.value.courseYearId && x.fk_regulation_id === this.examFeeCollectionForm.value.regulationId &&
      x.fk_subject_id === this.examFeeCollectionForm.value.subjectId));
  }
  selectedSection(examTimetableDetId): void {
    this.duplicateexamStudentList = [];
    this.labbatchsSubjects = [];
    this.labbatchs = [];
    this.totalField = false;
    ...
    if (this.examTimetableSubjectsList.filter(x => (x.fk_subject_id == this.examFeeCollectionForm.value.subjectId))[0].subject_type === 'LAB') {
      ...
      if (this.labbatchsSubjects && this.labbatchsSubjects.length > 0) {
        ...
        if (this.labbatchs.length > 0) {
          // this.examFeeCollectionForm.get('labBatchId').setValue(this.labbatchs[0].fk_stdbatch_id);
        }
      }
    }
    else {
  ...
  selectedLab(batchid) {
    if (this.examFeeCollectionForm.value.labBatchId !== 0) {
      // this.examFeeCollectionForm.get('examDate').setValue(this.labbatchs.filter(x => (x.fk_stdbatch_id === batchid))[0]?.exam_date);
      }
  }
  ...
  getStudentsList(list) {
    ...
    if (this.examFeeCollectionForm.value.subjectId !== null) {
      ...
      let request = [
        ...
        { paramName: 'in_group_section_id', paramValue: this.examFeeCollectionForm.value.groupSectionId },
        { paramName: 'in_stdbatch_id', paramValue: this.examFeeCollectionForm.value.labBatchId },
  ...
  download(): void {
    if (this.examFeeCollectionForm.valid) {
      ...
      xhr.open('GET', this.endURL + this.exammarksdownloadUrl + '?collegeId=' + this.examFeeCollectionForm.value.collegeId + '&subjectId=' + this.examFeeCollectionForm.value.subjectId +
        // tslint:disable-next-line:max-line-length
        '&examId=' + this.examFeeCollectionForm.value.examId + '&courseGroupId=' + this.examFeeCollectionForm.value.courseGroupId + '&courseYearId=' + this.examFeeCollectionForm.value.courseYearId, true);
