...
@Component({
  selector: 'app-exam-attendance-marks-entry',
  templateUrl: './exam-attendance-marks-entry.component.html',
  styleUrls: ['./exam-attendance-marks-entry.component.scss']
})
export class ExamAttendanceMarksEntryComponent implements OnInit {
  ...
  private addExamStudentAttendanceAndInternalMarksUrl=CONSTANTS.addExamStudentAttendanceAndInternalMarksUrl;
  ...
  getFiltersList(): void {
    ...
    let request = [
      {paramName: 'in_flag', paramValue: 'exam_published'},
          ...
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              ...
              for(let i=0; i<this.filtersDetailsList.length; i++){
                if(this.filtersDetailsList[i].length>0 && this.filtersDetailsList[i][0].flag === 'exam_published'){
...
getMarksSetup(data): void{
  if (data.length > 0){
    ...
    this.crudService.listDetailsByFourIds(this.examMarksSetupUrl, data[0].fk_college_id, data[0].fk_course_id, data[0].fk_regulation_id, 'true',
    this.getDetailsByCollegeIdUrl, this.getDetailsByCourseIdUrl, this.getDetailsByRegulationIdUrl, 'isActive')
    .subscribe(result => {
      this.spinner.hide();
      if (result.statusCode === 200) {
          if (result.data.resultList && result.data.resultList !== '') {
...
getStudentsList(list){
  ...
  this.examStudentsList =[]
    if (this.examFeeCollectionForm.value.examTimetableDetId !== null){
  //  this.examFeeCollectionForm.get('examDate').setValue(this.examTimetableSubjectsList.filter(x => (x.fk_exam_timetable_det_id === this.examFeeCollectionForm.value.examTimetableDetId))[0].exam_date);
      ...
      this.subjectDetails =  this.examTimetableSubjectsList.filter(x => (x.fk_exam_timetable_det_id === this.examFeeCollectionForm.value.examTimetableDetId))[0].subject_name + ' (' +
      this.examTimetableSubjectsList.filter(x => (x.fk_exam_timetable_det_id === this.examFeeCollectionForm.value.examTimetableDetId))[0].regulation_code + ')';
                    ...
                    this.totalPresents = presents.length;
                     this.totalAbsents = absents.length;
                        for (let i = 0; i < this.examStudentsList.length; i++){
                          this.examStudentsList[i].checked=true
...
selectedSubject(examTimetableDetId): void{
  ...
  if(this.examTimetableSubjectsList.filter(x=>(x.fk_subject_id==this.examFeeCollectionForm.value.subjectId))[0].subject_type == 'LAB'){
      ...
      this.labbatchsSubjects=this.CollegesListDetails.filter(x=>(x.fk_college_id== this.examFeeCollectionForm.value.collegeId && x.fk_course_id==this.examFeeCollectionForm.value.courseId && x.fk_academic_year_id==this.examFeeCollectionForm.value.academicYearId && x.fk_exam_id==this.examFeeCollectionForm.value.examId && x.fk_subject_id==this.examFeeCollectionForm.value.subjectId))
      if(this.labbatchsSubjects.length>0){
      const labbatchs = this.labbatchsSubjects.map(({ fk_eaxm_labbatch_id }) => fk_eaxm_labbatch_id);
      this.labbatchs = this.labbatchsSubjects.filter(({ fk_eaxm_labbatch_id }, index) => !labbatchs.includes(fk_eaxm_labbatch_id, index + 1));
      if(this.labbatchs.length>0){
        
        // this.examFeeCollectionForm.get('labBatchId').setValue(this.labbatchs[0].fk_eaxm_labbatch_id);
      }
     }
  ...
  selectedLab(batchid){
    if(batchid!=0){
      this.examFeeCollectionForm.get('examDate').setValue(this.labbatchs.filter(x=>(x.fk_eaxm_labbatch_id==batchid))[0]?.exam_date);

    }
    else{
      this.examFeeCollectionForm.get('examDate').setValue(this.labbatchs[0]?.exam_date);
    }
      ...
      if (this.examFeeCollectionForm.valid){
        ...
        if (this.examsList.filter(x => (x.fk_exam_id === this.examFeeCollectionForm.value.examId)).length > 0){
           if (this.examsList.filter(x => (x.fk_exam_id === this.examFeeCollectionForm.value.examId))[0].is_internal_exam){
              ...
              this.spinner.show();
              // ---------- EXAM INTERNAL MARKS ----------//
              this.crudService.add(this.addExamStudentAttendanceAndInternalMarksUrl, this.postMarksList)
              .subscribe(result => {
                  this.spinner.hide();
                  if (result.success){
                        this.snotifyService.success(result.message, 'Success!');
...
console.log(checked);
    ...
    this.check = checked; // Update global checkbox state

    // console.log(this.check);
    
     for (let i = 0; i < this.examStudentsList.length; i++){
        if (this.check){
    console.log(!this.check,'if');

           this.examStudentsList[i].checked = true;
           this.examStudentsList[i].isPresent = true;
        }else{
    console.log(this.check,'else');

         this.examStudentsList[i].checked = false;
         this.examStudentsList[i].isPresent = false;
        //  this.absents.push(this.examStudentsList[i]);
        }
     }  
