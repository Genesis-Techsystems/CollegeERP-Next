...
@Component({
  selector: 'app-internal-exam-entries',
  templateUrl: './internal-exam-entries.component.html',
  styleUrls: ['./internal-exam-entries.component.scss']
})
export class InternalExamEntriesComponent implements OnInit {
...
enteredEmployee(event): void{
  ...
  checkedItems(check, index, item): void{ 
    for(let i=0;i<this.examStudentsList.length;i++){
if(this.examStudentsList[i].examStdDetId==item.examStdDetId){
if(check){
  this.examStudentsList[i].isPresent==check
}
else{
  this.examStudentsList[i].isPresent==check
