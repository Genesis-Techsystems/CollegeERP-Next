...
@Component({
  selector: 'app-external-exam-attendance-marking',
  templateUrl: './external-exam-attendance-marking.component.html',
  styleUrls: ['./external-exam-attendance-marking.component.scss']
})
export class ExternalExamAttendanceMarkingComponent implements OnInit {
  ...
  selectedExam(examId){
    this.examAttendanceForm.get('employeeId').setValue(0)
    this.examAttendanceForm.get('roomId').setValue(0)
    this.examAttendanceForm.get('regulationId').setValue(0)
    ...
    if (this.examAttendanceForm.value.examId != null && examId != null) {
      /*----------- SUBJECTS -----------*/
      let request = [
            ...
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              ...
              for (const list of this.subjectListDetails) {
                ...
                if (this.subjectTypeList && this.subjectTypeList.length > 0) {
                  const subjectTypeList = this.subjectTypeList.map(({ fk_regulation_id }) => fk_regulation_id);
                  this.regulations = this.subjectTypeList.filter(({ fk_regulation_id }, index) => 
                    !subjectTypeList.includes(fk_regulation_id, index + 1));
                }
...
selectedRegulation(regulationId) {
    ...
    if (this.examAttendanceForm.value.regulationId != null && regulationId != null) {
      /*----------- SUBJECTS -----------*/
      this.subjectsDataList = this.subjectTypeList.filter(x => (x.fk_regulation_id === this.examAttendanceForm.value.regulationId))
      if (this.subjectsDataList.length > 0) {
                const examTimetableSubjects = this.subjectsDataList.map(({ fk_subject_id }) => fk_subject_id);
                this.examTimetableSubjectsList = this.subjectsDataList.filter(({ fk_subject_id }, index) =>
                  !examTimetableSubjects.includes(fk_subject_id, index + 1));
                this.examTimetableSubject = this.examTimetableSubjectsList
              }
              if(this.examTimetableSubjectsList && this.examTimetableSubjectsList.length > 0){
                  this.examAttendanceForm.get('subjectId').setValue(this.examTimetableSubjectsList[0]?.fk_subject_id);
                  this.selectedSubject(this.examAttendanceForm.value.subjectId);
              }
    }
  }
          ...
          if (result.statusCode === 200) {
            if (result.data && result.data !== '' && result.data.result.length > 0) {
              ...
              if (this.CollegesListDetails.length > 0) {
                  const groupList = this.CollegesListDetails.map(({ fk_course_group_id }) => fk_course_group_id);
                  this.courseGroups = this.CollegesListDetails.filter(({ fk_course_group_id }, index) => !groupList.includes(fk_course_group_id, index + 1));
                }
  ...
  selectedCourseGroup(courseGroupId): void {
    ...
    if (courseGroupId !== 0) {
      /*----------- COURSES Years -----------*/
      this.courseYearsList = this.CollegesListDetails.filter(x => (x.fk_course_group_id === this.examAttendanceForm.value.courseGroupId))
  ...
  getStudentsList(): void {
    ...
    if (this.examAttendanceForm.valid) {
      ...
      this.crudService.listByFiveteenIds(this.getExamAllotmentDetailsInvigilatorUrl,
        ...
        0, 0, 0, this.examDate, this.examDate, this.examAttendanceForm.value.subjectId,
        0, 0,
  ...
  roomDetails(): void {
    ...
    // this.crudService.listDetailsByIdsWithSort(this.examRoomAllotmentCrudUrl, this.examAttendanceForm.value.examId, this.getExamMasterDetailsUrl)
      .subscribe(result => {
        this.spinner.hide();
        if (result.statusCode === 200) {
