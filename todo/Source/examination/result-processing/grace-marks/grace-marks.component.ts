...
import { ViewGracemarksModelComponent } from './view-gracemarks-model/view-gracemarks-model.component';

@Component({
  selector: 'app-grace-marks',
  templateUrl: './grace-marks.component.html',
  styleUrls: ['./grace-marks.component.scss']
})
export class GraceMarksComponent implements OnInit {
...
getModerationList(){
  ...
  this.selectedData=
    ...
    if ( this.evaluatorForm.valid){
      this.crudService.listByFourteenIds(this.evaluatorModerationMarksUrl, 
      ...
      localStorage.getItem('employeeId'),
         'in_flag', 'in_orgid','in_academic_year', 'in_exam_month_yr', 'in_course_code', 'in_course_year_code','in_subject_code', 'in_evalutor_profileid','in_exam_date','in_regulation_code','in_emp_id','in_moderation_marks','in_questionpaper_id','in_loginuser_empid',
          )
       .subscribe(result => {
        this.flag = true;
           this.spinner.hide();
    ...
    this.evaluatorForm.get('subjectCode').setValue('')
     for(let i=0;i<this.examSubjects.length;i++){
      if(this.examSubjects[i].course_code==courseCodeId){
            this.academicyeardata.push(this.examSubjects[i])
            const academicYearData = this.academicyeardata.map(({ academic_year }) => academic_year);
            this.academicYear = this.academicyeardata.filter(({ academic_year }, index) =>
    ...
    this.evaluatorForm.get('subjectCode').setValue('')
       for(let i=0;i<this.examSubjects.length;i++){
                 if(this.examSubjects[i].course_code==this.evaluatorForm.value.CourseCode){
                       this.monthyear.push(this.examSubjects[i])
                       const exam_month_yrData = this.monthyear.map(({ exam_month_yr }) => exam_month_yr);
                       this.monthYear = this.monthyear.filter(({ exam_month_yr }, index) =>
  ...
  selectedMonthyr(exam_month_yr){
    ...
    for(let i=0;i<this.examSubjects.length;i++){
      if(this.examSubjects[i].exam_month_yr==exam_month_yr && this.examSubjects[i].course_code==this.evaluatorForm.value.CourseCode){
            this.courseyeardata.push(this.examSubjects[i])
            const courseyearcode = this.courseyeardata.map(({ course_year_code }) => course_year_code);
            this.courseyearcode = this.courseyeardata.filter(({ course_year_code }, index) =>
  ...
  selectedCourseYr(courseYr){
    ...
    for(let i=0;i<this.examSubjects.length;i++){
      if(this.examSubjects[i].course_year_code==courseYr && this.examSubjects[i].course_code==this.evaluatorForm.value.CourseCode && this.examSubjects[i].exam_month_yr==this.evaluatorForm.value.ExamMonthYear){
            this.subjectsdata.push(this.examSubjects[i])
            const subjectcode = this.subjectsdata.map(({ subject_code }) => subject_code);
            this.subjectcode = this.subjectsdata.filter(({ subject_code }, index) =>
      ...
      this.flag = true;
         ...
         if (result.statusCode === 200){
              if (result.data && result.data !== '' && result.data.result.length > 0) {
                  ...
                  const dialogRef = this.dialog.open(ViewGracemarksModelComponent, {
                    width: '1200px',
