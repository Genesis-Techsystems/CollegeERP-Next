...
import { getExamTimetables, deleteExamTimetable } from '@/services/exam-timetable'
...
export default function ExamTimetablePage() {
  ...
  const { data: exams = [], isLoading: loadingExams } = useQuery({
    queryKey: QK.examMaster.list(selectedCourseId ?? undefined, selectedAcademicYearId ?? undefined),
    queryFn: () => {
  ...
  const { data: courseYears = [] } = useQuery({
    queryKey: QK.collegeFilters.courseYears(selectedCourseId ?? 0),
    queryFn: () =>
  ...
  const { data: rows = [], isLoading: loadingRows } = useQuery({
    queryKey: QK.examTimetable.list(selectedExamId ?? undefined, selectedCourseYearId ?? undefined, selectedCourseId ?? undefined),
    queryFn: () =>
