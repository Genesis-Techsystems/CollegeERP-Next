...
export default function SeatingPlanSetupPage() {
  ...
  const { data: timetableSlots = [] } = useQuery({
    queryKey: QK.examTimetable.slots(selectedExamId ?? 0),
    queryFn: () => {
  ...
  const { data: rows = [], isLoading: loadingRows } = useQuery({
    queryKey: QK.seatingPlan.list(selectedExamId ?? undefined),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QK.seatingPlan.all })
