...
import { createExamSession, updateExamSession } from '@/services/exam-session'
...
function getDefaults(session: ExamSession | null): FormValues {
  if (session) {
    return {
      ...
      reason: session.reason ?? DEFAULT_ACTIVE_REASON,
    }
  }
  return {
    examSessionName: '',
...
export default function ExamSessionModal({
  ...
  const {
    ...
    formError,
    setFormError,
  } = useEntityForm<ExamSession, FormValues>(schema, getDefaults, open, editData)
  ...
  const { universities, sessionsIn } = useMemo(() => {
    ...
    for (const arr of result) {
      ...
      if (first.gm_code === GM_CODES.EXAM_SESSION) {
